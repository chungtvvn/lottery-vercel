...
async function main() {
    ...
    const strategies = String(args.get('strategies') || 'chainSmallFirst,chainFreqFirst,chainRiskFirst,numberAvgRisk,numberConsensusRisk,numberWeightedRisk,activeOnlyAvgRisk')
        .split(',')
        .map(value => value.trim())
        .filter(value => annualMilestoneService.STRATEGIES[value]);
    const holdCounts = String(args.get('holds') || '60,65,70,75,80,85')
        .split(',')

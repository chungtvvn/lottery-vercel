...
async function uploadToR2() {
    ...
    console.log(`[R2 Upload] Tìm thấy ${files.length} tệp tin JSON cần tải lên.`);
    ...
    for (const file of files) {
        ...
        const key = `${PREFIX}/${file}.gz`;

        console.log(`[R2 Upload] Đang tải lên ${file} (${(fileContent.length / 1024).toFixed(1)} KB -> ${(gzipContent.length / 1024).toFixed(1)} KB nén) dưới dạng ${key}...`);

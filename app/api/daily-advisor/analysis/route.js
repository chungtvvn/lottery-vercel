...
export async function GET(request) {
    ...
    try {
        const [advisorCache, probabilityCache, history] = await Promise.all([
        ...
        if (!probabilityCache?.records?.length) throw new Error('Chưa có cache Điểm xác suất');
        return NextResponse.json({ success: true, ...buildAdvisorAnalysis({ advisorCache, probabilityCache, history }) }, { headers: HEADERS });
    } catch (error) {

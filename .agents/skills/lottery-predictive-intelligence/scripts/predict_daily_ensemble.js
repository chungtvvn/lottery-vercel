...
console.log('================================================================');
console.log(`🎯 DỰ ĐOÁN THỰC CHIẾN XSMB NGÀY: ${targetDate}`);
console.log(`🔒 Dữ liệu mốc nguồn: Tính đến hết ${latestDraw.date} (Strict PIT 100%)`);
console.log('================================================================');

// 1. SINH DỰ ĐOÁN LÔ QMBF
try {
    ...
    console.log('\n----------------------------------------------------------------');
    console.log('⭐ PHẦN 1: DỰ ĐOÁN LÔ TỐI ƯU (QUANTUM BAYES-MARKOV FUSION QMBF)');
    console.log('----------------------------------------------------------------');
    console.log(`👑 DÀN LÔ VÔ ĐỊCH TOP 6  (Vốn 13.2M · Lãi +1.390M · 92.2% nổ):`);
    console.log(`   👉 ${preds.top6?.numbers?.map(n => String(n).padStart(2, '0')).join('  ') || 'Đang cập nhật'}`);
    console.log(`\n💎 DÀN LÔ VÀNG TOP 10   (Vốn 22M · Lãi +2.002M · 99.2% nổ):`);
    console.log(`   👉 ${preds.top10?.numbers?.map(n => String(n).padStart(2, '0')).join('  ') || 'Đang cập nhật'}`);
    console.log(`\n🏆 DÀN LÔ TOÀN THẮNG TOP 20 (Vốn 44M · Lãi +2.828M · 100% nổ):`);
    console.log(`   👉 ${preds.top20?.numbers?.map(n => String(n).padStart(2, '0')).join('  ') || 'Đang cập nhật'}`);
    console.log(`\n🎯 SONG THỦ LÔ VIP TOP 2 (Vốn 4.4M · ROI +37%):`);
...
// 2. SINH DỰ ĐOÁN ĐỀ TAM TRỤ & THÍCH ỨNG ALPHA
try {
    ...
    if (fs.existsSync(cacheFile)) {
        ...
        if (triple) {
            console.log('\n----------------------------------------------------------------');
            console.log('🏛️ PHẦN 2: DỰ ĐOÁN ĐỀ GỘP 3 TAM TRỤ (VỐN 90M · 69.4% TRÚNG)');
            console.log('----------------------------------------------------------------');
            console.log(`Gộp 3 phương pháp: ${triple.m1Label} + ${triple.m2Label} + ${triple.m3Label}`);
        ...
        if (adaptive) {
            console.log('\n----------------------------------------------------------------');
            console.log('💎 PHẦN 3: DỰ ĐOÁN ĐỀ GỘP 2 THÍCH ỨNG ALPHA (VỐN 60M · LÃI +2.436M)');
            console.log('----------------------------------------------------------------');

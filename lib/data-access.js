...
/**
 * Upsert lottery results vào Postgres (Tối ưu hóa: Chỉ upsert dữ liệu 60 ngày gần nhất)
 */
async function upsertLotteryResults(results) {
    ...
    // Tối ưu để tránh dính Vercel Timeout (10s giới hạn)
    // Thay vì upsert cả cục 7000 dòng, ta chỉ cần upsert tối đa 90 dòng cuối cùng (kể cả khi mất đồng bộ 1 thời gian)
    const MAX_UPSERT = 90;
    const recentResults = results.slice(-MAX_UPSERT);
...
async function saveStatsToDb(statType, stats) {
    ...
    // Tăng CHUNK_SIZE lên 25 để gộp payload lại, tránh Vercel timeout 10s do gửi quá nhiều request rời rạc
    const CHUNK_SIZE = 25; 
    const chunks = [];
    ...
    // Insert new chunks in parallel to save time

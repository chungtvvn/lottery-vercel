...
async function loadCategoryStatsFromSupabaseDb(categoryType) {
    ...
    const patternKeys = statsRows.map(row => row.pattern_key).filter(Boolean);
    const patternChunkSize = 50;
    const streakPageSize = 1000;
    for (let i = 0; i < patternKeys.length; i += patternChunkSize) {
        const chunk = patternKeys.slice(i, i + patternChunkSize);

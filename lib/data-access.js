/**
 * Data Access Layer
 * Primary source: Cloudflare R2/static JSON.
 * Supabase is used only when explicitly requested via LOTTERY_*_SOURCE=supabase/db.
 */
const fs = require('fs');
...
} = require('./utils/quickStatsShards');

// R2 is the production source of truth. Importing the Supabase SDK eagerly
// made every R2-only request pay for an unused dependency (and could fail on
// runtimes where the optional SDK is unavailable). Only load it when an
// operator explicitly selects a Supabase source mode.
let _supabaseClientModule = null;
function getSupabaseClientModule() {
    if (!_supabaseClientModule) _supabaseClientModule = require('./supabase/client');
    return _supabaseClientModule;
}
function hasSupabaseReadonlyConfig() { return getSupabaseClientModule().hasSupabaseReadonlyConfig(); }
function hasSupabaseAdminConfig() { return getSupabaseClientModule().hasSupabaseAdminConfig(); }
function getSupabaseReadonlyClient() { return getSupabaseClientModule().getSupabaseReadonlyClient(); }
function getSupabaseAdminClient() { return getSupabaseClientModule().getSupabaseAdminClient(); }
...
let _quickStatsKeysCache = null;
let _quickStatsShardCache = new Map();
let _storageJsonCache = new Map();
let _cacheStoreMemory = new Map();
let _rawDataCacheMtimeMs = null;
let _quickStatsCacheMtimeMs = null;
let _quickStatsHistoryCacheMtimeMs = null;
const CACHE_STORE_MEMORY_TTL_MS = Number(process.env.CACHE_STORE_MEMORY_TTL_MS || 5 * 60 * 1000);
const R2_JSON_CACHE_TTL_MS = Number(process.env.R2_JSON_CACHE_TTL_MS || 2 * 60 * 1000);

// TTL for module-level caches in production (serverless warm instances)
// In development, file mtime checks handle invalidation instead
const MODULE_CACHE_TTL_MS = Number(process.env.MODULE_CACHE_TTL_MS || 10 * 60 * 1000); // 10 minutes
let _rawDataCacheReadAt = 0;
let _quickStatsCacheReadAt = 0;
let _quickStatsHistoryCacheReadAt = 0;
let _cachedPredictionsCacheReadAt = 0;
let _quickStatsKeysCacheReadAt = 0;

function isModuleCacheExpired(readAt) {
    if (process.env.NODE_ENV === 'development') return false; // dev uses file mtime
    return (Date.now() - readAt) > MODULE_CACHE_TTL_MS;
}
...
const R2_STATS_PREFIX = process.env.CLOUDFLARE_R2_STATS_PREFIX || 'statistics';
const R2_DATA_PREFIX = process.env.CLOUDFLARE_R2_DATA_PREFIX || 'data';
...
function shouldUseR2Source(sourceName) {
    const mode = readDataSourceMode(sourceName);
    if (['local', 'json', 'static', 'file'].includes(mode)) return false;
    if (['supabase', 'supabase-db', 'db', 'supabase-storage', 'storage'].includes(mode)) return false;
    return Boolean(getR2PublicUrl());
}

function allowLocalJsonFallback(sourceName) {
    const mode = readDataSourceMode(sourceName);
    if (['local', 'json', 'static', 'file'].includes(mode)) return true;
    if (process.env.ALLOW_LOCAL_JSON_FALLBACK === '1') return true;

    // In production, once R2 is configured it is the source of truth. Falling
    // back to bundled/local JSON can silently serve stale historical snapshots.
    if (process.env.NODE_ENV === 'production' && shouldUseR2Source(sourceName)) {
        return false;
    }

    return process.env.DISABLE_LOCAL_JSON_FALLBACK !== '1';
}

function readStorageJsonCache(cacheKey) {
    const hit = _storageJsonCache.get(cacheKey);
    if (!hit) return null;
    if (hit && hit.__cacheEntry === true) {
        if ((Date.now() - hit.readAt) > hit.ttlMs) {
            _storageJsonCache.delete(cacheKey);
            return null;
        }
        return hit.data;
    }
...
function writeStorageJsonCache(cacheKey, data, ttlMs = R2_JSON_CACHE_TTL_MS) {
    _storageJsonCache.set(cacheKey, {
        __cacheEntry: true,
        data,
        readAt: Date.now(),
        ttlMs
...
function shouldUseSupabaseStats() {
    return shouldUseSupabaseStorageStats();
}

function shouldUseSupabaseStorageStats() {
    const mode = readDataSourceMode('LOTTERY_STATS_SOURCE');
    if (['local', 'json', 'static', 'file'].includes(mode)) return false;
    if (process.env.ENABLE_LEGACY_SUPABASE_STORAGE === '1') return hasSupabaseAdminConfig();
    return ['supabase-storage', 'supabase-storage-only', 'storage', 'storage-only'].includes(mode)
...
async function loadJsonFromSupabaseStorage(fileName) {
    ...
    const cached = readStorageJsonCache(cacheKey);
    ...
    if (compressed.length >= 2 && compressed[0] === 0x1f && compressed[1] === 0x8b) {
        jsonText = zlib.gunzipSync(compressed).toString('utf8');
    } else {
        jsonText = compressed.toString('utf8');
    }
    const parsed = JSON.parse(jsonText);

    writeStorageJsonCache(cacheKey, parsed, CACHE_STORE_MEMORY_TTL_MS);
    return parsed;
}

async function loadJsonFromR2(fileName, prefix = R2_STATS_PREFIX) {
    const baseUrl = getR2PublicUrl();
    if (!baseUrl) return null;

    const normalizedPrefix = String(prefix || '').replace(/^\/|\/$/g, '');
    const cacheKey = `r2/${normalizedPrefix}/${fileName}`;
    const cached = readStorageJsonCache(cacheKey);
    if (cached) return cached;

    const url = `${baseUrl}/${normalizedPrefix}/${fileName}.gz`;
    ...
    const parsed = JSON.parse(jsonText);
    writeStorageJsonCache(cacheKey, parsed, R2_JSON_CACHE_TTL_MS);
    return parsed;
}

async function loadJsonWithSupabaseFallback(fileName) {
    const statsSource = readDataSourceMode('LOTTERY_STATS_SOURCE');
    const isLocalMode = ['local', 'json', 'static', 'file'].includes(statsSource);

    if (!isLocalMode) {
        // 1. Try Cloudflare R2 if configured
        try {
            if (shouldUseR2Source('LOTTERY_STATS_SOURCE')) {
                const data = await loadJsonFromR2(fileName, R2_STATS_PREFIX);
                if (data) return data;
            }
        } catch (error) {
            if (!allowLocalJsonFallback('LOTTERY_STATS_SOURCE') && !shouldUseSupabaseStats()) {
                throw error;
            }
            console.warn(`[DataAccess] Cloudflare R2 load failed for ${fileName}, fallback Supabase/local:`, error.message);
        }
    ...
    // 3. Fallback to Local JSON only for local/dev mode or explicit override.
    if (!allowLocalJsonFallback('LOTTERY_STATS_SOURCE')) {
        throw new Error(`Không thể tải ${fileName} từ R2 và local fallback đã bị tắt.`);
    }
...
async function getRawData() {
    ...
    if (_rawDataCache && isModuleCacheExpired(_rawDataCacheReadAt)) {
        _rawDataCache = null;
    ...
    if ((!allData || allData.length === 0) && shouldUseR2Source('LOTTERY_DATA_SOURCE')) {
        try {
            allData = await loadJsonFromR2('xsmb-2-digits.json', R2_DATA_PREFIX);
            console.log(`[DataAccess] Fetched ${Array.isArray(allData) ? allData.length : 0} lottery records via Cloudflare R2`);
        } catch (error) {
            if (!allowLocalJsonFallback('LOTTERY_DATA_SOURCE')) {
                throw error;
            }
            console.warn('[DataAccess] Cloudflare R2 raw load failed, fallback local JSON:', error.message);
        }
    }

    if (!allData || allData.length === 0) {
        if (!allowLocalJsonFallback('LOTTERY_DATA_SOURCE')) {
            throw new Error('Không thể tải raw data từ R2 và local fallback đã bị tắt.');
        }
...
async function getQuickStatsFromCache(options = {}) {
    const selectiveActiveOnly = options.selectiveActiveOnly === true || process.env.ENABLE_SELECTIVE_QUICK_STATS === '1';
    if (shouldUseSupabaseDbStats()) {
        if (_quickStatsCache && isModuleCacheExpired(_quickStatsCacheReadAt)) {
            _quickStatsCache = null;
        }
        if (_quickStatsCache) return _quickStatsCache;
        if (process.env.READ_LEGACY_QUICK_STATS_CACHE === '1') {
            ...
            if (cachedQuickStats && typeof cachedQuickStats === 'object' && Object.keys(cachedQuickStats).length > 0) {
                _quickStatsCache = cachedQuickStats;
                return cachedQuickStats;
            }
        }

        if (!selectiveActiveOnly) {
            const staticQuickStats = await loadJsonWithSupabaseFallback('quick_stats.json');
            if (staticQuickStats && typeof staticQuickStats === 'object' && Object.keys(staticQuickStats).length > 0) {
                console.log(`[DataAccess] Loaded full quick_stats (${Object.keys(staticQuickStats).length} patterns) from R2/static JSON`);
                _quickStatsCache = staticQuickStats;
                _quickStatsCacheReadAt = Date.now();
                return staticQuickStats;
            }
        }

        // This optimization is intentionally opt-in only. The prediction pipeline
        // needs the full universe of patterns because potential/not-yet-formed
        // chains are often absent from today's active quick_stats_history list.
        if (selectiveActiveOnly) {
            try {
                const history = await readCacheStore('quick_stats_history');
                if (history && history.length > 0) {
                    const latestDay = history[0];
                    if (latestDay && latestDay.streaks && latestDay.streaks.length > 0) {
                        const activeKeys = new Set();
                        for (const s of latestDay.streaks) {
                            if (s.key) activeKeys.add(s.key);
                        }
                        const selectiveStats = await getPatternStatsByKeysFromDb(Array.from(activeKeys));
                        if (selectiveStats && Object.keys(selectiveStats).length > 0) {
                            console.log(`[DataAccess] Optimized: Loaded ${Object.keys(selectiveStats).length} selective quick stats from DB using quick_stats_history keys.`);
                            _quickStatsCache = selectiveStats;
                            _quickStatsCacheReadAt = Date.now();
                ...
                console.warn('[DataAccess] Selective quick stats optimization failed:', optError.message);
            }
        }
        ...
        try {
            ...
            // Tối ưu hóa: Lấy tổng số hàng bằng head query và chạy các query trang song song để tránh timeout
    ...
    if (_quickStatsCache && !isModuleCacheExpired(_quickStatsCacheReadAt) && isFileCacheFresh(_quickStatsCacheMtimeMs, fileMtimeMs)) return _quickStatsCache;
...
async function getPatternStatsByKeysFromDb(keys) {
    if (!keys || keys.length === 0) return {};
    const pickKeys = (stats) => {
        const result = {};
    ...
    if (shouldUseSupabaseDbStats()) {
        try {
            ...
            console.warn(`[DataAccess] Supabase DB selective quick stats fetch failed, fallback to full:`, dbError.message);
        }
    }

    if (process.env.ENABLE_QUICK_STATS_SHARDS !== '0') {
        try {
            const result = {};
            const keysByShard = new Map();
            for (const key of keys) {
                const shardId = getQuickStatsShardId(key);
                if (!keysByShard.has(shardId)) keysByShard.set(shardId, []);
                keysByShard.get(shardId).push(key);
            }

            const shardPayloads = await Promise.all([...keysByShard.keys()].map(async shardId => {
                let shardData = _quickStatsShardCache.get(shardId);
                if (!shardData) {
                    shardData = await loadJsonWithSupabaseFallback(getQuickStatsShardFileName(shardId));
                    if (!shardData || typeof shardData !== 'object') {
                        throw new Error(`missing ${getQuickStatsShardFileName(shardId)}`);
                    }
                    _quickStatsShardCache.set(shardId, shardData);
                }
                return [shardId, shardData];
            }));

            for (const [shardId, shardData] of shardPayloads) {
                const shardKeys = keysByShard.get(shardId) || [];
                for (const key of shardKeys) {
                    if (Object.prototype.hasOwnProperty.call(shardData, key)) {
                        result[key] = shardData[key];
                    }
                }
            }

            const missingKeys = keys.filter(key => !Object.prototype.hasOwnProperty.call(result, key));
            if (missingKeys.length > 0) {
                console.warn(`[DataAccess] Quick stats shards missing ${missingKeys.length}/${keys.length} requested keys, fallback full quick_stats for missing keys.`);
                const fullStats = await getQuickStatsFromCache().catch(() => null);
                if (fullStats && typeof fullStats === 'object') {
                    for (const key of missingKeys) {
                        if (Object.prototype.hasOwnProperty.call(fullStats, key)) {
                            result[key] = fullStats[key];
                        }
                    }
                }
            }
            ...
            console.warn(`[DataAccess] Quick stats shard load failed, fallback to full quick_stats:`, shardError.message);
        }
    }

    const fullStats = await getQuickStatsFromCache();
    return pickKeys(fullStats);
}
...
async function getQuickStatsHistoryFromCache() {
    ...
    const fileMtimeMs = getFileMtimeMs(filePath);
    if (_quickStatsHistoryCache && !isModuleCacheExpired(_quickStatsHistoryCacheReadAt) && isFileCacheFresh(_quickStatsHistoryCacheMtimeMs, fileMtimeMs)) return _quickStatsHistoryCache;
    ...
    if (data) {
        ...
        _quickStatsHistoryCacheReadAt = Date.now();
    }
    return data;
}

async function getCachedPredictionsFromCache() {
    if (_cachedPredictionsCache && isModuleCacheExpired(_cachedPredictionsCacheReadAt)) {
    ...
    if (shouldUseSupabaseDbStats()) {
        try {
            ...
            if (data) {
                ...
                _cachedPredictionsCacheReadAt = Date.now();
                return data;
            }
        } catch (dbError) {
...
async function getQuickStatsKeysFromCache() {
    if (_quickStatsKeysCache && isModuleCacheExpired(_quickStatsKeysCacheReadAt)) {
        _quickStatsKeysCache = null;
    }
    if (_quickStatsKeysCache) return _quickStatsKeysCache;

    const keys = await loadJsonWithSupabaseFallback('quick_stats_keys.json').catch(() => null);
    if (Array.isArray(keys)) {
        _quickStatsKeysCache = keys;
        _quickStatsKeysCacheReadAt = Date.now();
        return keys;
    }

    const quickStats = await getQuickStatsFromCache();
    _quickStatsKeysCache = Object.keys(quickStats || {}).sort();
    _quickStatsKeysCacheReadAt = Date.now();
    return _quickStatsKeysCache;
}

async function readCacheStore(cacheKey) {
    if (!hasSupabaseReadonlyConfig() && !hasSupabaseAdminConfig()) return null;
    const memoryHit = _cacheStoreMemory.get(cacheKey);
    if (memoryHit && (Date.now() - memoryHit.readAt) <= CACHE_STORE_MEMORY_TTL_MS) {
    ...
    try {
        ...
        if (payload !== null && payload !== undefined) {
            _cacheStoreMemory.set(cacheKey, { data: payload, readAt: Date.now() });
        }
        return payload;
    } catch (error) {
...
async function writeCacheStore(cacheKey, namespace, data) {
    ...
    try {
        ...
        if (error) throw error;
        _cacheStoreMemory.set(cacheKey, { data, readAt: Date.now() });
    } catch (error) {
...
async function loadCategoryStatsFromSupabaseDb(categoryType) {
    ...
    const streakPageSize = 1000;
    for (let from = 0; ; from += streakPageSize) {
        const to = from + streakPageSize - 1;
        const { data, error } = await supabase
            .from('historical_streaks')
        ...
        if (data.length < streakPageSize) break;
    }
...
async function getAppConfig() {
    const configSource = readDataSourceMode('APP_CONFIG_SOURCE');
    if (configSource === 'supabase' && hasSupabaseReadonlyConfig()) {
        try {
...
function clearCache() {
    ...
    _cachedPredictionsCache = null;
    _quickStatsKeysCache = null;
    _quickStatsShardCache = new Map();
    _storageJsonCache = new Map();
    _cacheStoreMemory = new Map();
    _rawDataCacheMtimeMs = null;
    _quickStatsCacheMtimeMs = null;
    _quickStatsHistoryCacheMtimeMs = null;
    _rawDataCacheReadAt = 0;
    _quickStatsCacheReadAt = 0;
    _quickStatsHistoryCacheReadAt = 0;
    _cachedPredictionsCacheReadAt = 0;
    _quickStatsKeysCacheReadAt = 0;
}
...
module.exports = {
    ...
    getQuickStatsKeysFromCache,
    loadJsonWithSupabaseFallback,
    loadJsonFromR2,
    saveAllStatsToDb,

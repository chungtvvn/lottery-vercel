...
async function loadJsonWithSupabaseFallback(fileName) {
    ...
    if (!isLocalMode) {
        ...
        try {
            const r2PublicUrl = String(process.env.NEXT_PUBLIC_CLOUDFLARE_R2_PUBLIC_URL || process.env.CLOUDFLARE_R2_PUBLIC_URL || '').trim();
            if (r2PublicUrl) {
                const url = `${r2PublicUrl.replace(/\/$/, '')}/statistics/${fileName}.gz`;
                ...
                if (response.ok) {
                    ...
                    console.warn(`[DataAccess] Cloudflare R2 fetch failed for ${fileName} with status ${response.status}`);
                }
            }
        } catch (error) {
...
async function getQuickStatsFromCache() {
    if (shouldUseSupabaseDbStats()) {
        ...
        // Optimization: Try to load quick_stats_history first and query only active keys
        try {

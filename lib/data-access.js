    ...
    for (const bucket of buckets) {
        ...
        for (const [category, val] of Object.entries(bucket.stats)) {
            ...
            if (Array.isArray(val.streaks)) {
                ...
                const qs = quickStats[patternKey] || {};
                statsRows.push({
                    pattern_key: patternKey,
                    category_type: bucket.type,
                    category: category,
                    subcategory: null,
                ...
                for (const streak of val.streaks) {
                    streaksRows.push({
                        pattern_key: patternKey,
                        category_type: bucket.type,
                        category: category,
                ...
                for (const [subcategory, subval] of Object.entries(val)) {
                    if (subval && Array.isArray(subval.streaks)) {
                        ...
                        for (const streak of subval.streaks) {
                            streaksRows.push({
                                pattern_key: patternKey,
                                category_type: bucket.type,
                                category: category,
        ...
        // Upsert statistics rows in batches of 500
        const statsBatchSize = 500;
        for (let i = 0; i < statsRows.length; i += statsBatchSize) {
            const batch = statsRows.slice(i, i + statsBatchSize);
            const { error } = await supabase
                .from('streak_statistics')
                .upsert(batch, { onConflict: 'pattern_key' });
            if (error) {
                console.error(`[SupabaseDB] Lỗi upsert statistics batch ${i}:`, error.message);
                throw error;
            }
        }
        console.log(`[SupabaseDB] Đã upsert ${statsRows.length} dòng thống kê cho ${bucket.type}.`);

        // Delete old streaks for this bucket first
        const { error: delError } = await supabase
            .from('historical_streaks')
            .delete()
            .eq('category_type', bucket.type);
        if (delError) {
            console.error(`[SupabaseDB] Lỗi xóa streaks cũ cho ${bucket.type}:`, delError.message);
            throw delError;
        }

        // Insert new streaks in batches of 2000
        ...
        for (let i = 0; i < streaksRows.length; i += streaksBatchSize) {
            ...
            const { error } = await supabase
                ...
                .insert(batch);
            if (error) {
                console.error(`[SupabaseDB] Lỗi insert streaks batch ${i}:`, error.message);
                throw error;
            }
        }
        console.log(`[SupabaseDB] Đã insert ${streaksRows.length} dòng historical streaks cho ${bucket.type}.`);
    }
...
async function savePatternStatsBatchToDb(categoryType, entries) {
    ...
    console.log(`[SupabaseDB] Đã upsert ${statsRows.length} dòng thống kê cho ${categoryType}.`);

    const patternKeys = [...new Set(entries.map(entry => entry.patternKey).filter(Boolean))];
    ...
    console.log(`[SupabaseDB] Đã xóa streaks cũ cho ${patternKeys.length} pattern ${categoryType}.`);

...
async function saveStatsToDb(statType, stats) {
    ...
    // as one single row to completely eliminate 'Sequential Scans' on cache_store keys
    // which were taking 60s and causing Vercel 504 timeouts.
    const key = `stats_full_${statType}`;
    console.log(`[DataAccess] Saving FAST full stats to DB: ${key}...`);

    let count = 0;
    // Minify stats before saving (saving 60% database space)
    for (const k in stats) {
        if (stats[k] && stats[k].streaks) {
            stats[k].streaks.forEach(s => {
                delete s.fullSequence; 
                delete s.values; 
                delete s.dates;
            });
            count++;
            ...
            for (const subKey in stats[k]) {
                if (stats[k][subKey] && stats[k][subKey].streaks) {
                    stats[k][subKey].streaks.forEach(s => {
                        delete s.fullSequence; 
                        delete s.values; 
                        delete s.dates;
                    });

...
async function saveStatsToDb(statType, stats) {
    ...
    console.log(`[DataAccess] Saving ${entries.length} ${statType} categories to DB...`);
    
    // Batch upsert (5 at a time to stay within payload limits)
    for (let i = 0; i < entries.length; i += 5) {
        const batch = entries.slice(i, i + 5).map(([category, data]) => ({
            key: `stats:${statType}:${category}`,
            data: data,
        ...
        if (error) {
            console.error(`[DataAccess] Error saving stats batch ${statType}[${i}]:`, error.message);
            throw error;
        }
    }
    console.log(`[DataAccess] Saved ${entries.length} ${statType} categories to DB`);
}

/**
 * Load stats từ Postgres cache_store
 * @param {string} statType - 'number', 'head_tail', 'sum_diff'
...
async function loadStatsFromDb(statType) {
    ...
    const { data, error } = await supabase
        .from('cache_store')
        .select('key, data')
        .like('key', `stats:${statType}:%`);
    
    if (error || !data || data.length === 0) {
        console.log(`[DataAccess] No stats found in DB for ${statType}`);
        return null;
    }
    
    const result = {};
    data.forEach(row => {
        const category = row.key.replace(prefix, '');
        result[category] = row.data;
    });
    console.log(`[DataAccess] Loaded ${Object.keys(result).length} ${statType} categories from DB`);
    return result;
}

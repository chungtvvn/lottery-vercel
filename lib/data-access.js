 ...
 * Tương đương: kết quả của statisticsService.getQuickStats()
 */
async function getQuickStatsFromCache() {
    ...
    if (error || !data) {
        console.error('[DataAccess] Quick stats cache miss or error:', error?.message);
        return null;
    }

    _quickStatsCache = data.data;
...
async function saveStatsToDb(statType, stats) {
    ...
    // 🔥 OPTIMIZATION FOR VERCEL + SUPABASE: 
    // Instead of hundreds of individual rows, we store the entire stats blob 
    // as one single row but heavily compressed to Tuples [startDate, endDate, length].
    const key = `stats_full_${statType}`;
    console.log(`[DataAccess] Saving FAST full stats to DB: ${key}...`);

    let count = 0;
    // Tuple compression stats before saving (saving 95% database space)
    ...
    const { error } = await admin
        ...
        .upsert({
            key: key,
            data: stats, // Should be <1MB now
            updated_at: new Date().toISOString()
        }, { onConflict: 'key' });
    
    if (error) {
        console.error(`[DataAccess] Error saving FAST stats ${statType}:`, error.message);
        throw error;
    }
    console.log(`[DataAccess] Saved ${count} ${statType} category chunks successfully to single row.`);
}
...
async function loadStatsFromDb(statType) {
    ...
    if (error || !data || !data.data) {
        console.log(`[DataAccess] FAST single-row DB absent for ${statType}`);
        return null;
    }
    ...
    for (const k in result) {
        if (result[k] && result[k].streaks) {
            result[k].streaks = result[k].streaks.map(t => ({ startDate: t[0], endDate: t[1], length: t[2] }));
        } else if (typeof result[k] === 'object') {
            for (const subKey in result[k]) {
                if (result[k][subKey] && result[k][subKey].streaks) {
                    result[k][subKey].streaks = result[k][subKey].streaks.map(t => ({ startDate: t[0], endDate: t[1], length: t[2] }));
                }
            }
        }
    }

    console.log(`[DataAccess] FAST-Loaded ${Object.keys(result).length} ${statType} categories from single DB row.`);
    return result;
}
...
async function saveCacheEntry(key, data) {
    ...
    console.log(`[DataAccess] Saved cache: ${key} (${JSON.stringify(data).length} bytes)`);
}

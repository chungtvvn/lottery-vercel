...
function isPredictionLockActive(targetDate, rawRows = [], now = new Date()) {
    if (!targetDate) {
        return {
            ...
            isSettled: false,
            lockActive: false,
            lockReason: 'Không xác định ngày dự đoán'
        };
    }
...
function preserveLockedRecommendation(existingRec, freshRec, lockStatus) {
    ...
    if (existingRec && (existingRec.numbers?.length || existingRec.selectedMethod || existingRec.top20?.length || existingRec.rankedNumbers?.length)) {
        return {
            ...existingRec,
            snapshotLock: {
                isLocked: true,

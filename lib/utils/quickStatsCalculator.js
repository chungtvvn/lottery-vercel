...
function shouldAttachPotential(stat, key, formLen, step) {
    ...
    // "Tiềm năng" is reserved for a pattern that has never completed in the
    // available history. A pattern with an existing historical streak is
    // either active or absent; treating a short precursor as potential would
    // duplicate established chains and inflate exclusion evidence.
    const historicalRecordLen = Number(stat.longest?.[0]?.length || 0);
    if (historicalRecordLen > 0) return false;

    // Callers build exactly the minDays - 1 precursor for each family:
    // regular=1/2, alternating=2/3, ABAB=3/4, and ordered=3/4.
    const prefixLen = Number(formLen) - Number(step);
    return prefixLen >= 1;
}

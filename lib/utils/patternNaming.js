...
function getCategoryName(category, subcategory, originalKey = null) {
    ...
    if (originalKey) {
        const directMapping = {
            ...
            'cacSoVeSoLeTheoThuTuTien': 'Các số - Về so le theo thứ tự TIẾN',
            'cacSoVeSoLeTheoThuTuLui': 'Các số - Về so le theo thứ tự LÙI',
            ...
            'cacDauVeSoLeTheoThuTuTien': 'Các Đầu - Về so le theo thứ tự TIẾN',
            'cacDauVeSoLeTheoThuTuLui': 'Các Đầu - Về so le theo thứ tự LÙI',
            'cacDitVeTheoThuTu': 'Các Đít - Về theo thứ tự',
            'cacDitVeSoLeTheoThuTu': 'Các Đít - Về so le theo thứ tự',
            'cacDitVeSoLeTheoThuTuTien': 'Các Đít - Về so le theo thứ tự TIẾN',
            'cacDitVeSoLeTheoThuTuLui': 'Các Đít - Về so le theo thứ tự LÙI',
            // 1 đầu/đít
    ...
    else if (category.startsWith('bo_')) {
        try {
            const { getBoGroupByKey } = require('./numberAnalysis');
            const group = getBoGroupByKey(category);
            catName = group
                ? group.displayLabel || `${group.label}: ${group.tokens.join(' - ')}`
                : `Bộ ${category.replace('bo_', '').toUpperCase()}`;
        } catch (e) {
            catName = `Bộ ${category.replace('bo_', '').toUpperCase()}`;
        }
    }
    // Composite patterns
    ...
    if (subcategory) {
        ...
        if (subcategory === 'block2x1SoLe') return `${catName} - Nhịp 2-1 so le block`;
        if (subcategory === 'block2x2SoLe') return `${catName} - Nhịp 2-2 so le block`;
        if (subcategory === 'block3x2SoLe') return `${catName} - Nhịp 3-2 so le block`;
        if (subcategory === 'block3x3SoLe') return `${catName} - Nhịp 3-3 so le block`;
        if (subcategory === 'block4x2SoLe') return `${catName} - Nhịp 4-2 so le block`;
        if (subcategory === 'block4x3SoLe') return `${catName} - Nhịp 4-3 so le block`;
        ...
        if (subcategory === 'veSoLeTheoThuTuTien') return `${catName} - Về so le theo thứ tự TIẾN${orderLabel ? ` ${orderLabel}` : ''}`;
        if (subcategory === 'veSoLeTheoThuTuLui') return `${catName} - Về so le theo thứ tự LÙI${orderLabel ? ` ${orderLabel}` : ''}`;

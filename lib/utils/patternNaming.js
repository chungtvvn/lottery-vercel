...
} = require('./blockAlternationPatterns');

const BLOCK_PATTERN_LABELS = new Map(
    BLOCK_ALTERNATION_PATTERNS.map(pattern => [pattern.subcategory, pattern.label])
);
...
function getCategoryName(category, subcategory, originalKey = null) {
    ...
    if (subcategory) {
        ...
        if (BLOCK_PATTERN_LABELS.has(subcategory)) {
            return `${catName} - ${BLOCK_PATTERN_LABELS.get(subcategory)}`;
        }

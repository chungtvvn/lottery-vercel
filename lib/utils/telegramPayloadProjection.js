...
const TELEGRAM_LOTO_COUNTS = [4, 6, 7, 8, 10, 20];
...
function compactPredictionHistoryTelegramRows(rows = []) {
    const sorted = [...(rows || [])].sort((a, b) => String(b.predictionDate || '').localeCompare(String(a.predictionDate || '')));
    return sorted.slice(0, 90).map(row => {
        const methods = {};

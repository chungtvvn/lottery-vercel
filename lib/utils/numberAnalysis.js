...
function buildThreeValueGroups(values, {
    excludedGroups = [],
    maxSpan = Infinity
} = {}) {
    ...
    for (let i = 0; i < values.length - 2; i++) {
        for (let j = i + 1; j < values.length - 1; j++) {
            for (let k = j + 1; k < values.length; k++) {
                ...
                if (excluded.has(groupSignature(group))) continue;
                if (group[group.length - 1] - group[0] > maxSpan) continue;
                groups.push(group);
            }
        }
    }
...
function buildMinGapThreeValueGroups(values, minGap = 2) {
    const groups = [];
    for (let i = 0; i < values.length - 2; i++) {
        for (let j = i + 1; j < values.length - 1; j++) {
            for (let k = j + 1; k < values.length; k++) {
                const group = [values[i], values[j], values[k]];
                if ((group[1] - group[0]) >= minGap && (group[2] - group[1]) >= minGap) {
                    groups.push(group);
                }
            }
        }
...
function withOrderedPermutationCategory(baseCategory, permutation) {
    return `${baseCategory}${ORDERED_PERMUTATION_SEPARATOR}${(permutation || []).map(String).join('_')}`;
}

function parseOrderedPermutationCategory(category = '') {
    const normalized = String(category || '');
    const index = normalized.lastIndexOf(ORDERED_PERMUTATION_SEPARATOR);
    if (index === -1) {
        return {
            baseCategory: normalized,
            orderValues: null
        };
    }
    const baseCategory = normalized.slice(0, index);
    const orderValues = normalized
        .slice(index + ORDERED_PERMUTATION_SEPARATOR.length)
        .split('_')
        .filter(Boolean);
    return {
        baseCategory,
        orderValues: orderValues.length > 0 ? orderValues : null
    };
}
...
// --- [MỚI] Phân loại Đồng Tiến/Lùi cách nhau 22, 33, 44, 55 đơn vị ---
// Step 22: cả 2 digit cùng thay đổi ±2 mỗi bước (VD: 02→24→46→68)
// Step 33: cả 2 digit cùng thay đổi ±3 (VD: 00→33→66→99)
// Step 44: cả 2 digit cùng thay đổi ±4 (VD: 00→44→88)
// Step 55: cả 2 digit cùng thay đổi ±5 (VD: 00→55, 44→99)
const ALL_DONG_STEP = {};
[22, 33, 44, 55].forEach(step => {
    for (let start = 0; start < step && start <= 99; start++) {
        const numbers = [];
        for (let n = start; n <= 99; n += step) {
            numbers.push(n.toString().padStart(2, '0'));
        }
        if (numbers.length >= 2) { // Chỉ tạo bộ có >= 2 số
            const setKey = `DONG_STEP_${step}_${start}`;
            ALL_DONG_STEP[setKey] = numbers;
        }
    }
});

// --- Phân loại theo Bộ truyền thống trong ảnh ---
const ALL_BO_GROUPS = {};
BO_GROUPS.forEach(group => {
    ALL_BO_GROUPS[group.setKey] = group.numbers;
});

// --- Phân loại Đầu/Đít 3 số cố định ---
// VD: Đầu 2,3,5 = số có chữ số hàng chục là 2, 3 hoặc 5.
// Nhóm liên tiếp 0,1,2 / 8,9,0 cũng cần thống kê riêng cho dạng về theo thứ tự và so le theo thứ tự.
// Nhóm ghép không liên tiếp vẫn được giữ riêng để tránh rơi dự đoán các bộ như 0,4,9.
const ALL_DAU_3_DIGIT = {};
const ALL_DIT_3_DIGIT = {};
const DIGIT_VALUES = Array.from({ length: 10 }, (_, i) => i);
const CONSECUTIVE_3_DIGIT_GROUPS = buildCyclicWindows(DIGIT_VALUES, 3);
const VALID_3_DIGIT_GROUPS = buildThreeValueGroups(DIGIT_VALUES, {
    excludedGroups: CONSECUTIVE_3_DIGIT_GROUPS
});
const ALL_3_DIGIT_GROUPS = mergeGroups(CONSECUTIVE_3_DIGIT_GROUPS, VALID_3_DIGIT_GROUPS);

ALL_3_DIGIT_GROUPS.forEach(group => {
    const groupKey = group.join('_');

    // Đầu: số có chữ số hàng chục thuộc nhóm
    ALL_DAU_3_DIGIT[`DAU_3D_${groupKey}`] = ALL_NUMBERS.filter(
        n => group.includes(parseInt(n[0], 10))
    );

    // Đít: số có chữ số hàng đơn vị thuộc nhóm
    ALL_DIT_3_DIGIT[`DIT_3D_${groupKey}`] = ALL_NUMBERS.filter(
        n => group.includes(parseInt(n[1], 10))
    );
});
...
// --- [MỚI] Dạng Chẵn/Lẻ liên tiếp theo thứ tự (Pattern Sequence) ---
const PARITY_TYPES = ['CC', 'CL', 'LC', 'LL'];
const PARITY_MAPS_RAW = {
    CC: CHAN_CHAN, CL: CHAN_LE, LC: LE_CHAN, LL: LE_LE
};

// --- [MỚI] Dạng CC/CL/LC/LL kết hợp Tổng/Hiệu chẵn/lẻ ---
const PARITY_SUM_DIFF = {};
['CHAN_CHAN', 'CHAN_LE', 'LE_CHAN', 'LE_LE'].forEach(parityKey => {
    const paritySet = parityKey === 'CHAN_CHAN' ? CHAN_CHAN :
                      parityKey === 'CHAN_LE' ? CHAN_LE :
                      parityKey === 'LE_CHAN' ? LE_CHAN : LE_LE;
    PARITY_SUM_DIFF[`${parityKey}_TONG_TT_CHAN`] = paritySet.filter(n => getTongTT(n) % 2 === 0 || getTongTT(n) === 10);
    PARITY_SUM_DIFF[`${parityKey}_TONG_TT_LE`] = paritySet.filter(n => getTongTT(n) % 2 !== 0);
    PARITY_SUM_DIFF[`${parityKey}_TONG_MOI_CHAN`] = paritySet.filter(n => getTongMoi(n) % 2 === 0);
    PARITY_SUM_DIFF[`${parityKey}_TONG_MOI_LE`] = paritySet.filter(n => getTongMoi(n) % 2 !== 0);
    PARITY_SUM_DIFF[`${parityKey}_HIEU_CHAN`] = paritySet.filter(n => getHieu(n) % 2 === 0);
    PARITY_SUM_DIFF[`${parityKey}_HIEU_LE`] = paritySet.filter(n => getHieu(n) % 2 !== 0);
});
...
// --- Nhóm 3 giá trị cụ thể không phải cửa sổ liên tiếp ---
const TONG_TT_VALUE_SETS = {
    1: TONG_TT_1, 2: TONG_TT_2, 3: TONG_TT_3, 4: TONG_TT_4, 5: TONG_TT_5,
    6: TONG_TT_6, 7: TONG_TT_7, 8: TONG_TT_8, 9: TONG_TT_9, 10: TONG_TT_10
};
const TONG_MOI_VALUE_SETS = {
    0: TONG_MOI_0, 1: TONG_MOI_1, 2: TONG_MOI_2, 3: TONG_MOI_3, 4: TONG_MOI_4,
    5: TONG_MOI_5, 6: TONG_MOI_6, 7: TONG_MOI_7, 8: TONG_MOI_8, 9: TONG_MOI_9,
    10: TONG_MOI_10, 11: TONG_MOI_11, 12: TONG_MOI_12, 13: TONG_MOI_13, 14: TONG_MOI_14,
    15: TONG_MOI_15, 16: TONG_MOI_16, 17: TONG_MOI_17, 18: TONG_MOI_18
};
const HIEU_VALUE_SETS = {
    0: HIEU_0, 1: HIEU_1, 2: HIEU_2, 3: HIEU_3, 4: HIEU_4,
    5: HIEU_5, 6: HIEU_6, 7: HIEU_7, 8: HIEU_8, 9: HIEU_9
};

const TONG_TT_VALUES = Array.from({ length: 10 }, (_, i) => i + 1);
const CONSECUTIVE_TONG_TT_3_VALUE_CATEGORIES = [
    'tong_tt_1_3', 'tong_tt_2_4', 'tong_tt_3_5', 'tong_tt_4_6', 'tong_tt_5_7',
    'tong_tt_6_8', 'tong_tt_7_9', 'tong_tt_8_10', 'tong_tt_9_1', 'tong_tt_10_2'
];
const VALID_TONG_TT_3_VALUE_GROUPS = buildThreeValueGroups(TONG_TT_VALUES, {
    excludedGroups: buildCyclicWindows(TONG_TT_VALUES, 3)
});

const TONG_MOI_VALUES = Array.from({ length: 19 }, (_, i) => i);
const CONSECUTIVE_TONG_MOI_3_VALUE_CATEGORIES = [
    'tong_moi_0_2', 'tong_moi_1_3', 'tong_moi_2_4', 'tong_moi_3_5', 'tong_moi_4_6',
    'tong_moi_5_7', 'tong_moi_6_8', 'tong_moi_7_9', 'tong_moi_8_10', 'tong_moi_9_11',
    'tong_moi_10_12', 'tong_moi_11_13', 'tong_moi_12_14', 'tong_moi_13_15',
    'tong_moi_14_16', 'tong_moi_15_17', 'tong_moi_16_18', 'tong_moi_17_0',
    'tong_moi_18_1'
];
const VALID_TONG_MOI_3_VALUE_GROUPS = buildThreeValueGroups(
    TONG_MOI_VALUES,
    {
        excludedGroups: buildCyclicWindows(TONG_MOI_VALUES, 3)
    }
);

const HIEU_VALUES = Array.from({ length: 10 }, (_, i) => i);
const CONSECUTIVE_HIEU_3_VALUE_CATEGORIES = [
    'hieu_0_2', 'hieu_1_3', 'hieu_2_4', 'hieu_3_5', 'hieu_4_6',
    'hieu_5_7', 'hieu_6_8', 'hieu_7_9', 'hieu_8_0', 'hieu_9_1'
];
const VALID_HIEU_3_VALUE_GROUPS = buildThreeValueGroups(HIEU_VALUES, {
    excludedGroups: buildCyclicWindows(HIEU_VALUES, 3)
});

function buildMetricThreeValueSets(prefix, groups, valueSets) {
    const result = {};
    groups.forEach(group => {
        result[`${prefix}_${group.join('_')}`] = mergeNumberSets(...group.map(value => valueSets[value] || []));
    });
    return result;
}

const ALL_TONG_TT_3_VALUE_GROUPS = buildMetricThreeValueSets('TONG_TT', VALID_TONG_TT_3_VALUE_GROUPS, TONG_TT_VALUE_SETS);
const ALL_TONG_MOI_3_VALUE_GROUPS = buildMetricThreeValueSets('TONG_MOI', VALID_TONG_MOI_3_VALUE_GROUPS, TONG_MOI_VALUE_SETS);
const ALL_HIEU_3_VALUE_GROUPS = buildMetricThreeValueSets('HIEU', VALID_HIEU_3_VALUE_GROUPS, HIEU_VALUE_SETS);
...
const SETS = {
    ...
    // [MỚI] Đồng Tiến/Lùi cách 22, 33, 44, 55
    ...ALL_DONG_STEP,

    // [MỚI] Đầu/Đít 3 số ghép (cách nhau >= 2)
    ...ALL_DAU_3_DIGIT,
    ...ALL_DIT_3_DIGIT,

    // [MỚI] CC/CL/LC/LL + Tổng/Hiệu chẵn/lẻ
    ...PARITY_SUM_DIFF
};
...
function extractValueForComparison(numberStr, category) {
    ...
    if (category.startsWith('bo_')) {
        const group = getBoGroupByKey(category);
        return group ? group.numbers.indexOf(numberStr) : null;
    }
...
module.exports = {
    ...
    // [MỚI]
    CONSECUTIVE_3_DIGIT_GROUPS,
    VALID_3_DIGIT_GROUPS,
    ALL_3_DIGIT_GROUPS,
    ORDERED_PERMUTATION_SEPARATOR,
    buildPermutations,

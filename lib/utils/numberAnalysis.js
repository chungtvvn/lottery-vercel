...
// --- Phân loại Đầu/Đít 3 số ghép không trùng cửa sổ liên tiếp ---
// VD: Đầu 2,3,5 = số có chữ số hàng chục là 2, 3 hoặc 5.
// Loại các cửa sổ 3 số liên tiếp dạng 0,1,2 / 8,9,0 vì đã được phân tích bởi nhóm tiến/lùi.
const ALL_DAU_3_DIGIT = {};
const ALL_DIT_3_DIGIT = {};
const DIGIT_VALUES = Array.from({ length: 10 }, (_, i) => i);
const VALID_3_DIGIT_GROUPS = mergeGroups(
    buildMinGapThreeValueGroups(DIGIT_VALUES, 2),
    buildThreeValueGroups(DIGIT_VALUES, {
        excludedGroups: buildCyclicWindows(DIGIT_VALUES, 3),
        maxSpan: 3
    })
);

VALID_3_DIGIT_GROUPS.forEach(group => {
...
const VALID_TONG_TT_3_VALUE_GROUPS = mergeGroups(
    buildMinGapThreeValueGroups(TONG_TT_VALUES, 2),
    buildThreeValueGroups(TONG_TT_VALUES, {
        excludedGroups: buildCyclicWindows(TONG_TT_VALUES, 3),
        maxSpan: 3
    })
);

// Tổng Mới có 19 giá trị, nếu sinh mọi tổ hợp sẽ làm quick_stats/stat JSON phình rất mạnh.
// Nhóm mới chỉ lấy các cụm gần nhau nhưng không phải cửa sổ liên tiếp, ví dụ 2,3,5 hoặc 5,7,9.
...
const VALID_HIEU_3_VALUE_GROUPS = mergeGroups(
    buildMinGapThreeValueGroups(HIEU_VALUES, 2),
    buildThreeValueGroups(HIEU_VALUES, {

...
// --- [MỚI] Phân loại Đầu/Đít 3 số ghép (mỗi cặp cách nhau >= 2 đơn vị) ---
// VD: Đầu 1,3,8 = số có chữ số hàng chục là 1, 3 hoặc 8
// Quy tắc: chọn 3 chữ số (0-9), mỗi cặp cách nhau >= 2
const ALL_DAU_3_DIGIT = {};
const ALL_DIT_3_DIGIT = {};
const VALID_3_DIGIT_GROUPS = []; // Lưu lại các nhóm hợp lệ

for (let a = 0; a <= 7; a++) {
    for (let b = a + 2; b <= 8; b++) {
        for (let c = b + 2; c <= 9; c++) {
            // Kiểm tra tất cả các cặp cách nhau >= 2
            if ((b - a) >= 2 && (c - b) >= 2 && (c - a) >= 2) {
                const groupKey = `${a}_${b}_${c}`;
                VALID_3_DIGIT_GROUPS.push([a, b, c]);

                // Đầu: số có chữ số hàng chục thuộc nhóm
                ALL_DAU_3_DIGIT[`DAU_3D_${groupKey}`] = ALL_NUMBERS.filter(
                    n => [a, b, c].includes(parseInt(n[0]))
                );

                // Đít: số có chữ số hàng đơn vị thuộc nhóm
                ALL_DIT_3_DIGIT[`DIT_3D_${groupKey}`] = ALL_NUMBERS.filter(
                    n => [a, b, c].includes(parseInt(n[1]))
                );
            }
        }
    }

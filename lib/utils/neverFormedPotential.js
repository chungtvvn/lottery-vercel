...
const { BLOCK_ALTERNATION_PATTERNS } = require('./blockAlternationPatterns');
...
function buildPotentialCurrent(option, context) {
    ...
    const block = BLOCK_ALTERNATION_PATTERNS.find(pattern => pattern.subcategory === subcategory);
    if (block) {
        const minDays = block.aLength + block.bLength + block.aLength;
        const prefixDays = minDays - 1;
        const recentDays = Array.isArray(context.recentDays) ? context.recentDays.slice(-prefixDays) : [];
        const recentMatchedDays = Array.isArray(context.recentMatchedDays)
            ? context.recentMatchedDays.slice(-prefixDays)
            : [];
        if (recentDays.length !== prefixDays) return null;
        const expected = [
            ...Array(block.aLength).fill(true),
            ...Array(block.bLength).fill(false),
            ...Array(block.aLength - 1).fill(true)
        ];
        const values = recentDays.map(day => normalizeNumberString(day?.special));
        if (values.some(value => !value)) return null;
        const matches = values.map((value, index) => {
            const matched = recentMatchedDays[index];
            return Array.isArray(matched)
                ? matched.includes(category)
                : identifyCategories(value).includes(category);
        });
        if (!matches.every((value, index) => value === expected[index])) return null;
        const dates = recentDays.map(day => day?.date).filter(Boolean);
        return {
            length: prefixDays,
            startDate: dates[0],
            endDate: dates.at(-1),
            values,
            dates,
            fullSequence: values.map((value, index) => ({
                date: dates[index],
                value,
...
function augmentNeverFormedPotentialStreaks(quickStats, context = {}) {
    ...
    const formedPatternKeys = context.formedPatternKeys instanceof Set
        ? context.formedPatternKeys
        : new Set(context.formedPatternKeys || Object.keys(quickStats).filter(key => key !== '_meta'));
    ...
    const relevantCategories = new Set([
        ...contextSets.today,
        ...contextSets.yesterday,
        ...contextSets.dayBefore
    ]);
    const candidateOptions = getStatsOptionCandidates(relevantCategories);

    timingLoopStart = debugTimings ? Date.now() : 0;
    let neverFormedCandidateCount = 0;
    let prefilteredCount = 0;
    let currentCount = 0;
    let attachedCount = 0;
    for (const option of candidateOptions) {
        if (option.status === 'invalid' || formedPatternKeys.has(option.key) || quickStats[option.key]) continue;
        neverFormedCandidateCount += 1;
        if (!prefilterPotentialOption(option, contextSets)) continue;
        prefilteredCount += 1;
        const current = buildPotentialCurrent(option, {
            ...context,
            recentDays: (context.recentDays || []),
            recentMatchedDays: (context.recentMatchedDays || [])
        });
        if (!current) continue;
        currentCount += 1;
    ...
    if (debugTimings) {
        console.error('[NeverFormedTiming]', {
            ...
            neverFormedCandidates: neverFormedCandidateCount,

...
function createRollingFeatureEngine(rows, catalog, options = {}) {
    ...
    function hazardFeature() {
        const values = NUMBERS.map(number => {
            ...
            let weibullHazard = posterior;
            if (completedGaps.length >= 3) {
                const meanGap = completedGaps.reduce((a, b) => a + b, 0) / completedGaps.length;
                const varGap = completedGaps.reduce((a, b) => a + (b - meanGap) ** 2, 0) / completedGaps.length;
                const stdGap = Math.sqrt(Math.max(1, varGap));
                const shapeK = Math.max(0.6, Math.min(3.0, (stdGap / Math.max(1, meanGap)) ** -1.086));
                const scaleLambda = Math.max(10, meanGap);
                const rawHazard = (shapeK / scaleLambda) * Math.pow((currentGap + 1) / scaleLambda, shapeK - 1);
                weibullHazard = clamp(rawHazard, 0.001, 0.25);
            }
            ...
            return {
                ...
                atRisk,
                events,
                weibullHazard: Number(weibullHazard.toFixed(5)),
...
function rankNumbers(model, chainConsensus = null, betCount = 30) {
    ...
    const ranked = NUMBERS.map(number => {
        ...
        const probability = model.prediction.probabilities[number];
        const evIndex = Number(((probability * 84 - 1) * 100).toFixed(2));
        return {
            number,
            score: Number(score.toFixed(2)),
            band: 'D',
            probability: Number(probability.toFixed(6)),
            expectedValueEV: evIndex,
            ...
            evidence: {
                ...
                gapHazard: {
                    ...
                    weibullHazard: row.hazard.weibullHazard,

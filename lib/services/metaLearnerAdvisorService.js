...
function buildMetaLearnerAdvisor(historyPayload, rawRows, options = {}) {
    ...
    const existingMeta = options.existingMetaLearner || options.existingAdvisorCache?.metaLearner || advisorCache?.metaLearner || null;
    const existingSettled = existingMeta?.settledLedger || [];
    const prevRec = existingMeta?.latestRecommendation || null;
    ...
    const LOCKED_LIVE_DE_SNAPSHOTS = {
        '2026-09-20': {
            numbers: [0, 1, 2, 6, 10, 11, 15, 20, 24, 31, 33, 39, 40, 48, 51, 57, 60, 66, 71, 75, 79, 80, 84, 86, 88, 91, 93, 95, 97, 99],
    ...
    const canIncremental = existingSettled.length >= 30 && !options.forceSynthesize;

    if (canIncremental) {
        // FAST INCREMENTAL PATH: 100% Immutable Snapshots
        settledLedger = existingSettled.map(r => {
            const entryDate = r.date || r.predictionDate;
            const lockedSnap = LOCKED_LIVE_DE_SNAPSHOTS[entryDate];
            if (lockedSnap) {
                const numbers = lockedSnap.numbers;
                const actual = r.actual !== undefined ? r.actual : (r.actualSpecial !== undefined ? r.actualSpecial : null);
                const isHit = actual !== null && numbers.map(Number).includes(Number(actual));
                const stakeK = 30000;
            ...
            return { ...r, isLocked: true, settled: true };
        });
        const settledDates = new Set(settledLedger.map(r => r.date || r.predictionDate));
        const latestRawDate = rawRows?.at(-1)?.date ? String(rawRows.at(-1).date).slice(0, 10) : '';

        for (const row of (rawRows || [])) {
            const drawDate = String(row.date || row.ngay || '').slice(0, 10);
            if (!drawDate || drawDate < '2026-01-01' || settledDates.has(drawDate)) continue;

            const rawActualStr = String(row.actual !== undefined ? row.actual : (row.special ?? row.db ?? row.giaiDb ?? row.giai_dac_biet ?? '')).trim();
            const actual = /^\d+$/.test(rawActualStr) ? Number(rawActualStr.slice(-2)) : null;
            if (actual === null) continue;

            let numbers = [];
            if (LOCKED_LIVE_DE_SNAPSHOTS[drawDate]) {
                numbers = LOCKED_LIVE_DE_SNAPSHOTS[drawDate].numbers;
            } else if (prevRec && prevRec.predictionDate === drawDate) {
                numbers = prevRec.standard30 || prevRec.numbers || [];
            } else {
                const d = (metaPolicy?.decisions || []).find(x => x.date === drawDate);
                numbers = Array.isArray(d?.numbers) ? d.numbers : [];
            }

            const isHit = actual !== null && numbers.map(Number).includes(actual);
            const isLive = drawDate >= '2026-08-28';
            const stakeK = 30000;
            const payoutK = isHit ? 84000 : 0;
            const profitK = payoutK - stakeK;
            ...
            settledLedger.push({
                ...
                payoutK,
                profitK,
                liveCumulativeProfitK: null,
                cumulativeProfitK: null,
        ...
        settledLedger.forEach(r => {
            const isLive = r.isLiveSnapshot || r.sourceType === 'live-snapshot' || r.date >= '2026-08-28';
            if (isLive) {
        ...
        // Determine target prediction date
        const targetDate = analysis.predictionDate || (advisorCache?.records?.at(-1)?.predictionDate);
        if (prevRec && prevRec.predictionDate === targetDate && (prevRec.standard30 || prevRec.numbers) && !options.forceSynthesize) {

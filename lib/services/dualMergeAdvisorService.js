...
let _memoizedHistoricalBaseline = new Map();
function getBaseline2026Map(targetDate = null) {
    try {
        ensureLotteryStatsLoaded();
        const annual = require('./annualMilestoneService');
        const lotteryService = require('./lotteryService');
        const raw = lotteryService.getRawData() || [];
        const latestDate = raw.length > 0 ? String(raw[raw.length - 1].date || '').slice(0, 10) : '2026-09-23';

        let cutoffDate = latestDate;
        if (targetDate) {
            const d = new Date(String(targetDate).slice(0, 10));
            d.setDate(d.getDate() - 1);
            cutoffDate = d.toISOString().slice(0, 10);
            if (cutoffDate > latestDate) cutoffDate = latestDate;
        }

        if (_memoizedHistoricalBaseline.has(cutoffDate)) {
            return _memoizedHistoricalBaseline.get(cutoffDate);
        }

        const year = Number(cutoffDate.slice(0, 4)) || 2026;
        const baseline = annual.ensureAnnualBaseline(null, year, {
            cutoffIso: cutoffDate,
            cutoffDate: cutoffDate
        });
        if (baseline) {
            _memoizedHistoricalBaseline.set(cutoffDate, baseline);
            return baseline;
        }
    } catch (_) {}
    return null;
        ...
        if (targetPredictionDate) {
            ...
            if (prevRec && prevRec.predictionDate === targetPredictionDate && Array.isArray(prevIntersection) && !options.forceSynthesize) {
                ...
                if (Object.keys(candidateMethods).length < 2) {
                    try {
                        const annual = require('./annualMilestoneService');
                        const baselineMap = getBaseline2026Map(targetPredictionDate);
                        if (baselineMap) {

...
function selectBestMethodPair(priorSettledRuns, candidateMethodsMap, predictionDate, options = {}) {
    ...
    const plainReasons = [
        ...
        `🎯 Vùng đồng thuận Vàng X2 (${champion.overlapCount} số trùng · Jaccard ${jaccardPercent}%): Cả 2 thuật toán độc lập cùng chỉ ra ${champion.overlapCount} số có xác suất về cao nhất hôm nay. Tỷ lệ cùng trúng trong lịch sử đạt ${bothPercent}%, cho phép dồn trọng số cược X2 (2K/số) để ăn lãi tối đa +108K.`,
        `🛡️ Mạng lưới bọc lót an toàn X1 (${champion.uniqueSingles.length} số riêng): ${champion.uniqueSingles.length} số riêng biệt tạo thành tấm chắn bảo hiểm (Độ phủ dàn hợp đạt ${unionPercent}%). Nếu chỉ 1 trong 2 phương pháp trúng, hệ thống vẫn đạt giải X1 (84K) và sinh lãi dương +24K.`,
        `💰 Quản trị vốn & Tối ưu kinh tế (Cố định 60K/ngày): 25 số trùng × 2K (50K) + 10 số riêng × 1K (10K) = 60K vốn. Khi trúng X2 lãi ròng +108K (ROI +180%), khi trúng X1 lãi ròng +24K (ROI +40%), rủi ro mất vốn được triệt tiêu ở mức tối thiểu.`,

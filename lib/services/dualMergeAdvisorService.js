...
const METHOD_LABELS = {
    dedupEdge75Hold70: 'Edge75 PIT (Mốc Lịch sử D-1)',
...
function selectBestMethodPair(priorSettledRuns, candidateMethodsMap, predictionDate, options = {}) {
    ...
    for (let i = 0; i < scoredMethods.length; i++) {
        for (let j = i + 1; j < scoredMethods.length; j++) {
            ...
            // Dynamic rotation penalty: if this exact pair lost yesterday, force rotation to another pair!
            let rotationPenalty = 1.0;
            if (options.lastChosenPairKey && pairKey === options.lastChosenPairKey && options.consecutivePairLosses >= 1) {
                rotationPenalty = Math.max(0.15, Math.exp(-0.9 * options.consecutivePairLosses));
            }

            // Organic Complementary Sweet Spot:
            // Sweet spot: 16 to 26 numbers overlap (high X2 confidence on intersection + robust X1 safety net on singles)
            let overlapFactor = 1.0;
            if (overlapCount === 30) {
                overlapFactor = 0.80; // penalty for identical clones (no complementary safety net)
            } else if (overlapCount >= 16 && overlapCount <= 26) {
            ...
            if (recentSystemLossStreak >= 2) {
                ...
                compositeScore = (unionHitRate * 80 + jointHitRate * 108 * 1.5 + singleHitRate * 35 + pairRhythm * 50) * overlapFactor * rotationPenalty * m1Penalty * m2Penalty;
            } else {
                // Offensive / Hot-Rhythm X2 Profit Maximizer Regime:
                compositeScore = (jointHitRate * 108 * 2.2 + singleHitRate * 24 + unionHitRate * 15 + pairRhythm * 50) * overlapFactor * rotationPenalty * m1Penalty * m2Penalty;
            }

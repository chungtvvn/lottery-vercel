...
const METHOD_LABELS = {
    dedupEdge75PitHold70: 'Edge75 PIT (Mốc Lịch sử D-1)',
    ...
    deParallelBlock85Small65Hold70: 'Đề Song Song Lịch sử (Block 85 · Small 65)'
};
...
function selectBestMethodPair(priorSettledRuns, candidateMethodsMap, predictionDate, options = {}) {
    ...
    const plainReasons = [
        ...
        `🎯 Vùng đồng thuận Vàng X2 (${champion.overlapCount} số trùng · Jaccard ${jaccardPercent}%): Cả 2 thuật toán độc lập cùng chỉ ra ${champion.overlapCount} số có xác suất về cao nhất hôm nay. Tỷ lệ cùng trúng trong lịch sử đạt ${bothPercent}%, cho phép dồn trọng số cược X2 (0.002M/số) để ăn lãi tối đa +0.108M.`,
        `🛡️ Mạng lưới bọc lót an toàn X1 (${champion.uniqueSingles.length} số riêng): ${champion.uniqueSingles.length} số riêng biệt tạo thành tấm chắn bảo hiểm (Độ phủ dàn hợp đạt ${unionPercent}%). Nếu chỉ 1 trong 2 phương pháp trúng, hệ thống vẫn đạt giải X1 (0.084M) và sinh lãi dương +0.024M.`,
        `💰 Quản trị vốn & Tối ưu kinh tế (Cố định 0.060M/ngày): 25 số trùng × 0.002M (0.050M) + 10 số riêng × 0.001M (0.010M) = 0.060M vốn. Khi trúng X2 lãi ròng +0.108M (ROI +180%), khi trúng X1 lãi ròng +0.024M (ROI +40%), rủi ro mất vốn được triệt tiêu ở mức tối thiểu.`,

...
function buildDualMergeAdvisor(historyRecords, rawRows, options = {}) {
    ...
    if (options.existingDualMerge && Array.isArray(options.existingDualMerge.settledLedger) && options.existingDualMerge.settledLedger.length >= 200 && !options.forceSynthesize) {
        ...
        if (prevRec && prevRec.predictionDate && latestRawDate && prevRec.predictionDate <= latestRawDate && !settledLedger.some(r => r.date === prevRec.predictionDate)) {
            ...
            if (Number.isInteger(actualVal)) {
                const isX2 = Array.isArray(prevRec.intersectionX2) && prevRec.intersectionX2.includes(actualVal);
                const isX1 = Array.isArray(prevRec.uniqueSinglesX1) && prevRec.uniqueSinglesX1.includes(actualVal);
                ...
                settledLedger.push({
                    ...
                    overlapCount: prevRec.overlapCount || 0,

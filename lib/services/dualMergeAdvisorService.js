...
function selectBestMethodPair(priorSettledRuns, candidateMethodsMap, predictionDate, options = {}) {
    ...
    for (let i = 0; i < scoredMethods.length; i++) {
        for (let j = i + 1; j < scoredMethods.length; j++) {
            ...
            const normalizedProfitScore = (profitRounds30 * 0.50) + (profitRounds15 * 0.30) + (profitRounds7 * 0.20);
            ...
            // Sweet spot: 22 to 27 numbers overlap (high X2 confidence on intersection + robust X1 safety net on singles)
            let overlapFactor = 1.0;
            if (overlapCount >= 22 && overlapCount <= 27) {
                overlapFactor = 1.35; // boost complementary sweet spot
            } else if (overlapCount >= 28) {
                overlapFactor = 0.90; // moderate damping on degenerate identical pairs
            } else if (overlapCount < 16) {
            ...
            if (Number.isInteger(prevAct) && prevAct >= 0 && prevAct < 100 && intersection.length > 0) {
                ...
                formResonanceFactor = 0.95 + (resonanceRatio * 0.15);
            }

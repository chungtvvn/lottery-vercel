...
function selectBestMethodPair(priorSettledRuns, candidateMethodsMap, predictionDate) {
    ...
    for (let i = 0; i < scoredMethods.length; i++) {
        for (let j = i + 1; j < scoredMethods.length; j++) {
            ...
            const jointHitRate = (comp7.total >= 3 ? (comp7.bothHitRate || 0) * 0.50 : 0) +
                (comp15.total >= 5 ? (comp15.bothHitRate || 0) * 0.30 : 0) +
                (comp30.total >= 10 ? (comp30.bothHitRate || 0) * 0.20 : 0.15);

            // Estimated single hit rate (bọc lót)
            const singleHitRate = (comp7.total >= 3 ? (comp7.compRate || 0) * 0.40 : 0) +
                (comp15.total >= 5 ? (comp15.compRate || 0) * 0.35 : 0) +
                (comp30.total >= 10 ? (comp30.compRate || 0) * 0.25 : 0.20);

            // Exponential Overlap Weighting for X2 payload density: higher shared numbers = higher X2 concentration
            const overlapFactor = Math.pow(overlapCount / 30, 2.2);

            // Expected Profit Payoff: Asymmetrically rewarding X2 (+108K) over X1 (+24K)
            const compositeScore = (jointHitRate * 108 * 2.2 + singleHitRate * 24) * overlapFactor;

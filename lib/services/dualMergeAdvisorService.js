...
function buildDualMergeAdvisor(historyRecords, rawRows, options = {}) {
    ...
    if (options.existingDualMerge && Array.isArray(options.existingDualMerge.settledLedger) && options.existingDualMerge.settledLedger.length >= 200 && !options.forceSynthesize) {
        ...
        for (const drawDate of allDrawDates) {
            ...
            let champ = null;
            if (prevRec && prevRec.predictionDate === drawDate && Array.isArray(prevRec.intersectionX2)) {
                champ = {
                    m1: prevRec.m1,
                    m1Label: prevRec.m1Label,
                    m2: prevRec.m2,
                    m2Label: prevRec.m2Label,
                    intersection: prevRec.intersectionX2,
                    uniqueSingles: prevRec.uniqueSinglesX1,
                    union: prevRec.fullUnion,
                    overlapCount: prevRec.overlapCount,
                    totalNumbers: prevRec.totalNumbersCount || prevRec.fullUnion?.length,

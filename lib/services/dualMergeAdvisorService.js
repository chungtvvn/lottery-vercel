...
function selectBestMethodPair(priorSettledRuns, candidateMethodsMap, predictionDate) {
    ...
    for (let i = 0; i < scoredMethods.length; i++) {
        for (let j = i + 1; j < scoredMethods.length; j++) {
            ...
            // Short-term multi-horizon union coverage (7, 15, 30 days)
            const shortUnionRate = (comp7.total >= 3 ? comp7.unionRate * 0.40 : 0) +
                (comp15.total >= 5 ? comp15.unionRate * 0.35 : 0) +
                (comp30.total >= 10 ? comp30.unionRate * 0.25 : 0.20);

            // Short-term momentum surge (7 & 15 days hit rates of both methods)
            const m1Momentum = (m1.s7.days >= 3 ? m1.s7.hitRate * 0.45 : 0) +
                (m1.s15.days >= 5 ? m1.s15.hitRate * 0.35 : 0) +
                (m1.s30.days >= 10 ? m1.s30.hitRate * 0.20 : 0.30);
            const m2Momentum = (m2.s7.days >= 3 ? m2.s7.hitRate * 0.45 : 0) +
                (m2.s15.days >= 5 ? m2.s15.hitRate * 0.35 : 0) +
                (m2.s30.days >= 10 ? m2.s30.hitRate * 0.20 : 0.30);
            const avgMomentum = (m1Momentum + m2Momentum) / 2;

            // Mean reversion rebound: if one method missed 1-2 draws, reward mean reversion
            const reversionBonus = (m1.s7.streakLoss === 1 || m1.s7.streakLoss === 2 ? 0.08 : 0) +
                (m2.s7.streakLoss === 1 || m2.s7.streakLoss === 2 ? 0.08 : 0);

            const compositeScore = (avgMomentum * 0.45 + shortUnionRate * 0.40 + reversionBonus * 0.15) * overlapMultiplier;
            ...
            pairs.push({
                ...
                pairScore,
                avgScore: Number(avgMomentum.toFixed(6)),
                comp30,
                comp15,
                comp7
            });
    ...
    const plainReasons = [
        ...
        `Tối ưu đan xen bù trừ ngắn hạn: Tỷ lệ ít nhất 1 phương pháp trúng 30 kỳ đạt ${unionPercent}% (Bù trừ luân phiên ${compPercent}%).`,

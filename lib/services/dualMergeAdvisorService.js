...
function selectBestMethodPair(priorSettledRuns, candidateMethodsMap, predictionDate, options = {}) {
    ...
    for (let i = 0; i < scoredMethods.length; i++) {
        for (let j = i + 1; j < scoredMethods.length; j++) {
            ...
            // STRICT COMPLEMENTARITY: Disqualify redundant pairs (e.g. 100% duplicate sets > 26 numbers) or overly disjoint pairs (< 12 numbers)
            if (overlapCount < 12 || overlapCount > 26) {
                continue;
            }
            ...
            // Trailing Profit & Multi-Horizon Momentum
            const profit30 = comp30.total >= 5 ? comp30.profitK : 0;
            const profit15 = comp15.total >= 3 ? comp15.profitK : 0;
            const profit7 = comp7.total >= 2 ? comp7.profitK : 0;
            const trailingProfitScore = (profit30 * 0.50) + (profit15 * 0.30) + (profit7 * 0.20);
            ...
            const jointHitRate = (comp7.total >= 3 ? (comp7.bothHitRate || 0) * 0.45 : 0) +
                ...
                (comp30.total >= 10 ? (comp30.bothHitRate || 0) * 0.20 : (overlapCount / 100) * 0.20);
            ...
            const singleHitRate = (comp7.total >= 3 ? (comp7.compRate || 0) * 0.45 : 0) +
                ...
                (comp30.total >= 10 ? (comp30.compRate || 0) * 0.20 : (uniqueSingles.length / 100) * 0.20);
            ...
            const unionHitRate = (comp7.total >= 3 ? (comp7.unionRate || 0) * 0.50 : 0) +
                ...
                (comp30.total >= 10 ? (comp30.unionRate || 0) * 0.20 : (union.length / 100) * 0.20);

            // Streak decay penalties for cold individual methods
            const m1Loss = m1.s7?.streakLoss || 0;
            const m2Loss = m2.s7?.streakLoss || 0;
            const m1Penalty = Math.max(0.70, 1 - m1Loss * 0.05);
            const m2Penalty = Math.max(0.70, 1 - m2Loss * 0.05);

            // Gentle rotation stability (prevent thrashing away from champions on single random variance)
            let rotationPenalty = 1.0;
            if (options.lastChosenPairKey && pairKey === options.lastChosenPairKey && options.consecutivePairLosses >= 3) {
                rotationPenalty = 0.85;
            }

            // Organic Complementary Sweet Spot:
            // Sweet spot: 16 to 24 numbers overlap (high X2 confidence on intersection + robust X1 safety net on singles)
            let overlapFactor = 1.0;
            if (overlapCount >= 16 && overlapCount <= 24) {
                overlapFactor = 1.35; // boost complementary sweet spot
            } else if (overlapCount < 14) {
                overlapFactor = Math.max(0.50, Math.pow(overlapCount / 20, 1.2));
            }
            ...
            if (recentSystemLossStreak >= 2) {
                ...
                compositeScore = (trailingProfitScore * 0.04 + unionHitRate * 80 + jointHitRate * 108 * 1.5 + singleHitRate * 35 + pairRhythm * 50) * overlapFactor * rotationPenalty * m1Penalty * m2Penalty;
            } else {
                // Offensive / Hot-Rhythm X2 Profit Maximizer Regime:
                compositeScore = (trailingProfitScore * 0.05 + jointHitRate * 108 * 2.2 + singleHitRate * 24 + unionHitRate * 25 + pairRhythm * 50) * overlapFactor * rotationPenalty * m1Penalty * m2Penalty;
            }
    ...
    const plainReasons = [
        `🏆 Lựa chọn cặp phương pháp dẫn đầu: [${champion.m1Label}] và [${champion.m2Label}] đạt điểm số tín hiệu tổng hợp cao nhất (${champion.pairScore} điểm) trong toàn bộ 21 cặp tổ hợp.`,
        ...
        `🎯 Vùng đồng thuận Vàng X2 (${champion.overlapCount} số trùng · Jaccard ${jaccardPercent}%): Cả 2 thuật toán độc lập cùng chỉ ra ${champion.overlapCount} số có xác suất về cao nhất hôm nay. Tỷ lệ cùng trúng trong lịch sử đạt ${bothPercent}%, cho phép dồn trọng số cược X2 (2K/số) để ăn lãi tối đa +108.000đ.`,
        `🛡️ Mạng lưới bọc lót an toàn X1 (${champion.uniqueSingles.length} số riêng): ${champion.uniqueSingles.length} số riêng biệt tạo thành tấm chắn bảo hiểm (Độ phủ dàn hợp đạt ${unionPercent}%). Nếu chỉ 1 trong 2 phương pháp trúng, hệ thống vẫn đạt giải X1 (84K) và sinh lãi dương +24.000đ.`,
        `💰 Quản trị vốn & Tối ưu kinh tế (Cố định 60.000đ/ngày): 25 số trùng × 2.000đ (50K) + 10 số riêng × 1.000đ (10K) = 60K vốn. Khi trúng X2 lãi ròng +108K (ROI +180%), khi trúng X1 lãi ròng +24K (ROI +40%), rủi ro mất vốn được triệt tiêu ở mức tối thiểu.`,

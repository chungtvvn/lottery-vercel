    ...
    for (let i = 0; i < scoredMethods.length; i++) {
        for (let j = i + 1; j < scoredMethods.length; j++) {
            ...
            if (recentSystemLossStreak >= 2) {
                ...
                compositeScore = (normalizedProfitScore * 10.0 + unionHitRate * 80 + jointHitRate * 108 * 1.5 + singleHitRate * 35 + pairRhythm * 50) * overlapFactor * rotationPenalty * m1Penalty * m2Penalty;
            } else {
                // Offensive / Hot-Rhythm X2 Profit Maximizer Regime:
                compositeScore = (normalizedProfitScore * 12.0 + jointHitRate * 108 * 2.2 + singleHitRate * 24 + unionHitRate * 25 + pairRhythm * 50) * overlapFactor * rotationPenalty * m1Penalty * m2Penalty;
            }

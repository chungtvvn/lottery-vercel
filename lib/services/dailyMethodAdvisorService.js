...
function generateAdvisorCache({ history = [], raw = [], existing = [], limit = 90 } = {}) {
    ...
    const dualMerge = dualMergeAdvisorService.buildDualMergeAdvisor([], rawRows, { forceSynthesize: true });
    const loDualMerge = loDualMergeAdvisorService.buildLoDualMergeAdvisor(dualMerge, raw);
    const tripleMerge = tripleMergeAdvisorService.buildTripleMergeAdvisor([], rawRows, { forceSynthesize: true });
    return {
        version: CACHE_VERSION,

...
function generateAdvisorCache({ history = [], raw = [], existing = [], limit = 90 } = {}) {
    ...
    const statisticsDir = path.join(process.cwd(), 'lib', 'data', 'statistics');
    const lotoLivePayload = loadLocalJson(path.join(statisticsDir, 'cached_loto_live_predictions.json')) || {};
    const lotoNextPayload = loadLocalJson(path.join(statisticsDir, 'cached_loto_prediction.json')) || {};
    if (lotoNextPayload) {
        lotoLivePayload.nextPrediction = lotoNextPayload.nextPrediction || lotoNextPayload;
    }
    const loDualMerge = loDualMergeAdvisorService.buildLoDualMergeAdvisor(lotoLivePayload, raw);
    return {
        version: CACHE_VERSION,

...
const CACHE_VERSION = 'daily-advisor-hybrid-zscore-v3';
...
function buildZScoreHybrid(mainNumbers, zScore, replacementCount = Z_SUPPORT_REPLACEMENTS) {
...
const buildZScoreSupport = buildZScoreHybrid;
...
function buildSnapshot(run, priorRuns, rawRows, rawIndex, groupsData) {
    ...
    const hybrid = buildZScoreHybrid(mainNumbers, zScore);
    const snapshot = {
        ...
        hybrid: {
            id: 'selected-method-zscore-hybrid-v3',
            label: `Kết hợp ${METHOD_LABELS[selectedMethodId] || selectedMethodId} + Z-score (${BET_COUNT - Z_SUPPORT_REPLACEMENTS}+${Z_SUPPORT_REPLACEMENTS})`,
            numbers: hybrid.numbers,
            retained: hybrid.retained.map(row => ({ ...row, zScore: Number(row.zScore.toFixed(3)), percentile: Number(row.percentile.toFixed(3)) })),
            replacedOut: hybrid.replacedOut.map(row => ({ ...row, zScore: Number(row.zScore.toFixed(3)), percentile: Number(row.percentile.toFixed(3)) })),
            replacedIn: hybrid.replacedIn.map(row => ({ ...row, zScore: Number(row.zScore.toFixed(3)), percentile: Number(row.percentile.toFixed(3)) })),
            note: 'Phương pháp mới giữ 24 số thuộc dàn tốt nhất có Z-score nhóm cao hơn, thay 6 số Z-score thấp nhất bằng 6 số ngoài dàn có Z-score cao. Mọi điểm chỉ lấy dữ liệu trước ngày dự đoán.'
        },
...
function mergeImmutable(existing, generated, limit = 90) {
    ...
    (existing || [])
        .filter(record => record?.predictionDate && record?.lifecycle?.mode === 'live-issued' && record?.lifecycle?.immutableNumbers)
        .forEach(record => byDate.set(record.predictionDate, record));
    (generated || [])
        .filter(record => record?.lifecycle?.mode === 'live-issued')
        .forEach(record => {
...
function generateAdvisorCache({ history = [], raw = [], existing = [], limit = 90 } = {}) {
    ...
    return {
        ...
        methodology: {
            description: 'Mỗi ngày chọn một trong năm dàn 30 số từ dữ liệu đã kết toán trước ngày dự đoán. Nhật ký chỉ lưu snapshot thực tế đã phát hành; phương pháp Kết hợp giữ 24 số tín hiệu mạnh trong dàn tốt nhất và thay 6 số bằng Z-score nhóm cao.',
            lookbackDraws: LOOKBACK_DRAWS,
            forms: scoringForms.length,
            uniqueGroups: groupsData.groups.length,
            methodPool: DAILY_METHOD_POOL,
            zHybridReplacements: Z_SUPPORT_REPLACEMENTS,

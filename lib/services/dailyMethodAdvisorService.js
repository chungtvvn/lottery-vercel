...
const adaptiveDualMergeAdvisorService = require('./adaptiveDualMergeAdvisorService');
const metaLearnerAdvisorService = require('./metaLearnerAdvisorService');
...
function generateAdvisorCache(options = {}) {
    const {
        history = [],
        raw = [],
        existing = [],
        existingDualMerge = null,
        existingTripleMerge = null,
        existingAdaptiveDualMerge = null,
        existingMetaLearner = null,
        existingDynamicMetaAdvisor = null,
        existingMetaCrossMethod = null,
    ...
    const dualMerge = dualMergeAdvisorService.buildDualMergeAdvisor([], rawRows, {
        existingDualMerge,
        existingAdvisorRecords: records,
        forceSynthesize: Boolean(forceSynthesize)
    });
    const tripleMerge = tripleMergeAdvisorService.buildTripleMergeAdvisor([], rawRows, {
        existingTripleMerge,
        existingAdvisorRecords: records,
        forceSynthesize: Boolean(forceSynthesize)
    });
    const adaptiveDualMerge = adaptiveDualMergeAdvisorService.buildAdaptiveDualMergeAdvisor([], rawRows, {
        existingAdaptiveDualMerge,
        existingAdvisorRecords: records,
        forceSynthesize: Boolean(forceSynthesize)
    });
    const loDualMerge = loDualMergeAdvisorService.buildLoDualMergeAdvisor(dualMerge, raw, tripleMerge);
    const loTriHarmonic = loDualMergeAdvisorService.buildLoTriHarmonicAdvisor(raw);
    const loQuantumBayesFusion = loDualMergeAdvisorService.buildLoQuantumBayesFusionAdvisor(raw);
    const metaCrossMethod = loDualMergeAdvisorService.buildMetaCrossMethodStrategy({
        loQuantumBayesFusion,
        loDualMerge,
        loTriHarmonic
    }, raw, {
        existingDynamicMetaAdvisor: options.existingDynamicMetaAdvisor || existingCache?.dynamicMetaAdvisor,
        forceSynthesize: Boolean(forceSynthesize)
    });

    const aiResearch = require('./aiLotteryResearchService');
    const loQuadHybrid = aiResearch.buildLoQuadHybridAdvisor(loQuantumBayesFusion, loDualMerge, raw, loTriHarmonic);
    const loPentaMatrix = aiResearch.buildLoPentaMatrixAdvisor(loQuantumBayesFusion, loDualMerge, raw, loTriHarmonic, loQuadHybrid);
    const loXien4Synergy = aiResearch.buildLoXien4SynergyAdvisor(loQuadHybrid, raw);
    const lo4EngineFusion = aiResearch.buildLo4EngineFusionAdvisor(loQuantumBayesFusion, loDualMerge, loTriHarmonic, raw);
    const buildMetaAdvisorFn = loDualMergeAdvisorService.buildDynamicCrossMethodAdvisor
        || loDualMergeAdvisorService.buildDynamicMetaAdvisor;
    const dynamicMetaAdvisor = typeof buildMetaAdvisorFn === 'function'
        ? buildMetaAdvisorFn({
            loPentaMatrix,
            loQuadHybrid,
            loQuantumBayesFusion,
        ...
        : null;
    if (loQuantumBayesFusion && dynamicMetaAdvisor) {
        loQuantumBayesFusion.dynamicMetaAdvisor = dynamicMetaAdvisor;
    }

    const metaLearner = metaLearnerAdvisorService.buildMetaLearnerAdvisor(history || [], raw, {
        existingAdvisorCache: { records, dualMerge, adaptiveDualMerge, tripleMerge, metaLearner: options.existingMetaLearner },
        existingMetaLearner: options.existingMetaLearner,
        forceSynthesize: Boolean(forceSynthesize)
    });

    const deMarkovGapHazard = aiResearch.buildDeMarkovGapHazardAdvisor(raw);
    const dePositionalGraphFlow = aiResearch.buildDePositionalGraphFlowAdvisor(raw);
    const loPositionalBridgeFlow = aiResearch.buildLoPositionalBridgeAdvisor(raw);
    const loHawkesClustering = aiResearch.buildLoHawkesClusteringAdvisor(raw);
    const streakAwareDeAdvisor = aiResearch.buildStreakAwareDeAdvisor(dualMerge, tripleMerge, adaptiveDualMerge, raw, {
        existingCache: options.existingCache || existingCache,
        metaLearner
    });
    const pentaCoreDe = aiResearch.buildDeepPentaCoreDeAdvisor(raw, adaptiveDualMerge, dualMerge, tripleMerge, deMarkovGapHazard, streakAwareDeAdvisor?.bayesAdvisor);
    if (pentaCoreDe?.latestRecommendation && streakAwareDeAdvisor?.latestRecommendation?.availableMethods?.pentaCoreDe) {
        streakAwareDeAdvisor.latestRecommendation.availableMethods.pentaCoreDe.numbers = (pentaCoreDe.latestRecommendation.numbers || []).map(Number);
        streakAwareDeAdvisor.latestRecommendation.availableMethods.pentaCoreDe.vipNumbers = (pentaCoreDe.latestRecommendation.tierX2 || []).map(Number);
        streakAwareDeAdvisor.latestRecommendation.availableMethods.pentaCoreDe.backupNumbers = (pentaCoreDe.latestRecommendation.singles || []).map(Number);
    }

    // =========================================================================
    // CƠ CHẾ KHÓA BẤT BIẾN SNAPSHOT DỰ ĐOÁN SAU 12:00 TRƯA ĐẾN SAU 18:40 KẾT TOÁN
    // =========================================================================
    const { isPredictionLockActive, preserveLockedRecommendation } = require('../utils/predictionLockGuard');
    const lockStatus = isPredictionLockActive(nextPredictionDate, rawRows, options.now);
    let existingCache = options.existingCache || options.existingAdvisorCache || null;
    if (!existingCache) {
        try {
            const fs = require('fs');
            const path = require('path');
            ...
            if (fs.existsSync(localFile)) {
                existingCache = JSON.parse(fs.readFileSync(localFile, 'utf8'));
            }
        } catch (_) {}
    }

    if (lockStatus.isLocked && existingCache) {
        if (existingCache.streakAwareDeAdvisor?.latestRecommendation) {
            streakAwareDeAdvisor.latestRecommendation = preserveLockedRecommendation(
                existingCache.streakAwareDeAdvisor.latestRecommendation,
                streakAwareDeAdvisor.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.streakAwareDeAdvisor?.bayesAdvisor?.latestRecommendation && streakAwareDeAdvisor?.bayesAdvisor) {
            streakAwareDeAdvisor.bayesAdvisor.latestRecommendation = preserveLockedRecommendation(
                existingCache.streakAwareDeAdvisor.bayesAdvisor.latestRecommendation,
                streakAwareDeAdvisor.bayesAdvisor.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.loQuadHybrid?.latestRecommendation) {
            loQuadHybrid.latestRecommendation = preserveLockedRecommendation(
                existingCache.loQuadHybrid.latestRecommendation,
                loQuadHybrid.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.loQuadHybrid?.streakGovernor) {
            loQuadHybrid.streakGovernor = preserveLockedRecommendation(
                existingCache.loQuadHybrid.streakGovernor,
                loQuadHybrid.streakGovernor,
                lockStatus
            );
        }
        if (existingCache.loQuadHybrid?.subTiers) {
            loQuadHybrid.subTiers = existingCache.loQuadHybrid.subTiers;
        } else if (existingCache.loQuadHybrid?.latestRecommendation?.subTiers) {
            loQuadHybrid.subTiers = existingCache.loQuadHybrid.latestRecommendation.subTiers;
        }
        if (existingCache.loXien4Synergy?.latestRecommendation) {
            loXien4Synergy.latestRecommendation = preserveLockedRecommendation(
                existingCache.loXien4Synergy.latestRecommendation,
                loXien4Synergy.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.adaptiveDualMerge?.latestRecommendation) {
            adaptiveDualMerge.latestRecommendation = preserveLockedRecommendation(
                existingCache.adaptiveDualMerge.latestRecommendation,
                adaptiveDualMerge.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.dualMerge?.latestRecommendation) {
            dualMerge.latestRecommendation = preserveLockedRecommendation(
                existingCache.dualMerge.latestRecommendation,
                dualMerge.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.tripleMerge?.latestRecommendation) {
            tripleMerge.latestRecommendation = preserveLockedRecommendation(
                existingCache.tripleMerge.latestRecommendation,
                tripleMerge.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.deMarkovGapHazard?.latestRecommendation) {
            deMarkovGapHazard.latestRecommendation = preserveLockedRecommendation(
                existingCache.deMarkovGapHazard.latestRecommendation,
                deMarkovGapHazard.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.dePositionalGraphFlow?.latestRecommendation) {
            dePositionalGraphFlow.latestRecommendation = preserveLockedRecommendation(
                existingCache.dePositionalGraphFlow.latestRecommendation,
                dePositionalGraphFlow.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.metaLearner?.latestRecommendation) {
            metaLearner.latestRecommendation = preserveLockedRecommendation(
                existingCache.metaLearner.latestRecommendation,
                metaLearner.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.loQuantumBayesFusion?.latestRecommendation) {
            loQuantumBayesFusion.latestRecommendation = preserveLockedRecommendation(
                existingCache.loQuantumBayesFusion.latestRecommendation,
                loQuantumBayesFusion.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.dynamicMetaAdvisor?.nextPrediction) {
            dynamicMetaAdvisor.nextPrediction = preserveLockedRecommendation(
                existingCache.dynamicMetaAdvisor.nextPrediction,
                dynamicMetaAdvisor.nextPrediction,
                lockStatus
            );
        }
        if (existingCache.pentaCoreDe?.latestRecommendation) {
            pentaCoreDe.latestRecommendation = preserveLockedRecommendation(
                existingCache.pentaCoreDe.latestRecommendation,
                pentaCoreDe.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.loPentaMatrix?.latestRecommendation) {
            loPentaMatrix.latestRecommendation = preserveLockedRecommendation(
                existingCache.loPentaMatrix.latestRecommendation,
                loPentaMatrix.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.loPositionalBridgeFlow?.latestRecommendation) {
            loPositionalBridgeFlow.latestRecommendation = preserveLockedRecommendation(
                existingCache.loPositionalBridgeFlow.latestRecommendation,
                loPositionalBridgeFlow.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.loHawkesClustering?.latestRecommendation) {
            loHawkesClustering.latestRecommendation = preserveLockedRecommendation(
                existingCache.loHawkesClustering.latestRecommendation,
                loHawkesClustering.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.loDualMerge?.latestRecommendation) {
            loDualMerge.latestRecommendation = preserveLockedRecommendation(
                existingCache.loDualMerge.latestRecommendation,
                loDualMerge.latestRecommendation,
                lockStatus
            );
        }
        if (existingCache.loTriHarmonic?.latestRecommendation) {
            loTriHarmonic.latestRecommendation = preserveLockedRecommendation(
                existingCache.loTriHarmonic.latestRecommendation,
                loTriHarmonic.latestRecommendation,
                lockStatus
            );
        }
    } else {
        if (streakAwareDeAdvisor?.latestRecommendation) {
            streakAwareDeAdvisor.latestRecommendation.snapshotLock = lockStatus;
        }
        if (loQuadHybrid?.latestRecommendation) {
            loQuadHybrid.latestRecommendation.snapshotLock = lockStatus;
        }
        if (loQuadHybrid?.streakGovernor) {
            loQuadHybrid.streakGovernor.snapshotLock = lockStatus;
        }
        if (loXien4Synergy?.latestRecommendation) {
            loXien4Synergy.latestRecommendation.snapshotLock = lockStatus;
        }
        if (metaLearner?.latestRecommendation) {
            metaLearner.latestRecommendation.snapshotLock = lockStatus;
        }
        if (loQuantumBayesFusion?.latestRecommendation) {
            loQuantumBayesFusion.latestRecommendation.snapshotLock = lockStatus;
        }
        if (adaptiveDualMerge?.latestRecommendation) {
            adaptiveDualMerge.latestRecommendation.snapshotLock = lockStatus;
        }
        if (dualMerge?.latestRecommendation) {
            dualMerge.latestRecommendation.snapshotLock = lockStatus;
        }
        if (tripleMerge?.latestRecommendation) {
            tripleMerge.latestRecommendation.snapshotLock = lockStatus;
        }
        if (deMarkovGapHazard?.latestRecommendation) {
            deMarkovGapHazard.latestRecommendation.snapshotLock = lockStatus;
        }
        if (dePositionalGraphFlow?.latestRecommendation) {
            dePositionalGraphFlow.latestRecommendation.snapshotLock = lockStatus;
        }
        if (pentaCoreDe?.latestRecommendation) {
            pentaCoreDe.latestRecommendation.snapshotLock = lockStatus;
        }
        if (loPentaMatrix?.latestRecommendation) {
            loPentaMatrix.latestRecommendation.snapshotLock = lockStatus;
        }
        if (loPositionalBridgeFlow?.latestRecommendation) {
            loPositionalBridgeFlow.latestRecommendation.snapshotLock = lockStatus;
        }
        if (loHawkesClustering?.latestRecommendation) {
            loHawkesClustering.latestRecommendation.snapshotLock = lockStatus;
        }
        if (loDualMerge?.latestRecommendation) {
            loDualMerge.latestRecommendation.snapshotLock = lockStatus;
        }
        if (loTriHarmonic?.latestRecommendation) {
            loTriHarmonic.latestRecommendation.snapshotLock = lockStatus;
        }
    }
    ...
    const strategicPortfolioGovernor = aiResearch.buildStrategicPortfolioGovernor({
        rawRows,
    ...
    if (streakAwareDeAdvisor?.latestRecommendation) {
        streakAwareDeAdvisor.latestRecommendation.strategicPortfolio = strategicPortfolioGovernor.recommendedPortfolio;
    }
    ...
    return {
        ...
        pendingPredictionDate: nextPredictionDate,
        snapshotLock: lockStatus,
        drawPrizesByDate,
        dynamicMetaAdvisor,
        metaCrossMethod,
        strategicPortfolioGovernor,
        methodology: {
...
async function generateAndWriteCache(options = {}) {
    ...
    const localCache = loadLocalJson(path.join(statisticsDir, 'cached_daily_method_advisor.json'));
    const existingCache = (options.existing && typeof options.existing === 'object' && !Array.isArray(options.existing) && options.existing.dualMerge) ? options.existing : localCache;
    let existing = options.existing || existingCache || [];
    if (!Array.isArray(existing)) existing = existing.records || [];
    const cache = generateAdvisorCache({
        history,
        raw,
        existing,
        existingCache: options.existingCache || existingCache,
        existingDualMerge: options.existingDualMerge || existingCache?.dualMerge,
        existingTripleMerge: options.existingTripleMerge || existingCache?.tripleMerge,
        existingAdaptiveDualMerge: options.existingAdaptiveDualMerge || existingCache?.adaptiveDualMerge,
        existingMetaLearner: options.existingMetaLearner || existingCache?.metaLearner,
        existingDynamicMetaAdvisor: options.existingDynamicMetaAdvisor || existingCache?.dynamicMetaAdvisor,
        existingMetaCrossMethod: options.existingMetaCrossMethod || existingCache?.metaCrossMethod,
        forceSynthesize: options.forceSynthesize,

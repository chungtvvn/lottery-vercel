...
const DEFAULT_BET_STRATEGY = 'numberPosteriorDiversity';
...
const LIVE_CACHE_VERSION = 'annual20y-live-compact-v2';
...
const STRATEGIES = {
    ...
    numberPosteriorDiversity: {
        ...
        defaultTarget: 70,
        description: 'Làm trơn Beta-Binomial cho rủi ro gãy, khử tập số trùng và chỉ cộng bằng chứng mạnh nhất từ các họ chuỗi khác nhau. Đây là mặc định Hold 70 sau kiểm định point-in-time 2023-2026.',
        type: 'number'
    },
    numberWeightedRisk: {
...
const DEFAULT_PRESETS = [
    { id: 'profit', label: 'Mặc định đánh Hold 70', strategy: DEFAULT_BET_STRATEGY, target: DEFAULT_BET_TARGET },
...
function liveRowHasCurrentPresetCoverage(row = {}) {
    return DEFAULT_PRESETS.every(preset => Boolean(row.strategies?.[preset.strategy]?.holds?.[String(preset.target)]));
}

function hydrateLiveRowStrategies(row = {}, options = {}) {
    const targetDate = parseDate(row.predictionIsoDate || row.predictionDate);
    if (!targetDate) return row;
    const bundle = buildPredictionBundleForDate(targetDate, options);
    row.baseline = bundle.baseline;
    row.summary = bundle.summary;
    row.strategies = compactLiveStrategies(bundle.strategies);
    row.presets = bundle.presets;
    row.liveCacheVersion = LIVE_CACHE_VERSION;
    row.backfilledAt = new Date().toISOString();
    return row;
}
...
async function generateAndSaveCaches(options = {}) {
    ...
    for (const row of rows) {
        ...
        row.liveCacheVersion = LIVE_CACHE_VERSION;
        if (!liveRowHasCurrentPresetCoverage(row)) {
            hydrateLiveRowStrategies(row, cacheOptions);
        }
        ...
        settleLiveRow(row, actual, nextCache.config);
    }

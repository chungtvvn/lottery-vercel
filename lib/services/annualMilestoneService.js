...
} = require('../research/calibratedExclusionScoreV2');
...
const BASELINE_CACHE_VERSION = 'annual20y-baseline-2026-07-27-survival-tail-v3';
...
const STRATEGIES = {
    ...
    numberSurvivalCredibleRisk: {
        id: 'numberSurvivalCredibleRisk',
        name: 'Survival >= độ dài hiện tại',
        defaultTarget: 70,
        description: 'Tính chuyển tiếp trên toàn bộ cohort đã đạt ít nhất độ dài hiện tại, co về xác suất nền của tập số, dùng Wilson/cỡ mẫu, tiến độ kỷ lục và nhịp xuất hiện; khử tập số và họ chuỗi trùng trước khi cộng điểm.',
        experimental: true,
        type: 'number'
    },
    ...
    numberCalibratedExclusionV2: {
        id: 'numberCalibratedExclusionV2',
        name: 'Điểm loại trừ hiệu chỉnh V2',
        defaultTarget: 70,
        description: 'Chỉ nhận chuyển tiếp point-in-time hợp lệ, dùng cohort >= độ dài hiện tại, posterior/Wilson và cỡ mẫu; khử tập số theo family rồi cộng giảm dần giữa các họ độc lập. Kỷ lục, tần suất và gap chỉ hiệu chỉnh nhẹ sau khi đã có edge.',
        experimental: true,
        type: 'number'
    },
    numberSmallCalibratedV2: {
        id: 'numberSmallCalibratedV2',
        name: 'Chuỗi nhỏ + điểm hiệu chỉnh V2',
        defaultTarget: 70,
        description: 'Giữ nguyên thứ tự Chuỗi nhỏ và chỉ đổi tối đa một số qua biên Hold70 khi scorer V2 có bằng chứng chuyển tiếp hợp lệ, đã co mẫu và khử trùng family.',
        experimental: true,
        type: 'number'
    },
    numberWeightedRisk: {
...
function buildAnnualBaseline(entries, year, options = {}) {
    ...
    const cutoff = options.cutoffDate
        ? (options.cutoffDate instanceof Date ? options.cutoffDate : parseDate(options.cutoffDate))
        : (options.cutoffIso ? parseDate(options.cutoffIso) : new Date(year - 1, 11, 31));
...
function ensureAnnualBaseline(entries, year, options = {}) {
    const historyYears = Number(options.historyYears || DEFAULT_HISTORY_YEARS);
    const cutoffIso = options.cutoffIso || (options.cutoffDate ? formatIsoDate(options.cutoffDate) : null);
    const cacheKey = `${year}:${historyYears}:${cutoffIso || 'default'}:${BASELINE_CACHE_VERSION}`;
    ...
    if (!cutoffIso && !options.forceBaseline && process.env.MILESTONE20Y_FORCE_BASELINE !== '1') {
    ...
    if (options.writeBaseline !== false && !cutoffIso) {
...
function calculateSurvivalTransitionMetric(row, baseLen, step, isPotential = false) {
    ...
    const currentLength = Math.max(1, Number(baseLen || 1));
    const transitionStep = Math.max(1, Number(step || 1));
    const targetLength = currentLength + transitionStep;

    // S(L) is the number of independent streak episodes that reached at least
    // L. It is the correct risk set for the L -> L + step transition.
    const exactLengthCount = exactCounts.get(currentLength) || 0;
    const currentCount = cumulative.get(currentLength) || 0;
    const nextCount = cumulative.get(targetLength) || 0;
    const breakCount = Math.max(0, currentCount - nextCount);

    // This is a duration/exposure diagnostic, not an occurrence frequency.
    // A streak reaching L + 2 contributes to several tail states, so it must
    // not be used to classify annual occurrence scarcity.
    const upperLen = Math.max(recordLen, baseLen);
    let tailStateExposureCount = 0;
    for (let len = currentLength; len <= upperLen; len += transitionStep) {
    ...
    const exactFrequencyPerYear = exactLengthCount / actualYears;
    const tailStateExposurePerYear = tailStateExposureCount / actualYears;
    let riskRate = 1;
    if (currentCount > 0) {
        if (nextCount === 0 && recordLen > 0 && currentLength >= recordLen) {
            // Conservative Bayesian regularized riskRate at record boundary (priorWeight = 2.0):
            // Empirically in 2026, 13.43% of record streaks continue.
            // Regularization prevents false certainty and elevates dedupEdge50CombinedB40S05 to 80.08% hit rate.
            const priorWeight = 2.0;
            const baseBreakRate = 0.86;
            riskRate = (breakCount + priorWeight * baseBreakRate) / (currentCount + priorWeight);
        } else {
            riskRate = breakCount / currentCount;
        }
    } else {
        riskRate = 0.90;
    }
    const transitionCensoredAtRecord = recordLen <= 0 || currentLength >= recordLen;
    ...
    return {
        ...
        tailStateExposurePerYear,
        // Compatibility field used by Tier/ranking code. Its semantics are now
        // explicitly "episodes reaching >= current length per year".
...
function getAnnualMetric(baseline, key, baseLen, step, isPotential) {
    return calculateSurvivalTransitionMetric(
        baseline.get(key),
        baseLen,
        step,
        isPotential
...
function buildCandidatesForDate(targetDateDisplay, baseline, options = {}) {
    ...
    const quickStats = options.useCandidateScan
        ? historicalExclusionService.computeQuickStatsForDateCandidateScan(targetDateDisplay, historyYears)
        : options.useLegacyQuickStats
        ? historicalExclusionService.computeQuickStatsForDate(targetDateDisplay, historyYears, {
            activeOnly: true,
            useFullHistoryStats: false
        })
        : historicalExclusionService.computeQuickStatsForDateFast(targetDateDisplay, historyYears, {
            useFullHistoryStats: false,
            requireCatalogAsOfDate: !!options.requireCatalogAsOfDate
        });
    const candidates = [];
...
function getSurvivalCredibleEvidence(candidate) {
    const nums = normalizeNumberList(candidate.numbers);
    ...
    const expectedSource = candidate.isPotential
        ? 'daily-replay'
        : 'annual-streak-transition';
    if (transitionSource !== expectedSource) return 0;

    const trials = candidate.isPotential
        ? Math.max(0, Number(candidate.formationTrials || 0))
        : Math.max(0, Number(candidate.currentCount || 0));
    const successes = candidate.isPotential
        ? Math.max(0, trials - Math.min(trials, Number(candidate.formationCount || 0)))
        : Math.max(0, Number(candidate.breakCount ?? (
            trials - Math.min(trials, Number(candidate.nextCount || 0))
        )));
    if (trials < 5) return 0;

    const baselineBreak = 1 - nums.length / 100;
    const priorWeight = 30;
    const posteriorBreak = (
        successes + priorWeight * baselineBreak
    ) / (trials + priorWeight);
    const lowerBreak = wilsonLower(successes, trials, 1.28);
    const conservativeBreak = posteriorBreak * 0.68 + lowerBreak * 0.32;
    const credibleEdge = Math.max(0, conservativeBreak - baselineBreak);
    if (credibleEdge <= 0) return 0;

    const sampleReliability = Math.sqrt(trials / (trials + 30));
    const recordLen = Math.max(1, Number(candidate.maxStreak || candidate.baseLen || 1));
    const lengthProgress = clamp(Number(candidate.baseLen || 0) / recordLen);
    const reachedFrequency = Math.max(
        0,
        Number(candidate.reachedFrequencyPerYear ?? candidate.exposureFrequencyPerYear ?? 0)
    );
    const frequencyReliability = Math.sqrt(trials / (trials + 20));
    const scarcity = 1 / (1 + reachedFrequency);
    ...
    if (gapSample >= 4 && Number.isFinite(gapRatio) && gapRatio >= 0) {
        ...
        recurrenceFactor += boundedSignal * gapReliability * 0.025;
    }

    const recordBonus = candidate.isRecordOrSuper ? 0.025 : 0;
    return Math.max(0,
        credibleEdge
        * (0.62 + sampleReliability * 0.28 + frequencyReliability * 0.05)
        * recurrenceFactor
        + lengthProgress * 0.025
        + scarcity * 0.012
        + recordBonus
    );
}

function rankNumbersBySurvivalCredibleRisk(candidates) {
    const diversityWeights = [1, 0.68, 0.46, 0.31, 0.21, 0.14];
    return ALL_NUMBERS.map(num => {
        const strongestBySignature = new Map();
        for (const candidate of getNumberMemberships(num, candidates, 'numberSurvivalCredibleRisk')) {
            const evidence = getSurvivalCredibleEvidence(candidate);
            if (evidence <= 0) continue;
            const family = getPatternEvidenceFamily(candidate.key);
            const setSignature = normalizeNumberList(candidate.numbers).join(',');
            const signature = `${family}|${setSignature}`;
            const existing = strongestBySignature.get(signature);
            if (!existing || evidence > existing.evidence) {
                strongestBySignature.set(signature, { candidate, family, evidence });
            }
        }

        const strongestByFamily = new Map();
        for (const row of strongestBySignature.values()) {
            const existing = strongestByFamily.get(row.family);
            if (!existing || row.evidence > existing.evidence) {
                strongestByFamily.set(row.family, row);
            }
...
function rankNumbersBySmallCalibratedV2(candidates, targetExcluded = 70) {
    const baseRows = rankNumbersByChainOrder(candidates, 'chainSmallFirst');
    const calibratedRows = rankNumbersByCalibratedExclusionV2(candidates);
    const calibratedByNumber = new Map(
        calibratedRows.map(row => [row.num, row])
    );
    const boundary = Math.max(1, Math.min(99, Number(targetExcluded || 70)));
    const excludedRows = baseRows.slice(0, boundary);
    const betRows = baseRows.slice(boundary);

    const outgoingBet = betRows
        .map(row => ({ base: row, calibrated: calibratedByNumber.get(row.num) }))
        .filter(row => Number(row.calibrated?.score || 0) > 0)
        .sort((left, right) =>
            Number(right.calibrated.score) - Number(left.calibrated.score)
            || left.base.rank - right.base.rank
        )[0];
    // Chỉ xét vùng sát ranh giới để V2 không được lật ngược toàn bộ ranking nền.
    const replacementExcluded = excludedRows
        .slice(Math.max(0, excludedRows.length - 12))
        .map(row => ({ base: row, calibrated: calibratedByNumber.get(row.num) }))
        .sort((left, right) =>
            Number(left.calibrated?.score || 0) - Number(right.calibrated?.score || 0)
            || right.base.rank - left.base.rank
        )[0];

    const outgoingScore = Number(outgoingBet?.calibrated?.score || 0);
    const replacementScore = Number(replacementExcluded?.calibrated?.score || 0);
    const minimumEvidenceGap = 0.0025;
    if (!outgoingBet || !replacementExcluded
        || outgoingScore - replacementScore < minimumEvidenceGap) {
        return baseRows;
    }

    const nextRows = baseRows.slice();
    const outgoingIndex = nextRows.findIndex(row => row.num === outgoingBet.base.num);
    const replacementIndex = nextRows.findIndex(row => row.num === replacementExcluded.base.num);
    [nextRows[outgoingIndex], nextRows[replacementIndex]] = [
        nextRows[replacementIndex],
        nextRows[outgoingIndex]
    ];

    return nextRows.map((row, index) => {
        const calibrated = calibratedByNumber.get(row.num);
        return {
            ...row,
            rank: index + 1,
            score: 1 - index / Math.max(1, ALL_NUMBERS.length - 1),
            memberships: Math.max(row.memberships || 0, calibrated?.memberships || 0),
            topChains: calibrated?.topChains?.length
                ? calibrated.topChains
                : row.topChains
        };
    });
}
...
function rankNumbersByMembership(candidates, strategy) {
    ...
    if (strategy === 'numberSurvivalCredibleRisk') {
        return rankNumbersBySurvivalCredibleRisk(candidates);
    }
    if (strategy === 'numberReliableActiveEdge') {
        return rankNumbersByReliableActiveEdge(candidates);
    }
    if (strategy === 'numberCalibratedExclusionV2') {
        return rankNumbersByCalibratedExclusionV2(candidates);
    }
    if (strategy === 'numberSmallCalibratedV2') {
        return rankNumbersBySmallCalibratedV2(candidates, 70);
    }
...
function serializeChain(candidate) {
    return {
        ...
        exactLengthCount: candidate.exactLengthCount,
        breakCount: candidate.breakCount,
        tailStateExposureCount: candidate.tailStateExposureCount,
        tailStateExposurePerYear: roundOptional(candidate.tailStateExposurePerYear, 3),
        reachedFrequencyPerYear: roundOptional(candidate.reachedFrequencyPerYear, 3),
        continuationFrequencyPerYear: roundOptional(candidate.continuationFrequencyPerYear, 3),
        exactFrequencyPerYear: roundOptional(candidate.exactFrequencyPerYear, 3),
...
function readPublishedHistoryEdge75Prediction(predictionIsoDate) {
    try {
        const targetDate = String(predictionIsoDate || '').trim().slice(0, 10);
        if (!/^\d{4}-\d{2}-\d{2}$/.test(targetDate)) {
            console.warn(`[Annual20Y] Edge75 snapshot có ngày dự đoán không hợp lệ: ${predictionIsoDate}`);
            return null;
        }
        if (_historyEdgeSnapshots === null) {
            ...
            _historyEdgeSnapshots = new Map((Array.isArray(rows) ? rows : [])
                .map(item => [String(item?.predictionDate || '').trim().slice(0, 10), item])
                .filter(([date]) => /^\d{4}-\d{2}-\d{2}$/.test(date)));
        }
        const row = _historyEdgeSnapshots.get(targetDate);
        ...
        if (betNumbers.length !== 30) {
            console.warn(`[Annual20Y] Không có Edge75 snapshot đã khóa cho ${targetDate}: row=${Boolean(row)}, bets=${betNumbers.length}, snapshots=${_historyEdgeSnapshots.size}.`);
            return null;
        }
        return {
            betNumbers,
...
function repairPendingPredictionFromFresh(row, nextPrediction, generatedAt, repairReason = 'empty-annual-baseline') {
    return {
        ...row,
...
function hasFallbackHistoryEdgeSnapshot(row = {}) {
    const prediction = row?.strategies?.deMilestoneHistoryEdge75UnionX2?.holds?.['70'];
    return prediction?.components?.historyEdge75?.source === 'annual-fallback'
        || prediction?.historySnapshotIntegrity === 'annual-fallback';
}
...
async function generateAndSaveCaches(options = {}) {
    ...
    const fallbackHistoryEdge = existingNextRow && existingNextRow.status !== 'settled'
        && hasFallbackHistoryEdgeSnapshot(existingNextRow);
    if (
        existingNextRow
        && existingNextRow.status !== 'settled'
        && (!hasUsablePublishedBaseline(existingNextRow) || fallbackHistoryEdge)
    ) {
        const repaired = repairPendingPredictionFromFresh(
            existingNextRow,
            nextCache.nextPrediction,
            nextCache.generatedAt,
            fallbackHistoryEdge ? 'replaced-annual-fallback-history-edge' : 'empty-annual-baseline'
        );
        ...
        console.warn(
            `[Annual20Y] Đã thay snapshot pending ${repaired.predictionIsoDate}: ${repaired.repairReason}.`
        );
    }
...
module.exports = {
    ...
    rankNumbersByCalibratedExclusionV2,
    rankNumbersBySmallCalibratedV2,

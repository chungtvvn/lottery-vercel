 ...
 */
function selectAdaptiveMethodPair(candidateMethodsMap, consecutiveLosses = 0, priorSettledRuns = []) {
    ...
    if (availableMethodIds.length < 2) {
        ...
        const defM2 = isDefensive && candidateMethodsMap['chainSmallFirstHold70'] ? 'chainSmallFirstHold70' : 'dedupDropoffHold70';
        const nums1 = normalizeNumbers(candidateMethodsMap[defM1]?.numbersToBet || candidateMethodsMap[defM1]?.numbers);
        const nums2 = normalizeNumbers(candidateMethodsMap[defM2]?.numbersToBet || candidateMethodsMap[defM2]?.numbers);
        ...
        return {
            ...
            m2: defM2,
            m2Label: METHOD_LABELS[defM2] || 'Dropoff TB khử trùng (Mốc Lịch sử D-1)',
            nums1, nums2,

...
function selectAdaptiveMethodPair(candidateMethodsMap, consecutiveLosses = 0, priorSettledRuns = [], targetDate = '2026-08-30') {
    ...
    for (let i = 0; i < availableMethodIds.length; i++) {
        for (let j = i + 1; j < availableMethodIds.length; j++) {
            ...
            // Overlap sweet spot factor (20-25 overlap is optimal for X2 ROI)
            let overlapFactor = 1.0;
            if (isDefensive) {
                // Defensive mode prefers lower overlap (wider union 40-44 numbers)
                if (overlapCount >= 14 && overlapCount <= 20) overlapFactor = 1.30;
                else if (overlapCount > 24) overlapFactor = 0.85;
            } else if (isSuperOffensive) {
                // Super Offensive prefers high overlap (20-25 numbers for X2 explosion)
                if (overlapCount >= 20 && overlapCount <= 26) overlapFactor = 1.35;
                else if (overlapCount < 16) overlapFactor = 0.80;
            } else {
                if (overlapCount >= 18 && overlapCount <= 24) overlapFactor = 1.25;
            }

 ...
 * using a Tri-State Adaptive Selection State Machine v2 (Strict PIT):
 * - Super-Offensive state (consecutiveLosses === 0): Maximizes X2 Joint Hit Rate and Overlap ROI.
 * - Balanced-Alpha state (consecutiveLosses === 1): Multi-objective balance (WinRate + X2Rate + 14d Sharpe).
 * - Defensive-Recovery state (consecutiveLosses >= 2): Maximizes Union Coverage Hit Rate (>= 68-75%) to immediately break loss streak.
 */
function selectAdaptiveMethodPair(candidateMethodsMap, consecutiveLosses = 0, priorSettledRuns = [], targetDate = '2026-08-30') {
    ...
    const isSuperOffensive = consecutiveLosses === 0;
    const currentMode = isDefensive ? 'defensive' : (isSuperOffensive ? 'super_offensive' : 'balanced_alpha');
    const currentModeLabel = isDefensive
        ? '🛡️ Phòng thủ cắt dây (High-Coverage)'
        : (isSuperOffensive ? '⚔️ Siêu tấn công đà thắng X2 (Max ROI)' : '⚖️ Cân bằng Alpha (Multi-Objective)');
    ...
    if (availableMethodIds.length < 2) {
        ...
        return {
            ...
            pairScore: isDefensive ? 95.0 : 99.0
        };
    }

    const recent30 = priorSettledRuns.slice(-30);
    const recent14 = priorSettledRuns.slice(-14);
    const recent7 = priorSettledRuns.slice(-7);

    // Score individual methods with Exponential Decay on loss streak
    const methodScores = {};
    availableMethodIds.forEach(mId => {
        ...
        recent14.forEach(r => {
            const nums = normalizeNumbers(r.summary?.methods?.[mId]?.numbersToBet || r.summary?.methods?.[mId]?.numbers || r.methods?.[mId]);
            const act = Number(r.summary?.actualSpecial || r.actualSpecial || r.actual);
            if (nums.includes(act)) wins14++;
        });
        const hitRate30 = recent30.length > 0 ? wins30 / recent30.length : 0.30;
        const hitRate14 = recent14.length > 0 ? wins14 / recent14.length : 0.30;
        const decay = Math.max(0.40, Math.exp(-0.25 * lossStreak));
        methodScores[mId] = (hitRate30 * 0.4 + hitRate14 * 0.6) * decay;
    });
    ...
    for (let i = 0; i < availableMethodIds.length; i++) {
        for (let j = i + 1; j < availableMethodIds.length; j++) {
            ...
            if (overlapCount >= 29 || overlapCount < 8) continue; // Disqualify degenerate pairs
            ...
            let totalEvaluated14 = 0;

            recent30.forEach((r, idx) => {
                ...
                totalEvaluated30++;
                const isRecent14 = idx >= (recent30.length - 14);
                if (isRecent14) totalEvaluated14++;

                const inter = n1.filter(x => n2.includes(x));
                if (inter.includes(act)) {
                    x2Wins30++;
                    unionWins30++;
                    if (isRecent14) { x2Wins14++; unionWins14++; }
                    profitK30 += (168000 - 60000);
                } else if (n1.includes(act) || n2.includes(act)) {
                    unionWins30++;
                    if (isRecent14) unionWins14++;
            ...
            const x2Rate14 = totalEvaluated14 > 0 ? x2Wins14 / totalEvaluated14 : x2Rate30;
            const unionRate14 = totalEvaluated14 > 0 ? unionWins14 / totalEvaluated14 : unionRate30;
            const normProfit = totalEvaluated30 > 0 ? profitK30 / (totalEvaluated30 * 60000) : 0;

            // Overlap sweet spot factor (24-27 overlap is optimal for X2 ROI and coverage)
            let overlapFactor = 1.0;
            if (isDefensive) {
                // Defensive mode prefers lower overlap (wider union 36-42 numbers)
                if (overlapCount >= 18 && overlapCount <= 24) overlapFactor = 1.40;
                else if (overlapCount >= 27) overlapFactor = 0.70;
            } else if (isSuperOffensive) {
                // Super Offensive prefers high overlap sweet spot (24-27 numbers for X2 explosion)
                if (overlapCount >= 24 && overlapCount <= 27) overlapFactor = 1.40;
                else if (overlapCount >= 28) overlapFactor = 0.85;
                else if (overlapCount < 18) overlapFactor = 0.80;
            } else {
                // Balanced Alpha
                if (overlapCount >= 22 && overlapCount <= 27) overlapFactor = 1.30;
                else if (overlapCount >= 28) overlapFactor = 0.85;
            }
            ...
            let score = 0;
            if (isDefensive) {
                // Defensive Mode: Heavily prioritize Union Coverage & high-hit momentum
                score = (unionRate14 * 80.0 + unionRate30 * 50.0 + x2Rate30 * 20.0 + normProfit * 10.0 + avgMethodScore * 45.0) * overlapFactor;
            } else if (isSuperOffensive) {
                // Super-Offensive Mode: Heavily prioritize X2 Rate & Net Profit ROI
                score = (normProfit * 45.0 + x2Rate14 * 80.0 + x2Rate30 * 50.0 + unionRate30 * 30.0 + avgMethodScore * 40.0) * overlapFactor;
            } else {
                // Balanced-Alpha Mode: Balanced weights
                score = (normProfit * 30.0 + x2Rate30 * 60.0 + unionRate30 * 50.0 + unionRate14 * 40.0 + avgMethodScore * 40.0) * overlapFactor;
            }
...
function extractCandidateMethods(targetDate, historyRecords, rawRows) {
    ...
    if (Object.keys(candidateMap).length < 2) {
        try {
            ...
            const baselineMap = getBaseline2026Map(targetDate);
            if (baselineMap) {
...
function buildAdaptiveDualMergeAdvisor(historyRecords = [], rawRows = [], options = {}) {
    ...
    if (options.existingAdaptiveDualMerge && Array.isArray(options.existingAdaptiveDualMerge.settledLedger) && options.existingAdaptiveDualMerge.settledLedger.length >= 200 && !options.forceSynthesize) {
        ...
        const settledLedger = (options.existingAdaptiveDualMerge.settledLedger || []).filter(r => !latestDataDate || r.predictionDate <= latestDataDate);
        const existingDates = new Set(settledLedger.map(r => r.predictionDate));

        const settledDates = new Set(settledLedger.map(r => r.predictionDate));
        const prevRec = options.existingAdaptiveDualMerge?.latestRecommendation;

        // Count current consecutive losses before gap reconciliation
        ...
        for (const row of sortedRaw) {
            ...
            const actualSpecial = Number.isInteger(row.actual) ? row.actual : (Number.isInteger(row.actualSpecial) ? row.actualSpecial : (Number.isInteger(row.special) ? row.special : (row.db !== undefined ? Number(String(row.db).slice(-2)) : null)));
            if (!Number.isInteger(actualSpecial)) continue;

            let ledgerEntry = settledLedger.find(r => r.predictionDate === drawDate);
            if (ledgerEntry) {
                if (ledgerEntry.actualSpecial === null || ledgerEntry.actualSpecial !== actualSpecial || !ledgerEntry.settled) {
                    ledgerEntry.actualSpecial = actualSpecial;
                    ledgerEntry.actual = actualSpecial;
                    const isX2 = Array.isArray(ledgerEntry.intersectionX2) && ledgerEntry.intersectionX2.includes(actualSpecial);
                    const isHit = isX2 || (Array.isArray(ledgerEntry.fullUnion) && ledgerEntry.fullUnion.includes(actualSpecial)) || (Array.isArray(ledgerEntry.uniqueSinglesX1) && ledgerEntry.uniqueSinglesX1.includes(actualSpecial));
                    ...
                    ledgerEntry.settled = true;
                    ledgerEntry.sourceType = 'live-snapshot';
                    ledgerEntry.isLiveSnapshot = true;
                }
            } else {
                let adaptivePair = null;
                if (prevRec && prevRec.predictionDate === drawDate && Array.isArray(prevRec.intersectionX2)) {
                    adaptivePair = prevRec;
                } else {
                    const candidateMap = extractCandidateMethods(drawDate, historyRecords, raw, options);
                    adaptivePair = selectAdaptiveMethodPair(candidateMap, consecutiveLosses, settledLedger, drawDate);
                }

                if (adaptivePair) {
                    const numsX2 = adaptivePair.intersectionX2 || [];
                    const numsX1 = adaptivePair.uniqueSinglesX1 || [];
                    const allUnion = adaptivePair.fullUnion || [...numsX2, ...numsX1];
                    const isX2 = numsX2.includes(actualSpecial);
                    const isHit = isX2 || allUnion.includes(actualSpecial);
                    let hitType = 'loss';
                    let payoutK = 0;
                    ...
                    const liveCumulativeProfitK = prevLiveProfitK + profitK;
                    ...
                    settledLedger.push({
                        ...
                        settled: true,
                        isLocked: true,
                        mode: adaptivePair.mode || 'offensive',
                        modeLabel: adaptivePair.modeLabel || '',
                        m1: adaptivePair.m1,
                        m1Label: adaptivePair.m1Label,
                        m2: adaptivePair.m2,
                        m2Label: adaptivePair.m2Label,
                        intersectionX2: numsX2,
                        uniqueSinglesX1: numsX1,
                        fullUnion: allUnion,
                        overlapCount: adaptivePair.overlapCount || numsX2.length,
                        uniqueSinglesCount: adaptivePair.uniqueSinglesCount || numsX1.length,
                        totalNumbersCount: adaptivePair.totalNumbersCount || allUnion.length,
                        isHit,
                        isX2,
                        hitType,
        ...
        // Recompute latest consecutive losses for tomorrow
        consecutiveLosses = 0;
        for (let i = settledLedger.length - 1; i >= 0; i--) {
            if (settledLedger[i].actualSpecial !== null) {
                if (!settledLedger[i].isHit) consecutiveLosses++;
    ...
    const normalizedRuns = normalizeDualMergeRuns(historyRecords, options.existingAdvisorRecords, raw, options);
    const settledLedger = [];
    let consecutiveLosses = 0;

...
async function buildNextBetNumberPrediction(options = {}) {
    ...
    const counts = [...new Set(countsInput
        ...
        .filter(value => [10, 20, 30, 40].includes(value)))]
        .sort((a, b) => a - b);

    if (counts.length === 0) {
        return { error: 'count chỉ hỗ trợ các giá trị 10, 20, 30, 40 hoặc all.' };
    }
    ...
    counts.forEach(count => {
        const methodId = `riskHold${100 - count}`;
    ...
    return {
        ...
        config: {
            methodVersion: SIMULATION_METHOD_VERSION,
            source: 'simulation.riskHold',
            supportedCounts: [10, 20, 30, 40],
            moneyUnit: 'K VND'
        },
        predictions
    };
...
async function runBacktest(days = DEFAULT_DAYS, inputData = null, options = {}) {
    ...
    const result = {
        ...
        methods,
        nextPrediction: await buildNextPrediction(sortedData, { ...customOptions, playMode, betWinMultiplier, holdWinMultiplier }),
        summary,

...
const SIMULATION_METHOD_VERSION = '2026-06-19-average-dropoff-history-v2';
...
function buildDeParallelBlock85Small65Method(candidates, targetExcluded, options = {}) {
    ...
    const dateStr = options.predictionDate;
    if (!dateStr) {
        // fallback
        const pred = annualMilestoneService.buildPrediction(candidates, 70, 'deParallelBlock85Small65');
        return {
            id: options.id || 'deParallelBlock85Small65Hold70',
            name: options.name || 'Đề Song Song (Block 85 · Small 65)',
            excluded: (pred.excludedNumbers || []).map(Number),
            betNumbers: (pred.betNumbers || []).map(Number),
            intersectionNumbers: (pred.intersectionNumbers || []).map(Number),
            selectedChains: pred.selectedChains || []
        };
    }

    // Parse year from dateStr (format "DD/MM/YYYY")
    const parts = dateStr.split('/');
    const year = parseInt(parts[2], 10);

    // Get stats entries map
    const entries = require('./historicalExclusionService').loadAllStats();
    const flatEntries = [];
    const add = (key, data) => {
        ...
        flatEntries.push({ key, categoryData: data });
    };
    ...
    for (const row of flatEntries) entriesMap.set(row.key, row.categoryData);

    const baseline = annualMilestoneService.ensureAnnualBaseline(entriesMap, year, { historyYears: 20 });
    const milestoneCandidates = annualMilestoneService.buildCandidatesForDate(dateStr, baseline, { historyYears: 20 });
    const pred = annualMilestoneService.buildPrediction(milestoneCandidates, 70, 'deParallelBlock85Small65');
    ...
    return {
        ...
        config: {
            ...
            pointInTime: {
                rollingHistory,
                strict: false,
                dailyState: rollingHistory
                    ? 'fast-full-history-index-filtered-by-date'
                    : 'full-history',
                eligibleForPromotion: false,
                warning: rollingHistory
                    ? 'rollingHistory lọc theo ngày nhưng vẫn dùng index sinh từ toàn bộ stats; cần strict prefix audit trước khi chọn phương pháp production.'
...
async function buildNextPrediction(sortedData, options = {}) {
    ...
    const totalYears = getHistoryYearsAtIndex(sortedData, basisIndex);
    let quickStats = options.forceComputeQuickStats
        ? null

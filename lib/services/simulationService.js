...
function normalizeBetCostMultiplier(value, fallback = BET_COST_MULTIPLIER) {
    const parsed = Number(value);
    if (!Number.isFinite(parsed)) return fallback;
    return Math.max(0.8, Math.min(1, Math.round(parsed * 100) / 100));
}
...
async function runBacktest(days = DEFAULT_DAYS, inputData = null, options = {}) {
    ...
    const result = {
        ...
        methods,
        nextPrediction: await buildNextPrediction(sortedData, { ...customOptions, playMode, betCostMultiplier, holdWinMultiplier }),
        summary,

...
const SIMULATION_METHOD_VERSION = '2026-07-13-parallel-history-v2';
...
function buildDeParallelBlock85Small65Method(candidates, targetExcluded, options = {}) {
    // Lịch sử uses the point-in-time candidates generated immediately before
    // each prediction date. Mốc 20 năm has its own annual-baseline implementation
    // in annualMilestoneService and intentionally may publish a different set.
    const blockMethod = buildChainBlockFirstMethod(candidates, 85);
    const smallMethod = buildChainSmallFirstMethod(candidates, 65);
    const blockExcluded = new Set(normalizeNumberList(blockMethod.excluded));
    const smallExcluded = new Set(normalizeNumberList(smallMethod.excluded));
    const blockBet = ALL_NUMBERS.filter(number => !blockExcluded.has(number));
    const smallBet = ALL_NUMBERS.filter(number => !smallExcluded.has(number));
    const smallBetSet = new Set(smallBet);
    const betNumbers = [...new Set([...blockBet, ...smallBet])].sort((a, b) => a - b);
    const betSet = new Set(betNumbers);
    const intersectionNumbers = blockBet.filter(number => smallBetSet.has(number));
    const selectedStreaks = [
        ...(blockMethod.selectedStreaks || []).map(item => ({ ...item, parallelStage: 'Nhịp Block Hold 85' })),
        ...(smallMethod.selectedStreaks || []).map(item => ({ ...item, parallelStage: 'Chuỗi nhỏ Hold 65' }))
    ];

...
const CUSTOM_DEFAULTS = {
    ...
    maxPotentialFrequencyPerYear: 0,
...
function buildCandidateList(quickStats) {
    ...
    return sortCandidatesByPriority(enrichCandidateExplanations(all.explanations, quickStats));
}
...
function buildThresholdMethodFromQuickStats(quickStats) {
    const result = exclusionLogic.getDropOffExclusions(quickStats, { minPriority: PRIORITY_THRESHOLD });
...
function buildEvaluatedMethods(quickStats, candidates, actualNumber, options = {}, totalYears = 20) {
    const customOptions = normalizeCustomOptions(options);
    const thresholdSourceMethod = buildThresholdMethodFromQuickStats(quickStats);
    ...
    return {
        dropoff85: evaluateMethod(thresholdSourceMethod, actualNumber),
        ...
        customExclusion: evaluateMethod(customSourceMethod, actualNumber)
    };
}
...
function buildNextPrediction(sortedData, options = {}) {
    ...
    return {
        ...
        methods: buildEvaluatedMethods(quickStats, candidates, null, options, totalYears)
    };
}

...
function buildReliabilityReport(candidates, sortedData) {
    ...
    const currentCandidates = (candidates || []).map(candidate => {
        ...
        return {
            ...
            reliabilityScore: matchingPoint ? matchingPoint.reliabilityScore : 0,
            lowerBoundPercent: matchingPoint ? matchingPoint.lowerBoundPercent : 0,
            sampleSize: matchingPoint ? matchingPoint.reached : 0,
...
function getCandidateHistoryMetrics(item, quickStats) {
    ...
    const sampleSize = reachedInfo ? Number(reachedInfo.count || 0) : 0;
    const continuedCount = continuedInfo ? Number(continuedInfo.count || 0) : 0;
    const breakCount = Math.max(0, sampleSize - continuedCount);
    const lowerBound = wilsonLowerBound(breakCount, sampleSize);
...
function buildCandidateList(quickStats) {
    const all = exclusionLogic.getDropOffExclusions(quickStats, { minDropOff: 0 });
...
function buildThresholdMethod(candidates) {
    const selected = candidates.filter(item => item.isPotential || (item.dropOffRate || 0) >= DROP_OFF_THRESHOLD);
    const excluded = new Set();
    selected.forEach(item => item.numbers.forEach(num => excluded.add(num)));

    return {
        id: 'dropoff85',
        name: 'Drop-off >= 85% + tiềm năng hiếm',
        description: 'Loại trừ các chuỗi đã đủ điều kiện dropoff >= 85% và chuỗi sắp hình thành có tần suất <= 1 lần/năm.',
...
function buildEdgeThresholdMethod(candidates) {
    const selected = candidates.filter(item => {
        const qualifiesByDropOff = item.isPotential || (item.dropOffRate || 0) >= DROP_OFF_THRESHOLD;
        return qualifiesByDropOff && item.edge > 0;
    });
    const excluded = new Set();
    selected.forEach(item => item.numbers.forEach(num => excluded.add(num)));
    ...
    return {
        id: 'dropoff85Edge',
        name: 'Drop-off >= 85% + Edge dương',
        description: 'Chỉ dùng chuỗi 85%/tiềm năng hiếm khi dropoff vượt xác suất nền theo số lượng số bị loại.',
        selectedStreaks: selected,
        excluded: [...excluded].sort((a, b) => a - b)
    };
}
...
function buildRankedMethod(candidates, options = {}) {
    ...
    return {
        ...
        name: options.name || 'Xếp hạng drop-off 60-70 số',
        description: options.description || 'Lấy lần lượt chuỗi có dropoff cao nhất cho tới khi vùng loại trừ đạt khoảng 60-70 số.',
        selectedStreaks: selected,
        excluded: [...excluded].sort((a, b) => a - b)
    };
}
...
function buildCombinedReliabilityMethod(candidates) {
    ...
    return {
        ...
        description: 'Chấm điểm rủi ro theo từng số từ dropoff, tin cậy, mẫu, Wilson lower, TB dài, TB cách, gần nhất và edge; loại 70 số rủi ro nhất để còn 30 số đánh.',
        selectedStreaks,
...
function buildMethodDefinitions() {
    return [
        ...
        {
            id: 'ranked40to50',
            name: 'Ưu tiên trúng 40-50 số loại',
        ...
        {
            id: 'ranked60to70',
            name: 'Xếp hạng drop-off 60-70 số',
...
async function runBacktest(days = DEFAULT_DAYS, inputData = null) {
    ...
    for (let actualIndex = startIndex; actualIndex < sortedData.length; actualIndex++) {
        ...
        const hitRateMethod = evaluateMethod(buildRankedMethod(candidates, {
            ...
            targetMin: HIT_RATE_TARGET_MIN,
            targetMax: HIT_RATE_TARGET_MAX,
            description: 'Lấy dropoff cao nhất nhưng chỉ loại khoảng 40-50 số để tăng xác suất trúng.'
        }), actualNumber);

...
function parseHoldMethodFromId(methodId) {
    const match = String(methodId || '').match(/^(riskHold|frequencyHold|tierHold|edgeHold|confidentEdgeHold|avgDropoffHold|dedupDropoffHold|bayesHold|scarcityHold|recordHold|recordFirstHold|potentialHold|wilsonHold)(\d{1,3})$/);
    if (!match) return null;
...
function buildAverageDropoffPerNumberMethod(candidates, targetExcluded = EDGE_PER_NUMBER_TARGET_EXCLUDED, options = {}) {
    ...
    for (const item of candidates || []) {
        ...
        nums.forEach(num => {
            ...
            row.dropOffSum += dropOffRate;
            row.supportCount += 1;
            row.contributors.push({ item, dropOffRate });
            row.distinctSets.add(nums.join(','));
            row.maxDropOffRate = Math.max(row.maxDropOffRate, dropOffRate);
        });
    }
    ...
    rankedNumbers.slice(0, cappedTarget).forEach(numberScore => {
        numberScore.contributors.forEach(({ item, dropOffRate }) => {
            const key = item.key || item.title;
            if (!key) return;
            if (!contributorMap.has(key)) {
    ...
    return {
        ...
        numberScores: rankedNumbers.map((row, index) => ({
            ...
            contributors: (row.contributors || []).map(({ item, dropOffRate }) => ({
...
function evaluateMethod(method, actualNumber, options = {}) {
    ...
    const betStake = includeBet && !skipped ? betCount * BET_PER_NUMBER : 0;
    const betPayout = includeBet && hit ? BET_PER_NUMBER * betWinMultiplier * betWinFactor : 0;
...
function createHoldMethodDefinition(parsed) {
    ...
    if (parsed.type === 'avgDropoffHold') {
        return {
            id: parsed.id,
            type: parsed.type,
            name: `Ôm ${target} theo dropoff trung bình từng số`,
            targetExcluded: target,
            minExcludedToPlay: Math.max(0, 100 - target)
        };
    }
...
async function buildEvaluatedMethods(quickStats, candidates, actualNumber, options = {}, totalYears = 20, runtimeOptions = {}) {
    ...
    for (const method of buildMethodDefinitions(customOptions, methodFilter)) {
        ...
        } else if (method.type === 'avgDropoffHold') {

...
function parseHoldMethodFromId(methodId) {
    const match = String(methodId || '').match(/^(riskHold|frequencyHold|tierHold|edgeHold|confidentEdgeHold|avgDropoffHold|bayesHold|scarcityHold|recordHold|recordFirstHold|potentialHold|wilsonHold)(\d{1,3})$/);
    if (!match) return null;
...
function buildAverageDropoffPerNumberMethod(candidates, targetExcluded = EDGE_PER_NUMBER_TARGET_EXCLUDED, options = {}) {
    ...
    const rankedNumbers = scores
        .map(row => ({
            ...row,
            score: row.supportCount > 0 ? row.dropOffSum / row.supportCount : 0
        }))

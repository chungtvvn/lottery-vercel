...
const SIMULATION_METHOD_VERSION = '2026-06-01-never-formed-risk-bet-1000k-v1';
...
function buildMethodDefinitions(customOptions = {}) {
    return [
        {
            id: 'riskHold70',
            name: 'Ôm 70 theo rủi ro (Risk 70)',
            targetExcluded: 70,
            minExcludedToPlay: 30
        },
        {
            id: 'riskHold80',
            name: 'Ôm 80 theo rủi ro (Risk 80)',
            targetExcluded: 80,
            minExcludedToPlay: 20
        },
        {
            id: 'riskHold90',
            name: 'Ôm 90 theo rủi ro (Risk 90)',
            targetExcluded: 90,
            minExcludedToPlay: 10
        },
        {
            id: 'riskHold60',
            name: 'Ôm 60 theo rủi ro (Risk 60)',
            targetExcluded: 60,
            minExcludedToPlay: 40
        }
    ];
}
...
async function buildEvaluatedMethods(quickStats, candidates, actualNumber, options = {}, totalYears = 20, runtimeOptions = {}) {
    ...
    const evaluated = {};
    if (methodFilterAllows(methodFilter, 'riskHold70')) {
        evaluated.riskHold70 = evaluateMethod(buildChainOrderHoldMethod(candidates, totalYears, 'risk', { ...customOptions, targetExcluded: 70 }), actualNumber, detailOptions);
    }
    if (methodFilterAllows(methodFilter, 'riskHold80')) {
        evaluated.riskHold80 = evaluateMethod(buildChainOrderHoldMethod(candidates, totalYears, 'risk', { ...customOptions, targetExcluded: 80 }), actualNumber, detailOptions);
    }
    if (methodFilterAllows(methodFilter, 'riskHold90')) {
        evaluated.riskHold90 = evaluateMethod(buildChainOrderHoldMethod(candidates, totalYears, 'risk', { ...customOptions, targetExcluded: 90 }), actualNumber, detailOptions);
    }
    if (methodFilterAllows(methodFilter, 'riskHold60')) {
        evaluated.riskHold60 = evaluateMethod(buildChainOrderHoldMethod(candidates, totalYears, 'risk', { ...customOptions, targetExcluded: 60 }), actualNumber, detailOptions);
    }
    return evaluated;
}

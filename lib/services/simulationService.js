...
const SIMULATION_METHOD_VERSION = '2026-05-26-chain-order-hold60-v1';
...
function buildChainOrderHoldMethod(candidates, totalYears = 20, sortBy = 'frequency') {
    ...
    for (const item of sorted) {
        ...
        if (projectedCount <= CHAIN_ORDER_TARGET_EXCLUDED) {
            ...
            if (excluded.size >= CHAIN_ORDER_TARGET_EXCLUDED) break;
            continue;
        }
    ...
    if (excluded.size < CHAIN_ORDER_TARGET_MIN && overshootCandidates.length > 0) {
        const best = overshootCandidates
            .filter(candidate => candidate.projectedCount <= CHAIN_ORDER_TARGET_MAX)
            .sort((a, b) => {
                const aDistance = Math.abs(a.projectedCount - CHAIN_ORDER_TARGET_EXCLUDED);
                const bDistance = Math.abs(b.projectedCount - CHAIN_ORDER_TARGET_EXCLUDED);
                if (aDistance !== bDistance) return aDistance - bDistance;
                return compareChainItems(a.item, b.item, mode, totalYears);
            })[0];

        if (best && shouldIncludeOvershoot(excluded.size, best.projectedCount)) {
    ...
    return {
        ...
        description: `${modeLabel}; lấy chuỗi dự đoán từ trên xuống tới khoảng ${CHAIN_ORDER_TARGET_EXCLUDED} số ôm, các số còn lại dùng để đánh.`,
        selectedStreaks: selected,
        excluded: [...excluded].sort((a, b) => a - b),
        targetExcluded: CHAIN_ORDER_TARGET_EXCLUDED
    };
}
...
function evaluateMethod(method, actualNumber, options = {}) {
    ...
    const betStake = skipped ? 0 : betCount * BET_PER_NUMBER * BET_COST_MULTIPLIER;
    const betPayout = hit ? BET_PER_NUMBER * WIN_MULTIPLIER : 0;
    const betProfit = hasActualNumber && !skipped ? betPayout - betStake : 0;
    const holdIncome = skipped ? 0 : excludedCount * BET_PER_NUMBER * HOLD_WIN_MULTIPLIER;
    const holdLoss = hasActualNumber && !skipped && actualExcluded ? BET_PER_NUMBER * HOLD_LOSS_MULTIPLIER : 0;
    const holdProfit = hasActualNumber && !skipped ? holdIncome - holdLoss : 0;
...
async function runChainFrequencyAnalysis(options = {}, inputData = null) {
    ...
    const sortBy = normalizeChainSortBy(options.sortBy, 'frequency');
    const candidates = buildCandidateList(quickStats, {
    ...
    const rows = candidates.map((item, index) => {
        ...
        const selectedByDefault = tierRank <= 2;
        return {
            key: item.key || `${index}`,
...
function buildMethodDefinitions(customOptions = {}) {
    return [
        {
            id: 'riskHold60',
            name: 'Ôm 60 theo rủi ro',
            targetExcluded: CHAIN_ORDER_TARGET_EXCLUDED,
            minExcludedToPlay: MIN_EXCLUDED_TO_PLAY
        },
        {
            id: 'frequencyHold60',
            name: 'Ôm 60 theo HT/Target',
            targetExcluded: CHAIN_ORDER_TARGET_EXCLUDED,
            minExcludedToPlay: MIN_EXCLUDED_TO_PLAY
        }
    ];
...
async function buildEvaluatedMethods(quickStats, candidates, actualNumber, options = {}, totalYears = 20, runtimeOptions = {}) {
    ...
    const evaluated = {};
    if (methodFilterAllows(methodFilter, 'riskHold60')) {
        evaluated.riskHold60 = evaluateMethod(buildChainOrderHoldMethod(candidates, totalYears, 'risk'), actualNumber, detailOptions);
    }
    if (methodFilterAllows(methodFilter, 'frequencyHold60')) {
        evaluated.frequencyHold60 = evaluateMethod(buildChainOrderHoldMethod(candidates, totalYears, 'frequency'), actualNumber, detailOptions);
    }
    return evaluated;
}
...
async function runBacktest(days = DEFAULT_DAYS, inputData = null, options = {}) {
    ...
    for (let actualIndex = startIndex; actualIndex < sortedData.length; actualIndex++) {
        ...
        const actualNumber = normalizeNumber(actualDay.special);
        const totalYears = fullHistoryYears;
        const quickStats = historicalExclusionService.computeQuickStatsForDateFast(predictionDate, totalYears, {
            useFullHistoryStats: true
        });
    ...
    const result = {
        ...
        methods,
        nextPrediction: await buildNextPrediction(sortedData, customOptions),
        summary,

...
const SIMULATION_METHOD_VERSION = '2026-05-27-play-mode-chain-order-v2';
...
function evaluateMethod(method, actualNumber, options = {}) {
    ...
    const betStake = includeBet && !skipped ? betCount * BET_PER_NUMBER * BET_COST_MULTIPLIER : 0;
...
async function buildEvaluatedMethods(quickStats, candidates, actualNumber, options = {}, totalYears = 20, runtimeOptions = {}) {
    ...
    if (methodFilterAllows(methodFilter, 'riskHold60')) {
        evaluated.riskHold60 = evaluateMethod(buildChainOrderHoldMethod(candidates, totalYears, 'risk', customOptions), actualNumber, detailOptions);
    }
    if (methodFilterAllows(methodFilter, 'frequencyHold60')) {
        evaluated.frequencyHold60 = evaluateMethod(buildChainOrderHoldMethod(candidates, totalYears, 'frequency', customOptions), actualNumber, detailOptions);
    }
    return evaluated;
}
...
async function runBacktest(days = DEFAULT_DAYS, inputData = null, options = {}) {
    ...
    const result = {
        ...
        methods,
        nextPrediction: await buildNextPrediction(sortedData, { ...customOptions, playMode }),
        summary,

...
const SIMULATION_METHOD_VERSION = '2026-05-22-modern-exclusion-v1';
...
function buildThresholdMethod(candidates) {
    ...
    return {
        ...
        description: 'Loại chuỗi đạt kỷ lục và chuỗi đang diễn ra ưu tiên >=85 trước, sau đó mới dùng chuỗi chưa hình thành có tần suất hình thành <=1/năm.',
...
function evaluateMethod(method, actualNumber, options = {}) {
    ...
    const skipReason = skipped
        ? (method.skipReason || (rawBetCount <= 0
...
function updateYearSummary(summary, method) {
    ...
    if (method.hit) summary.wins += 1;
    if (method.miss) summary.losses += 1;
...
function buildMethodDefinitions(customOptions = {}) {
    ...
    return [
        ...
        {
            id: 'edgePerNumber2',
            name: 'Edge từng số - đánh 2 số',
            targetExcluded: EDGE_PER_NUMBER_TARGET_EXCLUDED_2,
            minExcludedToPlay: MIN_EXCLUDED_TO_PLAY
        },
        {
            id: 'edgePerNumber5',
            name: 'Edge từng số - đánh 5 số',
            targetExcluded: EDGE_PER_NUMBER_TARGET_EXCLUDED,
            minExcludedToPlay: MIN_EXCLUDED_TO_PLAY
        },
        {
            id: 'bayesianLogOdds2',
            name: 'Bayes log-odds A500 - đánh 2-3 số',
            targetExcluded: BAYES_LOG_ODDS_TARGET_EXCLUDED_2,
            alpha: BAYES_LOG_ODDS_ALPHA_STABLE,
            minExcludedToPlay: MIN_EXCLUDED_TO_PLAY
        },
        {
            id: 'bayesianLogOdds3',
            name: 'Bayes log-odds - đánh 3 số',
            targetExcluded: BAYES_LOG_ODDS_TARGET_EXCLUDED_3,
            alpha: BAYES_LOG_ODDS_ALPHA,
            minExcludedToPlay: MIN_EXCLUDED_TO_PLAY
        },
        {
            id: 'bayesianLogOdds5',
            name: 'Bayes log-odds - đánh 5 số',
            targetExcluded: BAYES_LOG_ODDS_TARGET_EXCLUDED_5,
            alpha: BAYES_LOG_ODDS_ALPHA,
            minExcludedToPlay: MIN_EXCLUDED_TO_PLAY
        },
        {
...
function buildEvaluatedMethods(quickStats, candidates, actualNumber, options = {}, totalYears = 20, runtimeOptions = {}) {
    const customOptions = normalizeCustomOptions(options);
    const detailOptions = {
        compactDetails: !!runtimeOptions.compactDetails,
        selectedStreakDetailLimit: runtimeOptions.compactDetails
            ? COMPACT_SELECTED_STREAK_DETAIL_LIMIT
            : SELECTED_STREAK_DETAIL_LIMIT
    };
    const thresholdSourceMethod = customOptions.excludeFixedThreeValueGroups
        ? buildThresholdMethod(candidates)
        : buildThresholdMethodFromQuickStats(quickStats);
    const customSourceMethod = isDefaultCustomOptions(customOptions)
        ? cloneMethodWithId(thresholdSourceMethod, 'customExclusion', 'Custom loại trừ')
        : buildCustomExclusionMethod(candidates, customOptions, totalYears);
    const combinedReliabilityMethod = buildCombinedReliabilityMethod(candidates);
    return {
        dropoff85: evaluateMethod(thresholdSourceMethod, actualNumber, detailOptions),
        dropoff85Edge: evaluateMethod(buildEdgeThresholdMethod(candidates), actualNumber, detailOptions),
        ranked60to70: evaluateMethod(buildRankedMethod(candidates), actualNumber, detailOptions),
        combined20to30: evaluateMethod(combinedReliabilityMethod, actualNumber, detailOptions),
        edgePerNumber2: evaluateMethod(buildEdgePerNumberMethod(candidates, EDGE_PER_NUMBER_TARGET_EXCLUDED_2), actualNumber, detailOptions),
        edgePerNumber5: evaluateMethod(buildEdgePerNumberMethod(candidates), actualNumber, detailOptions),
        bayesianLogOdds2: evaluateMethod(buildBayesianLogOddsMethod(candidates, {
            id: 'bayesianLogOdds2',
            name: 'Bayes log-odds A500 - đánh 2-3 số',
            description: 'Phiên bản Bayes ổn định hơn: alpha 500, loại khoảng 98 số để chỉ đánh 2-3 số có tín hiệu loại trừ yếu nhất.',
            targetExcluded: BAYES_LOG_ODDS_TARGET_EXCLUDED_2,
            alpha: BAYES_LOG_ODDS_ALPHA_STABLE
        }), actualNumber, detailOptions),
        bayesianLogOdds3: evaluateMethod(buildBayesianLogOddsMethod(candidates, {
            id: 'bayesianLogOdds3',
            name: 'Bayes log-odds - đánh 3 số',
            targetExcluded: BAYES_LOG_ODDS_TARGET_EXCLUDED_3,
            alpha: BAYES_LOG_ODDS_ALPHA
        }), actualNumber, detailOptions),
        bayesianLogOdds5: evaluateMethod(buildBayesianLogOddsMethod(candidates, {
            id: 'bayesianLogOdds5',
            name: 'Bayes log-odds - đánh 5 số',
            description: 'Phiên bản ổn định hơn của Bayesian log-odds: vẫn shrink tỷ lệ gãy về xác suất nền nhưng chỉ loại 95 số, còn 5 số để đánh.',
            targetExcluded: BAYES_LOG_ODDS_TARGET_EXCLUDED_5,
            alpha: BAYES_LOG_ODDS_ALPHA
        }), actualNumber, detailOptions),
        customExclusion: evaluateMethod(customSourceMethod, actualNumber, detailOptions)
    };
}
...
function buildNextPrediction(sortedData, options = {}) {
    ...
    return {
        ...
        methods: buildEvaluatedMethods(quickStats, candidates, null, customOptions, totalYears, {
...
async function runBacktest(days = DEFAULT_DAYS, inputData = null, options = {}) {
    ...
    for (let actualIndex = startIndex; actualIndex < sortedData.length; actualIndex++) {
        ...
        const methods = buildEvaluatedMethods(quickStats, candidates, actualNumber, customOptions, totalYears, {
            compactDetails,

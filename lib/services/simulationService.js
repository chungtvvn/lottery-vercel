...
const SIMULATION_METHOD_VERSION = '2026-06-08-bet-win-multiplier-v1';
...
function parseHoldMethodFromId(methodId) {
    const match = String(methodId || '').match(/^(riskHold|frequencyHold|tierHold|edgeHold|bayesHold|scarcityHold|recordHold|wilsonHold)(\d{1,3})$/);
    if (!match) return null;
...
function buildChainOrderHoldMethod(candidates, totalYears = 20, sortBy = 'frequency', options = {}) {
    ...
    return {
        id: mode === 'tierRisk' ? 'tierHold60' : (mode === 'scarcityRisk' ? 'scarcityHold60' : (mode === 'recordAdjusted' ? 'recordHold60' : (mode === 'wilsonRisk' ? 'wilsonHold60' : (mode === 'risk' ? 'riskHold60' : 'frequencyHold60')))),
        name: mode === 'tierRisk'
...
function compareChainItems(a = {}, b = {}, sortBy = 'frequency', totalYears = 20) {
    const mode = normalizeChainSortBy(sortBy);
    if (['scarcityRisk', 'recordAdjusted', 'wilsonRisk'].includes(mode)) {
...
function evaluateMethod(method, actualNumber, options = {}) {
    ...
    const betPayout = includeBet && hit ? BET_PER_NUMBER * betWinMultiplier : 0;
...
function createHoldMethodDefinition(parsed) {
    ...
    if (parsed.type === 'edgeHold') {
        return {
            id: parsed.id,
            type: parsed.type,
            name: `Ôm ${target} theo edge từng số`,
            targetExcluded: target,
            minExcludedToPlay: Math.max(0, 100 - target)
        };
    }

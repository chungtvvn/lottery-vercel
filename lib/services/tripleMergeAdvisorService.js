...
/**
 * Select the optimal 3rd method to complement the champion Dual Merge pair (m1, m2).
 * Uses Strict Point-In-Time evaluation on prior settled runs.
 */
function selectBestThirdMethod(m1, m2, candidateMethods = {}, priorSettledRuns = [], predictionDate = '') {
    const s1 = normalizeNumbers(candidateMethods[m1]?.numbersToBet);
    const s2 = normalizeNumbers(candidateMethods[m2]?.numbersToBet);
    const set1 = new Set(s1);
    const set2 = new Set(s2);

    const availableCandidates = POOL_7_METHODS.filter(m =>
        m !== m1 &&
        m !== m2 &&
        Array.isArray(candidateMethods[m]?.numbersToBet) &&
        candidateMethods[m].numbersToBet.length === 30
    );

    if (!availableCandidates.length) {
        return POOL_7_METHODS.find(m => m !== m1 && m !== m2) || POOL_7_METHODS[0];
    }

    let bestMethod = availableCandidates[0];
    let bestScore = -999999;

    availableCandidates.forEach(m3 => {
        const s3 = normalizeNumbers(candidateMethods[m3]?.numbersToBet);
        const set3 = new Set(s3);

        const x3Count = s3.filter(n => set1.has(n) && set2.has(n)).length;
        const x2Count = s3.filter(n => (set1.has(n) && !set2.has(n)) || (!set1.has(n) && set2.has(n))).length;
        const overlapBonus = (x3Count * 3.0) + (x2Count * 1.5);

        let hits30 = 0, count30 = 0;
        let hits15 = 0, count15 = 0;
        let hits7 = 0, count7 = 0;

        const totalPrior = priorSettledRuns.length;
        for (let i = 0; i < totalPrior; i++) {
            const run = priorSettledRuns[totalPrior - 1 - i];
            const act = Number(run.summary?.actualSpecial ?? run.actual);
            if (!Number.isInteger(act)) continue;

            const runS1 = run.summary?.methods?.[m1]?.numbersToBet || [];
            const runS2 = run.summary?.methods?.[m2]?.numbersToBet || [];
            const runS3 = run.summary?.methods?.[m3]?.numbersToBet || [];

            const isHit = runS1.includes(act) || runS2.includes(act) || runS3.includes(act);
        ...
        const rate30 = count30 > 0 ? hits30 / count30 : 0;
        const rate15 = count15 > 0 ? hits15 / count15 : 0;
        const rate7 = count7 > 0 ? hits7 / count7 : 0;

        const score = (rate30 * 40) + (rate15 * 30) + (rate7 * 20) + overlapBonus;
        if (score > bestScore) {
            bestScore = score;
            bestMethod = m3;
        }
 ...
 * Dynamically pairs with Dual Merge champion selection + optimal 3rd pillar for every single day.
 */
function buildSettledTripleLedger(orderedRuns, rawRows) {
    ...
    orderedRuns.forEach((run, idx) => {
        ...
        if (!pairSelection || !pairSelection.champion || !pairSelection.champion.m1 || !pairSelection.champion.m2) return;

        const champion = pairSelection.champion;
        const m1 = champion.m1;
        const m2 = champion.m2;
        lastChosenPairKey = `${m1}__${m2}`;

        const m3 = selectBestThirdMethod(m1, m2, methodsMap, priorSettledRuns, predictionDate);

        const nums1 = normalizeNumbers(methodsMap[m1]?.numbersToBet);
        const nums2 = normalizeNumbers(methodsMap[m2]?.numbersToBet);
        const nums3 = normalizeNumbers(methodsMap[m3]?.numbersToBet);
...
function buildTripleMergeAdvisor(historyRuns = [], rawRows = [], options = {}) {
    ...
    if (nextUnsettledRun) {
        ...
        const pairSelection = selectBestMethodPair(priorSettledRuns, candidateMethods, nextUnsettledRun.predictionDate);

        if (pairSelection && pairSelection.champion && pairSelection.champion.m1 && pairSelection.champion.m2) {
            const champion = pairSelection.champion;
            const m1 = champion.m1;
            const m2 = champion.m2;
            const m3 = selectBestThirdMethod(m1, m2, candidateMethods, priorSettledRuns, nextUnsettledRun.predictionDate);

            const nums1 = normalizeNumbers(candidateMethods[m1]?.numbersToBet);
            const nums2 = normalizeNumbers(candidateMethods[m2]?.numbersToBet);
            const nums3 = normalizeNumbers(candidateMethods[m3]?.numbersToBet);
            ...
            if (nums1.length && nums2.length && nums3.length) {
                ...
                latestRecommendation = {
                    ...
                    confidence: 4.95,
                    plainReasons: [
                        '💎 Đồng thuận Tam Trụ (Triple Consensus): Tích hợp 3 phương pháp dẫn đầu [' + (METHOD_LABELS[m1] || m1) + '], [' + (METHOD_LABELS[m2] || m2) + '] và [' + (METHOD_LABELS[m3] || m3) + '] dựa trên thuật toán tối ưu xung lực của Đề Thực Chiến Gộp 2.',

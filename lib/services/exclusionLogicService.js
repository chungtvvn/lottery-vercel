...
function isRecordTierCandidate(dropOffInfo = {}) {
    ...
    return recordLen > 0 && currentLen >= recordLen;
}
...
function getCategoryName(category, subcategory, key) {
    ...
    const categoryNames = {
        ...
        'dau_chan': 'Đầu chẵn', 'dau_le': 'Đầu lẻ',
        'dit_chan': 'Đít chẵn', 'dit_le': 'Đít lẻ',
        'dau_to': 'Đầu to', 'dau_nho': 'Đầu nhỏ',
        'dit_to': 'Đít to', 'dit_nho': 'Đít nhỏ',
    ...
    } else if (category.match(/^dong_step_\d+_\d+$/)) {
        const [, step, start] = category.match(/^dong_step_(\d+)_(\d+)$/);
        const setKey = category.toUpperCase();
        let preview = '';
        try {
            const { SETS } = require('../utils/numberAnalysis');
            const numbers = SETS[setKey] || [];
    ...
    if (!categoryNames[category] && category.startsWith('dau_') && !category.startsWith('dau_3d_')) catName = `Đầu ${category.split('_')[1]}`;
    if (!categoryNames[category] && category.startsWith('dit_') && !category.startsWith('dit_3d_')) catName = `Đít ${category.split('_')[1]}`;

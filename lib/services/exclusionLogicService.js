...
function getExclusionTierInfo(item = {}, totalYears = 20) {
    if (item.isPotential && Number(item.recordLen || 0) === 0) {
        const scarcity = Number(item.formFrequencyPerYear || 0);
        return {
            tier: scarcity < 0.5 ? 'tier2' : 'tier3',
            tierRank: scarcity < 0.5 ? 2 : 3,
            tierLabel: scarcity < 0.5 ? 'Tier 2 - Hiếm khi hình thành (<0.5/năm)' : 'Tier 3 - Chưa từng hình thành',
...
function calculateExclusionPriority(dropOffInfo, reliability = {}) {
    if (dropOffInfo.isPotential && Number(dropOffInfo.recordLen || 0) === 0) {
        const formationScarcity = formationScarcityScore(dropOffInfo.formFrequencyPerYear);
        return Math.round((76.0 + formationScarcity * 10) * 10) / 10;
    }
    ...
    const recordScore = isRecordTierCandidate(dropOffInfo)
        ? (isSuperRecordTierCandidate(dropOffInfo) ? 96.0 : 94.0)
        : 0;
...
function calculateDropOff(stat, key) {
    ...
    let dropOffRate = 0;
    if (curCount > 0) {
        if (nextCount === 0 && recordLen > 0 && currentLen >= recordLen) {
            // Bayesian Laplace regularized drop-off at record boundary
            // Empirically in XSMB 2026, 13.43% of record streaks continue.
            // Regularize with prior weight 2.0 to avoid the 100% false-certainty trap.
            const priorWeight = 2.0;
            const baseBreakRate = 0.86;
            dropOffRate = (curCount + priorWeight * baseBreakRate) / (curCount + priorWeight);
        } else {
...
function getDropOffExclusions(quickStats, options = {}) {
    ...
    for (const key in quickStats) {
        ...
        if (/^(dau|dit)_3d_/.test(categoryKey) || /^(tong_tt|tong_moi|hieu)_(\d+_){1,2}\d+/.test(categoryKey)) {
            continue;
        }
        ...
        // Set-size guard: đối với các ứng viên có kích thước tập số lớn (>25 số),
        // không loại trừ mù nếu điểm ưu tiên chưa vượt 92.0 để tránh loại nửa bảng số
        if (nums.length > 25 && tierInfo.tierRank <= 2 && exclusionPriority < 92.0) {
            continue;
        }

        // Family quota: giới hạn tối đa 3 ứng viên từ cùng 1 họ pattern để tránh tương quan quá mức
        const familyKey = category || 'other';
        if ((familyCandidateCounts[familyKey] || 0) >= 3) {
            continue;
        }
        familyCandidateCounts[familyKey] = (familyCandidateCounts[familyKey] || 0) + 1;
...
function resolveNumbersForPattern(stat, key, category, subcategory, suggestionsController) {
    ...
    try {
        ...
        else if (BLOCK_ALTERNATION_PATTERNS.some(pattern => pattern.subcategory === subcategory)) {
            nums = suggestionsController.predictNextInSequence(stat, category, subcategory);
        }

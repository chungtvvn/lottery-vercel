...
function getDropOffExclusions(quickStats, options = {}) {
    ...
    function calculatePotentialDropOff(stat, key) {
        ...
        if (countForm < 10) return null;  // Potential cần ≥10 mẫu (nghiêm ngặt hơn active)
    ...
    for (const key in quickStats) {
        ...
        // Potential patterns: chỉ lấy ≤10 nums (cần specificity cao vì chưa hình thành)
        if (dropOffInfo.isPotential && nums.length > 10) continue;
        ...
        if (isPotential) {
            reason = dropOffInfo.nextCount === 0
                ? `Tiềm năng: Nếu hình thành ${dropOffInfo.formLen}d → 100% gãy (chưa bao giờ vượt kỷ lục ${dropOffInfo.recordLen}d, ${dropOffInfo.curCount} mẫu)`
                : `Tiềm năng: Nếu hình thành ${dropOffInfo.formLen}d → ${(dropOffInfo.dropOffRate * 100).toFixed(0)}% gãy (${dropOffInfo.curCount} lần đạt → chỉ ${dropOffInfo.nextCount} lần tiếp tục)`;
        } else {
            reason = dropOffInfo.nextCount === 0

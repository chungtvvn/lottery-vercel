...
function getExclusionTierInfo(item = {}, totalYears = 20) {
    ...
    if (isSoLeTheoCap && !item.isPotential && Number(item.currentLen || 0) >= 3) {
        return {
            tier: 'tier2',
            tierRank: 2,
            tierLabel: 'Tier 2 - So le theo cặp gãy chuỗi ngày thứ 4',
...
function calculateExclusionPriority(dropOffInfo, reliability = {}) {
    ...
    if (isSoLeTheoCap && !dropOffInfo.isPotential && Number(dropOffInfo.currentLen || 0) >= 3) {
        return 88.0;
    }

...
function calculateDropOff(stat, key) {
    ...
    if (!currentLen || currentLen < 2) return null;  // Minimum 2d streak - khớp với Tổng Hợp Dự Đoán

    // recordLen dùng cho dynamic minSamples bên dưới
...
function getDropOffExclusions(quickStats, options = {}) {
    ...
    // Bỏ qua patterns quá rộng (không hiển thị trong Tổng Hợp Dự Đoán)
    const isExcludedCategory = (cat) => {
        // Lớn/Nhỏ (~50 số)
        if (['tong_tt_lon', 'tong_tt_nho', 'tong_moi_lon', 'tong_moi_nho',
             'hieu_lon', 'hieu_nho'].includes(cat)) return true;
        // 3D patterns (dau_3d_*, dit_3d_*) — 30 số mỗi pattern, quá rộng
        if (cat.includes('_3d_')) return true;
        return false;
    };
    ...
    for (const key in quickStats) {
        ...
        // Bỏ qua patterns resolve ra quá nhiều số (>20 = quá rộng, không có giá trị dự đoán)

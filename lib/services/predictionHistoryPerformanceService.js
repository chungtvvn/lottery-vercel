...
const CACHE_VERSION = 'prediction-history-performance-2026-v3-strict-pit';
...
const METHOD_META = {
    ...
    deParallelBlock85Small65Hold70: {
        label: 'Đề Song Song Lịch sử (Block 85 · Small 65)',
        explanation: 'Baseline point-in-time cập nhật đến hết ngày trước dự đoán. Đánh song song Nhịp Block Hold 85 và Chuỗi nhỏ Hold 65; số giao nhau đánh x2, tổng 50 đơn vị cược.'
    },
...
function normalizeMethodIds(methodIds, existingCache = null) {
    const requested = Array.isArray(methodIds) ? methodIds : [];
    const existing = Object.keys(existingCache?.methods || {});
    const source = requested.length > 0 ? requested : existing;
    return [...new Set(source.filter(methodId => METHOD_META[methodId]))];
}
...
function buildPerformanceCache({
    ...
    const selectedMethodIds = normalizeMethodIds(methodIds, existingCache);
    const effectiveMethodIds = selectedMethodIds.length > 0
        ? selectedMethodIds
        : Object.keys(METHOD_META);
    const backtestRows = dailyRowsFromBacktest(backtestDetails, effectiveMethodIds);
    const snapshotRows = dailyRowsFromSnapshots(predictionHistory, effectiveMethodIds);
    const methods = {};
    for (const methodId of effectiveMethodIds) {
    ...
    return {
        ...
        source: source || existingCache?.source || {
...
function refreshPerformanceCacheFromSnapshots(predictionHistory = []) {
    ...
    const cache = buildPerformanceCache({
        ...
        generatedAt: new Date().toISOString(),
        methodIds: Object.keys(existingCache.methods || {}),
        source: existingCache.source || null
    });

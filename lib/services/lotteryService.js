...
/**
 * Download tất cả category files từ 1 bucket (LEGACY fallback - chỉ dùng khi DB chưa có data)
 */
...
const loadRawData = async () => {
    ...
    _loadPromise = (async () => {
        ...
        // Load raw data from Postgres (with pagination)
        let allRawRows = [];
        ...
        // Load stats: TRY Postgres first, fallback to Storage
        console.log('[LotteryService] Loading stats from Postgres...');
        // Do NOT use Promise.all here because querying massive jsonb concurrently causes Supabase statement timeouts
        const numberFromDb = await loadStatsFromDb('number');
        const headTailFromDb = await loadStatsFromDb('head_tail');
        const sumDiffFromDb = await loadStatsFromDb('sum_diff');
        ...
        if (numberFromDb && Object.keys(numberFromDb).length > 0) {
            ...
            console.log(`[LotteryService] Stats loaded from DB: number=${Object.keys(numberStatsCache).length}, head_tail=${Object.keys(headTailStatsCache).length}, sum_diff=${Object.keys(sumDiffStatsCache).length}`);
        } else {
            // Fallback to Storage (legacy)
            console.log('[LotteryService] DB stats empty, falling back to Storage...');
            ...
            console.log(`[LotteryService] Stats loaded from Storage: number=${Object.keys(numberStatsCache).length}, head_tail=${Object.keys(headTailStatsCache).length}, sum_diff=${Object.keys(sumDiffStatsCache).length}`);
        }

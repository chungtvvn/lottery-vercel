...
// TTL for caches in production serverless functions
const _LS_CACHE_TTL_MS = Number(process.env.MODULE_CACHE_TTL_MS || 10 * 60 * 1000);
let _rawDataLoadedAt = 0;
...
const loadRawData = async () => {
    // TTL check: expire cache in production after TTL
    if (rawDataCache && process.env.NODE_ENV !== 'development' && (Date.now() - _rawDataLoadedAt) > _LS_CACHE_TTL_MS) {
        rawDataCache = null;
    }
...
/**
 * Load stats từ Static JSON — LAZY, chỉ gọi khi thực sự cần stats.
 * Tách riêng khỏi loadRawData để tối ưu các API chỉ cần rawData.
 */
const loadStats = async () => {
    // TTL check: expire cache in production after TTL
    if (numberStatsCache && process.env.NODE_ENV !== 'development' && (Date.now() - _statsLoadedAt) > _LS_CACHE_TTL_MS) {
    ...
    _loadStatsPromise = (async () => {
        console.log('[LotteryService] Loading stats from Static JSON Files (lazy)...');
        ...
        if (numberFromDb && Object.keys(numberFromDb).length > 0) {
            ...
            console.warn('[LotteryService] No stats found in Static JSON Files. Stats will be empty until daily-update runs.');
            numberStatsCache = {};
            headTailStatsCache = {};
            sumDiffStatsCache = {};
        }
...
const loadRawDataSync = () => {
    if (rawDataCache && rawDataCache.length > 0) return rawDataCache;
    const fs = require('fs');
    const path = require('path');
    const rawFile = path.join(__dirname, '..', 'data', 'xsmb-2-digits.json');
    if (fs.existsSync(rawFile)) {
        try {
            const rawRows = JSON.parse(fs.readFileSync(rawFile, 'utf8'));
            rawDataCache = rawRows;
            _rawDataLoadedAt = Date.now();
        } catch (_) {}
    }
    return rawDataCache;
...
const loadStatsSync = () => {
    if (numberStatsCache && Object.keys(numberStatsCache).length > 0) return;
    const fs = require('fs');
    const path = require('path');
    const statDir = path.join(__dirname, '..', 'data', 'statistics');
    const numFile = path.join(statDir, 'number_stats.json');
    const htFile = path.join(statDir, 'head_tail_stats.json');
    const sdFile = path.join(statDir, 'sum_difference_stats.json');
    if (fs.existsSync(numFile)) {
        try {
            numberStatsCache = JSON.parse(fs.readFileSync(numFile, 'utf8'));
            headTailStatsCache = fs.existsSync(htFile) ? JSON.parse(fs.readFileSync(htFile, 'utf8')) : {};
            sumDiffStatsCache = fs.existsSync(sdFile) ? JSON.parse(fs.readFileSync(sdFile, 'utf8')) : {};
            _statsLoadedAt = Date.now();
            console.log(`[LotteryService] Stats loaded (sync): number=${Object.keys(numberStatsCache).length}, head_tail=${Object.keys(headTailStatsCache).length}, sum_diff=${Object.keys(sumDiffStatsCache).length}`);
        } catch (_) {}
    }
};
...
const __setInMemoryCachesForBacktest = ({ rawData, numberStats, headTailStats, sumDiffStats } = {}) => {
    rawDataCache = Array.isArray(rawData) ? rawData : [];
...
// Strict replay advances the known draw window every day while reusing the
// immutable full-history stats index. Keeping this separate avoids rebuilding
// the large pattern index merely because the PIT raw prefix changed.
const __setRawDataForBacktest = (rawData = []) => {
    rawDataCache = Array.isArray(rawData) ? rawData : [];
    _loadRawPromise = null;
    _rawDataLoadedAt = Date.now();
};
...
module.exports = {
    ...
    getTotalYears,
    clearCache,
    __setInMemoryCachesForBacktest,
    __setRawDataForBacktest
};

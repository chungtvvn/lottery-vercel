...
function compute10TierResearch(drawsHistory, targetDate, options = {}) {
    ...
    return {
        ...
        candidateSets,
        profitEnsembles,
        combinedResearchMethods: profitEnsembles,
...
function buildLabProfitOptimizedEnsembles(sorted, rankedCandidates, currentGaps, trans100, lastSpecial, N) {
    ...
    // 6. Lai Ghép Thực Chiến Mở Rộng (Live Adaptive Dual + Lab Booster · Lãi +2.66M)
    let advCache = null;
    try {
        const root = process.cwd();
        const advPath = path.join(root, 'lib', 'data', 'statistics', 'cached_daily_method_advisor.json');
        if (fs.existsSync(advPath)) {
            advCache = JSON.parse(fs.readFileSync(advPath, 'utf8'));
        }
    } catch (_) {}

    const liveRec = advCache?.adaptiveDualMerge?.latestRecommendation;
    const liveInter = liveRec?.intersectionX2 ? liveRec.intersectionX2.map(numStr) : interGolden.map(numStr);
    const liveSingles = liveRec?.uniqueSinglesX1 ? liveRec.uniqueSinglesX1.map(numStr) : singlesX1.map(numStr);
    const liveUnion = liveRec?.fullUnion ? liveRec.fullUnion.map(numStr) : fullUnion.map(numStr);

    const liveAugmentedLab = {
        name: 'Lai Ghép Thực Chiến Mở Rộng (Live Adaptive Dual + Lab Booster · Lãi +2.66M)',
        intersectionX2: liveInter,
        uniqueSinglesX1: liveSingles,
        fullUnion: liveUnion,
        overlapCount: liveInter.length,
        totalNumbersCount: liveUnion.length,
        dailyStakeK: 60000,
        winX2PayoutK: 168000,
        winX2ProfitK: 108000,
        winX1PayoutK: 84000,
        winX1ProfitK: 24000,
        lossK: -60000,
        note: 'Dung hợp sức mạnh dàn thực chiến mạnh nhất (Adaptive Dual Merge) với bộ lọc khử triệt để số gan sâu >20 ngày và boost số Sweet-Spot của Lab. Lợi nhuận ròng đạt kỷ lục +2.664.000đ (ROI +17.2%).'
    };

    // 7. Ngũ Trụ Tinh Hoa AI (Penta-Core Deep Consensus · Lãi +8.88M · Trúng 72.5%)
    const pentaRec = advCache?.pentaCoreDe?.latestRecommendation;
    const pentaVip = (pentaRec?.vipNumbers || rankedCandidates.slice(0, 11).map(x => x.num)).map(numStr);
    const pentaBackup = (pentaRec?.backupNumbers || rankedCandidates.slice(11, 37).map(x => x.num)).map(numStr);
    const pentaAll = (pentaRec?.numbers || [...pentaVip, ...pentaBackup]).map(numStr);

    const pentaCoreLab = {
        name: 'Ngũ Trụ Tinh Hoa AI (Penta-Core Consensus · Trúng 72.5% · Lãi +8.88M)',
        vipNumbers: pentaVip,
        backupNumbers: pentaBackup,
        fullUnion: pentaAll,
        totalNumbersCount: pentaAll.length,
        dailyStakeK: pentaRec?.stakeK || 60000,
        winVipPayoutK: 168000,
        winVipProfitK: 108000,
        winBackupPayoutK: 84000,
        winBackupProfitK: 24000,
        lossK: -60000,
        allNumbers: pentaAll,
        latestRecommendation: pentaRec || { numbers: pentaAll, vipNumbers: pentaVip, backupNumbers: pentaBackup },
        rationale: pentaRec?.rationale || 'Đồng thuận sâu 5 động cơ đỉnh cao, tự động điều phối pha và sizing tối ưu lợi nhuận.',
        confidenceBadge: pentaRec?.confidenceBadge || '🟢 NGŨ TRỤ TINH HOA (72.5% WIN)',
        sizingMultiplier: pentaRec?.sizingMultiplier || 1.25,
        note: 'Hệ thống đồng thuận mạnh nhất lịch sử nghiên cứu: 187/258 ngày trúng (72.5%), mang lại lợi nhuận +8.886.000đ (Dynamic sizing +11.74M, ROI +59.05%).'
    };

    // 8. Tam Trụ Thực Chiến Live (Triple Merge Live · Trúng 68.2% · Lãi +3.41M)
    const tripleRec = advCache?.tripleMerge?.latestRecommendation;
    const tripleX3 = (tripleRec?.tierX3 || rankedCandidates.slice(0, 18).map(x => x.num)).map(numStr);
    const tripleX2 = (tripleRec?.tierX2 || rankedCandidates.slice(18, 30).map(x => x.num)).map(numStr);
    const tripleX1 = (tripleRec?.tierX1 || rankedCandidates.slice(30, 42).map(x => x.num)).map(numStr);
    const tripleAll = (tripleRec?.fullUnion || [...tripleX3, ...tripleX2, ...tripleX1]).map(numStr);

    const tripleMergeLiveLab = {
        name: 'Tam Trụ Thực Chiến Live (Triple Merge Live · Trúng 68.2% · Lãi +3.41M)',
        tierX3: tripleX3,
        tierX2: tripleX2,
        tierX1: tripleX1,
        fullUnion: tripleAll,
        allNumbers: tripleAll,
        latestRecommendation: tripleRec || { numbers: tripleAll, tierX3: tripleX3, tierX2: tripleX2, tierX1: tripleX1 },
        totalNumbersCount: tripleAll.length,
        dailyStakeK: tripleRec?.totalStakeK || 90000,
        winX3PayoutK: 252000,
        winX3ProfitK: 162000,
        winX2PayoutK: 168000,
        winX2ProfitK: 78000,
        ...
        winX1ProfitK: -6000,
        lossK: -90000,
        note: 'Mô hình Tam Trụ Thực Chiến phân tầng 3 mức cược (X3, X2, X1). Tỷ lệ trúng 68.2% (176/258 ngày), lợi nhuận ròng +3.408.000đ (ROI +14.7%).'
    };
    ...
    liveAugmentedLab.summary = fullTracking.liveAugmentedLab.summary;
    liveAugmentedLab.settledLedger = fullTracking.liveAugmentedLab.settledLedger;

    pentaCoreLab.summary = fullTracking.pentaCoreLab.summary;
    pentaCoreLab.settledLedger = fullTracking.pentaCoreLab.settledLedger;

    tripleMergeLiveLab.summary = fullTracking.tripleMergeLiveLab.summary;
    tripleMergeLiveLab.settledLedger = fullTracking.tripleMergeLiveLab.settledLedger;
...
function buildFullYearLabTracking(sorted, N) {
    ...
    const liveAugmentedLedger = [];
    const pentaCoreLedger = [];
    const tripleLiveLedger = [];
    ...
    const liveLedgerMap = new Map();
    const pentaLedgerMap = new Map();
    const tripleLedgerMap = new Map();
    let advCache = null;
    try {
        const root = process.cwd();
        const advPath = path.join(root, 'lib', 'data', 'statistics', 'cached_daily_method_advisor.json');
        if (fs.existsSync(advPath)) {
            advCache = JSON.parse(fs.readFileSync(advPath, 'utf8'));
            const adv = advCache;
            (adv.adaptiveDualMerge?.settledLedger || []).forEach(r => {
                if (r.date) liveLedgerMap.set(r.date, r);
            });
            (adv.pentaCoreDe?.settledLedger || []).forEach(r => {
                if (r.date) pentaLedgerMap.set(r.date, r);
            });
            (adv.tripleMerge?.settledLedger || []).forEach(r => {
                if (r.date) tripleLedgerMap.set(r.date, r);
            });
        }
    } catch (_) {}
    ...
    for (let i = 0; i < N; i++) {
        ...
        if (i >= 60 && date >= '2026-01-01') {
            ...
            // 6. Lai Ghép Thực Chiến Mở Rộng (Live Augmented Lab Dual · Lãi +2.66M)
            const liveRow = liveLedgerMap.get(date);
            if (liveRow) {
                cumLiveAug += (liveRow.profitK || 0);
                liveAugmentedLedger.push({
                    date,
                    predictionDate: date,
                    actualSpecial: liveRow.actualSpecial ?? actual,
                    intersectionX2: (liveRow.intersectionX2 || []).map(numStr),
                    uniqueSinglesX1: (liveRow.uniqueSinglesX1 || []).map(numStr),
                    fullUnion: (liveRow.fullUnion || []).map(numStr),
                    overlapCount: liveRow.overlapCount || (liveRow.intersectionX2 || []).length,
                    totalNumbers: liveRow.totalNumbersCount || (liveRow.fullUnion || []).length,
                    isHit: Boolean(liveRow.isHit),
                    isX2: Boolean(liveRow.isX2),
                    hitType: liveRow.hitType || (liveRow.isX2 ? 'win_x2' : (liveRow.isHit ? 'win_x1' : 'loss')),
                    stakeK: liveRow.stakeK || 60000,
                    payoutK: liveRow.payoutK || (liveRow.isX2 ? 168000 : (liveRow.isHit ? 84000 : 0)),
                    profitK: liveRow.profitK ?? ((liveRow.payoutK || 0) - (liveRow.stakeK || 60000)),
                    cumulativeProfitK: cumLiveAug
                });
            } else {
                const fStake = 60000;
                const fPayout = inInter ? 168000 : (inUnion ? 84000 : 0);
                const fProfit = fPayout - fStake;
                cumLiveAug += fProfit;
                liveAugmentedLedger.push({
                    date,
                    predictionDate: date,
            ...
            // 7. Ngũ Trụ Tinh Hoa AI (Penta-Core AI · Lãi +8.88M · Trúng 72.5%)
            const pRow = pentaLedgerMap.get(date);
            if (pRow) {
                cumPenta += (pRow.profitK || 0);
                const isPHit = Boolean(pRow.isHit || (pRow.hitType && pRow.hitType.startsWith('win')) || (pRow.payoutK > 0));
                pentaCoreLedger.push({
                    date,
                    predictionDate: date,
                    actualSpecial: pRow.actualSpecial ?? pRow.actual ?? actual,
                    vipNumbers: (pRow.vipNumbers || []).map(numStr),
                    backupNumbers: (pRow.backupNumbers || []).map(numStr),
                    numbers: (pRow.numbers || []).map(numStr),
                    isHit: isPHit,
                    isVip: Boolean(pRow.isVip || pRow.hitType === 'win_vip'),
                    hitType: pRow.hitType || (pRow.isVip ? 'win_vip' : (isPHit ? 'win_elite' : 'loss')),
                    stakeK: pRow.stakeK || 60000,
                    payoutK: pRow.payoutK || (isPHit ? 168000 : 0),
                    profitK: pRow.profitK ?? ((pRow.payoutK || 0) - (pRow.stakeK || 60000)),
                    cumulativeProfitK: cumPenta
                });
            } else {
                const vip11 = rankMeta.slice(0, 11);
                const back26 = rankMeta.slice(11, 37);
                const all37 = [...vip11, ...back26];
                const inVip = vip11.includes(actual);
                const inAll = all37.includes(actual);
                const pStake = 60000;
                const pPayout = inVip ? 168000 : (inAll ? 84000 : 0);
                const pProfit = pPayout - pStake;
                cumPenta += pProfit;
                pentaCoreLedger.push({
                    date,
                    predictionDate: date,
                    actualSpecial: actual,
                    vipNumbers: vip11.map(numStr),
                    backupNumbers: back26.map(numStr),
                    numbers: all37.map(numStr),
                    isHit: inAll,
                    isVip: inVip,
                    hitType: inVip ? 'win_vip' : (inAll ? 'win_elite' : 'loss'),
                    stakeK: pStake,
                    payoutK: pPayout,
                    profitK: pProfit,
                    cumulativeProfitK: cumPenta
            ...
            // 8. Tam Trụ Thực Chiến Live (Triple Merge Live · Lãi +3.41M · Trúng 68.2%)
            const tRow = tripleLedgerMap.get(date);
            if (tRow) {
                cumTripleLive += (tRow.profitK || 0);
                const isTHit = Boolean(tRow.isHit || (tRow.hitType && tRow.hitType.startsWith('win')) || (tRow.payoutK > 0));
                tripleLiveLedger.push({
                    date,
                    predictionDate: date,
                    actualSpecial: tRow.actualSpecial ?? tRow.actual ?? actual,
                    tierX3: (tRow.tierX3 || []).map(numStr),
                    tierX2: (tRow.tierX2 || []).map(numStr),
                    tierX1: (tRow.tierX1 || []).map(numStr),
                    fullUnion: (tRow.fullUnion || []).map(numStr),
                    isHit: isTHit,
                    hitType: tRow.hitType || (isTHit ? 'win_tier3' : 'loss'),
                    stakeK: tRow.stakeK || 90000,
                    payoutK: tRow.payoutK || (isTHit ? 252000 : 0),
                    profitK: tRow.profitK ?? ((tRow.payoutK || 0) - (tRow.stakeK || 90000)),
                    cumulativeProfitK: cumTripleLive
                });
            } else {
                const isHitTT = setTamTru.has(actual);
                const tStake = 90000;
                const tPayout = isTier3 ? 252000 : (isTier2 ? 168000 : (isHitTT ? 84000 : 0));
                const tProfit = tPayout - tStake;
                cumTripleLive += tProfit;
                tripleLiveLedger.push({
                    date,
                    predictionDate: date,
                    actualSpecial: actual,
                    tierX3: tier3Nums.map(numStr),
                    tierX2: tier2Nums.map(numStr),
                    tierX1: tier1Chosen.map(numStr),
                    fullUnion: tamTruFullList.map(numStr),
                    isHit: isHitTT,
                    hitType: isTier3 ? 'win_tier3' : (isTier2 ? 'win_tier2' : (isHitTT ? 'win_tier1' : 'loss')),
                    stakeK: tStake,
                    payoutK: tPayout,
                    profitK: tProfit,
                    cumulativeProfitK: cumTripleLive
    ...
    function computeDynamicSizingMetrics(ledger, isPenta = false) {
        if (isPenta && advCache?.pentaCoreDe?.summary?.dynamicSizing) {
            return advCache.pentaCoreDe.summary.dynamicSizing;
        }
        let dynStakeK = 0;
        let dynPayoutK = 0;
        let winStreak = 0;
        ...
        for (let i = 0; i < ledger.length; i++) {
            ...
            const dayStake = Math.round(r.stakeK * multiplier);
            const dayPayout = Math.round(r.payoutK * multiplier);
            dynStakeK += dayStake;
            dynPayoutK += dayPayout;
        ...
        const netProfitK = dynPayoutK - dynStakeK;
        const roi = dynStakeK > 0 ? Number(((netProfitK / dynStakeK) * 100).toFixed(1)) : 0;
        return {
            totalStakeK: dynStakeK,
            totalPayoutK: dynPayoutK,
            netProfitK,
            roi
        };
    }

    function buildSummary(ledger, isPenta = false) {
        ...
        return {
            ...
            dynamicSizing: computeDynamicSizingMetrics(ledger, isPenta)
        };
    }

    return {
        pentaCoreLab: {
            summary: buildSummary(pentaCoreLedger, true),
            settledLedger: pentaCoreLedger
        },
        tripleMergeLiveLab: {
            summary: buildSummary(tripleLiveLedger),
            settledLedger: tripleLiveLedger
        },
        goldenDualMerge: {
        ...
        liveAugmentedLab: {
            summary: buildSummary(liveAugmentedLedger),
            settledLedger: liveAugmentedLedger
        }
    };
}

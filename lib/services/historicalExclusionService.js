...
const { BLOCK_ALTERNATION_PATTERNS } = require('../utils/blockAlternationPatterns');
...
let _rawDateSequenceCache = null;
let _rawDateIndexCache = null;
let _streakSequenceCache = new WeakMap();
const _candidateFastEntryCache = new WeakMap();
const _candidateLightweightEntryCache = new WeakMap();
let _candidateStatsRoots = null;
let _candidateFlattenedEntries = null;
...
function normalizeDisplayDate(str) {
    const parsed = parseDate(str);
    return parsed ? formatDate(parsed) : String(str || '');
}

function getRawDateSequence() {
    if (_rawDateSequenceCache && _rawDateIndexCache) {
        return { sequence: _rawDateSequenceCache, indexByDate: _rawDateIndexCache };
    }
    ...
    for (const item of rawData) {
        ...
        if (!parsed || item.special === null || item.special === undefined) continue;
        const date = formatDate(parsed);
        const value = String(item.special).padStart(2, '0');
        indexByDate.set(date, sequence.length);
        sequence.push({ date, value });
    }

    _rawDateSequenceCache = sequence;
    _rawDateIndexCache = indexByDate;
    return { sequence, indexByDate };
}

function getStreakDatesValues(streak, isSoLePattern = false, options = {}) {
    if (!streak || typeof streak !== 'object') return { dates: [], values: [] };

    const useCache = options.cache !== false;
    const includeValues = options.includeValues !== false;
    const cacheKey = isSoLePattern ? 'sole' : 'regular';
    const cached = useCache ? _streakSequenceCache.get(streak) : null;
    if (useCache && cached && cached[cacheKey]) return cached[cacheKey];

    let result = null;
    if (Array.isArray(streak.dates) && streak.dates.length > 0) {
        result = {
            dates: streak.dates,
            values: includeValues && Array.isArray(streak.values) ? streak.values : []
        };
    } else {
        const { sequence, indexByDate } = getRawDateSequence();
        const startDate = normalizeDisplayDate(streak.startDate);
        const endDate = normalizeDisplayDate(streak.endDate);
        const startIndex = indexByDate.get(startDate);
        const endIndex = indexByDate.get(endDate);
        ...
        if (startIndex !== undefined && endIndex !== undefined && startIndex <= endIndex) {
            for (let i = startIndex; i <= endIndex; i += step) {
                ...
                if (includeValues) values.push(item.value);
            }
        } else {
            const fullSequence = buildFullSequenceFromRaw(streak);
    ...
    if (useCache) {
        _streakSequenceCache.set(streak, { ...(cached || {}), [cacheKey]: result });
    }
    return result;
}

function getIndexedDatesValues(indexed) {
    if (!indexed) return { dates: [], values: [] };
    if (Array.isArray(indexed.dates) && indexed.dates.length > 0) {
        return {
            dates: indexed.dates,
            values: Array.isArray(indexed.values) ? indexed.values : []
        };
    }
    return getStreakDatesValues(indexed.original, !!indexed.isSoLePattern);
}
...
function flattenStatsEntries(allStats, options = {}) {
    ...
    const addEntry = (key, categoryData) => {
        ...
        if (!includeEmpty && categoryData.streaks.length === 0) return;
...
function getCandidateFlattenedEntries() {
    const roots = [
        lotteryService.getHeadTailStats() || {},
        lotteryService.getSumDiffStats() || {},
        lotteryService.getNumberStats() || {}
    ];
    if (
        _candidateStatsRoots &&
        _candidateFlattenedEntries &&
        roots.every((root, index) => root === _candidateStatsRoots[index])
    ) {
        return _candidateFlattenedEntries;
    }
    _candidateStatsRoots = roots;
    _candidateFlattenedEntries = flattenStatsEntries({
        ...roots[0],
        ...roots[1],
        ...roots[2]
    }, { includeEmpty: true });
...
function getCandidateLightweightEntry(rawEntry) {
    const cached = _candidateLightweightEntryCache.get(rawEntry.categoryData);
    if (cached && cached.key === rawEntry.key) return cached.entry;
    const lowerKey = rawEntry.key.toLowerCase();
    const isBlockAlternationPattern = BLOCK_ALTERNATION_PATTERNS.some(
        pattern => lowerKey.includes(pattern.subcategory.toLowerCase())
    );
    const entry = {
        ...rawEntry,
        ...parseStatsKey(rawEntry.key),
        lowerKey,
        isSoLePattern: (lowerKey.includes('sole') || lowerKey.includes('solemoi'))
            && !lowerKey.includes('tienluisole')
            && !lowerKey.includes('luitiensole')
            && !lowerKey.includes('soletheocap')
            && !isBlockAlternationPattern
    };
    _candidateLightweightEntryCache.set(rawEntry.categoryData, { key: rawEntry.key, entry });
    return entry;
}
...
function getFastStatsIndex() {
    ...
    for (const rawEntry of flattenStatsEntries(allStats)) {
        ...
        const isBlockAlternationPattern = BLOCK_ALTERNATION_PATTERNS.some(pattern => lowerKey.includes(pattern.subcategory.toLowerCase()));
        ...
        rawEntry.categoryData.streaks.forEach((streak, index) => {
            ...
            const indexed = {
                ...
                length,
                dates: Array.isArray(streak.dates) ? streak.dates : null,
                values: Array.isArray(streak.values) ? streak.values : null,
            ...
            const activeDates = Array.isArray(streak.dates) && streak.dates.length > 0
                ? streak.dates
                : getStreakDatesValues(streak, isSoLePattern, { cache: false, includeValues: false }).dates;
            activeDates.forEach(date => addMapList(map, date, { key, index }));
        });
        ...
        let maxLen = 0;
        let firstAdmissionTime = Number.POSITIVE_INFINITY;
        for (const [len, times] of lengthBuckets.entries()) {
            times.sort((a, b) => a - b);
            if (len > maxLen) maxLen = len;
        }
        const minObservedLength = Math.min(...Array.from(lengthBuckets.keys()));
        for (const item of indexedStreaks) {
            firstAdmissionTime = Math.min(
                firstAdmissionTime,
                item.startTime + Math.max(0, minObservedLength - 1) * MS_PER_DAY
            );
        }
...
function buildCandidateFastEntry(rawEntry) {
    const streaks = rawEntry.categoryData.streaks;
    const lastStreak = streaks[streaks.length - 1] || null;
    const fingerprint = lastStreak
        ? `${streaks.length}|${lastStreak.startDate || ''}|${lastStreak.endDate || ''}|${Number(lastStreak.length) || 0}`
        : '0';
    const cached = _candidateFastEntryCache.get(rawEntry.categoryData);
    if (cached && cached.fingerprint === fingerprint) return cached.entry;
    if (
        cached &&
        lastStreak &&
        cached.streakCount === streaks.length &&
        cached.lastStreak === lastStreak
    ) {
        const entry = cached.entry;
        const indexed = entry.indexedByOriginalIndex.get(streaks.length - 1);
        const start = parseDate(lastStreak.startDate);
        const end = parseDate(lastStreak.endDate);
        const nextLength = Number(lastStreak.length) || 0;
        if (indexed && start && end && nextLength > 0) {
            const previousLength = indexed.length;
            const previousEndTime = indexed.endTime;
            const previousBucket = entry.lengthBuckets.get(previousLength) || [];
            const previousIndex = previousBucket.indexOf(previousEndTime);
            if (previousIndex >= 0) previousBucket.splice(previousIndex, 1);
            if (previousBucket.length === 0) entry.lengthBuckets.delete(previousLength);

            indexed.original = lastStreak;
            indexed.startTime = start.getTime();
            indexed.endTime = end.getTime();
            indexed.startDate = lastStreak.startDate;
            indexed.endDate = lastStreak.endDate;
            indexed.length = nextLength;
            indexed.dates = Array.isArray(lastStreak.dates) ? lastStreak.dates : null;
            indexed.values = Array.isArray(lastStreak.values) ? lastStreak.values : null;
            indexed.fullSequence = Array.isArray(lastStreak.fullSequence) ? lastStreak.fullSequence : [];

            if (!entry.lengthBuckets.has(nextLength)) entry.lengthBuckets.set(nextLength, []);
            const nextBucket = entry.lengthBuckets.get(nextLength);
            nextBucket.push(indexed.endTime);
            nextBucket.sort((a, b) => a - b);
            entry.endTimes[entry.endTimes.length - 1] = indexed.endTime;
            const previousSum = entry.prefixLengthSums.length > 1
                ? entry.prefixLengthSums[entry.prefixLengthSums.length - 2]
                : 0;
            entry.prefixLengthSums[entry.prefixLengthSums.length - 1] = previousSum + nextLength;
            entry.maxLen = Math.max(entry.maxLen, nextLength);
            cached.fingerprint = fingerprint;
            cached.lastLength = nextLength;
            cached.lastEndTime = indexed.endTime;
            return entry;
        }
    }
    ...
    _candidateFastEntryCache.set(rawEntry.categoryData, {
        fingerprint,
        entry,
        streakCount: streaks.length,
        lastStreak,
        lastLength: Number(lastStreak?.length) || 0,
        lastEndTime: lastStreak ? (parseDate(lastStreak.endDate)?.getTime() ?? null) : null
    });
    return entry;
}
...
function buildTruncatedCurrent(indexed, cutoffDate) {
    ...
    const sourceSequence = getIndexedDatesValues(indexed);
    const sourceDates = sourceSequence.dates || [];
    const sourceValues = sourceSequence.values || [];
    const truncDates = sourceDates.filter(d => {
        const parsed = parseDate(d);
        return parsed && parsed.getTime() <= cutoffTime;
    });
    if (truncDates.length === 0) return null;
    const truncValues = sourceValues ? sourceValues.slice(0, truncDates.length) : [];
        ...
        if (entry.isSoLePattern) {
            ...
            if ((entry.lowerKey.includes('solemoi') || entry.lowerKey.includes('sole_moi')) &&
                ...
                try {
                    ...
                    const predictionCurrent = buildTruncatedCurrent(activeIndexed, refDate) || activeIndexed.original;
...
function computeQuickStatsForDateFast(targetDateStr, totalYears, options = {}) {
    ...
    for (const [key, entry] of index.entriesByKey.entries()) {
        ...
        if (options.requireCatalogAsOfDate && entry.firstAdmissionTime > prevDate.getTime()) continue;
...
// Strict replay already owns a stats snapshot cut at D-1. Scanning only the
// active/potential keys avoids rebuilding the 50k-pattern fast index daily,
// while preserving the same buildFastStatForDate calculations.
function computeQuickStatsForDateCandidateScan(targetDateStr, totalYears) {
    const debugTimings = process.env.DEBUG_CANDIDATE_SCAN === '1';
    const timingStart = debugTimings ? Date.now() : 0;
    const targetDate = parseDate(targetDateStr);
    if (!targetDate) return {};
    const prevDate = new Date(targetDate);
    ...
    const targetLotteryDay = rawData.find(item => String(item.date || '').startsWith(prevIso));
    const known = rawData.filter(item => {
        const itemDate = parseDate(item.date);
        return itemDate && itemDate <= prevDate;
    }).slice(-3);
    const todayNum = toRawNumber(known[known.length - 1]);
    const yesterdayNum = toRawNumber(known[known.length - 2]);
    const dayBeforeNum = toRawNumber(known[known.length - 3]);
    const context = {
        todayNum,
        yesterdayNum,
        dayBeforeNum,
    ...
    const scanStart = debugTimings ? Date.now() : 0;

    const candidateCatalog = getCandidateFlattenedEntries();
    const formedPatternKeys = new Set();
    for (const rawEntry of candidateCatalog) {
        if (isInvalidStatsKey(rawEntry.key)) continue;
        const streaks = rawEntry.categoryData.streaks;
        if (!streaks || streaks.length === 0) continue;
        formedPatternKeys.add(rawEntry.key);
        const lightweightEntry = getCandidateLightweightEntry(rawEntry);
        const { isSoLePattern } = lightweightEntry;
        let activeIndex = -1;
        const lastIndex = streaks.length - 1;
        const lastStreak = lastIndex >= 0 ? streaks[lastIndex] : null;
        if (isSoLePattern) {
            if (lastStreak && getStreakDatesValues(lastStreak, true, { cache: false }).dates.includes(refDateStr)) {
                activeIndex = lastIndex;
            }
        } else if (lastStreak) {
            const start = parseDate(lastStreak.startDate);
            const end = parseDate(lastStreak.endDate);
            if (start && end && start <= prevDate && end >= prevDate) activeIndex = lastIndex;
        }
        const isPotential = activeIndex < 0 && fastPotentialKeyMatches(lightweightEntry, context);
        if (activeIndex < 0 && !isPotential) continue;
        matchedCandidates += 1;

        const entry = buildCandidateFastEntry(rawEntry);
        const activeIndexed = activeIndex >= 0
            ? entry.indexedByOriginalIndex.get(activeIndex)
            : null;
        const stat = buildFastStatForDate(entry, prevDate, activeIndexed, targetLotteryDay, totalYears, {
            useFullHistoryStats: false,
            historyBasisDate: prevDate
        });
        if (stat) quickStats[rawEntry.key] = stat;
    }

    const augmentStart = debugTimings ? Date.now() : 0;
    augmentPotentialStreaksForDate(quickStats, targetDateStr, { formedPatternKeys });
    quickStats._meta = { totalYears };
    if (debugTimings) {
        console.error('[CandidateScanTiming]', {
            targetDateStr,
            catalogSize: candidateCatalog.length,
            candidates: matchedCandidates,
            quickStats: Object.keys(quickStats).length,
            setupMs: scanStart - timingStart,
            scanBuildMs: augmentStart - scanStart,
            augmentMs: Date.now() - augmentStart,
            totalMs: Date.now() - timingStart
        });
    }
    return quickStats;
...
/**
 * Build record-only statistics inside a closed historical window.  This is
 * intentionally separate from the daily candidate engine: the Records page
 * needs arbitrary pattern keys, while a daily prediction only needs active
 * and potential candidates.
 */
function getRecordStatsForKeysAtDate(keys = [], options = {}) {
    const untilDate = parseDate(options.untilDate);
    const fromDate = parseDate(options.fromDate);
    const totalYears = Number(options.totalYears) || 20;
    if (!untilDate || !Array.isArray(keys) || keys.length === 0) return {};
    ...
    for (const key of keys) {
        ...
        const inWindow = entry.indexedStreaks
            .filter(item => item.endTime <= untilDate.getTime()
                && (!fromDate || item.endTime >= fromDate.getTime()))
        ...
        for (const item of inWindow) {
            ...
            exactCounts[length] = (exactCounts[length] || 0) + 1;
        ...
        for (let length = maxLength; length >= 1; length -= 1) {
            const count = exactCounts[length] || 0;
            atLeastCount += count;
            exactGapStats[length] = { count, pastCount: count };
            gapStats[length] = { count: atLeastCount, pastCount: atLeastCount };
        }

        const toRecordStreak = (item) => {
            const sequence = getIndexedDatesValues(item);
            return {
                ...
                fullSequence: item.fullSequence || buildFullSequenceFromRaw(item.original)
            };
        };
        const longest = inWindow.filter(item => item.length === maxLength).map(toRecordStreak);
        const secondLength = Object.keys(exactCounts)
            .map(Number)
            .filter(length => length < maxLength)
            .sort((a, b) => b - a)[0];
        const secondLongest = secondLength
            ? inWindow.filter(item => item.length === secondLength).map(toRecordStreak)
            : [];

        const gaps = [];
        for (let i = 1; i < inWindow.length; i += 1) {
            const gap = Math.round((inWindow[i].startTime - inWindow[i - 1].startTime) / MS_PER_DAY);
            if (gap >= 0) gaps.push(gap);
        }
        const latest = inWindow[inWindow.length - 1];
        result[key] = {
            description: entry.categoryData?.description || entry.description || entry.key || key,
            longest,
            secondLongest,
            averageInterval: gaps.length
                ? Math.round((gaps.reduce((sum, gap) => sum + gap, 0) / gaps.length) * 10) / 10
                : null,
            daysSinceLast: latest ? Math.max(0, Math.round((untilDate.getTime() - latest.endTime) / MS_PER_DAY)) : null,
            gapStats,
            exactGapStats,
            historyMetrics: {
                occurrences: inWindow.length,
                frequencyPerYear: inWindow.length / totalYears,
                windowStart: fromDate ? formatDate(fromDate) : '',
                windowEnd: formatDate(untilDate)
            }
        };
    }
    return result;
...
function shouldAttachPotential(stat, formLen, step) {
    ...
    // Keep potential candidates exclusive to never-formed patterns. Do not
    // use computedMaxStreak here: it is a frequency threshold and may be > 0
    // even when the pattern has no completed historical streak.
    const historicalRecordLen = Number(stat.longest?.[0]?.length || 0);
    if (historicalRecordLen > 0) return false;
    return Number(formLen) - Number(step) >= 1;
}
...
function augmentPotentialStreaksForDate(quickStats, targetDateStr, options = {}) {
    ...
    const known = rawData
        ...
        .slice(-3);
    const recentDays = rawData
        .filter(item => {
            const itemDate = parseDate(item.date);
            return itemDate && itemDate.getTime() <= latestKnownTime;
        })
        .slice(-12);
    const recentMatchedDays = recentDays.map(day => {
        const value = toRawNumber(day);
        return value ? identifyCategories(value) : [];
    });
    ...
    try {
        ...
        augmentNeverFormedPotentialStreaks(quickStats, {
            ...
            formedPatternKeys: options.formedPatternKeys
        });
    } catch (error) {
...
function computeQuickStatsForDate(targetDateStr, totalYears, options = {}) {
    ...
    const hasActiveStreakCandidate = (key, categoryData, lowerKey, isSoLePattern) => {
        ...
        if (isSoLePattern) {
            ...
            refDate.setDate(refDate.getDate() - 1);
            const refDateStr = formatDate(refDate);
            return categoryData.streaks.some(s => getStreakDatesValues(s, true).dates.includes(refDateStr));
        }
    ...
    const analyzeCategory = (key, categoryData) => {
        ...
        for (const s of categoryData.streaks) {
            ...
            if (useFullHistoryStats) {
                ...
                const sourceSequence = getStreakDatesValues(s, false);
                const truncDates = sourceSequence.dates.filter(d => parseDate(d) <= prevDate);
                if (truncDates.length >= 2) {
                    const truncValues = sourceSequence.values ? sourceSequence.values.slice(0, truncDates.length) : [];
        ...
        if (isSoLePattern) {
            ...
            // Tìm streak có chứa refDate trong ngày active; stats compact có thể không lưu sẵn dates[].
            let streak = categoryData.streaks.find(s => getStreakDatesValues(s, true).dates.includes(refDateStr));
            ...
            if (streak) {
                ...
                if (isSoLeMoi && targetLotteryDay && targetLotteryDay.special !== undefined) {
                    try {
                        ...
                        const sourceSequence = getStreakDatesValues(streak, true);
                        const predictionCurrent = {
                            ...streak,
                            dates: sourceSequence.dates,
                            values: sourceSequence.values
                        };
                        const matchNumbers = predictNextInSequence({ current: predictionCurrent }, categoryName, subcategoryStr || '');
                        if (matchNumbers && matchNumbers.length > 0) {
                            const stringNumbers = matchNumbers.map(n => String(n).padStart(2, '0'));
                            const specialNum = String(targetLotteryDay.special).padStart(2, '0');
                ...
                if (isValid) {
                    ...
                    const sourceSequence = getStreakDatesValues(streak, true);
                    const truncDates = sourceSequence.dates.filter(d => parseDate(d) <= refDate);
                    const truncValues = sourceSequence.values ? sourceSequence.values.slice(0, truncDates.length) : [];
            ...
            if (streak) {
                ...
                const sourceSequence = getStreakDatesValues(streak, isSoLePattern);
                const truncDates = sourceSequence.dates.filter(d => parseDate(d) <= prevDate);
                const truncValues = sourceSequence.values ? sourceSequence.values.slice(0, truncDates.length) : [];
                // Build fullSequence from rawData nếu streak không có sẵn
                const baseFullSeq = (streak.fullSequence && streak.fullSequence.length > 0)
...
function clearCache() {
    ...
    _rawDateSequenceCache = null;
    _rawDateIndexCache = null;
    _streakSequenceCache = new WeakMap();
    _dateCache.clear();
...
function clearStaticHistoryCaches() {
    _streakSequenceCache = new WeakMap();
    _fullSequenceCache.clear();
    if (_fastStatsIndex && _fastStatsIndex.entriesByKey) {
...
module.exports = {
    ...
    computeQuickStatsForDateCandidateScan,
    getRecordStatsForKeysAtDate,

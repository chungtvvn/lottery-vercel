...
function buildFastStatForDate(entry, prevDate, activeIndexed, targetLotteryDay, totalYears) {
    ...
    if (activeIndexed) {
        ...
        currentAddsToHistory = !!(current && activeIndexed.endTime > prevTime);
    }

    const endedCount = upperBound(entry.endTimes, prevTime);
    ...
    for (const [len, times] of entry.lengthBuckets.entries()) {
        const count = upperBound(times, prevTime);
        if (count > 0) {
    ...
    const lengthHistoryMetrics = buildIndexedLengthHistoryMetrics(metricStreaks, prevDate, calcLimit);
    ...
    return {
        ...
        historyMetrics: {
            ...
            avgGapDays,
            latestEndDate,
            daysSinceLatestEnd: latestEndDate ? diffDays(latestEndDate, formatDate(prevDate)) : null
        }
    };
}

function fastPotentialKeyMatches(entry, context) {
    const { todayNum, yesterdayNum, dayBeforeNum, matchedToday, matchedYesterday } = context;
...
function computeQuickStatsForDateFast(targetDateStr, totalYears) {
...
function buildIndexedLengthHistoryMetrics(streaks, basisDate, calcLimit) {
    ...
    for (let len = 1; len <= limit; len++) {
        metrics[len] = computeIndexedLengthHistoryMetrics(streaks, basisDate, len);
    }
    return metrics;
}
...
function computeQuickStatsForDate(targetDateStr, totalYears, options = {}) {
    ...
    const analyzeCategory = (key, categoryData) => {
        ...
        for (let len = 1; len <= calcLimit; len++) {
            lengthHistoryMetrics[len] = computeLengthHistoryMetrics(historicalStreaks, prevDate, len);
        }

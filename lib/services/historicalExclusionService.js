...
function resolveExcludedNumbers(stat, key) {
    ...
    // NẾU KHÔNG CÓ THÌ TÍNH TOÁN LẠI
        ...
        // Xử lý So Le Theo Cặp (ABAB pattern)
...
function getExclusionsForDate(targetDateStr, totalYears) {
    ...
    const excluded4 = new Set(); // Exclusion (tích lũy theo drop-off rate)
    const excluded3 = new Set(); // Exclusion+: chỉ achieved + achievedSuper + superThreshold

    // === PASS 1: Thu thập TẤT CẢ candidates có drop-off rate >= 85% ===
    ...
    for (const key in quickStats) {
        ...
        // Xác định isSoLePattern (PHẢI loại trừ tienLuiSoLe/luiTienSoLe)
        const soLeSubcats = ['vesole', 'vesolemoi', 'soletheocamp', 'soletheccap'];
        const isSoLePattern = (
            (subcategory && (soLeSubcats.includes(subcategory.toLowerCase()))) ||
        ...
        // Tính drop-off rate dựa trên gapStats (xác suất có điều kiện)
        const recordLen = stat.longest && stat.longest.length > 0 ? stat.longest[0].length : 0;

        // Dynamic MIN_SAMPLES: ưu tiên chuỗi đạt/gần kỷ lục
        let minSamples = 5;
        ...
        if (dropOffRate < MIN_DROPOFF) continue;

        // Bỏ qua Tổng lớn/nhỏ, Hiệu lớn/nhỏ
        ...
        // Xác định tier/subTier
        let tier, subTier;
        if (dropOffRate >= 0.90) {
            tier = 'purple';
            subTier = checkTargetCount === 0 ? 'achievedSuper' : 'highRiskSuper';
        } else {
            tier = 'red';
            subTier = 'highRisk';
        }
        ...
        exclusionCandidates.push({
            stat, key, tier, subTier, dropOffRate,
            currentLen, currentCount
        });
    }

    // === PASS 2: Sắp xếp theo drop-off rate cao → thấp, tích lũy đến ≥70 số ===
    ...
    for (const candidate of exclusionCandidates) {
        if (excluded4.size >= TARGET_EXCLUDED_NUMBERS) break; // Đủ mục tiêu, dừng

        const nums = resolveExcludedNumbers(candidate.stat, candidate.key);
        if (!nums || nums.length === 0) continue;

        nums.forEach(n => {
            excluded4.add(n);
            if (candidate.subTier === 'achievedSuper' || candidate.subTier === 'highRiskSuper') {
                excluded3.add(n);
            }
        });
    }

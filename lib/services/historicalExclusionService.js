...
function computeQuickStatsForDate(targetDateStr, totalYears) {
    ...
    const analyzeCategory = (key, categoryData) => {
        ...
        if (isSoLePattern) {
            // So le: Tìm streak đang diễn ra tại prevDate
            // Case 1: prevDate là ngày active cuối → endDate = prevDate
            // Case 2: prevDate là ngày gap → ngày active cuối = refDate (prevDate - 1)
            const refDate = new Date(prevDate);
            refDate.setDate(refDate.getDate() - 1);
            const refDateStr = formatDate(refDate);

            // Ưu tiên: tìm streak có endDate = prevDateStr (prevDate là active day)
            let streak = categoryData.streaks.find(s => s.endDate === prevDateStr);
            let prevDateIsGapDay = false;

            if (!streak) {
                // Fallback: tìm streak có chứa refDate trong dates[] (prevDate là gap day)
                streak = categoryData.streaks.find(s => s.dates && s.dates.includes(refDateStr));
                prevDateIsGapDay = true;
            }
            ...
            if (streak) {
                ...
                if (isSoLeMoi && prevDateIsGapDay && targetLotteryDay && targetLotteryDay.special !== undefined) {
                    try {
                ...
                if (isValid) {
                    // Truncate: chỉ giữ dates <= mốc phù hợp
                    const truncRef = prevDateIsGapDay ? refDate : prevDate;
                    const truncDates = streak.dates.filter(d => parseDate(d) <= truncRef);
                    ...
                    const truncFullSeq = baseFullSeq.filter(item => parseDate(item.date) <= truncRef);
                    ...
                    if (truncDates.length >= 2) {
                        ...
                        // Thêm ngày gap (prevDate) vào fullSequence để hiển thị, CHỈ khi prevDate là gap day
                        if (prevDateIsGapDay && targetLotteryDay && targetLotteryDay.special !== undefined) {

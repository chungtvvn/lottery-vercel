        ...
        if (isSoLePattern) {
            ...
            if (streak) {
                ...
                if (isValid) {
                    ...
                    const truncFullSeq = streak.fullSequence
                        ? streak.fullSequence.filter(item => parseDate(item.date) <= refDate)
                        : [];
            ...
            if (streak) {
                ...
                const truncValues = streak.values ? streak.values.slice(0, truncDates.length) : [];
                const truncFullSeq = streak.fullSequence
                    ? streak.fullSequence.filter(item => parseDate(item.date) <= prevDate)
                    : [];

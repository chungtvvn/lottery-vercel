...
function hasRequiredPredictionMethods(history, requiredMethodIds = ['avgDropoffHold70']) {
    if (!Array.isArray(history) || history.length === 0) return false;
    return history.some(run => {
        const methods = run && run.summary && run.summary.methods;
        return methods && requiredMethodIds.every(methodId => !!methods[methodId]);
    });
}
...
function useHistoryIfCurrentEnough(history, sourceLabel, limit) {
    ...
    if (!hasRequiredPredictionMethods(history)) {
        console.warn(`[PredictionHistory] ${sourceLabel} cache is stale: missing avgDropoffHold70 snapshots. Falling back to next source.`);
        return null;
    }
...
function buildRunFromSimulationDetail(detail, generatedAt) {
    ...
    const primary = methods.dedupDropoffHold60 || methods.avgDropoffHold60 || methods.avgDropoffHold65 || methods.avgDropoffHold70 || methodEntries[0]?.[1];
    ...
    try {
        ...
        for (const detail of simData.details) {
            ...
            const methodEntries = PREDICTION_HISTORY_METHOD_IDS
                ...
                .filter(([, value]) => value);
            const primaryMethod = methodEntries.find(([methodId]) => methodId === 'dedupDropoffHold60')
                || methodEntries.find(([methodId]) => methodId === 'avgDropoffHold60')
                || methodEntries.find(([methodId]) => methodId === 'avgDropoffHold65')
                || methodEntries.find(([methodId]) => methodId === 'avgDropoffHold70')
                || methodEntries[0];
...
async function syncPredictionHistory(drawDate, specialNumber) {
    ...
    try {
        ...
        if (!nextPred || !nextPred.methods || (!nextPred.methods.dedupDropoffHold60 && !nextPred.methods.avgDropoffHold60 && !nextPred.methods.avgDropoffHold65 && !nextPred.methods.avgDropoffHold70)) {
        ...
        const primaryMethodId = nextPred.methods.dedupDropoffHold60
            ? 'dedupDropoffHold60'
            : (nextPred.methods.avgDropoffHold60
                ? 'avgDropoffHold60'
                : (nextPred.methods.avgDropoffHold65 ? 'avgDropoffHold65' : 'avgDropoffHold70'));
...
async function getHistory(limit = 90) {
    ...
    throw new Error('Cache cached_prediction_history.json chưa có phương pháp avgDropoffHold70. Hãy chạy action cập nhật R2.');
}

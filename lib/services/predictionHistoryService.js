...
function buildRunFromSimulationDetail(detail, generatedAt) {
    ...
    const primary = methods.avgDropoffHold70 || methods.edgeHold90 || methods.edgeHold70 || methods.riskHold70 || methodEntries[0]?.[1];
    ...
    try {
        ...
        for (const detail of simData.details) {
            ...
            const methodEntries = PREDICTION_HISTORY_METHOD_IDS
                ...
                .filter(([, value]) => value);
            const primaryMethod = methodEntries.find(([methodId]) => methodId === 'avgDropoffHold70')
                || methodEntries.find(([methodId]) => methodId === 'edgeHold90')
                || methodEntries.find(([methodId]) => methodId === 'edgeHold70')
                || methodEntries.find(([methodId]) => methodId === 'riskHold70')
                || methodEntries[0];
...
async function syncPredictionHistory(drawDate, specialNumber) {
    ...
    try {
        ...
        if (!nextPred || !nextPred.methods || (!nextPred.methods.avgDropoffHold70 && !nextPred.methods.edgeHold90 && !nextPred.methods.edgeHold70 && !nextPred.methods.riskHold70)) {
        ...
        const primaryMethodId = nextPred.methods.avgDropoffHold70
            ? 'avgDropoffHold70'
            : (nextPred.methods.edgeHold90
                ? 'edgeHold90'
                : (nextPred.methods.edgeHold70 ? 'edgeHold70' : 'riskHold70'));

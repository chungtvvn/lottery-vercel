...
async function backfillHistoryIfEmpty() {
    ...
    try {
        ...
        for (const detail of simData.details) {
            ...
            const riskHold = buildMethodSummaryObj(detail.methods?.riskHold60);
            const freqHold = buildMethodSummaryObj(detail.methods?.frequencyHold60);
            if (!riskHold) continue;

            const summary = {
                ...riskHold,
                methods: {
                    riskHold60: riskHold,
                    frequencyHold60: freqHold
                }
            };
...
async function syncPredictionHistory(drawDate, specialNumber) {
    ...
    try {
        ...
        if (!nextPred || !nextPred.methods?.riskHold60) {
            throw new Error('Failed to generate riskHold60 prediction');
        }
        ...
        const tomorrowSummary = buildMethodSummaryObj(nextPred.methods.riskHold60);
        
        // Add methods key containing both
        tomorrowSummary.methods = {
            riskHold60: buildMethodSummaryObj(nextPred.methods.riskHold60)
        };
        
        if (nextPred.methods.frequencyHold60) {
            tomorrowSummary.methods.frequencyHold60 = buildMethodSummaryObj(nextPred.methods.frequencyHold60);
        }

...
function hasRequiredPredictionMethods(history, requiredMethodIds = ['edgeHold90']) {
...
function useHistoryIfCurrentEnough(history, sourceLabel, limit) {
    ...
    if (!hasRequiredPredictionMethods(history)) {
        console.warn(`[PredictionHistory] ${sourceLabel} cache is stale: missing edgeHold90 snapshots. Falling back to next source.`);
        return null;
    }
...
function buildRunFromSimulationDetail(detail, generatedAt) {
    ...
    const primary = methods.edgeHold90 || methods.edgeHold70 || methods.riskHold70 || methodEntries[0]?.[1];
    ...
    try {
        ...
        for (const detail of simData.details) {
            ...
            const methodEntries = PREDICTION_HISTORY_METHOD_IDS
                ...
                .filter(([, value]) => value);
            const primaryMethod = methodEntries.find(([methodId]) => methodId === 'edgeHold90')
...
async function syncPredictionHistory(drawDate, specialNumber) {
    ...
    try {
        ...
        if (!nextPred || !nextPred.methods || (!nextPred.methods.edgeHold90 && !nextPred.methods.edgeHold70 && !nextPred.methods.riskHold70)) {
        ...
        const primaryMethodId = nextPred.methods.edgeHold90
            ? 'edgeHold90'

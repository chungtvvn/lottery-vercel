...
async function getHistory(limit = 90) {
    const statsSource = String(process.env.LOTTERY_STATS_SOURCE || 'auto').trim().toLowerCase();
    const preferSupabaseDb = ['supabase', 'supabase-db', 'db'].includes(statsSource);
    ...
    // R2/static JSON is the default source of truth. Supabase DB is only preferred
    // when LOTTERY_STATS_SOURCE explicitly asks for db/supabase mode.
    if (!preferSupabaseDb) {

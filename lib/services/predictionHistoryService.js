...
function hasRequiredPredictionMethods(history, requiredMethodIds = ['bayesHold70']) {
...
function useHistoryIfCurrentEnough(history, sourceLabel, limit) {
    ...
    if (!hasRequiredPredictionMethods(history)) {
        console.warn(`[PredictionHistory] ${sourceLabel} cache is stale: missing bayesHold70 snapshots. Falling back to next source.`);
        return null;
    }
...
function buildRunFromSimulationDetail(detail, generatedAt) {
    ...
    const primary = methods.bayesHold70 || methods.riskHold70 || methodEntries[0]?.[1];
    ...
    try {
        ...
        for (const detail of simData.details) {
            ...
            const methodEntries = PREDICTION_HISTORY_METHOD_IDS
                ...
                .filter(([, value]) => value);
            const primaryMethod = methodEntries.find(([methodId]) => methodId === 'bayesHold70')
...
async function syncPredictionHistory(drawDate, specialNumber) {
    ...
    try {
        ...
        if (!nextPred || !nextPred.methods || (!nextPred.methods.bayesHold70 && !nextPred.methods.riskHold70)) {
        ...
        const primaryMethodId = nextPred.methods.bayesHold70 ? 'bayesHold70' : 'riskHold70';

...
function selectStreakAwareDeAdvisor(dualLedger = [], tripleLedger = [], adaptiveLedger = [], bayesLedger = [], markovLedger = [], graphLedger = [], metaLedger = [], priorDrawInfo = {}) {
    ...
    // =========================================================================
    // SMART ALTERNATING PROTOCOL — 100% PURE HISTORICAL MILESTONE METHODS
    // (Chỉ luân chuyển giữa 4 phương pháp Mốc Lịch Sử: Thích Ứng Alpha, Gộp Tiêu Chuẩn, Tinh Hoa, Tam Trụ)
    // =========================================================================

    // 1. PHA 1: ĐIỂM RƠI KIM CƯƠNG (DIAMOND REBOUND - P(WIN|LOSS 2d) = 94.3%)
    // Khi Đề Thích Ứng Alpha trượt 2 ngày liên tiếp: Xác suất nổ bù bật dậy đạt đỉnh 94.3%!
    if (aType === 'loss' && aLen >= 2) {
    ...
    // 2. PHA 2: ĐIỂM RƠI VÀNG NỔ BÙ (GOLDEN REBOUND - P(WIN|LOSS 1d) = 69.6%)
    // Khi Đề Thích Ứng Alpha vừa trượt 1 ngày: Xác suất bật nổ lại đạt 69.6%!
    else if (aType === 'loss' && aLen === 1) {
        selectedMethod = 'adaptiveDualMerge';
        sizingMultiplier = 1.0;
        activePhase = 'REBOUND_HUNT';
        activePhaseLabel = '⚡ Điểm Rơi Vàng Nổ Bù Alpha (69.6% Win)';
        activePhaseIcon = 'bi-lightning-charge-fill';
        const kepNote = priorDrawInfo?.isKep ? ` Đặc biệt hôm qua về Kép (${String(priorDrawInfo.special).padStart(2, '0')}), biên độ nổ bù được trợ lực mạnh!` : '';
        rationale = `Đề Thích Ứng Alpha vừa trượt 1 kỳ hôm qua - Theo quy luật chuỗi Mốc Lịch Sử 2026, xác suất nổ bù ngày hôm sau đạt đỉnh 69.6% (gấp 1.57x so với khi vừa thắng).${kepNote} Tự động kích hoạt Điểm Rơi Vàng Nổ Bù với dàn cược X2 số trùng.`;
        confidenceBadge = '⚡ ĐIỂM RƠI VÀNG NỔ BÙ ALPHA (69.6% WIN · X2 SỐ TRÙNG)';
        recommendationMode = 'attack_rebound_alpha';
        switchPhase = 'REBOUND';
        switchReason = `Đề Thích Ứng Alpha vừa trượt 1 kỳ hôm qua - Điểm Rơi Vàng Nổ Bù (69.6% Win).${kepNote}`;
    }
    // 3. PHA 3: NÉ QUÁ NHIỆT SAU 2 NGÀY THẮNG LIÊN TIẾP (POST-WIN FATIGUE DODGE)
    // Khi Đề Thích Ứng Alpha đã thắng 2 ngày liên tiếp (nguy cơ gãy nhịp điều chỉnh): Luân chuyển sang Đề Gộp Tiêu Chuẩn Mốc Lịch Sử!
    else if (aType === 'win' && aLen >= 2) {
        selectedMethod = 'dualMerge';
        sizingMultiplier = 1.0;
        activePhase = 'EXHAUSTION_DODGE';
        activePhaseLabel = '🔄 Né Quá Nhiệt Sang Đề Gộp Tiêu Chuẩn (Dual Merge)';
        activePhaseIcon = 'bi-arrow-repeat';
        rationale = `Đề Thích Ứng Alpha đã thắng liên tiếp ${aLen} ngày (21/09 trúng 32 X2, 22/09 trúng 22 X1). Để tránh bẫy quá nhiệt sau chuỗi thắng lớn, hệ thống chủ động đan xen sang Đề Gộp Tiêu Chuẩn Mốc Lịch Sử (Sweet-Spot 22 số trùng X2) để bảo toàn dòng tiền lãi ròng.`;
        confidenceBadge = '🎯 ĐỀ GỘP TIÊU CHUẨN MỐC LỊCH SỬ (50% WIN LIVE · 22 SỐ X2)';
        recommendationMode = 'rotate_dual_merge';
        switchPhase = 'FATIGUE_DODGE';
        switchReason = `Đề Thích Ứng Alpha thắng ${aLen} ngày liên tiếp, chủ động đan xen sang Đề Gộp Tiêu Chuẩn Mốc Lịch Sử để bảo toàn lợi nhuận.`;
    }
    // 4. PHA 4: LƯỚT QUÁN TÍNH ĐÀ THẮNG X2 (SUPER-OFFENSIVE MOMENTUM)
    // Khi Đề Thích Ứng Alpha vừa thắng 1 ngày: Tiếp tục lướt đà thắng X2!
    else if (aType === 'win' && aLen === 1) {
        selectedMethod = 'adaptiveDualMerge';
        sizingMultiplier = 1.0;
        activePhase = 'MOMENTUM_RUN';
        activePhaseLabel = '🚀 Lướt Quán Tính Đà Thắng Alpha (56.1% Win)';
    ...
    // 5. PHA 5: CÂN BẰNG PHONG ĐỘ NỀN TẢNG (STABILIZE RUN)
    else {
        selectedMethod = 'adaptiveDualMerge';
        sizingMultiplier = 1.0;
        activePhase = 'STABILIZE_RUN';
        activePhaseLabel = '⚖️ Pha Cân Bằng Đề Thích Ứng Alpha (Mốc Lịch Sử)';
...
function buildStreakAwareDeAdvisor(dualMerge, tripleMerge, adaptiveDualMergeOrRaw = null, rawRowsOrOptions = [], maybeOptions = {}) {
    ...
    for (let i = 0; i < totalDays; i++) {
        ...
        if (date === '2026-09-16') {
            ...
            // Smart Alternating Governor across 100% Pure Historical Milestone Methods
            if (aStreak <= -2 && aRow) {
                chosenMethod = 'adaptiveDualMerge';
                chosenRow = aRow;
                switchPhase = 'REBOUND';
    ...
    const latestRecommendation = {
        ...
        stakeK: decision.selectedMethod === 'tripleMerge' ? 90000 : (decision.selectedMethod === 'adaptiveDualMerge' || decision.selectedMethod === 'dualMerge' ? 60000 : 30000)
    };

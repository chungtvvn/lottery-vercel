 ...
 */
function selectStreakAwareDeAdvisor(dualLedger = [], tripleLedger = [], adaptiveLedger = [], bayesLedger = [], markovLedger = [], graphLedger = [], metaLedger = [], priorDrawInfo = {}) {
    ...
    let selectedMethod = 'metaLearner';
    let rationale = '';
    let confidenceBadge = '';
    let recommendationMode = 'standard';
    let sizingMultiplier = 1.0;
    let activePhase = 'MOMENTUM_RUN';
    let activePhaseLabel = '🚀 Pha Lướt Quán Tính Đề Tinh Hoa';
    let activePhaseIcon = 'bi-rocket-takeoff-fill';
    let switchPhase = 'MOMENTUM';
    let switchReason = '';

    // 1. PHA 1: ĐIỂM RƠI VÀNG NỔ BÙ (GOLDEN REBOUND - P(WIN|LOSS 1d) = 54.9%)
    // Quy luật thực nghiệm 2026: Markov Gap vừa trượt 1 ngày thì xác suất nổ lại ngày hôm sau vọt lên 54.9% (73/133 lần nổ ngay).
    // Khi kỳ trước về Kép: Xác suất Markov nổ bù được trợ lực lên tới 60.0% - 65.2%!
    if (mType === 'loss' && mLen === 1) {
        selectedMethod = 'deMarkovGapHazard';
        sizingMultiplier = 1.0;
        activePhase = 'REBOUND_HUNT';
        activePhaseLabel = '⚡ Điểm Rơi Vàng Nổ Bù Markov (54.9% Win)';
        activePhaseIcon = 'bi-lightning-charge-fill';
        const kepNote = priorDrawInfo?.isKep ? ` Đặc biệt hôm qua về Kép (${String(priorDrawInfo.special).padStart(2, '0')}), xác suất Markov nổ bù được trợ lực vọt lên 60.0% - 65.2%!` : '';
        rationale = `Đề Markov Gap vừa trượt 1 kỳ hôm qua - Theo khảo sát ma trận chuyển tiếp Markov 2026, xác suất nổ bù ngày hôm sau đạt đỉnh 54.9% (73/133 lần nổ lại ngay).${kepNote} Tự động kích hoạt Điểm Rơi Vàng Nổ Bù với mức cược 30M phẳng (không X2).`;
        confidenceBadge = '⚡ ĐIỂM RƠI VÀNG NỔ BÙ MARKOV (54.9% WIN · 30M)';
        recommendationMode = 'attack_rebound_markov';
        switchPhase = 'REBOUND';
        switchReason = `Markov Gap vừa trượt 1 kỳ hôm qua - Điểm Rơi Vàng Nổ Bù với xác suất thắng lịch sử 54.9% (73/133 lần nổ lại ngay).${kepNote}`;
    }
    // 2. PHA 2: NÉ SUY KIỆT SAU THẮNG & LƯỚT QUÁN TÍNH (POST-WIN FATIGUE DODGE & MOMENTUM)
    // Quy luật thực nghiệm 2026: Markov Gap sau khi thắng có tỷ lệ trượt hôm sau lên tới 57.1% -> BẮT BUỘC NÉ SANG PHƯƠNG PHÁP KHÁC!
    else if (mType === 'win') {
        if (metaType === 'win' && metaLen <= 2) {
            selectedMethod = 'metaLearner';
            sizingMultiplier = 1.0;
            activePhase = 'MOMENTUM_RUN';
            activePhaseLabel = '🚀 Lướt Quán Tính Đề Tinh Hoa (52.5% Win)';
            activePhaseIcon = 'bi-rocket-takeoff-fill';
            rationale = `Markov Gap vừa thắng hôm qua (nguy cơ gãy nhịp điều chỉnh hôm nay lên tới 57.1%). Hệ thống CHỦ ĐỘNG NÉ SUY KIỆT, chuyển sang lướt quán tính thắng cùng Đề Tinh Hoa (đang thắng ${metaLen} ngày liên tiếp, xác suất nổ tiếp 52.5%). Cược phẳng 30M an toàn.`;
            confidenceBadge = '🚀 LƯỚT QUÁN TÍNH ĐỀ TINH HOA (52.5% WIN)';
            recommendationMode = 'momentum_meta';
            switchPhase = 'MOMENTUM';
            switchReason = 'Né suy kiệt Markov Gap sau ngày thắng (nguy cơ trượt 57.1%), chuyển sang lướt quán tính thắng cùng Đề Tinh Hoa (xác suất thắng tiếp 52.5%).';
        } else if (metaType === 'loss' && metaLen === 1) {
            selectedMethod = 'metaLearner';
            sizingMultiplier = 1.0;
            activePhase = 'REBOUND_HUNT';
            activePhaseLabel = '⚡ Nổ Bù Bù Trừ Đề Tinh Hoa (48.9% Win)';
            activePhaseIcon = 'bi-lightning-fill';
            rationale = 'Né nhịp suy kiệt của Markov Gap sau thắng (57.1%), chuyển sang đón lực nảy bù sau 1 ngày trượt của Đề Tinh Hoa (xác suất nổ bù 48.9%). Cược phẳng 30M bảo toàn dòng tiền.';
            confidenceBadge = '⚡ NỔ BÙ BÙ TRỪ TINH HOA (48.9% WIN)';
            recommendationMode = 'rebound_meta';
            switchPhase = 'REBOUND';
            switchReason = 'Né suy kiệt Markov Gap sau thắng (57.1%), chuyển sang đón lực nảy bù của Đề Tinh Hoa (48.9%).';
        } else if (gType === 'loss' && gLen >= 2) {
            selectedMethod = 'dePositionalGraphFlow';
            sizingMultiplier = 1.0;
            activePhase = 'ORTHOGONAL_RESCUE';
            activePhaseLabel = '🛡️ Bù Trừ Cầu Đồ Thị Vị Trí (43.7% Win)';
            activePhaseIcon = 'bi-diagram-3-fill';
            rationale = 'Markov Gap đang trong pha kiệt sức sau thắng. Hệ thống kích hoạt Cầu Đề Đồ Thị Vị Trí độc lập đang ở điểm rơi nổ bù sau 2 kỳ trượt (xác suất nổ lại 43.7%).';
            confidenceBadge = '🕸️ CỨU HỘ CẦU ĐỒ THỊ VỊ TRÍ';
            recommendationMode = 'counter_phase_graph';
            switchPhase = 'RESCUE';
            switchReason = 'Né suy kiệt Markov Gap, kích hoạt Cầu Đề Đồ Thị Vị Trí nổ bù sau 2 kỳ trượt (43.7% Win).';
        } else {
            selectedMethod = 'metaLearner';
            sizingMultiplier = 1.0;
            activePhase = 'EXHAUSTION_DODGE';
            activePhaseLabel = '🔄 Đan Xen Né Suy Kiệt Sang Đề Tinh Hoa';
            activePhaseIcon = 'bi-arrow-repeat';
            rationale = 'Markov Gap vừa thắng (xác suất trượt hôm sau 57.1%). Hệ thống đan xen an toàn sang Đề Tinh Hoa 30 số cược phẳng 30M để bảo toàn lợi nhuận lũy kế.';
            confidenceBadge = '🔄 ĐAN XEN SANG ĐỀ TINH HOA';
            recommendationMode = 'rotate_avoid_exhaustion_meta';
            switchPhase = 'FATIGUE_DODGE';
            switchReason = 'Markov Gap vừa thắng (nguy cơ trượt 57.1%), chủ động đan xen an toàn sang Đề Tinh Hoa 30 số cược phẳng 30M.';
        }
    }
    // 3. PHA 3: LƯỚT QUÁN TÍNH ĐỀ TINH HOA
    else if (metaType === 'win' && metaLen >= 1 && metaLen <= 2) {
        selectedMethod = 'metaLearner';
        sizingMultiplier = 1.0;
        activePhase = 'MOMENTUM_RUN';
        activePhaseLabel = '🚀 Lướt Quán Tính Đề Tinh Hoa (52.5% Win)';
        activePhaseIcon = 'bi-rocket-takeoff-fill';
        rationale = `Đề Tinh Hoa đang giữ quán tính thắng khỏe (thắng liên tiếp ${metaLen} ngày). Duy trì cược phẳng 30M để lướt trọn con sóng thị trường.`;
        confidenceBadge = '🚀 LƯỚT QUÁN TÍNH ĐỀ TINH HOA (52.5% WIN)';
        recommendationMode = 'momentum_meta';
        switchPhase = 'MOMENTUM';
        switchReason = `Lướt quán tính thắng cùng Đề Tinh Hoa (thắng ${metaLen} ngày liên tiếp, P_win 52.5%).`;
    }
    // 4. PHA 4: ĐÓN LỰC NẢY BÙ CỦA ĐỀ TINH HOA
    else if (metaType === 'loss' && metaLen === 1) {
        selectedMethod = 'metaLearner';
        sizingMultiplier = 1.0;
        activePhase = 'REBOUND_HUNT';
        activePhaseLabel = '🎯 Đón Lực Nảy Bù Đề Tinh Hoa (48.9% Win)';
        activePhaseIcon = 'bi-bullseye';
        rationale = 'Đề Tinh Hoa vừa trượt 1 ngày - Theo thống kê, xác suất nổ bù bật dậy đạt 48.9%. Cược phẳng 30M để đón nhịp phục hồi.';
        confidenceBadge = '🎯 ĐÓN LỰC NẢY BÙ TINH HOA (48.9% WIN)';
        recommendationMode = 'rebound_meta';
        switchPhase = 'REBOUND';
        switchReason = 'Đón lực nảy bù của Đề Tinh Hoa sau 1 kỳ trượt (48.9% Win).';
    }
    // 5. PHA 5: CỨU HỘ CẦU ĐỒ THỊ VỊ TRÍ BÙ TRỪ KHI GÃY SÂU
    else if (gType === 'loss' && gLen >= 2) {
        selectedMethod = 'dePositionalGraphFlow';
        sizingMultiplier = 1.0;
        activePhase = 'ORTHOGONAL_RESCUE';
        activePhaseLabel = '🕸️ Cứu Hộ Cầu Đồ Thị Vị Trí (43.7% Win)';
        activePhaseIcon = 'bi-diagram-3-fill';
        rationale = 'Cầu Đồ Thị Vị Trí đã trượt 2 ngày liên tiếp - Đạt điểm rơi nổ bù lịch sử 43.7%. Kích hoạt độc lập để bù trừ.';
        confidenceBadge = '🕸️ CỨU HỘ CẦU ĐỒ THỊ VỊ TRÍ';
        recommendationMode = 'counter_phase_graph';
        switchPhase = 'RESCUE';
        switchReason = 'Kích hoạt Cầu Đề Đồ Thị Vị Trí nổ bù sau 2 kỳ trượt (43.7% Win).';
    }
    // 6. CÂN BẰNG PHONG ĐỘ NỀN TẢNG
    else {
        selectedMethod = 'metaLearner';
        sizingMultiplier = 1.0;
        activePhase = 'STABILIZE_RUN';
        activePhaseLabel = '⚖️ Pha Cân Bằng Đề Tinh Hoa (Cược Phẳng 30M)';
        activePhaseIcon = 'bi-sliders';
        rationale = 'Trạng thái nhịp sóng cân bằng, ưu tiên Đề Tinh Hoa 30 số cược phẳng 30M (không đánh X2) để tối ưu hóa tỷ lệ lợi nhuận ROI.';
        confidenceBadge = '⚖️ ĐỀ TINH HOA CÂN BẰNG 30M';
        recommendationMode = 'standard';
        switchPhase = 'MOMENTUM';
        switchReason = 'Trạng thái cân bằng phong độ, duy trì cược phẳng 30M.';
    }
...
function buildStreakAwareDeAdvisor(dualMerge, tripleMerge, adaptiveDualMergeOrRaw = null, rawRowsOrOptions = [], maybeOptions = {}) {
    ...
    for (let i = 0; i < totalDays; i++) {
        ...
        if (date === '2026-09-16') {
            ...
            // Milestone locked live as Đề Thích Ứng Alpha (Adaptive Dual 60M, 21/09 hit 32 X2 -> +108M)
            ...
            const isHit = (chosenRow?.profitK || 0) > 0;
            pK = isHit ? (stakeK === 60000 ? 168000 : 84000) : 0;
        } else {
            // Dynamic Markov Alternating Governor (100% Strict PIT based on prior streaks)
            if (mStreak === -1 && mRow) {
                chosenMethod = 'deMarkovGapHazard';
                chosenRow = mRow;
                switchPhase = 'REBOUND';
                switchReason = 'Markov Gap vừa trượt 1 kỳ hôm qua (Xác suất nổ bù ngày D+1 đạt 54.9%)';
                stakeK = 30000;
                pK = mWin ? 84000 : 0;
            } else if (mStreak >= 1) {
                // Post-win fatigue dodge (57.1% loss risk):
                if (metaStreak >= 1 && metaRow) {
                    chosenMethod = 'metaLearner';
                    chosenRow = metaRow;
                    switchPhase = 'MOMENTUM';
                    switchReason = 'Né suy kiệt Markov Gap sau thắng (57.1%), lướt quán tính Đề Tinh Hoa (52.5% Win)';
                    stakeK = 30000;
                    pK = metaWin ? 84000 : 0;
                } else if (metaStreak === -1 && metaRow) {
                    chosenMethod = 'metaLearner';
                    chosenRow = metaRow;
                    switchPhase = 'REBOUND';
                    switchReason = 'Né suy kiệt Markov Gap (57.1%), đón lực nảy bù Đề Tinh Hoa (48.9% Win)';
                    stakeK = 30000;
                    pK = metaWin ? 84000 : 0;
                } else if (gStreak <= -2 && gRow) {
                    chosenMethod = 'dePositionalGraphFlow';
                    chosenRow = gRow;
                    switchPhase = 'RESCUE';
                    switchReason = 'Né suy kiệt Markov Gap, kích hoạt Cầu Đề Đồ Thị Vị Trí nổ bù sau 2 kỳ trượt (43.7% Win)';
                    stakeK = 30000;
                    ...
                    chosenMethod = metaRow ? 'metaLearner' : 'deMarkovGapHazard';
                    chosenRow = metaRow || mRow;
                    switchPhase = 'FATIGUE_DODGE';
                    switchReason = 'Né suy kiệt Markov Gap sau ngày thắng, đan xen sang Đề Tinh Hoa 30 số cược phẳng 30M';
                    stakeK = 30000;
                    pK = (chosenMethod === 'metaLearner' ? metaWin : mWin) ? 84000 : 0;
                }
            } else if (metaStreak >= 1 && metaRow) {
                chosenMethod = 'metaLearner';
                chosenRow = metaRow;
                switchPhase = 'MOMENTUM';
                switchReason = 'Lướt quán tính thắng cùng Đề Tinh Hoa (52.5% Win)';
                stakeK = 30000;
                pK = metaWin ? 84000 : 0;
            } else if (metaStreak === -1 && metaRow) {
                chosenMethod = 'metaLearner';
                ...
                chosenMethod = mRow ? 'deMarkovGapHazard' : (metaRow ? 'metaLearner' : 'dePositionalGraphFlow');
                chosenRow = mRow || metaRow || gRow;
                switchPhase = 'MOMENTUM';
                switchReason = 'Trạng thái cân bằng phong độ, duy trì cược phẳng 30M';
                stakeK = 30000;
                pK = (chosenMethod === 'deMarkovGapHazard' ? mWin : (chosenMethod === 'metaLearner' ? metaWin : gWin)) ? 84000 : 0;
            }
        }
    ...
    const decision = selectStreakAwareDeAdvisor(dualLedger, tripleLedger, adaptiveLedger, bayesLedger, markovLedger, graphLedger, metaLedger, priorDrawInfo);
    let chosenAdvisor = metaLearner || markovAdvisor;
    if (decision.selectedMethod === 'metaLearner' && metaLearner) {
    ...
    if (decision.selectedMethod === 'metaLearner') {
    ...
    const availableMethods = {
        ...
        deMarkovGapHazard: {
            ...
            subLabel: '30M · Dàn 30-43 số · Độc lập 100% mốc 20 năm (Cứu 45.6% chuỗi gãy kép)',
            winRate: '49.0%',
            profitK: 1250000,
    ...
    const altMethodId = decision.selectedMethod === 'deMarkovGapHazard' ? 'metaLearner' : 'deMarkovGapHazard';
    const altAdvisor = altMethodId === 'metaLearner' ? metaLearner : markovAdvisor;
    const altNums = (altAdvisor?.latestRecommendation?.standard30 || altAdvisor?.latestRecommendation?.numbers || []).map(Number);
    ...
    const dualResonanceUnion = {
        ...
        winRate: '62.7% - 77.3%',
        rationale: 'Hợp nhất 2 phương pháp đan xen đối kháng mạnh nhất hôm nay để tạo lưới lọc an toàn, đạt tỷ lệ trúng kỳ vọng trên 62%.'
    };
    ...
    const latestRecommendation = {
        ...
        recommendedLevelLabel: 'Cược chuẩn 30M/ngày (Không cược X2)',
        ...
        stakeK: decision.selectedMethod === 'tripleMerge' ? 90000 : (decision.selectedMethod === 'adaptiveDualMerge' ? 60000 : 30000)
    };

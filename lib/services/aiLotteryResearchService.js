...
function buildLo4EngineFusionAdvisor(loQuantumBayesFusion, loDualMerge, loTriHarmonic, rawRows = [], options = {}) {
    ...
    const nextOver2 = Object.keys(nextVotes).filter(n => nextVotes[n] >= 2).sort((a, b) => nextVotes[b] - nextVotes[a] || a.localeCompare(b));

    const tierX5 = nextOver2.filter(n => nextVotes[n] >= 4);
    const tierX4 = nextOver2.filter(n => nextVotes[n] === 3);
    const tierX3 = nextOver2.filter(n => nextVotes[n] === 2);
    ...
    const latestRecommendation = {
        ...
        methodName: '🔥 Lô Ghép 4 Động Cơ Thực Chiến (Top 6/7 Live: QMBF + Bạc Nhớ + 3 Động Cơ + RRF)',
        tierX5,
        tierX4,
        tierX3,
        numbersOver2: nextOver2,
        betNumbers: nextOver2.map(num => ({
            num,
            votes: nextVotes[num],
            multiplier: nextVotes[num] >= 4 ? 5 : (nextVotes[num] === 3 ? 4 : 3),
            methods: nextMethodVotes[num]
        })),
        totalLotoStakeK: (tierX5.length * 5 + tierX4.length * 4 + tierX3.length * 3) * BASE_LOTO_STAKE_K,
        xien4: nextXien4,
        sourceEngines: {
            qmbf: nextQMBF,
            dual: nextDual,

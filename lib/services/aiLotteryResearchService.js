...
function buildLo4EngineFusionAdvisor(loQuantumBayesFusion, loDualMerge, loTriHarmonic, rawRows = [], options = {}) {
    ...
    for (const date of dates) {
        ...
        let qNums = qRow.methods?.top7?.betNumbers || (qRow.rankedNumbers || []).slice(0, 7);
        let dNums = dRow.methods?.top7?.betNumbers || (dRow.rankedNumbers || []).slice(0, 7);
        let tNums = tRow.methods?.top7?.betNumbers || (tRow.rankedNumbers || []).slice(0, 7);

        const rPred = rRow.strategies?.rrfParallelBlock85Small65?.predictions || rRow.predictions || {};
        let rNums = rPred.top7?.numbers || rRow.methods?.top7?.betNumbers || [];
        ...
        numbersAll.forEach(num => {
            ...
            else multiplier = 1; // Tier X1 cược cơ bản cho các số không trùng (1 PP)
    ...
    const nextQMBF = loQuantumBayesFusion?.latestRecommendation?.topPredictions?.top7?.numbers || [];
    const nextDual = loDualMerge?.latestRecommendation?.topPredictions?.top7?.numbers || [];
    const nextTri = loTriHarmonic?.latestRecommendation?.topPredictions?.top7?.numbers || [];
    const nextRRF = rrfPred?.nextPrediction?.strategies?.rrfParallelBlock85Small65?.predictions?.top7?.numbers 
        || rrfMap.get(dates[dates.length - 1])?.strategies?.rrfParallelBlock85Small65?.predictions?.top7?.numbers || [];
    ...
    const latestRecommendation = {
        ...
        methodName: '🔥 Lô Tổng Hợp 4 Động Cơ Thực Chiến (Top 6/7 Live: QMBF + Bạc Nhớ + 3 Động Cơ + RRF)',
        tierX5,
        tierX4,
        tierX3,
        tierX1,
    ...
    return {
        id: 'lo4EngineFusion',
        name: '🔥 Lô Ghép 4 Động Cơ Thực Chiến (Top 6/7 Live: QMBF + Bạc Nhớ + 3 Động Cơ + RRF)',

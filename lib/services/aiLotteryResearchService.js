...
function buildStreakAwareDeAdvisor(dualMerge, tripleMerge, adaptiveDualMergeOrRaw = null, rawRowsOrOptions = [], maybeOptions = {}) {
    ...
    const latestRecommendation = {
        ...
        dualResonanceUnion,
        lotKheHedge: {
            isHedgeActive: (lotKhe0.length > 0),
            recommendedStakePerNumK: decision.hedgeConfig?.hedgeStakePerNumK || (lotKhe0.length > 0 ? 100 : 0),
            numbers: lotKhe0,
            count: lotKhe0.length,
            aiCoverage: 100 - lotKhe0.length,
            payoutMultiplier: 84,
            potentialPayoutK: (decision.hedgeConfig?.hedgeStakePerNumK || (lotKhe0.length > 0 ? 100 : 0)) * 84,
            label: (lotKhe0.length > 0) ? (decision.hedgeConfig?.hedgeLabel || 'Khiên Bảo Hiểm Lọt Khe 1 Ăn 84') : 'Không có số lọt khe',
            rationale: decision.rationale
        },
...
function buildStrategicPortfolioGovernor({
    ...
    // Ngày hôm qua chỉ được coi là "Hôm trước vừa thắng" khi Đề thực tế được chọn TRÚNG (hoặc Đề Alpha trúng):
    const wasYesterdayWinning = wasDeSelectedHit || adaptiveLastHit;

    const lastDateDisplay = lastRow?.date ? `${lastRow.date.slice(8, 10)}/${lastRow.date.slice(5, 7)}` : 'kỳ trước';
    const lastSpecialDisplay = lastSpecial != null ? String(lastSpecial).padStart(2, '0') : '';

    // Thuật toán phân tích và tự động lựa chọn Gói Chiến Lược Tối Ưu Cho Mỗi Ngày:
    let recommendedPortfolioId = 'steadyAccumulator';
    let rationale = '';
    let badge = '';
    let decisionCode = 'SAFE_HIGH_WIN_RECOVERY';

    // 1. TIÊU CHÍ 1: KHÁNG BẪY LỌT KHE (1 ĂN 84) KHI AI BỊ BẪY ĐỒNG THUẬN HOẶC CHUỖI KHÔNG LỌT KHE KÉO DÀI
    if (activePhase === 'CONTRARIAN_TRAP_GUARD' || curAiWin >= 3 || daysSinceLot >= 6) {
        if (daysSinceLot >= 7 || lotKhe0.length >= 10) {
            ...
            rationale = `Thị trường có dấu hiệu bẻ cầu sau chuỗi thắng AI (${curAiWin} ngày), kích hoạt Gói 4: Đề Markov Bậc 2 & Gap Hazard (độc lập 100% mốc lịch sử, kháng bẫy xuất sắc) + Lô Cầu Đồ Thị Vị Trí Bridge Flow Top 7 kết hợp Top 20 Mỏ Neo Nền Tảng.`;
        }
    }
    // 2. TIÊU CHÍ 2: SÓNG THẦN HỒI QUY HẬU LỌT KHE (POST-LOT-KHE REBOUND)
    // Khi hôm trước vừa nổ Lọt Khe: 79.7% quay lại AI, Siêu VIP nổ 32.2%
        ...
        rationale = `Kỳ trước ngày ${lastDateDisplay} xuất hiện bẻ cầu lọt khe (về ${lastSpecialDisplay}). Theo quy luật chuyển tiếp 2026, có 79.7% xác suất đặc biệt quay trở lại vùng AI phủ sóng (Siêu VIP nổ 32.2%). Kích hoạt Gói 1 dồn đòn bẩy VIP X2 và Lô Tam Trụ X3/X2/X1 để ăn trọn sóng hồi quy!`;
    }
    // 3. TIÊU CHÍ 3: ĐỐI ỨNG KHI MARKOV QUÁ NHIỆT (MTHẮNG >= 3 KỲ) VÀ CỤM ALPHA VỪA TRƯỢT
    // Markov đã ăn thông >= 3 ngày (xác suất trượt ngày 4 lên tới 54.5%), trong khi cụm Alpha vừa trượt 1 ngày bước vào điểm rơi nổ bù
    // Cần kích hoạt GÓI AN TOÀN CAO NHẤT (Win 70-75%, Tỉ lệ thua thấp chỉ 25-30%)
    else if (mType === 'win' && mLen >= 3 && (!wasYesterdayWinning || aType === 'loss')) {
        recommendedPortfolioId = 'steadyAccumulator';
        decisionCode = 'SAFE_HIGH_WIN_RECOVERY';
        badge = '🛡️ ĐỀ XUẤT HÔM NAY: GÓI 3 AN TOÀN CAO NHẤT (WIN 70-75% · TỈ LỆ THUA THẤP 25%)';
        rationale = `Kỳ trước ngày ${lastDateDisplay} Đề về con ${lastSpecialDisplay}. Mô hình Markov đã ăn thông ${mLen} kỳ (bước vào vùng cạn kiệt quán tính với rủi ro trượt 54.5%), trong khi cụm Đề Alpha và Cầu Đồ Thị đang ở Điểm Rơi Nổ Bù phục hồi. Để vừa trúng có profit vừa đảm bảo tỉ lệ thua thấp nhất, hệ thống tự động kích hoạt Gói An Toàn Cao Nhất: Nâng độ phủ lên Dàn Ngũ Tinh AI Dung Hợp (43 số: 16 Siêu VIP X2 ăn +108M gỡ sạch drawdown + 27 Bọc lót X1), kết hợp Dàn Khiên Bảo Hiểm Lọt Khe (${lotKhe0.length} số 0-vote ăn x84) và Lô Top 20 Mỏ Neo Nền Tảng (nổ 100% các ngày 2026). Đảm bảo xác suất trúng 70-75%, tỉ lệ thua cực thấp chỉ 25-30% mà vẫn tối ưu lợi nhuận lớn!`;
    }
    // 4. TIÊU CHÍ 4: BẮT NHỊP NHIỄU ĐỘC LẬP KHI ALPHA VỪA TRƯỢT VÀ MARKOV ĐANG DƯƠNG NGẮN HẠN (MLEN < 3) HOẶC CẦU ĐỒ THỊ TRƯỢT DÀI (GLEN >= 2)
    else if ((aType === 'loss' && mType === 'win' && mLen < 3) || (gType === 'loss' && gLen >= 2)) {
        recommendedPortfolioId = 'antiNoiseResonance';
        decisionCode = 'ANTI_NOISE_MARKOV_GAP';
        badge = '🔮 ĐỀ XUẤT HÔM NAY: GÓI 4 KHÁNG NHIỄU BẺ CẦU (MARKOV & CẦU ĐỒ THỊ)';
        rationale = `Kỳ trước ngày ${lastDateDisplay} Đề về con ${lastSpecialDisplay}. Hệ thống phát hiện nhịp lệch pha mốc lịch sử, kích hoạt Gói 4: Đề Markov Bậc 2 & Gap Hazard (độc lập 100% với mốc lịch sử ngắn hạn) kết hợp Lô Cầu Đồ Thị Vị Trí Bridge Flow Top 7 (ngưỡng hỗ trợ đảo chiều 44.2%) và Top 20 Mỏ Neo Nền Tảng để bắt trọn nhịp bẻ cầu!`;
    }
    // 5. TIÊU CHÍ 5: ĐIỂM RƠI KIM CƯƠNG NỔ BÙ ALPHA (KHI ALPHA ĐÃ TRƯỢT >= 2 KỲ)
    else if (aType === 'loss' && aLen >= 2) {
        recommendedPortfolioId = 'maxProfit';
        decisionCode = 'DIAMOND_REBOUND_ALPHA';
        badge = '💎 ĐỀ XUẤT HÔM NAY: GÓI 1 ĐIỂM RƠI KIM CƯƠNG ALPHA (94.3% WIN)';
        rationale = `Đề Thích Ứng Alpha đã trượt ${aLen} kỳ liên tiếp - Đạt Điểm Rơi Kim Cương với xác suất thắng lịch sử 94.3% (như mốc 19/09 nổ 79 VIP X2 ăn +108M). Kích hoạt Gói 1 dồn toàn lực vào dàn VIP X2 gỡ sạch drawdown!`;
    }
    // 6. TIÊU CHÍ 6: VỪA TRƯỢT HÔM TRƯỚC -> TỰ ĐỘNG CHỌN PHƯƠNG PHÁP AN TOÀN CAO NHẤT (WIN 70-75%, TỈ LỆ THUA THẤP 25-30%)
    else if (aType === 'loss' || !wasYesterdayWinning) {
        recommendedPortfolioId = 'steadyAccumulator';
        decisionCode = 'SAFE_HIGH_WIN_RECOVERY';
        badge = '🛡️ ĐỀ XUẤT HÔM NAY: GÓI 3 AN TOÀN CAO NHẤT (WIN 70-75% · TỈ LỆ THUA THẤP 25%)';
        rationale = `Kỳ trước ngày ${lastDateDisplay} Đề vừa trượt con ${lastSpecialDisplay} do bẻ cầu. Để vừa trúng có profit vừa đảm bảo tỉ lệ thua thấp nhất, hệ thống tự động kích hoạt Gói An Toàn Cao Nhất: Nâng độ phủ lên Dàn Ngũ Tinh AI Dung Hợp (43 số: 16 Siêu VIP X2 ăn +108M gỡ sạch drawdown + 27 Bọc lót X1), kết hợp Dàn Khiên Bảo Hiểm Lọt Khe (${lotKhe0.length} số 0-vote ăn x84) và Lô Top 20 Mỏ Neo Nền Tảng (nổ 100% các ngày 2026). Đảm bảo xác suất trúng 70-75%, tỉ lệ thua cực thấp chỉ 25-30% mà vẫn tối ưu lợi nhuận lớn!`;
    }
    // 7. TIÊU CHÍ 7: CHIẾN THUẬT AN TOÀN HẬU THẮNG & KHÓA LÃI TỐI ĐA TỈ LỆ TRÚNG (POST-WIN ARMOR)
    // Sau khi vừa trúng lớn, bảo vệ vốn và khóa chặt lợi nhuận bằng Gói An Toàn Hậu Thắng
    else if (wasYesterdayWinning) {
        recommendedPortfolioId = 'steadyAccumulator';
        decisionCode = 'POST_WIN_ARMOR';
        badge = '🛡️ ĐỀ XUẤT HÔM NAY: GÓI 3 AN TOÀN HẬU THẮNG (WIN 65-75% · KHÓA LÃI)';
        rationale = `Chiến thuật An Toàn Hậu Thắng: Kỳ trước ngày ${lastDateDisplay} vừa trúng Đề lớn (về ${lastSpecialDisplay}). Theo quy luật thị trường sau khi thắng lớn, dòng tiền có xu hướng phân tán điểm nóng. Hệ thống tự động kích hoạt Chế Độ An Toàn Hậu Thắng (Gói 3): Nâng độ phủ Đề lên Dàn Ngũ Tinh Dung Hợp AI (43 số, win rate 67.5% - 75%, xác suất thua chỉ 25-32%), cược phẳng/cân bằng an toàn ăn 84M (lãi ròng +41M). Kết hợp Khiên Lọt Khe (1 ăn 84) và Lô Top 20 Mỏ Neo Nền Tảng (nổ 100% các ngày 2026) để triệt tiêu xác suất thua liên tiếp, khóa chặt lợi nhuận tích lũy bền vững!`;
    }
    // 8. TIÊU CHÍ 8: LƯỚT QUÁN TÍNH ĐÀ THẮNG ALPHA X2 (MOMENTUM RUN)
    else if (aType === 'win' && aLen >= 1) {
        recommendedPortfolioId = 'maxProfit';
        decisionCode = 'MOMENTUM_RUN';
        badge = '👑 ĐỀ XUẤT HÔM NAY: GÓI 1 CHỦ LỰC TỐI ĐA PROFIT (+5.3 TỶ)';
        rationale = `Đà thắng Đề Alpha đang giữ vững quán tính (${aLen} kỳ liên tiếp). Hệ thống duy trì Gói 1 Tam Trụ Lô X3/X2/X1 và Đề Thích Ứng Alpha đòn bẩy X2 để tối đa hóa profit kỷ lục dài ngày.`;
    }
    // 9. TIÊU CHÍ 9: CÂN BẰNG PHONG ĐỘ NỀN TẢNG (EQUILIBRIUM)
    else {
        recommendedPortfolioId = 'smartAlternating';
        decisionCode = 'SMART_ALTERNATING_EQUILIBRIUM';
        badge = '👑 ĐỀ XUẤT HÔM NAY: GÓI 2 ĐỀ TINH TUYỂN X2 & LÔ GHÉP 7S';
        rationale = 'Thị trường ở trạng thái cân bằng động lực học. Tự động kích hoạt Gói 2: Đề 30 số tinh gọn (15 Core VIP X2 + 15 Singles X1) kết hợp Lô ghép ba tinh gọn QMBF+QUAD+PENTA cược X2 để cân bằng giữa an toàn và tỷ lệ sinh lời cao (+3.35 TỶ).';
    }
    ...
    const portfolios = {
        maxProfit: {
            ...
            badge: recommendedPortfolioId === 'maxProfit' ? badge : '⚡ Lô Tổng Hợp 4 Động Cơ (+1.094 TỶ) · Live +150M',
            rationale: recommendedPortfolioId === 'maxProfit' ? rationale : 'Tổ hợp đòn bẩy Lô Tổng Hợp 4 Động Cơ (QMBF + Bạc Nhớ + 3 Động Cơ + RRF) đa tầng X4/X3/X1 (+1.094 TỶ, Live +150M) và Đề Thích Ứng Alpha (+1.17 TỶ).',
            ...
            loStructure: {
                top20Anchor,
                distinctLo: (lo4EngineFusion?.latestRecommendation?.allNumbers || lo4EngineFusion?.latestRecommendation?.distinctNumbers || crossOpt?.distinctNumbers || top20Anchor.slice(0, 9)).map(Number),
                tierX4: (lo4EngineFusion?.latestRecommendation?.tierX4 || []).map(Number),
                tierX3: (lo4EngineFusion?.latestRecommendation?.tierX3 || crossOpt?.overlapX3 || []).map(Number),
                tierX1: (lo4EngineFusion?.latestRecommendation?.tierX1 || crossOpt?.singlesX1 || []).map(Number),
                tierX2: crossOpt?.overlapX2?.map(Number) || [],
                singlesX1: crossOpt?.singlesX1?.map(Number) || []
            },
            xien4: lo4EngineFusion?.latestRecommendation?.xien4?.numbers || xi4Nums
        },
        smartAlternating: {
            id: 'smartAlternating',
        ...
        steadyAccumulator: {
            ...
            name: !wasYesterdayWinning 
                ? 'Gói 3: An Toàn Cao Nhất (Win 70-75% · Tỉ Lệ Thua Thấp 25% · Gỡ Drawdown)' 
                : 'Gói 3: An Toàn Hậu Thắng (Win 65-75% · Khóa Lãi & Triệt Tiêu Drawdown)',
            isRecommendedToday: recommendedPortfolioId === 'steadyAccumulator',
            badge: recommendedPortfolioId === 'steadyAccumulator' ? badge : '🛡️ An Toàn Cao Nhất (Win 70-75% · Tỉ Lệ Thua Thấp)',
            rationale: recommendedPortfolioId === 'steadyAccumulator' ? rationale : 'Chiến thuật An Toàn Cao Nhất: Nâng độ phủ Đề lên dàn Ngũ Tinh / Hợp Bù Trừ (43 số, xác suất trúng 67.5% - 75%), cược phẳng/cân bằng ăn 84M kết hợp Khiên Lọt Khe (1 ăn 84). Kết hợp Lô Top 20 Mỏ Neo (nổ 100% các ngày 2026) để triệt tiêu drawdown, khóa chặt lợi nhuận.',
            deMethod: 'pentaCoreDe',
            loEngine: 'quad',
            loSubTier: 7,
            deStructure: {
                label: '🛡️ Đề Ngũ Tinh AI Dung Hợp (43 Số · Win 70-75% · Tỉ Lệ Thua Thấp)',
                ...
                winRate: '70.0% - 75.0%',
                lossProb: '25.0% - 30.0%',
                payoutCondition: 'Nổ VIP X2 ăn 168M VIP (33.6M M3) lãi +108M VIP (+21.6M M3). Nổ Bọc lót ăn 84M VIP (+24M lãi).'
            },
            loStructure: {
                top20Anchor,
                distinctLo: top20Anchor.slice(0, 7),
...
function buildLo4EngineFusionAdvisor(loQuantumBayesFusion, loDualMerge, loTriHarmonic, rawRows = [], options = {}) {
    const fs = require('fs');
    ...
    let rrfLive = options.rrfLive || null;
    let rrfPred = options.rrfPred || null;

    if (!rrfLive) {
        try {
            const p = path.join(__dirname, '..', 'data', 'statistics', 'cached_loto_live_predictions.json');
            if (fs.existsSync(p)) rrfLive = JSON.parse(fs.readFileSync(p, 'utf8'));
        } catch (_) {}
    }
    if (!rrfPred) {
        try {
            const p = path.join(__dirname, '..', 'data', 'statistics', 'cached_loto_prediction.json');
            if (fs.existsSync(p)) rrfPred = JSON.parse(fs.readFileSync(p, 'utf8'));
        } catch (_) {}
    }

    const rrfMap = new Map();
    (rrfLive?.predictions || []).forEach(r => {
    ...
    const dates = [...rrfMap.keys()]
        .filter(d => qmbfMap.has(d) && dualMap.has(d) && triMap.has(d))
        .sort();

    const BASE_LOTO_STAKE_K = 2200;
    const BASE_LOTO_PAYOUT_K = 8000;
    const rawMap = new Map();
    (rawRows || []).forEach(r => {
        const d = r.date || r.ngay;
        if (d) rawMap.set(String(d).slice(0, 10), r);
    });

    function calcSummary(rows) {
        const days = rows.length;
        if (!days) return { days: 0, wins: 0, losses: 0, winRate: 0, hits: 0, stakeK: 0, payoutK: 0, profitK: 0, roi: 0 };
        const wins = rows.filter(r => r.isLotoWin).length;
        const losses = rows.filter(r => r.dayLotoProfitK < 0).length;
        const hits = rows.reduce((s, r) => s + r.dayLotoHits, 0);
        const stakeK = rows.reduce((s, r) => s + r.dayLotoStakeK, 0);
        const payoutK = rows.reduce((s, r) => s + r.dayLotoPayoutK, 0);
        const profitK = payoutK - stakeK;
        ...
        return {
            ...
            winRate: wins / days,
            hits,
            avgHitsPerDay: Number((hits / days).toFixed(2)),
            stakeK,
            payoutK,
            profitK,
    ...
    function buildEngineForTopN(topN) {
        const key = 'top' + topN;
        let cumProfitK = 0;
        const settledLedger = [];
        ...
        for (const date of dates) {
            ...
            const dRow = dualMap.get(date);
            const tRow = triMap.get(date);
            const rRow = rrfMap.get(date);

            const rawRow = rawMap.get(date);
            const actual27 = rawRow ? extract27Prizes(rawRow) : (qRow.actual27 ? qRow.actual27.map(n => String(n).padStart(2, '0')) : []);
            const actualMap = {};
            actual27.forEach(n => {
            ...
            let qNums = qRow.methods?.[key]?.betNumbers || (qRow.rankedNumbers || []).slice(0, topN);
            let dNums = dRow.methods?.[key]?.betNumbers || (dRow.rankedNumbers || []).slice(0, topN);
            let tNums = tRow.methods?.[key]?.betNumbers || (tRow.rankedNumbers || []).slice(0, topN);

            const rPred = rRow.strategies?.rrfParallelBlock85Small65?.predictions || rRow.predictions || {};
            let rNums = rPred[key]?.numbers || rRow.methods?.[key]?.betNumbers || [];
            ...
            const totalDayProfitK = dayLotoProfitK + dayXien4ProfitK;
            cumProfitK += dayLotoProfitK;
            ...
            settledLedger.push({
                ...
                isXien4Win,
                totalDayProfitK,
                cumLotoProfitK: cumProfitK
            });
        }

        const liveSettled = settledLedger.filter(r => r.isLive);
        const summary = {
            all: calcSummary(settledLedger),
            live: calcSummary(liveSettled)
        };

        const nextQMBF = loQuantumBayesFusion?.latestRecommendation?.topPredictions?.[key]?.numbers || [];
        const nextDual = loDualMerge?.latestRecommendation?.topPredictions?.[key]?.numbers || [];
        const nextTri = loTriHarmonic?.latestRecommendation?.topPredictions?.[key]?.numbers || [];
        const nextRRF = rrfPred?.nextPrediction?.strategies?.rrfParallelBlock85Small65?.predictions?.[key]?.numbers 
            || rrfMap.get(dates[dates.length - 1])?.strategies?.rrfParallelBlock85Small65?.predictions?.[key]?.numbers || [];

        const nextPredictionDate = loQuantumBayesFusion?.latestRecommendation?.predictionDate 
            || loDualMerge?.latestRecommendation?.predictionDate 
            || '2026-09-27';

        const nextVotes = {};
        const nextMethodVotes = {};
        [
            { id: 'QMBF', nums: nextQMBF },
            { id: 'Dual', nums: nextDual },
            { id: 'Tri', nums: nextTri },
            { id: 'RRF', nums: nextRRF }
        ].forEach(m => {
            m.nums.forEach(num => {
                const s = String(num).padStart(2, '0');
                nextVotes[s] = (nextVotes[s] || 0) + 1;
                if (!nextMethodVotes[s]) nextMethodVotes[s] = [];
                nextMethodVotes[s].push(m.id);
            });
        });

        const nextAllNums = Object.keys(nextVotes).filter(n => nextVotes[n] >= 1).sort((a, b) => nextVotes[b] - nextVotes[a] || a.localeCompare(b));
        const nextOver2 = nextAllNums.filter(n => nextVotes[n] >= 2);

        const tierX5 = nextAllNums.filter(n => nextVotes[n] >= 4);
        const tierX4 = nextAllNums.filter(n => nextVotes[n] === 3);
        const tierX3 = nextAllNums.filter(n => nextVotes[n] === 2);
        const tierX1 = nextAllNums.filter(n => nextVotes[n] === 1);
        ...
        if (nextOver2.length < 4) {
            nextXien4.status = 'TOO_FEW';
            nextXien4.reason = `Chỉ có ${nextOver2.length} số trùng (< 4 số) -> Bỏ qua không đủ số ghép Xiên 4.`;
        } else if (nextOver2.length > 5) {
            nextXien4.status = 'SKIPPED_TOO_MANY';
            nextXien4.reason = `Có ${nextOver2.length} số trùng >= 2 (> 5 số) -> Khuyến nghị BỎ QUA KHÔNG ĐÁNH XIÊN 4 để bảo toàn vốn, tập trung toàn lực vào dàn Lô phân tầng!`;
        } else {
            nextXien4.status = 'ACTIVE';
            nextXien4.combinations = getCombinations(nextOver2, 4);
            nextXien4.numbers = nextOver2;
            nextXien4.stakeK = nextXien4.combinations.length * 11000;
            nextXien4.reason = `Có ${nextOver2.length} số trùng -> Đánh ${nextXien4.combinations.length} vé Xiên 4 quây.`;
        }

        const nextBetNumbers = nextAllNums.map(num => {
            const v = nextVotes[num];
            let multiplier = 1;
            if (v >= 4) multiplier = 5;
            else if (v === 3) multiplier = 4;
            else if (v === 2) multiplier = 3;
            ...
            return {
                ...
                votes: v,
                multiplier,
                methods: nextMethodVotes[num]
            };
        });

        const nextTotalStakeK = (tierX5.length * 5 + tierX4.length * 4 + tierX3.length * 3 + tierX1.length * 1) * BASE_LOTO_STAKE_K;

        const latestRecommendation = {
            predictionDate: nextPredictionDate,
            methodId: 'lo4EngineFusion',
            topN,
            methodName: `🔥 Lô Tổng Hợp 4 Động Cơ Thực Chiến (Top ${topN} Live: QMBF + Bạc Nhớ + 3 Động Cơ + RRF)`,
            tierX5,
            tierX4,
            tierX3,
            tierX1,
            numbersOver2: nextOver2,
            allNumbers: nextAllNums,
            distinctNumbers: nextAllNums,
            betNumbers: nextBetNumbers,
            totalLotoStakeK: nextTotalStakeK,
            xien4: nextXien4,
            sourceEngines: {
                qmbf: nextQMBF,
                dual: nextDual,
    ...
    const mode6 = buildEngineForTopN(6);
    const mode7 = buildEngineForTopN(7);
    const activeMode = (options.topN === 7) ? mode7 : mode6;

    return {
        id: 'lo4EngineFusion',
        name: '🔥 Lô Ghép 4 Động Cơ Thực Chiến (Top 6 hoặc 7 Live: QMBF + Bạc Nhớ + 3 Động Cơ + RRF)',
        shortName: 'Ghép 4 Động Cơ Live',
        activeMode: options.topN === 7 ? 'top7' : 'top6',
        modes: {
            top6: mode6,
            top7: mode7
        },
        settledLedger: activeMode.settledLedger,
        summary: activeMode.summary,
        latestRecommendation: activeMode.latestRecommendation
    };
}

 ...
 */
function selectStreakAwareDeAdvisor(dualLedger = [], tripleLedger = [], adaptiveLedger = [], bayesLedger = []) {
    ...
    // 2. ƯU TIÊN 2: BÙ TRỪ NHỊP GÃY CHUỖI MỐC 20 NĂM (COUNTER-PHASE RECOVERY)
    else if (dType === 'loss' && aType === 'loss') {
        if (bType === 'win' || (bType === 'loss' && bLen === 1)) {
            selectedMethod = 'bayesFormResonance';
            sizingMultiplier = 1.2;
            rationale = 'Đề Gộp và Alpha cùng chịu nhịp gãy chuỗi mốc 20 năm. Kích hoạt Đề Ngũ Hành Dạng Số Bayes độc lập để bù trừ chuỗi xịt (tỷ lệ cứu 41.2%).';
            confidenceBadge = '🔮 KÍCH HOẠT DẠNG SỐ BAYES BÙ TRỪ (CƯỢC X1.2)';
    ...
    // 3. ƯU TIÊN 3: BÁM THEO ĐÀ THẮNG KHỎE (70% - 81% WIN RATE)
    ...
    // 4. ƯU TIÊN 4: XOAY TRỤC TRÁNH BÃO HÒA THẮNG (WIN EXHAUSTION < 3.6%)
    ...
    // 5. TRẠNG THÁI CÂN BẰNG / NHỊP THƯỜNG
    else {
...
function buildStreakAwareDeAdvisor(dualMerge, tripleMerge, adaptiveDualMergeOrRaw = null, rawRowsOrOptions = [], options = {}) {
    ...
    let dStreak = 0, tStreak = 0, aStreak = 0, bStreak = 0;
    ...
    for (let i = 0; i < totalDays; i++) {
        ...
        // Quad-Governor dispatch rules (100% Strict PIT based on streaks prior to day i)
        if (aStreak === -2 && tStreak !== -3 && aRow) {
            ...
            if (bStreak >= 1 && bRow) {
                ...
                chosenMethod = 'bayesFormResonance'; chosenRow = bRow; sizing = 1.0;
            } else {
        ...
        } else if (bStreak === 1 && aStreak < 2 && dStreak < 2 && bRow) {
            chosenMethod = 'bayesFormResonance'; chosenRow = bRow; sizing = 1.1;
        } else if (tStreak < 2 && (aStreak >= 2 || dStreak >= 2)) {
    ...
    const decision = selectStreakAwareDeAdvisor(dualLedger, tripleLedger, adaptiveLedger, bayesLedger);
    ...
    const availableMethods = {
        ...
        adaptiveDualMerge: {
            ...
            subLabel: '60M · Dàn 43 số · Chọn 2/21 cặp động',
            winRate: '56.1%',
            profitK: 2676000,
        ...
        dualMerge: {
            ...
            subLabel: '60M · Dàn 43 số · Sweet-Spot Giao Thoa',
            winRate: '54.9%',
            profitK: 1584000,
        ...
        tripleMerge: {
            ...
            subLabel: '90M · 50-60 số · Khung 3 Tầng Cược',
            winRate: '41.2%',
            profitK: 3510000,
    ...
    return {
        version: 'v3.0-quad-method-governor',
        description: 'Bộ Điều Phối Đổi Pha Đa Phương Pháp (Quad-Method Governor): Luân chuyển thông minh giữa Đề Thích Ứng Alpha, Đề Gộp, Tam Trụ và Đề Dạng Số Bayes Bù Trừ (cứu 41% chuỗi xịt khi mốc 20 năm gãy).',
        latestRecommendation,
        settledLedger,
        summary,
        bayesAdvisor
...
function buildLoQuadHybridAdvisor(loQuantumBayesFusion, loDualMerge, rawRows = [], loTriHarmonic = null) {
    ...
    // Multi-Engine Smart Governor: Đổi pha động cơ & dàn số bù trừ
    let selectedEngine = 'quad';
    let selectedEngineLabel = 'Super-Hybrid Quad-Fusion v7.1';
    let selectedSubTier = 7;
    let confidenceBadge = '🟢 THEO ĐÀ THẮNG KHỎE QUAD-HYBRID (65.7% WIN · CƯỢC X2)';
    let rationale = `Dàn Thất Thủ (Top 7) Quad-Hybrid đang giữ đà nổ liên tiếp ${s7Quad.streakLen} ngày qua. Lịch sử kiểm chứng 3 năm cho thấy xác suất nổ tiếp theo đà thắng đạt 65.7%, tỷ lệ thắng tổng thể 65.1% cao nhất trong các dàn con, chuỗi thua tối đa chỉ 6 ngày. Đề xuất cược X2 để bứt phá lợi nhuận.`;
    let sizingMultiplier = 2.0;

    // Dispatch rules for engine selection (Strict PIT):
    if (s7Penta.streakType === 'win' && s7Penta.streakLen >= 1 && s7Quad.streakType === 'loss') {
        selectedEngine = 'penta';
        selectedEngineLabel = 'Siêu Động Cơ Ngũ Hợp Lô AI v8.0';
        selectedSubTier = 7;
        confidenceBadge = '⚡ KÍCH HOẠT NGŨ HỢP v8.0 (TOP 7 WIN 77.3% · CƯỢC X2)';
        rationale = `Quad-Hybrid vừa điều chỉnh. Siêu Động Cơ Ngũ Hợp Lô AI v8.0 đang giữ nhịp nổ (${s7Penta.streakLen}d), tỷ lệ thắng vượt trội 77.3% (+1.369 TỶ). Kích hoạt cược X2 Thất Thủ để tối đa hóa lợi nhuận!`;
    } else if (s7Quad.streakType === 'win' && s7Quad.streakLen >= 1 && s7Quad.streakLen < 3) {
        selectedEngine = 'quad';
        selectedEngineLabel = 'Super-Hybrid Quad-Fusion v7.1';
        selectedSubTier = 7;
        confidenceBadge = '🟢 THEO ĐÀ THẮNG KHỎE QUAD-HYBRID (65.7% WIN · CƯỢC X2)';
        rationale = `Dàn Thất Thủ (Top 7) Quad-Hybrid đang giữ đà nổ liên tiếp ${s7Quad.streakLen} ngày qua. Tỷ lệ thắng 65.1%, xác suất thắng tiếp 65.7%. Đề xuất cược X2.`;
    } else if (s7Quad.streakType === 'loss') {
        if (s7Qmbf.streakType === 'win') {
            selectedEngine = 'qmbf';
            selectedEngineLabel = 'Quantum Bayes Fusion 7D';
            selectedSubTier = 7;
            confidenceBadge = '🌌 KÍCH HOẠT QUANTUM BAYES 7D (BÙ TRỪ 45.6% · CƯỢC X2)';
            rationale = `Quad-Hybrid vừa điều chỉnh. Động cơ Quantum Bayes Fusion 7D đang giữ nhịp thắng (${s7Qmbf.streakLen}d), tỷ lệ thắng 80.8% và có khả năng cứu 45.6% số ngày khi Quad trượt. Kích hoạt đảo pha sang QMBF để bứt phá lợi nhuận!`;
        } else if (s7Dual.streakType === 'win') {
            selectedEngine = 'dual';
            selectedEngineLabel = 'Lô Gộp Tinh Hoa (LoDual)';
            selectedSubTier = 7;
            confidenceBadge = '💎 KÍCH HOẠT LÔ GỘP TINH HOA (BÙ TRỪ 38.6% · CƯỢC X2)';
            rationale = `Quad-Hybrid vừa điều chỉnh. Động cơ Lô Gộp Tinh Hoa đang giữ nhịp đỏ (${s7Dual.streakLen}d) và bù trừ 38.6% ngày thua của Quad. Kích hoạt cược X2.`;
        } else if (s7Tri.streakType === 'win') {
            selectedEngine = 'tri';
            selectedEngineLabel = 'Sóng 3 Điều Hòa (Tri-Harmonic)';
            selectedSubTier = 7;
            confidenceBadge = '🌊 KÍCH HOẠT SÓNG 3 ĐIỀU HÒA (BÙ TRỪ 33.3% · CƯỢC X2)';
            rationale = `Quad-Hybrid vừa điều chỉnh. Động cơ Sóng 3 Điều Hòa đang giữ nhịp thắng (${s7Tri.streakLen}d) và bù trừ 33.3% ngày thua của Quad.`;
        } else {
            selectedEngine = 'quad';
            selectedEngineLabel = 'Super-Hybrid Quad-Fusion v7.1';
            selectedSubTier = 7;
    ...
    } else if (s7Quad.streakLen >= 3) {
        if (s7Qmbf.streakLen < 2) {
            selectedEngine = 'qmbf';
            selectedEngineLabel = 'Quantum Bayes Fusion 7D';
            confidenceBadge = '🛡️ XOAY TRỤC TRÁNH BÃO HÒA THẮNG (SANG QMBF)';
            rationale = 'Quad-Hybrid đã nổ liên tiếp >= 3 ngày. Hệ thống chủ động xoay trục sang Quantum Bayes Fusion 7D để bảo vệ lợi nhuận và đón đầu chuỗi thắng mới.';
        } else {
            selectedEngine = 'dual';
            selectedEngineLabel = 'Lô Gộp Tinh Hoa';
            confidenceBadge = '🛡️ XOAY TRỤC TRÁNH BÃO HÒA THẮNG (SANG LÔ GỘP)';
            rationale = 'Quad-Hybrid đã nổ liên tiếp >= 3 ngày. Hệ thống xoay trục sang Lô Gộp Tinh Hoa.';
        }
    }

    let activeRanked = rankedNext;
    if (selectedEngine === 'penta' && pentaRankedNext.length > 0) activeRanked = pentaRankedNext;
    else if (selectedEngine === 'qmbf' && qmbfRankedNext.length > 0) activeRanked = qmbfRankedNext;
    else if (selectedEngine === 'dual' && dualRankedNext.length > 0) activeRanked = dualRankedNext;
    else if (selectedEngine === 'tri' && triRankedNext.length > 0) activeRanked = triRankedNext;
    ...
    const subTiers = {
        ...
        7: { size: 7, label: 'Thất Thủ ⭐', numbers: activeRanked.slice(0, 7), count: 7, stakeK: 15400, winCondition: 'Ăn 2 nháy lãi +600K', winRate: '65.1%', streak: `${s7.streakType}_${s7.streakLen}d`, recommended: selectedSubTier === 7 },
    ...
    const engines = {
        ...
        quad: {
            id: 'quad',
            label: '🚀 Super-Hybrid Quad-Fusion v7.1',
            shortLabel: 'Quad-Hybrid',
            description: 'Động cơ siêu dung hợp 4 tầng (QMBF + Bạc nhớ + T-PPR + Hawkes)',
            winRateTop7: '77.6%',
            winRateTop20: '75.7%',
            profitTop7K: 1345000,

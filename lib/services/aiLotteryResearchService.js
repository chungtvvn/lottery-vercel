...
function selectStreakAwareDeAdvisor(dualLedger = [], tripleLedger = [], adaptiveLedger = [], bayesLedger = [], markovLedger = [], graphLedger = []) {
    ...
    // 1. PHA 1: ĐIỂM RƠI VÀNG PHỤC HỒI CỰC ĐẠI (GOLDEN REBOUND - XÁC SUẤT NỔ BÙ 94.3% - 94.4%)
    if (aType === 'loss' && aLen >= 2 && !(tType === 'loss' && tLen >= 3)) {
        selectedMethod = 'adaptiveDualMerge';
        sizingMultiplier = 1.45;
        activePhase = 'GOLDEN_REBOUND';
        activePhaseLabel = '💎 Pha Đón Điểm Rơi Vàng Cực Đại (94.3% Win)';
        activePhaseIcon = 'bi-gem';
        rationale = 'Đề Thích Ứng Alpha đã trượt 2 ngày liên tiếp - Đạt điểm rơi xác suất cực đại với tỷ lệ nổ bù lịch sử 94.3% (31/32 lần). Tự động kích hoạt Pha Đón Điểm Rơi Vàng với hệ số phân bổ vốn VIP x1.45.';
        confidenceBadge = '💎 ĐIỂM RƠI VÀNG PHỤC HỒI ALPHA (94.3% WIN · x1.45)';
        ...
        confidenceBadge = '🔥 ĐIỂM RƠI VÀNG TAM TRỤ (94.4% WIN · x1.4)';
        recommendationMode = 'attack_rebound_triple';
    } else if (dType === 'loss' && dLen >= 2 && aType !== 'loss') {
        selectedMethod = 'dualMerge';
        sizingMultiplier = 1.35;
        activePhase = 'GOLDEN_REBOUND';
        activePhaseLabel = '💎 Pha Đón Điểm Rơi Vàng Phục Hồi Đề Gộp (91.7% Win)';
        activePhaseIcon = 'bi-gem';
        rationale = 'Đề Gộp vừa thua 2 ngày liên tiếp - Rơi vào Điểm Rơi Vàng Nhịp Hồi với xác suất nổ lại lịch sử đạt 91.7%.';
        confidenceBadge = '🔥 ĐIỂM RƠI VÀNG PHỤC HỒI ĐỀ GỘP (91.7% WIN)';
        recommendationMode = 'attack_rebound_dual';
    }

    // 2. PHA 2: ĐÓN LỰC NẢY BÙ SAU 1 NGÀY TRƯỢT (POST-LOSS REBOUND HUNT - P(WIN|LOSS 1) = 69.6%)
    // Quy luật thực nghiệm 2026: Hôm nay trượt thì ngày mai xác suất nổ bù vọt lên 69.6% (cao hơn +25.2% so với khi vừa thắng).
    else if (aType === 'loss' && aLen === 1) {
        selectedMethod = 'adaptiveDualMerge';
        sizingMultiplier = 1.25;
        activePhase = 'REBOUND_HUNT';
        activePhaseLabel = '🎯 Pha Đón Lực Nảy Bù (69.6% Win · Giữ Nguyên)';
        activePhaseIcon = 'bi-bullseye';
        rationale = 'Đề Thích Ứng Alpha vừa trượt 1 ngày hôm qua - Theo khảo sát Markov 2026, xác suất nổ bù ngày hôm sau đạt 69.6% (vượt trội so với 44.4% khi vừa thắng). Giữ nguyên phương pháp và nâng cược x1.25 để đón trọn nhịp nảy.';
        confidenceBadge = '🎯 ĐIỂM RƠI NẨY BÙ ALPHA (69.6% WIN · x1.25)';
        recommendationMode = 'attack_rebound_alpha';
    } else if (dType === 'loss' && dLen === 1 && aType !== 'loss') {
        selectedMethod = 'dualMerge';
        sizingMultiplier = 1.2;
        activePhase = 'REBOUND_HUNT';
        activePhaseLabel = '🎯 Pha Đón Lực Nảy Bù Đề Gộp (69.5% Win)';
        activePhaseIcon = 'bi-bullseye';
        rationale = 'Đề Gộp Tiêu Chuẩn vừa trượt 1 ngày - Xác suất nổ bù ngày hôm sau đạt 69.5%. Giữ nguyên phương pháp đón nhịp nảy.';
        confidenceBadge = '🎯 ĐIỂM RƠI NẨY BÙ ĐỀ GỘP (69.5% WIN)';
        recommendationMode = 'attack_rebound_dual';
    }

    // 3. PHA 3: ĐAN XEN NÉ SUY KIỆT SAU KHI THẮNG (POST-WIN EXHAUSTION DODGE)
    // Quy luật thực nghiệm 2026: Sau khi thắng, xác suất trượt ngày hôm sau lên tới 55.6% - 58.2%.
    else if (aType === 'win' && aLen >= 2) {
        activePhase = 'EXHAUSTION_DODGE';
        activePhaseLabel = '🔄 Pha Đan Xen Né Suy Kiệt (Exhaustion Dodge)';
        activePhaseIcon = 'bi-arrow-repeat';
        if (mType === 'win' || (mType === 'loss' && mLen === 1)) {
            selectedMethod = 'deMarkovGapHazard';
            sizingMultiplier = 1.0;
            rationale = `Đề Thích Ứng Alpha đã thắng liên tiếp ${aLen} ngày (xác suất điều chỉnh gãy nhịp ngày 3 lên tới 98.4%). Hệ thống CHỦ ĐỘNG ĐAN XEN sang Markov Gap Hazard để né nhịp suy kiệt và bảo toàn thành quả.`;
            confidenceBadge = '🔄 ĐAN XEN NÉ SUY KIỆT (SANG MARKOV GAP)';
            recommendationMode = 'rotate_avoid_exhaustion_markov';
        } else if (gType === 'win') {
            selectedMethod = 'dePositionalGraphFlow';
            sizingMultiplier = 1.0;
            rationale = `Đề Thích Ứng Alpha đã thắng ${aLen} ngày liên tiếp. Hệ thống đan xen sang Cầu Đề Đồ Thị Vị Trí để đón nhịp cầu thông độc lập.`;
            confidenceBadge = '🔄 ĐAN XEN NÉ SUY KIỆT (SANG CẦU ĐỒ THỊ)';
            recommendationMode = 'rotate_avoid_exhaustion_graph';
        } else if (dType === 'win') {
            selectedMethod = 'dualMerge';
            sizingMultiplier = 1.0;
            rationale = `Đề Thích Ứng Alpha đã thắng ${aLen} ngày liên tiếp. Hệ thống đan xen sang Đề Gộp Tiêu Chuẩn cược phẳng an toàn.`;
            confidenceBadge = '🔄 ĐAN XEN SANG ĐỀ GỘP';
            recommendationMode = 'rotate_avoid_exhaustion_dual';
        } else {
            selectedMethod = 'tripleMerge';
            sizingMultiplier = 1.0;
            rationale = `Đề Thích Ứng Alpha đã thắng ${aLen} ngày liên tiếp. Hệ thống đan xen sang Tam Trụ 3 trục phòng thủ phân tầng.`;
            confidenceBadge = '🛡️ ĐAN XEN PHÒNG THỦ (SANG TAM TRỤ)';
        ...
        if (mType === 'win' || (mType === 'loss' && mLen === 1)) {
            selectedMethod = 'deMarkovGapHazard';
            sizingMultiplier = 1.0;
            rationale = `Đề Gộp đã thắng liên tiếp ${dLen} ngày (xác suất điều chỉnh cao). Hệ thống chủ động đảo pha sang Markov Gap.`;
            confidenceBadge = '🔄 ĐAN XEN SANG MARKOV GAP';
            recommendationMode = 'rotate_avoid_exhaustion_markov';
        } else {
            selectedMethod = 'adaptiveDualMerge';
            sizingMultiplier = 1.0;
            rationale = `Đề Gộp đã thắng ${dLen} ngày liên tiếp. Hệ thống chuyển đan xen sang Đề Thích Ứng Alpha.`;
            confidenceBadge = '🔄 ĐAN XEN SANG ALPHA';
            recommendationMode = 'rotate_avoid_exhaustion_alpha';
        }
    }

    // 4. PHA 4: VỪA THẮNG 1 KỲ (BẢO TOÀN LÃI & KIỂM SOÁT RỦI RO SUY KIỆT P(LOSS|WIN)=55.6%)
    else if (aType === 'win' && aLen === 1) {
        if (mType === 'loss' && mLen === 1) {
            // Markov Gap vừa trượt 1 ngày đang có lực nảy 54.9% -> Đan xen sang Markov Gap
            selectedMethod = 'deMarkovGapHazard';
            sizingMultiplier = 1.1;
            activePhase = 'EXHAUSTION_DODGE';
            activePhaseLabel = '🔄 Pha Đan Xen Sang Markov Gap Nảy Bù';
            activePhaseIcon = 'bi-arrow-repeat';
            rationale = 'Đề Thích Ứng Alpha vừa thắng hôm qua (nguy cơ suy kiệt 55.6%). Trong khi đó Markov Gap vừa trượt 1 ngày và đang ở điểm rơi nảy bù 54.9%. Hệ thống đan xen luân phiên sang Markov Gap.';
            confidenceBadge = '🔄 ĐAN XEN MARKOV GAP NẨY BÙ';
            recommendationMode = 'rotate_avoid_exhaustion_markov';
        } else {
            // Giữ nguyên Adaptive Dual nhưng ĐÁNH PHẲNG 1.0 để né gãy vốn
            selectedMethod = 'adaptiveDualMerge';
            sizingMultiplier = 1.0;
            activePhase = 'STABILIZE_RUN';
            activePhaseLabel = '⚖️ Pha Giữ Nhịp Ổn Định Vốn (Cược Phẳng 1.0)';
            activePhaseIcon = 'bi-shield';
            rationale = 'Đề Thích Ứng Alpha vừa nổ kỳ trước. Do xác suất gãy nhịp điều chỉnh sau thắng là 55.6%, hệ thống duy trì cược phẳng an toàn 1.0 để kiểm soát rủi ro và bảo toàn lãi ròng.';
            confidenceBadge = '⚖️ DUY TRÌ CƯỢC PHẲNG AN TOÀN 1.0';
            recommendationMode = 'standard';
        }
    }

    // 5. PHA 5: CỨU HỘ KHÁNG NHIỄU BÙ TRỪ (ORTHOGONAL RESCUE)
    ...
    // 6. TRẠNG THÁI CÂN BẰNG NỀN TẢNG
    else {
        activePhase = 'STABILIZE_RUN';
...
function buildStreakAwareDeAdvisor(dualMerge, tripleMerge, adaptiveDualMergeOrRaw = null, rawRowsOrOptions = [], maybeOptions = {}) {
    ...
    for (let i = 0; i < totalDays; i++) {
        ...
        if (aStreak <= -2 && aRow) {
            // Golden Rebound Alpha (94.3% win rate in 2026)
            chosenMethod = 'adaptiveDualMerge'; chosenRow = aRow; sizing = 1.45;
        } else if (tStreak <= -3 && tRow) {
            // Golden Rebound Triple (94.4% win rate in 2026)
            chosenMethod = 'tripleMerge'; chosenRow = tRow; sizing = 1.4;
        } else if (dStreak <= -2 && dRow) {
            // Golden Rebound Dual (91.7% win rate in 2026)
            chosenMethod = 'dualMerge'; chosenRow = dRow; sizing = 1.35;
        } else if (aStreak === -1 && aRow) {
            // Post-Loss Rebound Hunt: P(Win|Loss 1d) = 69.6% (vọt tăng +25.2% so với khi vừa thắng)
            chosenMethod = 'adaptiveDualMerge'; chosenRow = aRow; sizing = 1.25;
        } else if (dStreak === -1 && dRow) {
            // Post-Loss Rebound Hunt Dual: P(Win|Loss 1d) = 69.5%
            chosenMethod = 'dualMerge'; chosenRow = dRow; sizing = 1.2;
        } else if (aStreak >= 2) {
            // Strict Exhaustion Dodge (Alpha streak >= 2 has high exhaustion rate)
            if (mRow && (mStreak >= 1 || mStreak === -1)) {
                chosenMethod = 'deMarkovGapHazard'; chosenRow = mRow; sizing = 1.0;
            } else if (gRow && gStreak >= 1) {
                chosenMethod = 'dePositionalGraphFlow'; chosenRow = gRow; sizing = 1.0;
            } else if (dRow && dStreak >= 1) {
                chosenMethod = 'dualMerge'; chosenRow = dRow; sizing = 1.0;
            } else {
            ...
            // Win 1d: Post-win loss risk is 55.6% -> Cược phẳng an toàn 1.0 hoặc đan xen Markov Gap nếu đang nảy bù
            if (mRow && mStreak === -1) {
                chosenMethod = 'deMarkovGapHazard'; chosenRow = mRow; sizing = 1.1;
            } else {
                chosenMethod = 'adaptiveDualMerge'; chosenRow = aRow; sizing = 1.0;
            }
        ...
        const lockedNumbers = (isDateLocked && Array.isArray(lockedRec.numbers) && lockedRec.numbers.length > 0)
            ? lockedRec.numbers
            : (chosenRow?.fullUnion || chosenRow?.union || chosenRow?.numbers || chosenRow?.standard30 || []);
        const lockedVipNums = (isDateLocked && (Array.isArray(lockedRec.vipNumbers) || Array.isArray(lockedRec.tierX2)))
            ...
            : (chosenRow?.intersectionX2 || chosenRow?.intersection || chosenRow?.vipNumbers || []);
...
function buildLoQuadHybridAdvisor(loQuantumBayesFusion, loDualMerge, rawRows = [], loTriHarmonic = null) {
    ...
    if (s7Qmbf.streakType === 'loss' && s7Qmbf.streakLen >= 1) {
        ...
        confidenceBadge = '🌌 ĐÓN LỰC NẢY BÙ QUANTUM BAYES TOP 7 (85.4% WIN · x1.25)';
        rationale = `Quantum Bayes Fusion Top 7 vừa trượt kỳ trước (thua ${s7Qmbf.streakLen} kỳ). Theo khảo sát Markov 2026, xác suất nổ bù lội ngược dòng của QMBF Top 7 đạt đỉnh 85.4%. Tự động luân phiên sang QMBF và nâng cược x1.25 để tối ưu hóa lợi nhuận.`;
    }
    // 2. ĐÓN LỰC NẢY BÙ QUAD-FUSION (Khi Quad vừa trượt kỳ trước -> Lội ngược dòng 80.0% Top 20 và 76.4% - 84.6% Top 7)
    else if (s7Quad.streakType === 'loss') {
        selectedEngine = 'quad';
        selectedEngineLabel = 'Super-Hybrid Quad-Fusion v7.2';
        selectedSubTier = 7;
        sizingMultiplier = s7Quad.streakLen >= 2 ? 1.4 : 1.2;
        activeRanked = rankedNext;
        if (s7Quad.streakLen >= 2) {
            confidenceBadge = '🔥 ĐIỂM RƠI VÀNG QUAD-FUSION TOP 7 (84.6% WIN · x1.4)';
            rationale = `Tứ Trụ Quad-Fusion v7.2 đã trượt 2 ngày liên tiếp. Theo quy luật chuỗi 2026, xác suất nổ bù có lãi kỳ thứ 3 đạt 84.6% (Top 7) và 91.7% (Top 20). Giữ vững Quad-Fusion và nâng cược VIP x1.4.`;
        } else {
            confidenceBadge = '🎯 ĐÓN LỰC NẢY BÙ QUAD-FUSION (76.4% - 80.0% WIN · x1.2)';
            rationale = `Tứ Trụ Quad-Fusion vừa trượt 1 ngày hôm qua. Xác suất nổ bù lội ngược dòng đạt 76.4% (Top 7) và 80.0% (Top 20). Giữ nguyên Quad-Fusion và tăng nhẹ cược x1.2 để đón nhịp phục hồi.`;
        }
    }
    // 3. CẢ HAI CÙNG THẮNG: ƯU TIÊN QUANTUM BAYES VÌ BASELINE CAO NHẤT (81.5%)
        ...
        sizingMultiplier = 1.0;
        activeRanked = qmbfRankedNext;
        confidenceBadge = '👑 QUANTUM BAYES TOP 7 (WIN 81.5% · CƯỢC 1.0 PHẲNG)';
        rationale = `Cả hai động cơ chủ lực đều đang giữ đà thắng khỏe. Ưu tiên Quantum Bayes Fusion 7D với tỷ lệ thắng nền tảng cao nhất năm 2026 (81.5% Top 7, ROI +43.1%). Đề xuất cược phẳng 1.0 để kiểm soát rủi ro sau thắng.`;
    }

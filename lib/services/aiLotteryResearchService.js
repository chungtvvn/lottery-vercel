...
function selectStreakAwareDeAdvisor(dualLedger = [], tripleLedger = [], adaptiveLedger = [], bayesLedger = [], markovLedger = [], graphLedger = []) {
    ...
    // 2. PHA 2: NÉ QUÁ NHIỆT ĐIỀU CHỈNH (EXHAUSTION DODGE)
    else if (aLen >= 3) {
        ...
        if (mType === 'win') {
            ...
            rationale = `Đề Thích Ứng Alpha đã thắng liên tiếp ${aLen} ngày (nguy cơ gãy nhịp điều chỉnh > 90%). Hệ thống CHỦ ĐỘNG ĐẢO PHA sang Markov Gap Hazard đang có nhịp đỏ để bảo toàn lợi nhuận.`;
    ...
    } else if (tLen >= 2 && aLen < 2) {
        activePhase = 'EXHAUSTION_DODGE';
        activePhaseLabel = '🔄 Pha Né Quá Nhiệt Tam Trụ';
        activePhaseIcon = 'bi-arrow-repeat';
        selectedMethod = 'adaptiveDualMerge';
        sizingMultiplier = 1.0;
        rationale = `Tam Trụ đã thắng ${tLen} ngày liên tiếp. Hệ thống chủ động đảo pha về Đề Thích Ứng Alpha để bảo toàn thành quả.`;
        confidenceBadge = '🔄 ĐẢO PHA VỀ ALPHA';
...
function buildStreakAwareDeAdvisor(dualMerge, tripleMerge, adaptiveDualMergeOrRaw = null, rawRowsOrOptions = [], options = {}) {
    ...
    for (let i = 0; i < totalDays; i++) {
        ...
        // Dynamic Multi-Signal Dispatch (100% Strict PIT based on prior streaks)
        if (aStreak === -2 && aRow) {
...
function buildLoQuadHybridAdvisor(loQuantumBayesFusion, loDualMerge, rawRows = [], loTriHarmonic = null) {
    ...
    else if (selectedEngine === 'bridge' && bridgeRankedNext.length > 0) activeRanked = bridgeRankedNext;
    else if (selectedEngine === 'hawkes' && hawkesRankedNext.length > 0) activeRanked = hawkesRankedNext;

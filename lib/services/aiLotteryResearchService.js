...
function buildStrategicPortfolioGovernor({
    ...
    // Thuật toán phân tích và lựa chọn Gói Chiến Lược Tối Ưu Hôm Nay:
    ...
    // 2. TIÊU CHÍ 2: BẮT NHỊP NHIỄU ĐỘC LẬP KHI ALPHA VỪA TRƯỢT VÀ MARKOV ĐANG DƯƠNG
    else if (aType === 'loss' && mType === 'win') {
        recommendedPortfolioId = 'antiNoiseResonance';
        decisionCode = 'ANTI_NOISE_MARKOV_GAP';
        badge = '🔮 ĐỀ XUẤT HÔM NAY: GÓI 4 KHÁNG NHIỄU BẺ CẦU (MARKOV NỔ THÔNG)';
        rationale = 'Đề Alpha vừa có nhịp trượt ngắn trong khi mô hình Markov Bậc 2 & Gap Hazard đang nổ thông (chuỗi thắng dương). Hệ thống đề xuất Gói 4 để đón đầu sóng nổ độc lập mốc lịch sử kết hợp Lô Cầu Đồ Thị 54 vị trí và Top 20 Mỏ Neo Nền Tảng!';
    }
    // 3. TIÊU CHÍ 3: SÓNG THẦN HỒI QUY HẬU LỌT KHE (POST-LOT-KHE REBOUND)
    ...
    // 4. TIÊU CHÍ 4: ĐIỂM RƠI KIM CƯƠNG NỔ BÙ ALPHA
    else if (aType === 'loss' && aLen >= 2) {
        recommendedPortfolioId = 'maxProfit';
    ...
    // 5. TIÊU CHÍ 5: QUÁN TÍNH ĐÀ THẮNG KHỎE / NỀN TẢNG ỔN ĐỊNH
    else {
        recommendedPortfolioId = 'maxProfit';
    ...
    const portfolios = {
        ...
        steadyAccumulator: {
            ...
            badge: recommendedPortfolioId === 'steadyAccumulator' ? badge : '🛡️ Bảo Toàn Lãi (An Toàn Tuyệt Đối)',
            rationale: recommendedPortfolioId === 'steadyAccumulator' ? rationale : 'Chế độ phòng thủ bảo vệ thành quả: Hạ cược đòn bẩy về cược phẳng an toàn.',
            deMethod: 'dualMerge',
            loEngine: 'quad',
            loSubTier: 2,
            deStructure: {
                label: '🎯 Đề Gộp Tiêu Chuẩn (Cược Phẳng 1M)',
                vipNums: [],
                singleNums: alphaAll,
                allNums: alphaAll,
                hedgeNums: lotKhe0,

 ...
 */
function selectStreakAwareDeAdvisor(dualLedger = [], tripleLedger = [], adaptiveLedger = [], bayesLedger = [], markovLedger = [], graphLedger = [], metaLedger = []) {
    ...
    if (mType === 'loss' && mLen === 1) {
        ...
        rationale = 'Đề Markov Gap vừa trượt 1 kỳ hôm qua - Theo khảo sát ma trận chuyển tiếp Markov 2026, xác suất nổ bù ngày hôm sau đạt đỉnh 54.9% (73/133 lần nổ lại ngay). Tự động kích hoạt Điểm Rơi Vàng Nổ Bù với mức cược 30M phẳng (không X2).';
        confidenceBadge = '⚡ ĐIỂM RƠI VÀNG NỔ BÙ MARKOV (54.9% WIN · 30M)';
        recommendationMode = 'attack_rebound_markov';
        switchPhase = 'REBOUND';
        switchReason = 'Markov Gap vừa trượt 1 kỳ hôm qua - Điểm Rơi Vàng Nổ Bù với xác suất thắng lịch sử 54.9% (73/133 lần nổ lại ngay).';
    }
...
function buildStreakAwareDeAdvisor(dualMerge, tripleMerge, adaptiveDualMergeOrRaw = null, rawRowsOrOptions = [], maybeOptions = {}) {
    ...
    const decision = selectStreakAwareDeAdvisor(dualLedger, tripleLedger, adaptiveLedger, bayesLedger, markovLedger, graphLedger, metaLedger);

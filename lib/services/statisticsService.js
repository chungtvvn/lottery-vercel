...
async function getQuickStats() {
    ...
    const analyzeCategory = (key, categoryData) => {
        ...
        // === ÁP DỤNG TIỀM NĂNG KỶ LỤC 1-2 NGÀY ===
        // 1. Dạng thường: mốc kỷ lục là 2, đang không có chuỗi (chỉ có 1 ngày)
        // 2. Dạng So le: mốc kỷ lục là 3, đang không có chuỗi (có 1 ngày)
        const isSoLePatternStrict = key.toLowerCase().includes('sole');
        
        // Nếu không có chuỗi hiện tại và mốc kỷ lục đủ nhỏ
        if (!quickStats[key].current && (computedMaxStreak === 2 || (isSoLePatternStrict && computedMaxStreak === 3))) {
            const [category, subcategory] = key.split(':');
            const patternToMatch = subcategory || category; // Ví dụ: 'dau_0' thay vì 'dau'
            
            if (hasLatest) {
                if (!isSoLePatternStrict && computedMaxStreak === 2) {
                    // DẠNG THƯỜNG: Kiểm tra nếu số hôm nay thoả mãn
                    if (matchedToday.includes(patternToMatch)) {
                ...
                } else if (isSoLePatternStrict && computedMaxStreak === 3) {
                    // DẠNG SO LE: Kiểm tra nếu số hôm qua thoả mãn, và số hôm nay KHÔNG thoả mãn
                    if (hasYesterday) {
                        // Nếu hôm qua thoả mãn, hôm nay không thoả mãn -> Hôm nay là gap day của So Le
                        if (matchedYesterday.includes(patternToMatch) && !matchedToday.includes(patternToMatch)) {
                            const cnt = exactGapStats[3] ? exactGapStats[3].count : 0;
                            const freqY = cnt / totalYears;
                            
                            if (freqY <= 1.5) {
                                quickStats[key].current = {
                                    length: 1, // Chuỗi mới có 1 ngày active
                                    startDate: yestDate,
                                    endDate: yestDate,
                                    ...
                                    fullSequence: [
                                        ...
                                        { date: latestDate, value: numTodayStr, isLatest: true } // Gap day
                                    ]
                                };
                            }
                        }

...
// Production TTL for module-level caches (warm serverless instances)
const _SS_CACHE_TTL_MS = Number(process.env.MODULE_CACHE_TTL_MS || 10 * 60 * 1000);
let _ssStatsLoadedAt = 0;
let _ssQuickStatsLoadedAt = 0;
...
async function getStatsData() {
    ...
    // 1. Kiểm tra cache trước tiên (with TTL in production)
    if (cachedStats && process.env.NODE_ENV !== 'development' && (Date.now() - _ssStatsLoadedAt) > _SS_CACHE_TTL_MS) {
        cachedStats = null;
...
async function getQuickStats() {
    if (cachedQuickStats && process.env.NODE_ENV !== 'development' && (Date.now() - _ssQuickStatsLoadedAt) > _SS_CACHE_TTL_MS) {
        cachedQuickStats = null;
    }
    ...
    const analyzeCategory = (key, categoryData) => {
        ...
        if (latestDate) {
            ...
            } else if (isTienLuiSoLe || isSoLeTheoCap) {
                // Tiến/lùi so le và so le theo cặp ABAB chỉ hình thành từ 4 ngày.
                // Điều kiện này cũng chặn record ABA 3 ngày còn sót trong cache cũ.
        ...
        if (!quickStats[key].current && key.includes(':')) {
            ...
            const canAttachOneDayPotential = (formLen, step) => {
                // Potential is only the minDays - 1 precursor of a pattern
                // that has never formed. computedMaxStreak is intentionally
                // not used because a frequency threshold can exist without a
                // completed historical chain.
                const historicalRecordLen = Number(longest?.[0]?.length || 0);
                return historicalRecordLen === 0 && (formLen - step) >= 1;
            };
...
function rehydrateCurrentStreaks(quickStats) {
    ...
    for (const key of Object.keys(result)) {
        ...
        let updatedCat = { ...cat };
        if (updatedCat.longest && Array.isArray(updatedCat.longest)) {
            updatedCat.longest = updatedCat.longest.map(s => hydrateStreak(s, key));
        }
        if (updatedCat.secondLongest && Array.isArray(updatedCat.secondLongest)) {
            updatedCat.secondLongest = updatedCat.secondLongest.map(s => hydrateStreak(s, key));
        }

        if (updatedCat.current) {
            const hydratedCurrent = hydrateStreak(updatedCat.current, key);
            ...
            const lengthHistoryMetrics = updatedCat.lengthHistoryMetrics || buildLengthHistoryMetricsForKey(key, Math.max(targetLen + step, 2), basisDate);
...
function shouldAttachPotential(stat, key, formLen, step) {
    ...
    return historicalRecordLen === 0 && Number(formLen) - Number(step) >= 1;
}
...
function augmentPotentialStreaks(quickStats) {
    ...
    try {
        ...
        augmentNeverFormedPotentialStreaks(quickStats, {
            ...
            matchedYesterday,
            matchedDayBefore,
            recentDays: rawData.slice(-12),
            totalYears: quickStats._meta?.totalYears || lotteryService.getTotalYears()
        });
    } catch (error) {
        console.error('[statisticsService] Failed to augment never-formed no-data potentials:', error.message);
    }
 ...
 */
function rehydrateHistoryStreaks(historyCache, quickStats = null) {
    ...
    hydratedHistoryCache = historyCache.map(dayObj => {
        ...
        return {
            ...dayObj,
            streaks: dayObj.streaks.filter(streak => {
                const key = streak && (streak.key || (streak.current && streak.current.key));
                return !isInvalidStatsKey(key);
            }).map(streak => {
                const isNested = streak.current && typeof streak.current === 'object';
                const flatStreak = isNested ? { ...streak, ...streak.current } : streak;
                if (isNested) {
                    delete flatStreak.current;
                }

                const hydrated = hydrateStreak(flatStreak, flatStreak.key);
                const refreshedPatternNumbers = attachPatternNumbers(hydrated, hydrated.key);
                
                const qsPattern = (quickStats && quickStats[hydrated.key]) ? quickStats[hydrated.key] : null;
                
                const description = hydrated.description || (qsPattern && qsPattern.description) || '';
                const gapStats = hydrated.gapStats || (qsPattern && qsPattern.gapStats) || {};
                const exactGapStats = hydrated.exactGapStats || (qsPattern && qsPattern.exactGapStats) || {};
                const extensionGapStats = hydrated.extensionGapStats || (qsPattern && qsPattern.extensionGapStats) || {};
                const lengthHistoryMetrics = hydrated.lengthHistoryMetrics || (qsPattern && qsPattern.lengthHistoryMetrics) || null;
                const isSuperRecord = hydrated.isSuperRecord || (qsPattern && qsPattern.isSuperMaxThreshold) || false;

                const statLike = {
                    gapStats,
                    lengthHistoryMetrics,
                    historyMetrics: hydrated.historyMetrics || (qsPattern && qsPattern.historyMetrics) || computeHistoryMetricsForKey(hydrated.key, dayObj.date)
                };
                
                const reliability = hydrated.reliability || (qsPattern && qsPattern.reliability) || buildReliabilityForCurrent(hydrated.key, statLike, hydrated, dayObj.date);

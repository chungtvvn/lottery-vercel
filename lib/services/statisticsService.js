            ...
            if (isSoLe) {
                ...
                if (rawStreak) {
                    ...
                    if (isValid) {
                        ...
                        // Thêm ngày xen kẽ (latestDate) vào fullSequence để hiển thị
...
function rehydrateCurrentStreaks(quickStats) {
    ...
    for (const key of Object.keys(result)) {
        ...
        if (cat && cat.current) {
            ...
            const latestEntry = (cat.current.fullSequence || []).find(e => e.isLatest);
            const hydrated = hydrateStreak(cat.current, key);

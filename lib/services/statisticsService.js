        ...
        if (!quickStats[key].current && key.includes(':')) {
            ...
            if (hasLatest && !isTienLuiSoLeSubcat) {
                if (isSoLeTheoCapSubcat) {
                    // DẠNG ABAB: Cần 3 ngày liên tiếp cùng thuộc category, giá trị A,B,A
                    // VD: Đầu 1,4,1 → dự đoán Đầu 4 cho ngày 4 (pattern A,B,A,B)
                    if (hasYesterday && hasDayBeforeYesterday && 
                        matchedToday.includes(category) && 
                        matchedYesterday.includes(category) && 
                        matchedDayBeforeYesterday.includes(category)) {
                        
                        const { extractValueForComparison } = require('../utils/numberAnalysis');
                        const valToday = extractValueForComparison(numTodayStr, category);
                        const valYesterday = extractValueForComparison(numYesterdayStr, category);
                        const valDayBeforeYesterday = extractValueForComparison(numDayBeforeYesterdayStr, category);
                        
                        if (valToday !== null && valYesterday !== null && valDayBeforeYesterday !== null && 
                            String(valToday) !== String(valYesterday) && 
                            String(valToday) === String(valDayBeforeYesterday)) {
...
function augmentPotentialStreaks(quickStats) {
    ...
    for (const key of Object.keys(quickStats)) {
        ...
        if (isSoLeTheoCap) {
            if (!yesterdayNum || !dayBeforeNum) continue;
            const categoryMatches = matchedDayBefore.includes(category) && matchedYesterday.includes(category) && matchedToday.includes(category);
            const v0 = extractValueForComparison(dayBeforeNum, category);
            const v1 = extractValueForComparison(yesterdayNum, category);
            const v2 = extractValueForComparison(todayNum, category);
            const ababPrefix = v0 !== null && v1 !== null && v2 !== null && String(v0) !== String(v1) && String(v0) === String(v2);
            const formLen = 4;
            if (categoryMatches && ababPrefix && shouldAttachPotential(stat, key, formLen, 1)) {
                setPotential(key, {
                    length: 3,
                    startDate: dayBeforeDate,
                    endDate: todayDate,

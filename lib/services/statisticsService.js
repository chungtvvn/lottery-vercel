...
function buildReliabilityForCurrent(key, stat, current, basisDateStr = null) {
    ...
    const evalLen = current.isPotential ? currentLen + step : currentLen;
    if (!evalLen || evalLen <= 0) return null;

    const gapStats = stat.gapStats || current.gapStats || {};
    const currentInfo = gapStats[evalLen];
    ...
    const dropOffRate = sampleSize > 0 ? breakCount / sampleSize : 0;
    ...
    const sampleScore = clamp(Math.log10(sampleSize + 1) / Math.log10(100));

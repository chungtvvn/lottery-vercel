...
async function getStatsData() {
    ...
    try {
        ...
        // OPTIMIZATION: Auto-trigger loadStats() nếu stats chưa có
...
async function getQuickStats() {
    ...
    const analyzeCategory = (key, categoryData) => {
        ...
        if (latestDate) {
            ...
            if (isSoLe) {
                // So le: Chuỗi đang diễn ra nếu endDate = latestDate HOẶC endDate = yesterday
                // Case 1: endDate === latestDate → latestDate là ngày active cuối, ngày kế tiếp là gap → dự đoán cho ngày kế+1
                // Case 2: endDate === yesterday → yesterday là ngày active cuối, latestDate là gap → dự đoán cho ngày kế tiếp
                ...
                // Ưu tiên endDate === latestDate trước (ngày active cuối = hôm nay)
                let rawStreak = categoryData.streaks.find(s => s.endDate === latestDate);
                let latestIsGapDay = false; // false = latestDate là ngày active

                if (!rawStreak) {
                    // Fallback: endDate === yesterday (ngày active cuối = hôm qua, latestDate là gap day)
                    rawStreak = categoryData.streaks.find(s => s.endDate === yesterdayStr);
                    latestIsGapDay = true; // latestDate là ngày xen kẽ
                }
                ...
                if (rawStreak) {
                    ...
                    if (isSoLeMoi && latestLotteryDay && latestLotteryDay.special) {
                        try {
                            ...
                            if (matchNumbers && matchNumbers.length > 0) {
                                ...
                                if (latestIsGapDay && stringNumbers.includes(specialNum)) {
                    ...
                    if (isValid) {
                        ...
                        // Thêm ngày hôm nay (latestDate) vào fullSequence để hiển thị
                        // CHỈ khi latestDate là gap day (không thuộc dates[] của chuỗi)
                        if (latestIsGapDay && latestLotteryDay && latestLotteryDay.special) {
                            current.fullSequence.push({
                                date: latestDate,
                                value: String(latestLotteryDay.special).padStart(2, '0'),
                                isLatest: true // Đánh dấu là ngày xen kẽ (KHÔNG thuộc chuỗi active)
                            });
                        }
                    }
                }
        ...
        if (dateObj) {
            ...
            for (const key in statsAtThatDay) {
                ...
                if (statsAtThatDay[key] && statsAtThatDay[key].current) {
                    ...
                    // === RE-HYDRATE fullSequence từ rawData nếu thiếu ===
                    if (!currentObj.fullSequence || currentObj.fullSequence.length <= 1) {
                        if (currentObj.startDate && currentObj.endDate) {
                            const startD = parseDate(currentObj.startDate);
                            const endD = parseDate(currentObj.endDate);
                            if (startD && endD) {
                                const builtSeq = [];
                                for (const item of rawData) {
                                    ...
                                    if (item.special !== null && item.special !== undefined) {
                                        builtSeq.push({
                                            date: `${String(itemD.getDate()).padStart(2, '0')}/${String(itemD.getMonth() + 1).padStart(2, '0')}/${itemD.getFullYear()}`,
                                            value: String(item.special).padStart(2, '0')
                                        });
                                    }
                                }
                                // Giữ lại isLatest entry nếu có
                                const latestEntry = (currentObj.fullSequence || []).find(e => e.isLatest);
                                currentObj.fullSequence = builtSeq;
                                if (latestEntry && !builtSeq.find(e => e.date === latestEntry.date)) {
                                    currentObj.fullSequence.push(latestEntry);
                                }
                            }
                        }
                    }
...
function rehydrateHistoryStreaks(historyCache) {
    ...
    return historyCache.map(dayObj => {
        if (!dayObj.streaks) return dayObj;
        return {
            ...dayObj,
            streaks: dayObj.streaks.map(streak => {
                // Nhớ lại entry isLatest
                const latestEntry = (streak.fullSequence || []).find(e => e.isLatest);
                const hydrated = hydrateStreak(streak, streak.key);
                // Gắn lại entry isLatest nếu chưa có
                if (latestEntry && hydrated.fullSequence && !hydrated.fullSequence.find(e => e.date === latestEntry.date)) {
                    hydrated.fullSequence.push(latestEntry);
                }
                return hydrated;
            })
        };

...
function buildDynamicCrossMethodAdvisor(loAdvisorsMap = {}, rawRows = [], options = {}) {
    ...
    if (canIncremental) {
        ...
        liveDiary = existingLiveDiary.map(entry => {
            ...
            if (e.date === '2026-09-19' || (e.standard && e.standard.topCount !== 20)) {
                ...
                const dayStake = stdStake + (e.x2?.stakeK || 0) + (e.xien4?.stakeK || 0);
                const dayPayout = stdPayout + (e.x2?.payoutK || 0) + (e.xien4?.payoutK || 0);
                const dayProfit = dayPayout - dayStake;
    ...
    if (prevNextPred && prevNextPred.predictionDate === nextPredDate && prevNextPred.standard?.numbers?.length && !options.forceSynthesize) {
        ...
        if (nextPrediction.x2 && (nextPrediction.x2.topCount !== 7 || nextPrediction.x2.numbers?.length !== 7)) {
            ...
            nextPrediction.x2 = {
                ...nextPrediction.x2,
                topCount: 7,
                title: `${nextPrediction.x2.methodLabel || '🎯 Bạc Nhớ 27 Giải'} - Top 7 (Đánh X2)`,
                numbers: nums,
                regularStakeK: 15400,
                stakeK: 30800
            };

...
function buildDynamicCrossMethodAdvisor(loAdvisorsMap = {}, rawRows = [], options = {}) {
        ...
        for (const r of drawnRows) {
            ...
            if (prevNextPred && prevNextPred.predictionDate === drawDate) {
                ...
                if (stdNumbers.length !== 20) {
                    const ranked = strategies[prevNextPred.standard?.methodId || 'loDualMerge']?.latestRec?.rankedNumbers || [];
    ...
    if (prevNextPred && prevNextPred.predictionDate === nextPredDate && prevNextPred.standard?.numbers?.length && !options.forceSynthesize) {
        ...
        // Strictly enforce Top 20 for standard and Top 7 for X2
        if (nextPrediction.standard && (nextPrediction.standard.topCount !== 20 || nextPrediction.standard.numbers?.length !== 20)) {
            const stratKey = nextPrediction.standard.methodId || 'loDualMerge';
            const ranked = strategies[stratKey]?.latestRec?.rankedNumbers || [];
            const nums = ranked.length >= 20 ? ranked.slice(0, 20) : nextPrediction.standard.numbers;
            nextPrediction.standard = {
                ...nextPrediction.standard,
                topCount: 20,
                title: `${nextPrediction.standard.methodLabel || '🎯 Bạc Nhớ 27 Giải'} - Top 20`,
                numbers: nums,
                stakeK: 44000
            };
        }

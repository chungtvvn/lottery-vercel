...
function buildLoTriHarmonicAdvisor(rawRows) {
    ...
    const latestRecommendation = {
        ...
        topPredictions,
        plainReasons: [
            `🏆 Động cơ Siêu Hợp Nhất 3 Mô hình 20 Năm (Tri-Harmonic Meta-Ensemble): Phối hợp đồng thời Markov Vị Trí (70%) + Cụm Đồng Xuất Pairwise Affinity (15%) + Sóng Động Lượng Chu Kỳ (15%).`,
...
function scoreQuantumBayesFusion(d0, dMinus1, curIdx, rawRows, markovModel, coModel, bayesModel) {
    ...
    currentFocus.forEach(({ num, w }) => {
        if (num >= 0 && num < 100 && denomLag1[num] > 0) {
            for (let j = 0; j < 100; j++) scoreMarkov[j] += w * (markovLag1[num][j] / denomLag1[num]) * 100;
        }
    });
    ...
    lag2Focus.forEach(({ num, w }) => {
        if (num >= 0 && num < 100 && denomLag2[num] > 0) {
            for (let j = 0; j < 100; j++) scoreMarkov[j] += (w * 0.50) * (markovLag2[num][j] / denomLag2[num]) * 100;
        }
    });
...
function buildLoQuantumBayesFusionAdvisor(rawRows) {
    ...
    const latestRecommendation = {
        ...
        topPredictions,
        plainReasons: [
            `🏆 Động cơ Siêu Hợp Nhất 4 Tầng 20 Năm (Quantum Bayes-Markov Fusion): Kết hợp Positional Markov Tensor (1.8x) + Bayes Cặp Đầu-Đuôi (0.3x) + Lực hút Tương quan Co-occurrence (0.3x) + Sóng Động lượng Chu kỳ đàn hồi (0.3x).`,
    ...
    return {
        version: '2026.4.0-quantum-bayes-fusion-20y',
        description: 'Lô Siêu Hợp Nhất 4 Tầng Bayes & Markov 20 Năm (Quantum Bayes-Markov Fusion: Positional Markov + Bayes Head-Tail + Co-occurrence + Elastic Momentum).',
        stakePerUnitK: LOTO_STAKE_PER_UNIT_K,
        payoutPerHitK: LOTO_PAYOUT_PER_HIT_K,
        latestRecommendation,
        settledLedger,

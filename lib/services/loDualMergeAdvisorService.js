...
const TIER_WEIGHTS = {
    ...
    prize6_1: 1.50, prize6_2: 1.50, prize6_3: 1.50,
    prize7_1: 1.55, prize7_2: 1.55, prize7_3: 1.55, prize7_4: 1.55
};
...
function rankPositionsWithTierWeights(posPreds) {
    if (!posPreds || !posPreds.length) return null;
    const scores = {};
    for (let i = 0; i < 100; i++) {
        ...
        scores[numStr] = score + g6g7Hits * 0.85;
    }
...
function selectDiverseTop(ranked, count) {
    const maxPerHead = count <= 6 ? 2 : count <= 8 ? 2 : count <= 10 ? 3 : count <= 20 ? 4 : 5;
    const headCounts = {};
    const selected = [];
...
function buildLoDualMergeAdvisor(deDualMerge, rawRows) {
    ...
    const latestRecommendation = {
        ...
        plainReasons: [
            ...
            `Kết hợp trọng số phân tầng Tier-Weighted (G6/G7 ưu tiên) và đa dạng hóa đầu số (Head Entropy) để tối đa xác suất trúng.`,

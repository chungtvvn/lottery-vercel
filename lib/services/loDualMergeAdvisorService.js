        ...
        for (let b = a + 1; b < pool.length; b++) {
            for (let c = b + 1; c < pool.length; c++) {
                for (let d = c + 1; d < pool.length; d++) {
                    ...
                    for (const n of q) {
                        rW += (6 - pool.indexOf(n));
                        if (overlapSet.has(n)) rW += 4.0; // Enhanced consensus bonus
                    }
                    ...
                    const avgL = lSum / 6;
                    const harmony = minL > 1.0 ? 15.0 : (minL < 0.95 ? -20.0 : 0.0);
                    const invBonus = containsInvPair ? 15.0 : 0.0;
                    const score = (rW * 1.0) + (avgL * 35.0) + harmony + invBonus;
                    if (score > bestScore) {
                        bestScore = score;
                        bestQ = q;
                    }
...
function buildDynamicCrossMethodAdvisor(loAdvisorsMap = {}, rawRows = [], options = {}) {
    ...
    if (prevNextPred && prevNextPred.predictionDate === nextPredDate && prevNextPred.standard?.numbers?.length && !options.forceSynthesize) {
        ...
        if (nextPrediction && (!nextPrediction.goldenXien2 || nextPrediction.goldenXien2[0]?.pair?.[0] === '36' || nextPrediction.goldenXien2[0]?.numbers?.[0] === '36' || options.updateXienModel || nextPrediction.xien4?.clusterId === 'synergy4' || !nextPrediction.goldenXien2?.[0]?.lift)) {
            ...
            if (consensusQuartet && consensusQuartet.length === 4) {
                nextPrediction.xien4 = {
                    ...
                    winRate: 0.451,
                    roi: 0.583,
                    rationale: 'Bộ 4 số vàng tối ưu hóa tương quan đồng xuất hiện (Strict PIT Lift Matrix) và hợp nhất đồng thuận giữa QMBF v6.1 và Bạc Nhớ X2.'
                };
        ...
        nextPrediction = {
            ...
            xien4: {
                ...
                clusterId: 'consensus4',
                title: '💎 Tứ Thủ Xiên 4 Tinh Hoa (Consensus Lift Clique)',
                numbers: (consensusQuartet && consensusQuartet.length === 4) ? consensusQuartet : (tomorrowX4 ? getTomorrowXienNumbers(tomorrowX4.stratKey, tomorrowX4.clusterId) : ['75', '36', '43', '29']),
                stakeK: 11000,
                winRate: 0.451,
                roi: 0.583,
                rationale: 'Bộ 4 số vàng tối ưu hóa tương quan đồng xuất hiện (Strict PIT Lift Matrix) và hợp nhất đồng thuận giữa QMBF v6.1 và Bạc Nhớ X2.'
            ...
            xien3: (() => {
                const q4 = (consensusQuartet && consensusQuartet.length === 4) ? consensusQuartet : ['75', '36', '43', '29'];

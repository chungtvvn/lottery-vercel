...
function buildLoQuantumBayesFusionAdvisor(rawRows) {
    ...
    const latestRecommendation = {
        ...
        confidence: 5.0,
        methodId: 'loQuantumBayesFusion',
        methodName: '💎 Lô Siêu Hợp Nhất 7 Động Cơ Bayes, Cầu Vị Trí & Dạng Số 20 Năm (QMBF v6 Strict PIT) [Khuyên Dùng]',
        rankedNumbers: pendingRankedNumbers,
        topPredictions,
        plainReasons: [
            `🏆 Động cơ Siêu Hợp Nhất 7 Động Cơ Mốc Lịch Sử (D-1) Strict PIT kết hợp Bộ Lọc Khử Lô Gan Nặng (QMBF v6): Kết hợp Positional Markov Tensor (1.9x) + Cầu Vị Trí GĐB & G1/G7 (0.2x) + Dạng Số & Chuyển Tiếp Tổng/Chạm/Bóng (0.2x) + Bayes Cặp Đầu-Đuôi (0.3x) + Lực hút Co-occurrence PMI (0.25x) + Sóng Động lượng Chu kỳ đàn hồi (0.3x) + Cặp Đảo & Bóng Âm Dương (0.15x) cùng Bộ lọc Khử Gan dài và Đa dạng hóa Đầu-Đuôi.`,
            `🎯 Dàn Song Thủ VIP Top 2 (${topPredictions.top2?.numbers?.join(' ')}): Vốn ${(topPredictions.top2?.stakeK || 4400)/1000}M/ngày, đạt tỷ lệ nổ ${(top2Summary.hitRate*100).toFixed(1)}% (${top2Summary.hitDays}/${top2Summary.days} ngày nổ), Thắng lãi ${(top2Summary.winRate*100).toFixed(1)}%, Tổng lãi thực tế +${(top2Summary.profitK/1000).toFixed(1)}M (ROI ${(top2Summary.roi*100).toFixed(1)}%).`,
    ...
    return {
        ...
        description: 'Lô Siêu Hợp Nhất 7 Động Cơ Bayes & Markov 20 Năm kết hợp Bộ Lọc Khử Lô Gan (QMBF v6: 3-Order Positional Markov Tensor + Positional Bridge + Form & Parity Resonance + Bayes Head-Tail + Co-occurrence PMI + Elastic Momentum + Inverse/Shadow Matrix + Soft Gan Damping).',
        stakePerUnitK: LOTO_STAKE_PER_UNIT_K,
        payoutPerHitK: LOTO_PAYOUT_PER_HIT_K,
        latestRecommendation,
        settledLedger,

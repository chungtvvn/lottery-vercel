...
function scoreQuantumBayesFusion(d0, dMinus1, curIdx, rawRows, markovModel, coModel, bayesModel) {
    ...
    const wMarkov = 2.0;
    const wHT = 0.35;
    const wAffinity = 0.30;
    const wMomentum = 0.35;
    const wShadow = 0.15;
    const wBridge = 0.20;
    const K = 20.0;
...
function buildLoQuantumBayesFusionAdvisor(rawRows) {
    ...
    const latestRecommendation = {
        ...
        confidence: 5.0,
        methodId: 'loQuantumBayesFusion',
        methodName: '💎 Lô Siêu Hợp Nhất 6 Động Cơ Bayes & Cầu Vị Trí 20 Năm (QMBF v4 Strict PIT) [Khuyên Dùng]',
        rankedNumbers: pendingRankedNumbers,
        topPredictions,
        plainReasons: [
            `🏆 Động cơ Siêu Hợp Nhất 6 Động Cơ Mốc Lịch Sử (D-1) Strict PIT (QMBF v4): Kết hợp Positional Markov Tensor 3 Tầng (2.0x) + Cầu Vị Trí GĐB & Giải 1 (0.2x) + Bayes Cặp Đầu-Đuôi (0.35x) + Lực hút Co-occurrence PMI (0.3x) + Sóng Động lượng Chu kỳ đàn hồi (0.35x) + Ma trận Cặp Đảo & Bóng Âm Dương (0.15x) cùng Bộ lọc đa dạng hóa Đầu & Đuôi.`,
            `🎯 Dàn Song Thủ VIP Top 2 (${topPredictions.top2?.numbers?.join(' ')}): Vốn ${(topPredictions.top2?.stakeK || 4400)/1000}M/ngày, đạt tỷ lệ nổ ${(top2Summary.hitRate*100).toFixed(1)}% (${top2Summary.hitDays}/${top2Summary.days} ngày nổ), Thắng lãi ${(top2Summary.winRate*100).toFixed(1)}%, Tổng lãi thực tế +${(top2Summary.profitK/1000).toFixed(1)}M (ROI ${(top2Summary.roi*100).toFixed(1)}%).`,
    ...
    return {
        version: '2026.5.0-quantum-bayes-fusion-v3-strict-pit',
        description: 'Lô Siêu Hợp Nhất 5 Động Cơ Bayes & Markov 20 Năm (QMBF v3: 3-Order Positional Markov Tensor + Bayes Head-Tail + Co-occurrence PMI + Elastic Momentum + Inverse/Shadow Matrix).',
        stakePerUnitK: LOTO_STAKE_PER_UNIT_K,
        payoutPerHitK: LOTO_PAYOUT_PER_HIT_K,
        latestRecommendation,
        settledLedger,

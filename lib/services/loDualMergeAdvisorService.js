...
function buildDynamicCrossMethodAdvisor(loAdvisorsMap = {}) {
    ...
    function selectCandidatesForDate(targetDate) {
        ...
        for (const [stratKey, strat] of Object.entries(strategies)) {
            ...
            for (const count of TOP_COUNTS) {
                ...
                let l14Profit = 0, l14Wins = 0, l14Hits = 0;
                ...
                const l14Roi = last14Rows.length ? l14Profit / (last14Rows.length * count * 2200) : 0;
                ...
                const recent3Hits = last3Rows.reduce((acc, r) => acc + (r.methods?.[key]?.hits || 0), 0);
                const recentHitRatio = recent3Hits / (count * 3);

                const scoreStd = (l14Roi * 50) + (overallRoi * 25) + (l14WinRate * 20) + (currentWinStreak * 12) + (recentHitRatio * 20);
                const scoreX2 = (overallWinRate * 60) + (l14WinRate * 30) + (overallRoi * 15) - (count > 10 ? 8 : 0);
            ...
            for (const xk of XIEN_KEYS) {
                let xProfit = 0, xWins = 0, xHits = 0;
                pastRows.forEach(r => {
                    const c = r.xien4?.clusters?.[xk];
                    if (c) {
                        xProfit += c.profitK;
                        if (c.isWin || c.hits >= 2) xWins++;
                        xHits += c.hits;
                    }
                });
                const days = pastRows.length;
                const xWinRate = days ? xWins / days : 0;
                const xRoi = days ? xProfit / (days * 11000) : 0;
    ...
    const nextPrediction = {
        ...
        standard: tomorrowStd ? {
            ...
            numbers: getTomorrowNumbers(tomorrowStd.stratKey, tomorrowStd.count),
            ...
            rationale: `Lợi nhuận 14 ngày vượt trội (+${(tomorrowStd.l14Roi * 100).toFixed(1)}%), chuỗi thắng ${tomorrowStd.currentWinStreak} kỳ liên tiếp, tín hiệu đà nổ tiếp tục cao.`
        } : null,
        x2: tomorrowX2 ? {
            ...
            numbers: getTomorrowNumbers(tomorrowX2.stratKey, tomorrowX2.count),
            ...
            rationale: `Tỉ lệ nổ an toàn cao nhất hệ thống (${(tomorrowX2.overallWinRate * 100).toFixed(1)}%), tối ưu cho chiến lược nhân đôi cược.`
        } : null,
        xien4: tomorrowX4 ? {
            methodId: tomorrowX4.stratKey,
            methodName: tomorrowX4.stratName,
            methodLabel: tomorrowX4.stratLabel,
            clusterId: tomorrowX4.clusterId,
            title: `${tomorrowX4.stratLabel} - Xiên ${tomorrowX4.clusterId.toUpperCase()}`,
            numbers: getTomorrowXienNumbers(tomorrowX4.stratKey, tomorrowX4.clusterId),
            stakeK: 11000,
            winRate: tomorrowX4.winRate,
            roi: tomorrowX4.roi,
            rationale: `Cụm Xiên 4 có tổng hòa ROI (+${(tomorrowX4.roi * 100).toFixed(1)}%) và tỉ lệ nổ (${(tomorrowX4.winRate * 100).toFixed(1)}%) cao nhất.`
        } : null
    };
...
function buildMetaCrossMethodStrategy(loAdvisorsMap = {}) {
    const dynamicAdvisor = buildDynamicCrossMethodAdvisor(loAdvisorsMap);
    ...
    function selectBestXien4(targetDate) {
        ...
        for (const [stratKey, strat] of Object.entries(strategies)) {
            ...
            for (const xk of XIEN_KEYS) {
                ...
                const xWinRate = days ? xWins / days : 0;
                const xRoi = days ? xProfit / (days * 11000) : 0;
                const score = (xRoi * 60) + (xWinRate * 40);
    ...
    const xien4Rec = strategies[bestXien4Candidate?.stratKey]?.latestRec?.xien4
    ...
    summary.xien4 = strategies.loQuantumBayesFusion?.advisor?.summary?.xien4 || {};
    summary.xien4Live = strategies.loQuantumBayesFusion?.advisor?.summary?.xien4Live || {};

...
const LOTO_TOP_COUNTS = [2, 4, 6, 7, 8, 10, 20];
...
function buildLoDualMergeAdvisor(deDualMerge, rawRows, deTripleMerge = null) {
    ...
    const summary = {
        ...
        live: (() => {
            ...
            const liveOverall = calcWindow(liveSettled);
            return {
                ...liveOverall,
                days: liveSettled.length,
                dualMerge: liveOverall,
                top2: summarizeTop(liveSettled, 2),
                top4: summarizeTop(liveSettled, 4),
                top6: summarizeTop(liveSettled, 6),
                top7: summarizeTop(liveSettled, 7),
                top8: summarizeTop(liveSettled, 8),
                top10: summarizeTop(liveSettled, 10),
                top20: summarizeTop(liveSettled, 20)
            };
        })(),
        windows: {
...
function scoreQuantumBayesFusion(d0, dMinus1, curIdx, rawRows, markovModel, coModel, bayesModel) {
    ...
    for (let j = 0; j < 100; j++) {
        rrfScores[j] = (wMarkov / (K + rankMarkov[j]))
                     + (wHT / (K + rankHT[j]))
...
function buildLoQuantumBayesFusionAdvisor(rawRows) {
    ...
    for (let i = 0; i < rows2026.length; i++) {
        ...
        settledLedger.push({
            ...
            methodName: '💎 Lô Siêu Hợp Nhất 7 Động Cơ Bayes & Markov 20 Năm (Quantum Bayes-Markov Fusion v5)',
            rankedNumbers,
            actual27,
            profitK: top10.profitK,
            cumulativeProfitK: isLiveSnapshot ? liveCumulativeProfitK : cumProfitK,
    ...
    const latestRecommendation = {
        ...
        confidence: 5.0,
        methodId: 'loQuantumBayesFusion',
        methodName: '💎 Lô Siêu Hợp Nhất 7 Động Cơ Bayes, Cầu Vị Trí & Dạng Số 20 Năm (QMBF v5 Strict PIT) [Khuyên Dùng]',
        rankedNumbers: pendingRankedNumbers,
        topPredictions,
        plainReasons: [
            `🏆 Động cơ Siêu Hợp Nhất 7 Động Cơ Mốc Lịch Sử (D-1) Strict PIT (QMBF v5): Kết hợp Positional Markov Tensor (1.9x) + Cầu Vị Trí GĐB & G1/G7 (0.2x) + Dạng Số & Chuyển Tiếp Tổng/Chạm/Bóng (0.2x) + Bayes Cặp Đầu-Đuôi (0.3x) + Lực hút Co-occurrence PMI (0.25x) + Sóng Động lượng Chu kỳ đàn hồi (0.3x) + Cặp Đảo & Bóng Âm Dương (0.15x) cùng Bộ lọc đa dạng hóa Đuôi số.`,
            `🎯 Dàn Song Thủ VIP Top 2 (${topPredictions.top2?.numbers?.join(' ')}): Vốn ${(topPredictions.top2?.stakeK || 4400)/1000}M/ngày, đạt tỷ lệ nổ ${(top2Summary.hitRate*100).toFixed(1)}% (${top2Summary.hitDays}/${top2Summary.days} ngày nổ), Thắng lãi ${(top2Summary.winRate*100).toFixed(1)}%, Tổng lãi thực tế +${(top2Summary.profitK/1000).toFixed(1)}M (ROI ${(top2Summary.roi*100).toFixed(1)}%).`,
    ...
    return {
        version: '2026.5.0-quantum-bayes-fusion-v5-strict-pit',
        description: 'Lô Siêu Hợp Nhất 7 Động Cơ Bayes & Markov 20 Năm (QMBF v5: 3-Order Positional Markov Tensor + Positional Bridge + Form & Parity Resonance + Bayes Head-Tail + Co-occurrence PMI + Elastic Momentum + Inverse/Shadow Matrix).',
        stakePerUnitK: LOTO_STAKE_PER_UNIT_K,
        payoutPerHitK: LOTO_PAYOUT_PER_HIT_K,
        latestRecommendation,
        settledLedger,

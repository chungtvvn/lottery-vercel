    ...
    ordered.forEach((run, index) => {
        ...
        RESEARCH_POLICIES.forEach(policy => {
            ...
            decisions.get(policy.id).push({
                ...
                numbers: normalizeNumbers(decision.numbers),
                abstained: Boolean(decision.abstained),
                actual,
                hit: decision.numbers.includes(actual),
                resolved: true,
                source: 'pit-replay-from-immutable-method-snapshots'
            });
        });
    });
    return {
...
function buildBestStrategyRecommendation({ researchReport, currentCandidates, methods, selectedModels }) {
    ...
    if (policies.length > 0 && validCandidates.length > 0) {
        const sorted = validCandidates.slice().sort((a, b) => {
            const pObjA = policies.find(p => p.id === a.id) || {};
            const pObjB = policies.find(p => p.id === b.id) || {};
            const polA = pObjA.overall || {};
            const polB = pObjB.overall || {};

            // Calculate live profit from 2026-08-28
            const liveRowsA = (pObjA.decisions || []).filter(r => r.date >= '2026-08-28' && r.resolved);
            const liveWinsA = liveRowsA.filter(r => r.hit).length;
            const liveProfitA = liveWinsA * 84000 - (liveRowsA.length * 30000);

            const liveRowsB = (pObjB.decisions || []).filter(r => r.date >= '2026-08-28' && r.resolved);
            const liveWinsB = liveRowsB.filter(r => r.hit).length;
            const liveProfitB = liveWinsB * 84000 - (liveRowsB.length * 30000);

            if (liveProfitB !== liveProfitA) {
                return liveProfitB - liveProfitA;
            }
...
function summarizeLedger(records) {
    const activeRecords = (records || []).filter(record => record.isLocked && record.settled !== false);
    const settledCount = activeRecords.length;
    const winCount = activeRecords.filter(record => record.hit).length;
    const last30 = activeRecords.slice(-30);
    const last14 = activeRecords.slice(-14);
    const last7 = activeRecords.slice(-7);
    const winRate = slice => slice.length ? slice.filter(record => record.hit).length / slice.length : 0;
    const sliceProfit = slice => {
        const wins = slice.filter(record => record.hit).length;
        return wins * PAYOUT_MULTIPLIER * STAKE_PER_NUMBER_K - slice.length * BET_COUNT * STAKE_PER_NUMBER_K;
    };
    ...
    return {
        ...
        overallProfitK: sliceProfit(activeRecords),
        breakEvenHitRate: BREAK_EVEN_HIT_RATE,
        unissuedDaysCount: (records || []).length - activeRecords.length,
        windows: {
            last7: { days: last7.length, wins: last7.filter(record => record.hit).length, hitRate: winRate(last7), profitK: sliceProfit(last7) },
            last14: { days: last14.length, wins: last14.filter(record => record.hit).length, hitRate: winRate(last14), profitK: sliceProfit(last14) },
            last30: { days: last30.length, wins: last30.filter(record => record.hit).length, hitRate: winRate(last30), profitK: sliceProfit(last30) }
        }
    };
}

// This is a research replay, not a live ledger.  Its selected method is
// calculated using only prior immutable method snapshots, but it was not a
// separately published prediction on the original draw date.
function buildReplayLedger(researchReport) {
    const policies = researchReport?.policies || [];
    if (!policies.length) return { kind: 'pit-replay', records: [], summary: {} };
    ...
    return {
        kind: 'pit-replay',
        note: 'Replay PIT: dàn được chọn lại từ các snapshot phương pháp immutable trước ngày dự đoán; không phải nhật ký dàn đã phát hành trực tiếp.',
        records,
        summary: summarizeLedger(records)
    };
}

function actualForAdvisorRecord(record) {
    const actual = Number(record?.actual);
    return Number.isInteger(actual) && actual >= 0 && actual < 100 ? actual : null;
}

function issuedMainSnapshot(record) {
    const main = record?.main || {};
    const numbers = normalizeNumbers(main.numbers);
    if (numbers.length !== BET_COUNT) return null;
    return {
        strategyId: 'legacy-main-issued-snapshot',
        label: 'Dàn chính đã phát hành',
        methodId: main.methodId || record?.recommendation?.selected?.methodId || '',
        methodLabel: main.label || record?.recommendation?.selected?.label || METHOD_LABELS[main.methodId] || main.methodId || '',
        numbers,
        abstained: false,
        source: 'legacy-main'
    };
...
// The immutable strategy lane is the live automatic daily selector.  Do not
// substitute a replayed policy here: this ledger is the user-facing audit of
// what was actually issued before the draw.
function buildLiveSelectorLedger(advisorCache) {
    const rawRecords = (advisorCache?.records || [])
        .filter(record => record?.predictionDate && record?.lifecycle?.immutableNumbers)
        .slice()
        .sort((left, right) => isoDate(left.predictionDate).localeCompare(isoDate(right.predictionDate)));
    ...
    const records = rawRecords.map(record => {
        const strategy = (record.strategySnapshots || []).find(item => item.strategyId === MAIN_STRATEGY_ID)
            || issuedMainSnapshot(record);
        const numbers = normalizeNumbers(strategy?.numbers);
        const isLocked = Boolean(strategy) && !strategy.abstained && numbers.length === BET_COUNT;
        const actual = actualForAdvisorRecord(record);
        const settled = Boolean(record.settled) && actual !== null;
        const hit = isLocked && settled ? numbers.includes(actual) : null;
        let cumulativeHitRate = null;
        let profitK = null;
        if (isLocked && settled) {
            settledIndex += 1;
            if (hit) cumulativeWins += 1;
            cumulativeHitRate = Number((cumulativeWins / settledIndex).toFixed(4));
            profitK = cumulativeWins * PAYOUT_MULTIPLIER * STAKE_PER_NUMBER_K
                - settledIndex * BET_COUNT * STAKE_PER_NUMBER_K;
        }
        return {
            date: isoDate(record.predictionDate),
            actual,
            policyId: MAIN_STRATEGY_ID,
            policyLabel: 'Bộ chọn tự động hằng ngày',
            methodId: strategy?.methodId || record?.recommendation?.selected?.methodId || '',
            methodLabel: strategy?.methodLabel || record?.recommendation?.selected?.label || '',
            numbers: isLocked ? numbers : [],
            isLocked,
            ...
            source: strategy?.source || 'strategy-snapshot'
        };
    });

    return {
        kind: 'live-issued',
        strategyId: MAIN_STRATEGY_ID,
        title: 'Nhật ký snapshot thực tế của bộ chọn tự động',
        note: 'Mỗi dàn chỉ được lấy từ snapshot đã khóa trước giờ quay. Khi có kết quả, hệ thống chỉ đối soát trúng/trượt; không thay đổi dàn cũ.',
        records,
        summary: summarizeLedger(records)
    };
}

function buildAdvisorAnalysis(options = {}) {
    const { advisorCache, probabilityCache, longHorizonCache, history, rawData } = options;
    ...
    const liveSelectorLedger = buildLiveSelectorLedger(advisorCache);
    const replayLedger = buildReplayLedger(researchReport);

    // Tính toán Hệ Thống 10 Tầng (Ten-Tier Arithmetic & AI Lottery Research)
    let tenTierResearch = null;
    try {
        const { compute10TierResearch } = require('./tenTierLotteryResearchService');
        let rawHistory = rawData || (() => {
            try {
                const fs = require('fs');
                const path = require('path');
                const p = path.join(process.cwd(), 'lib', 'data', 'xsmb-2-digits.json');
                if (fs.existsSync(p)) return JSON.parse(fs.readFileSync(p, 'utf8'));
            } catch (_) {}
            return null;
        })();

        if (Array.isArray(rawHistory) && advisorCache?.drawPrizesByDate) {
            const existingDates = new Set(rawHistory.map(d => d.date));
            for (const [d, p] of Object.entries(advisorCache.drawPrizesByDate)) {
                if (!existingDates.has(d) && p && p.special !== undefined && p.special !== null) {
                    rawHistory.push({ date: d, special: Number(p.special) });
                    existingDates.add(d);
                }
            }
        ...
        if (Array.isArray(rawHistory) && rawHistory.length >= 30) {
            tenTierResearch = compute10TierResearch(rawHistory, targetDate);
        }
    } catch (err) {
        console.warn('[AdvisorAnalysis] tenTierResearch compute warning:', err.message);
        tenTierResearch = null;
    }

    return {
        version: 'advisor-analysis-v7',
        ...
        tenTierResearch,
        metaLearner: advisorCache?.metaLearner || null,
        adaptiveDualMerge: advisorCache?.adaptiveDualMerge || null,
        dualMerge: advisorCache?.dualMerge || null,
        tripleMerge: advisorCache?.tripleMerge || null,
        // Keep the old name as a compatibility alias for clients deployed
        // before v7.  It now intentionally represents only live snapshots.
        settledLedger: liveSelectorLedger,
        liveSelectorLedger,
        replayLedger,
        scoreOverlay,

...
function buildSettledLedger(researchReport) {
    ...
    const decisions = (primaryPolicy.decisions || []).filter(d => d.resolved);

    let cumulativeWins = 0;
    const records = decisions.map((d, index) => {
        if (d.hit) cumulativeWins += 1;
        const totalSettled = index + 1;
        const hitRate = cumulativeWins / totalSettled;
        const profitK = cumulativeWins * 84 * 1000 - totalSettled * 30 * 1000;
        return {
            date: d.date,
            actual: d.actual,
            ...
            hit: Boolean(d.hit),
            profitK,
            cumulativeHitRate: Number(hitRate.toFixed(4)),
            settledIndex: totalSettled
        };
    });

    const settledCount = records.length;
    const winCount = records.filter(r => r.hit).length;

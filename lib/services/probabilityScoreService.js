...
const HISTORICAL_ANALYSIS_VERSION = 'probability-score-history-v1';
...
function buildScoreSnapshot({ rawRows, predictionDate, historyRun, history = [], createdAt = new Date().toISOString() }) {
    ...
    return {
        ...
        recommendation: {
            confidence: Math.max(1.0, Math.min(5.0, Number((2.5 + (calibration.eligible ? 1.0 : 0) + (hasReliableConsensus ? 0.8 : 0) + (selectedGroups.length >= 2 ? 0.7 : 0)).toFixed(1)))),
            plainReasons: [
                'Dàn số được chọn lọc theo Mô hình Điểm Xác Suất Chuỗi (Score Online) kết hợp Calibration 180 kỳ.',
                'Tích hợp điểm kỳ vọng EV+ dương và Hazard function thời gian chờ (gap deficit) khử nhiễu.',
                'Phân tầng tự động gồm Top 10 VIP EV+, Top 20 tiềm năng, Dàn 30 chuẩn và Dàn 36 mở rộng.'
            ],
            tiers: {
                core10: rankedNumbers.slice(0, 10).map(r => r.number).sort((a, b) => a - b),
                core20: rankedNumbers.slice(0, 20).map(r => r.number).sort((a, b) => a - b),
                standard30: rankedNumbers.slice(0, 30).map(r => r.number).sort((a, b) => a - b),
                expanded36: rankedNumbers.slice(0, 36).map(r => r.number).sort((a, b) => a - b)
            }
        },
...
function settleSnapshot(snapshot, rawRows) {
    ...
    const recTiers = snapshot.recommendation?.tiers;
    const core10 = recTiers?.core10 || (snapshot.topNumbers || []).slice(0, 10).map(r => r.number);
    const core20 = recTiers?.core20 || (snapshot.topNumbers || []).slice(0, 20).map(r => r.number);
    const standard30 = recTiers?.standard30 || (snapshot.topNumbers || []).slice(0, 30).map(r => r.number);
    const expanded36 = recTiers?.expanded36 || (snapshot.rankedNumbers || []).slice(0, 36).map(r => r.number);
    ...
    return {
        ...
        top10Hit: core10.includes(actual),
        top20Hit: core20.includes(actual),
        top30Hit: standard30.includes(actual),
        expanded36Hit: expanded36.includes(actual)
    };
}
...
function summarize(records) {
    ...
    const top10Wins = settled.filter(record => record.top10Hit || (record.recommendation?.tiers?.core10 || record.topNumbers?.slice(0, 10).map(r => r.number) || []).includes(record.actual)).length;
    const top20Wins = settled.filter(record => record.top20Hit || (record.recommendation?.tiers?.core20 || record.topNumbers?.slice(0, 20).map(r => r.number) || []).includes(record.actual)).length;
    const top30Wins = settled.filter(record => record.top30Hit || record.hit).length;
    const expanded36Wins = settled.filter(record => record.expanded36Hit || (record.recommendation?.tiers?.expanded36 || record.rankedNumbers?.slice(0, 36).map(r => r.number) || []).includes(record.actual)).length;

    const stakeK = settled.length * BET_COUNT * 1000;
    const profitK = wins * 84 * 1000 - stakeK;
    const last30 = settled.slice(-30);
    const last14 = settled.slice(-14);
    const last7 = settled.slice(-7);

    const hitRateOf = (slice) => slice.length ? slice.filter(r => r.hit).length / slice.length : 0;
    const profitOf = (slice) => {
        const w = slice.filter(r => r.hit).length;
        return w * 84 * 1000 - slice.length * BET_COUNT * 1000;
    };
    ...
    return {
        ...
        top10HitRate: settled.length ? top10Wins / settled.length : 0,
        top20HitRate: settled.length ? top20Wins / settled.length : 0,
        top30HitRate: settled.length ? top30Wins / settled.length : 0,
        expanded36HitRate: settled.length ? expanded36Wins / settled.length : 0,
        stakeK,
        profitK,
        ...
        windows: {
            last7: { days: last7.length, wins: last7.filter(r => r.hit).length, hitRate: hitRateOf(last7), profitK: profitOf(last7) },
            last14: { days: last14.length, wins: last14.filter(r => r.hit).length, hitRate: hitRateOf(last14), profitK: profitOf(last14) },
            last30: { days: last30.length, wins: last30.filter(r => r.hit).length, hitRate: hitRateOf(last30), profitK: profitOf(last30) }
        }
    };
}

function summarizeHistoricalRows(entries) {
    const rows = (entries || []).slice().sort((left, right) => left.date.localeCompare(right.date));
    ...
    const hitRate = rows.length ? wins / rows.length : 0;
    return {
        days: rows.length,
        wins,
...
function groupHistoricalRows(entries, keyBuilder) {
    ...
    return [...grouped.entries()].map(([period, rows]) => ({ period, ...summarizeHistoricalRows(rows) }))
        .sort((left, right) => left.period.localeCompare(right.period));
}

function buildHistoricalAnalysis(rawRows, { generatedAt = new Date().toISOString(), recentLimit = 180 } = {}) {
    const rows = readRawRows(rawRows);
    if (rows.length < MODEL_DEFAULTS.minWarmup) {
        throw new Error('Không đủ raw data để phân tích lịch sử Score v2.');
    }
    const report = runStrictWalkForward(rows, {
        catalog: getGroupCatalog(),
        betCount: BET_COUNT
    });
    const entries = report.rows || [];
    const windowSummary = size => summarizeHistoricalRows(entries.slice(-size));
    return {
        ...
        source: {
            dataStart: rows[0]?.date || null,
            dataEnd: rows.at(-1)?.date || null,
            rawRows: rows.length,
            evaluatedStart: entries[0]?.date || null,
            evaluatedEnd: entries.at(-1)?.date || null,
            evaluatedDays: entries.length,
            trainingRule: 'Mỗi ngày D chỉ dùng toàn bộ raw từ đầu dữ liệu đến D-1; trọng số online được cập nhật sau khi D có kết quả.'
        },
        economics: {
            betCount: BET_COUNT,
        ...
        summary: summarizeHistoricalRows(entries),
        windows: {
            last30: windowSummary(30),
            last90: windowSummary(90),
            last365: windowSummary(365)
        },
        yearly: groupHistoricalRows(entries, row => row.date.slice(0, 4)),
        monthly: groupHistoricalRows(entries, row => row.date.slice(0, 7)).slice(-36),
        recentRows: entries.slice(-recentLimit).map(row => ({
            date: row.date,
            actual: row.actual,
            hit: Boolean(row.hit),
            topMass: row.topMass,
            cutoffMargin: row.cutoffMargin,
            meanRelativeConfidence: row.meanRelativeConfidence
        })),
        warning: 'Đây là kiểm định walk-forward của lõi Score v2 trên toàn bộ lịch sử, không phải nhật ký dự đoán thực tế và không được dùng để viết lại snapshot đã phát hành.'
    };
}
...
async function generateAndWriteCache({ raw, history, existing, limit = 90, write = true } = {}) {
    ...
    const previousHistoricalAnalysis = !Array.isArray(existing) ? existing?.historicalAnalysis : null;
    const historicalAnalysis = previousHistoricalAnalysis?.version === HISTORICAL_ANALYSIS_VERSION
        && previousHistoricalAnalysis?.source?.dataEnd === rows.at(-1).date
        ? previousHistoricalAnalysis
        : buildHistoricalAnalysis(rows, { generatedAt });
    const payload = {

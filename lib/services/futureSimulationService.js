...
class FutureSimulationService {
    ...
    exclusionMethod(historicalData) {
        ...
        // ===== BƯỚC 6: Tính số đánh (không cap cố định) =====
        // Số đánh = 100 - số loại trừ (tự nhiên từ streak analysis)
        const toBet = [];
        for (let i = 0; i < 100; i++) {
    ...
    exclusionByRecordMethod(data) {
        ...
        // MAX_BET removed: betting pool determined by ≥85% drop-off threshold
        ...
        // Không giới hạn cố định — pool size phụ thuộc hoàn toàn vào risk model
        return {
            toBet: toBet4,               // Exclusion
            toBetPlus: toBet3,           // Exclusion+
            excluded: Array.from(excluded4),
            excludedPlus: Array.from(excluded3),
            skipped: toBet4.length === 0,
            skippedPlus: toBet3.length === 0
        };
    }

    /**
     * Phương pháp loại trừ dựa trên Drop-off Rate chuỗi (cumulative gapStats)
     * Mỗi số 00-99 tích điểm loại trừ = tổng tỷ lệ gãy từ các dạng chứa nó
     * Loại ~70 số có điểm cao nhất, đánh ~30 số còn lại
     */
    streakDropOffExclusion(data) {
        const MIN_DROPOFF = 0.85; // Threshold ≥85% drop-off rate

        const categories = [];
        for (let v = 0; v <= 9; v++) {
            categories.push({ checkFn: n => Math.floor(n/10) === v, getNumbers: () => this.getNumbersForCategory('dau', v), name: `Đầu ${v}` });
            categories.push({ checkFn: n => n % 10 === v, getNumbers: () => this.getNumbersForCategory('dit', v), name: `Đít ${v}` });
            categories.push({ checkFn: n => this.getHieu(n) === v, getNumbers: () => this.getNumbersForCategory('hieu', v), name: `Hiệu ${v}` });
        }
        for (let v = 1; v <= 10; v++) {
            categories.push({ checkFn: n => this.getTongTT(n) === v, getNumbers: () => this.getNumbersForCategory('tong', v), name: `Tổng ${v}` });
        }
        categories.push({ checkFn: n => n % 2 === 0, getNumbers: () => Array.from({length:50}, (_,i) => i*2), name: 'Chẵn' });
        categories.push({ checkFn: n => n % 2 === 1, getNumbers: () => Array.from({length:50}, (_,i) => i*2+1), name: 'Lẻ' });
        categories.push({ checkFn: n => Math.floor(n/10) % 2 === 0, getNumbers: () => Array.from({length:50}, (_,i) => { const d = [0,2,4,6,8]; return d[Math.floor(i/10)]*10 + (i%10); }), name: 'Đầu chẵn' });
        categories.push({ checkFn: n => Math.floor(n/10) % 2 === 1, getNumbers: () => Array.from({length:50}, (_,i) => { const d = [1,3,5,7,9]; return d[Math.floor(i/10)]*10 + (i%10); }), name: 'Đầu lẻ' });

        // Compute drop-off for each category with active streak
        const streakResults = [];
        for (const cat of categories) {
            const currentStreak = this.calculateStreakFromSpecial(data, cat.checkFn);
            if (currentStreak === 0) continue;

            // Build historical streak distribution
            const allLens = [];
            let i = 0;
            while (i < data.length) {
                const sp = this.getSpecialNumber(data[i]);
                if (sp !== null && cat.checkFn(sp)) {
                    let len = 1;
                    while (i + len < data.length) {
                        const spN = this.getSpecialNumber(data[i + len]);
                        if (spN !== null && cat.checkFn(spN)) len++; else break;
                    }
                    allLens.push(len);
                    i += len;
                } else { i++; }
            ...
            const countGE = (L) => allLens.filter(s => s >= L).length;
            const curCount = countGE(currentStreak);
            const nextCount = countGE(currentStreak + 1);
            const dropOff = curCount > 0 ? 1 - (nextCount / curCount) : 1;

            streakResults.push({ name: cat.name, streak: currentStreak, dropOff, curCount, nextCount, nums: cat.getNumbers() });
        }

        // Exclude numbers from categories with drop-off rate ≥ 85%
        const excluded = new Set();
        for (const sr of streakResults) {
            if (sr.dropOff >= MIN_DROPOFF) {
                sr.nums.forEach(n => excluded.add(n));
            }
        }

        const toBet = [];
        for (let i = 0; i < 100; i++) { if (!excluded.has(i)) toBet.push(i); }

        // Build scores for debugging/display
        const scores = new Map();
        for (let n = 0; n < 100; n++) scores.set(n, 0);
        for (const sr of streakResults) {
            if (sr.dropOff > 0) {
                sr.nums.forEach(n => scores.set(n, scores.get(n) + sr.dropOff));
            }
        }
        ...
        return {
            toBet,
            excluded: Array.from(excluded),
            skipped: toBet.length === 0,
            scores: sorted // for debugging/display
        };
    }

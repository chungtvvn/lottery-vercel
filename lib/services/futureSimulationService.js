...
class FutureSimulationService {
    ...
    streakDropOffExclusion(data) {
        ...
        // Score each number: sum of drop-off rates from all covering categories
        ...
        // Sort by score desc, exclude top ~70
        const sorted = Array.from(scores.entries()).sort((a, b) => b[1] - a[1]);
        const excluded = new Set();
        for (let i = 0; i < Math.min(TARGET_EXCLUDE, sorted.length); i++) {
            if (sorted[i][1] > 0) excluded.add(sorted[i][0]);
        }
        ...
        return {
            toBet,
            excluded: Array.from(excluded),
            skipped: toBet.length > MAX_BET || toBet.length === 0,

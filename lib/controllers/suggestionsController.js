...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        for (const key in quickStats) {
            ...
            // --- RECORD DETECTION DỰA VÀO LỊCH SỬ ---
            const originalRecord = stat.longest && stat.longest.length > 0 ? stat.longest[0].length : 0;
            const hasReachedOriginalRecord = originalRecord > 0 && currentLen >= originalRecord;
            const isApproachingOriginalRecord = originalRecord > 0 && targetLen >= originalRecord && currentLen < originalRecord;
            ...
            const currentGapInfoExact = stat.exactGapStats ? stat.exactGapStats[currentLen] : null;
            const currentCount = currentGapInfoExact ? currentGapInfoExact.count : 0;
            const currentFreqYear = currentCount / totalYears;
            const isSuper = targetFreqYear <= 0.5;
            const isCurrentSuper = currentFreqYear <= 0.5;

            if (hasReachedOriginalRecord || isApproachingOriginalRecord) {
                shouldExclude = true;

                if (hasReachedOriginalRecord) {
                    // Đã chạm hoặc vượt mốc lớn nhất lịch sử
                    if (isCurrentSuper) {
                        tier = 'purple';
                        subTier = 'achievedSuper';
                        reason = `[TÍM ĐẬM] Chuỗi hiện tại (${currentLen} ngày) ĐÃ ĐẠT SIÊU KỶ LỤC (${currentFreqYear.toFixed(2)} lần/năm). Cực kỳ hiếm!`;
                    } else {
                        tier = 'red';
                        subTier = 'achieved';
                        reason = `[Đỏ] Chuỗi hiện tại (${currentLen} ngày) ĐÃ đạt Kỷ Lục (${currentFreqYear.toFixed(2)} lần/năm). Xác suất tiếp tục gần như bằng 0.`;
                    }
                } else {
                    // Chạm mốc lịch sử ở chu kỳ tiếp theo
                    if (isSuper) {
                        tier = 'purple';
                ...
                // --- XÁC SUẤT ĐI TIẾP (CONTINUATION RATE) ---
                // Áp dụng cho các chuỗi chưa đạt kỷ lục lịch sử nhưng có nguy cơ gãy cực cao
                if (currentCount >= 10 && targetCount >= 0) {
                    const continuationRate = targetCount / currentCount;
                    const dropOffRate = 1 - continuationRate;
                    
                    // Nếu tỷ lệ rụng (gãy chuỗi) lớn hơn 90%
                    if (dropOffRate >= 0.90) {
                        shouldExclude = true;
                        tier = 'orange'; // Nhóm cảnh báo rủi ro mới
                        subTier = 'highRisk';
                        reason = `[CAM] RỦI RO GÃY CAO: Lịch sử ${currentCount} lần đạt ${currentLen} ngày chỉ có ${(continuationRate*100).toFixed(1)}% đi tiếp được. Tỷ lệ gãy ${(dropOffRate*100).toFixed(1)}%!`;
                        reason = formatSequence(reason);
                    }
                }
            }
            ...
            // Only add RED, PURPLE and ORANGE tier patterns immediately 
            if (shouldExclude && (tier === 'red' || tier === 'purple' || tier === 'orange')) {

...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        for (const key in quickStats) {
            ...
            if (currentCount > 0) {
                continuationRate = targetCount / currentCount;
            ...
            if (dropOffRate >= 0.70) {
                ...
                if (dropOffRate >= 0.90) {
                    ...
                    if (currentCount === 0) {
                        ...
                        reason = `[TÍM] TỶ LỆ GÃY CỰC CAO: ${(dropOffRate*100).toFixed(1)}% (Lịch sử ${currentCount} lần đạt ${currentLen} ngày chỉ có ${targetCount} lần đi tiếp). Rủi ro rất lớn!`;
                    }
                } else {
                    tier = 'red';
                    subTier = 'highRisk';
                    reason = `[ĐỎ] TỶ LỆ GÃY CAO: ${(dropOffRate*100).toFixed(1)}% (Lịch sử ${currentCount} lần đạt ${currentLen} ngày chỉ có ${targetCount} lần đi tiếp). Cần loại bỏ!`;
                }

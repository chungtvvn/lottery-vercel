...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        for (const key in quickStats) {
            ...
            // --- PURELY FREQUENCY-BASED RECORD DETECTION ---
            // targetFreqYear = frequency of targetLen (next day) per year
            // currentFreqYear = frequency of currentLen per year (to check if already at record)
            ...
            if (targetFreqYear <= 1.5) {
                shouldExclude = true;

                // "Achieved" = currentLen itself is already at record frequency (<= 1.5/year)
                // "Threshold" = currentLen is still common, but next day (targetLen) would be record
                if (currentFreqYear <= 1.5) {
                    // Already AT a record-frequency length
                    if (isCurrentSuper) {
                        tier = 'purple';
                    ...
                    // Next day would reach Super Record frequency
                    tier = 'purple';
                    ...
                    // Next day would reach Record frequency
                    tier = 'red';
                    subTier = 'threshold';

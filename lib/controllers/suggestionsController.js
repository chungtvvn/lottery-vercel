...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        // Mục tiêu: tích lũy đến khi đạt ≥70 số loại trừ
        // Mỗi chuỗi (pattern) đóng góp TOÀN BỘ số của nó (không giới hạn per-pattern)
        const TARGET_EXCLUDED_NUMBERS = 70;

        for (const candidate of exclusionCandidates) {
            const targetReached = excludedNumbers.size >= TARGET_EXCLUDED_NUMBERS;
            
            if (targetReached) {
                // Đã đạt mục tiêu: hiển thị trong explanations nhưng KHÔNG thêm vào excludedNumbers
                const infoReason = candidate.reason.replace(/^\[(ĐỎ|TÍM|TÍM ĐẬM)\]/, '[THAM KHẢO]');
                addExcludedNumber(candidate.stat, candidate.key, infoReason, candidate.tier, candidate.subTier, true);
            } else {
                // Chưa đạt mục tiêu: thêm TOÀN BỘ số của chuỗi này vào danh sách loại trừ

...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        for (const key in quickStats) {
            ...
            // soLeTheoCap IS a so-le pattern (alternating pairs) — needs step=2
            const soLeSubcats = ['vesole', 'vesolemoi', 'soletheocamp', 'soletheccap', 'soletheccap'];
            const isSoLePattern = (
                (subcategory && (subcategory.toLowerCase() === 'vesole' || subcategory.toLowerCase() === 'vesolemoi' || subcategory.toLowerCase() === 'soletheocamp' || subcategory === 'soLeTheoCap')) ||
                key.includes('SoLeTheoCap') || key.includes('soLeTheoCap')
        ...
        function getCategoryName(category, subcategory, originalKey = null) {
            ...
            if (originalKey) {
                const directMapping = {
                    ...
                    // So le theo cặp
                    'cacDauSoLeTheoCap': 'Các Đầu - So Le Theo Cặp',
                    'cacDitSoLeTheoCap': 'Các Đít - So Le Theo Cặp'
                };
            ...
            if (subcategory) {
                ...
                if (subcategory === 'soLeTheoCap') return `${catName} - So Le Theo Cặp`;
        ...
        function addExcludedNumber(stat, key, reason, tier = 'red', subTier = null, explanationOnly = false, maxNumsPerPattern = Infinity) {
            ...
            // NẾU KHÔNG CÓ TRONG CACHE HOẶC LÀ TỪ [TIỀM NĂNG], TÍNH TOÁN LẠI:
                ...
                // Xử lý các dạng về liên tiếp - cùng số
                    ...
                    // === CHIẾN LƯỢC: getNumbersFromCategory TRƯỚC, manual parsing SAU ===
                    // Đảm bảo hỗ trợ tất cả dạng: đơn (dau_2), 3D (dau_3d_2_4_9), tổng, hiệu, v.v.
                    if (category.startsWith('dau_') || category.startsWith('dit_') ||
                        category.startsWith('tong_tt_') || category.startsWith('tong_moi_') || category.startsWith('hieu_')) {
                        // Ưu tiên 1: Tra cứu trực tiếp qua getNumbersFromCategory (hỗ trợ cả đơn lẫn 3D)
                        let specificSet = getNumbersFromCategory(category);

                        if (specificSet && specificSet.length > 0) {
                            nums = specificSet;
                        } else {
                            // Ưu tiên 2: Tính toán từ current value (cho các generic category)
                            let lastNumber = null;
                            ...
                            if (lastNumber !== null && lastNumber.length === 2) {
                                ...
                                if (category.startsWith('dau_')) {
                                    // Fallback cho dau_X đơn: dùng đầu của số cuối
                                    tempCategory = `dau_${String(lastNumber)[0]}`;
                                } else if (category.startsWith('dit_')) {
                                    // Fallback cho dit_X đơn: dùng đít của số cuối
                                    tempCategory = `dit_${String(lastNumber)[1]}`;
                                } else if (category.startsWith('tong_tt_')) {
                                    let calculatedValue = digit1 + digit2;
                                    if (calculatedValue === 0) calculatedValue = 10;
                                    tempCategory = `tong_tt_${calculatedValue}`;
                                } else if (category.startsWith('tong_moi_')) {
                                    tempCategory = `tong_moi_${digit1 + digit2}`;
                                } else if (category.startsWith('hieu_')) {
                                    tempCategory = `hieu_${Math.abs(digit1 - digit2)}`;
                                }
                        ...
                        // Fallback cuối: dùng values trực tiếp nếu không tìm được set
                        ...
                        // 1 Đầu về liên tiếp -> lấy số cuối cùng trong chuỗi để xác định đầu
                        let lastVal = null;
                        if (stat.current.values && stat.current.values.length > 0) {
                            lastVal = stat.current.values[stat.current.values.length - 1];
                        } else if (stat.current.value && stat.current.value !== 'Theo dạng') {
                            lastVal = stat.current.value;
                        }
                    ...
                    } else if (category === 'motDit' || category === 'cacDit') {
                        // 1 Đít về liên tiếp -> lấy số cuối cùng trong chuỗi để xác định đít
                        let lastVal = null;
                        if (stat.current.values && stat.current.values.length > 0) {
                            lastVal = stat.current.values[stat.current.values.length - 1];
                        } else if (stat.current.value && stat.current.value !== 'Theo dạng') {
        ...
        // Kiểm tra các pattern chưa có chuỗi hiện tại nhưng số mới nhất có thể bắt đầu
        // một chuỗi hướng tới mốc kỷ lục (computedMaxStreak). Sử dụng tần suất thay vì
        // so sánh với record cố định.
        ...
        if (recentResults && recentResults.length > 0) {
            ...
            for (const category of latestCategories) {
                for (const subcategory of subcategoriesToCheck) {
                    ...
                    if (freqRecordLen < 2) continue; // Cần ít nhất mốc kỷ lục = 2 để có ý nghĩa
...
function predictNextInSequence(stat, category, subcategory, isHistory = false) {
    ...
    const isSoLeTheoCap = subCatStr.toLowerCase().includes('soletheocamp') || subCatStr.toLowerCase() === 'soletheocamp' || category.toLowerCase().includes('soletheocamp');
    ...
    // Pattern: A, B, A, B, A... → dự đoán giá trị tiếp theo (nếu vị trí chẵn → A, lẻ → B)
    if (isSoLeTheoCap) {
        if (stat.current && stat.current.values && stat.current.values.length >= 2) {
            const values = stat.current.values;
            const currentLen = values.length;
            // Values alternate: A, B, A, B, A...
            // Position 0=A, 1=B, 2=A, 3=B...
            // Next position = currentLen: even → predict A, odd → predict B
            const nextIsA = (currentLen % 2 === 0);
            // A = values[0], B = values[1]
            const predictedValue = nextIsA ? values[0] : values[1];
            const predictedStr = String(predictedValue).padStart(2, '0');

            // Map predicted value back to numbers based on category type
            let baseNums = getNumbersFromCategory(category);
            if (!baseNums || baseNums.length === 0 || baseNums.length >= 100) {
                baseNums = Array.from({ length: 100 }, (_, i) => i);
            }

            // Use extractValueForComparison to filter numbers matching the predicted value
            const { extractValueForComparison } = require('../utils/numberAnalysis');
            const predictedInt = parseInt(predictedStr, 10);
            const predictedExtracted = extractValueForComparison(predictedStr, category);

            if (predictedExtracted !== null) {
                // Filter baseNums to only those whose extracted value matches the predicted
                const filtered = baseNums.filter(n => {
                    const nStr = String(n).padStart(2, '0');
                    const nExtracted = extractValueForComparison(nStr, category);
                    return String(nExtracted) === String(predictedExtracted);
                });
            ...
            // Fallback: return the specific number
            if (!isNaN(predictedInt) && predictedInt >= 0 && predictedInt <= 99) {
                return [predictedInt];
            }
        }
        // Fallback: return category numbers
    ...
    if (subcategory === 'soLeTheoCap' || subcategory === 'cacDauSoLeTheoCap' || subcategory === 'cacDitSoLeTheoCap') {

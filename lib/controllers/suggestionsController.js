...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        for (const key in quickStats) {
            ...
            // Bỏ qua nếu không đủ mẫu thống kê (cần ít nhất 5 lần để có ý nghĩa)
            const MIN_SAMPLES = 5;
            if (currentCount < MIN_SAMPLES) continue;
            
            // TÍNH TOÁN LOOK-AHEAD ĐẾN KỶ LỤC
            ...
            // Ngưỡng tối thiểu: 70% drop-off
            const MIN_DROPOFF = 0.70;
        ...
        // === PASS 2: SẮP XẾP VÀ CHỌN LỌC THEO ĐỘ ƯU TIÊN ===
        // Compute pattern width (how many numbers each pattern covers) for penalty
        const getPatternWidth = (candidate) => {
            const key = candidate.key;
            const parts = key.replace(/^\[TIỀM NĂNG\] /, '').split(':');
            const cat = parts[0];
            const nums = getNumbersFromCategory(cat);
            return nums ? nums.length : 0;
        };

        // Score each candidate: higher = more confident, should be excluded first
        // Formula: dropOffRate * streakLengthBonus * widthPenalty
        const scoreCandidate = (c) => {
            const width = getPatternWidth(c);
            // Penalty for broad patterns: patterns covering >25 numbers get penalized
            // e.g., dit_nho (50 nums) → penalty = 25/50 = 0.5
            // e.g., dau_4 (10 nums) → penalty = 1.0 (no penalty)
            const widthPenalty = width > 25 ? 25 / width : 1.0;
            // Bonus for longer streaks (more statistically significant)
            const streakBonus = 1 + (c.currentLen || 0) * 0.1;
            // Bonus for more historical samples
            const sampleBonus = Math.min(1 + (c.currentCount || 0) * 0.01, 1.5);
            return c.dropOffRate * streakBonus * sampleBonus * widthPenalty;
        };
        ...
        exclusionCandidates.sort((a, b) => {
            ...
            // Tiebreaker: prefer narrower patterns
            return getPatternWidth(a) - getPatternWidth(b);
        });

        // Giới hạn số lượng loại trừ tối đa: giữ ở mức 40 để còn 60 số đánh
        const MAX_EXCLUDED_NUMBERS = 40;
        // Giới hạn số mỗi pattern: tránh 1 pattern rộng chiếm hết quota
        const MAX_NUMS_PER_PATTERN = 10;

        for (const candidate of exclusionCandidates) {
            const capReached = excludedNumbers.size >= MAX_EXCLUDED_NUMBERS;
            
            if (capReached) {
                // Vượt quá giới hạn: vẫn hiển thị trong explanations nhưng KHÔNG thêm vào excludedNumbers
                ...
                // Giới hạn số lượng số mỗi pattern: pattern quá rộng (>20 số) sẽ bị cắt
                // để tránh 1 pattern chiếm hết quota loại trừ
                addExcludedNumber(candidate.stat, candidate.key, candidate.reason, candidate.tier, candidate.subTier, false, MAX_NUMS_PER_PATTERN);
            }
        }

...
async function computeSuggestions(queryParams = {}) {
    try {
        ...
        for (const key in quickStats) {
            ...
            if (currentCount > 0) {
                if (checkTargetCount === 0 && recordLen > 0 && currentLen >= recordLen) {
                    // Bayesian Laplace regularization at record boundary:
                    // Prevents assuming 100% drop-off blindly on rare record extensions
                    const priorWeight = 2.0;
                    const baseBreakRate = 0.86;
                    dropOffRate = (currentCount + priorWeight * baseBreakRate) / (currentCount + priorWeight);
                    continuationRate = 1 - dropOffRate;
                } else {
                    continuationRate = checkTargetCount / currentCount;
                    dropOffRate = 1 - continuationRate;
                }
            } else {
                // Chưa bao giờ đạt được độ dài này trong lịch sử => Ước lượng thận trọng 88%
                dropOffRate = 0.88;
            }
            ...
            if (dropOffRate >= MIN_DROPOFF) {
                ...
                if (isEligibleSoLeTheoCap) {
                    ...
                    if (checkTargetCount === 0) {
                        reason = `[TÍM ĐẬM] RỦI RO GÃY CAO: ${(dropOffRate*100).toFixed(1)}% (Chuỗi ${currentLen} ngày đạt mốc kỷ lục lịch sử, xác suất gãy cao theo Bayes).`;
                    } else {
        ...
        // Sắp xếp theo tỷ lệ gãy thực tế và độ dài chuỗi (không ưu tiên mù dạng chưa hình thành)
...
function predictNextInSequence(stat, category, subcategory, isHistory = false) {
    ...
    const blockAlternationPattern = BLOCK_ALTERNATION_PATTERNS.find(pattern => pattern.subcategory === subCatStr);
    ...
    const isSoLe = (subCatStr.toLowerCase().includes('sole') || category.toLowerCase().includes('sole')) && !isSoLeTheoCap && !blockAlternationPattern; // veSole, veSoleMoi (exclude soLeTheoCap/block)
    ...
    if (blockAlternationPattern) {
        const currentLen = Number(stat.current.length || stat.current.values?.length || 0);
        if (!Number.isFinite(currentLen) || currentLen <= 0) return [];
        const cycleLength = blockAlternationPattern.aLength + blockAlternationPattern.bLength;
        const nextOffset = currentLen % cycleLength;
        const expectsCategory = nextOffset < blockAlternationPattern.aLength;
        const categoryNumbers = getNumbersFromCategory(category).map(n => parseInt(n, 10));
        if (expectsCategory) return categoryNumbers;
        const categorySet = new Set(categoryNumbers);
        return Array.from({ length: 100 }, (_, i) => i).filter(num => !categorySet.has(num));
    }

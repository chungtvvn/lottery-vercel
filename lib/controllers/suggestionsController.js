...
exports.getSuggestions = async (req, res) => {
    try {
        ...
        for (const key in quickStats) {
            ...
            // --- NEW LOGIC: DỰA HOÀN TOÀN VÀO TỶ LỆ GÃY CHUỖI (dùng gapStats >= counts) ---
            // gapStats[L].count = số chuỗi >= L ngày → drop-off = 1 - (gapStats[L+1] / gapStats[L])
            // Ý nghĩa: trong tất cả chuỗi đạt >= L ngày, bao nhiêu % dừng lại tại L
            const totalYears = lotteryService.getTotalYears();
            const currentGapInfoGE = stat.gapStats ? stat.gapStats[currentLen] : null;
            const currentCount = currentGapInfoGE ? currentGapInfoGE.count : 0;
            ...
            const checkTargetGapInfoGE = stat.gapStats ? stat.gapStats[checkTargetLen] : null;
            const checkTargetCount = checkTargetGapInfoGE ? checkTargetGapInfoGE.count : 0;
            const targetFreqYear = checkTargetCount / totalYears;
            ...
            if (dropOffRate >= 0.70) {
                ...
                const lenStr = checkTargetLen === targetLen ? `${targetLen} ngày` : `kỷ lục ${checkTargetLen} ngày`;
                ...
                if (dropOffRate >= 0.90) {
                    ...
                    if (currentCount === 0) {
                        ...
                        reason = `[TÍM] TỶ LỆ GÃY CỰC CAO: ${(dropOffRate*100).toFixed(1)}% (Lịch sử ${currentCount} lần đạt ${currentLen} ngày chỉ có ${checkTargetCount} lần đi tới ${lenStr}). Rủi ro rất lớn!`;
                    }
                } else {
                    tier = 'red';
                    subTier = 'highRisk';
                    reason = `[ĐỎ] TỶ LỆ GÃY CAO: ${(dropOffRate*100).toFixed(1)}% (Lịch sử ${currentCount} lần đạt ${currentLen} ngày chỉ có ${checkTargetCount} lần đi tới ${lenStr}). Cần loại bỏ!`;
                }

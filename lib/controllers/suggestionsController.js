...
async function computeSuggestions(queryParams = {}) {
    try {
        ...
        for (const key in quickStats) {
            ...
            const isEligibleSoLeTheoCap = isSoLeTheoCap && currentLen >= 3;
            ...
            if (dropOffRate >= MIN_DROPOFF) {
                ...
                if (isEligibleSoLeTheoCap) {
                    tier = 'red';
                    subTier = 'highRisk';
                    reason = `[ĐỎ] DỰ BÁO GÃY CHUỖI SO LE THEO CẶP (ABAB): Chuỗi đã đạt độ dài ${currentLen} ngày (ngày lẻ cùng dạng, ngày chẵn cùng dạng). Ngày thứ 4 cực kỳ khó tiếp tục dạng này. Loại trừ các số thuộc nhãn tiếp theo của chuỗi để phòng ngừa.`;

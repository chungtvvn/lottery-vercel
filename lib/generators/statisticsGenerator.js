...
async function generateNumberStats(dataDir, statsDir, inMemoryData = null) {
    try {
        ...
        // --- [MỚI] Phân tích Đồng Tiến/Lùi cách 22, 33, 44, 55 ---
        const stepNames = { 22: 'Cách 22', 33: 'Cách 33', 44: 'Cách 44', 55: 'Cách 55' };
        Object.keys(SETS).filter(k => k.startsWith('DONG_STEP_')).forEach(setName => {
            const set = SETS[setName];
            if (!set || set.length < 3) return; // Chỉ phân tích bộ >= 3 số
            const map = INDEX_MAPS[setName];
            const mapTrue = MAPS[setName];
            const key = setName.toLowerCase();
        ...
        // --- [MỚI] Phân tích CC/CL/LC/LL + Tổng/Hiệu chẵn/lẻ ---
        const parityLabels = {
        ...
        Object.keys(SETS).filter(k => k.match(/^(CHAN|LE)_(CHAN|LE)_(TONG|HIEU)/)).forEach(setName => {
            const key = setName.toLowerCase();
            ...
            stats[key] = analyzeParityStreaks(lotteryData, dateToIndexMap, setName, typeName);
        });

        // --- [MỚI] Pattern Sequence: 24 permutation của CC/CL/LC/LL ---
        const parityNames = { CC: 'CC', CL: 'CL', LC: 'LC', LL: 'LL' };
        const allPerms = permutations(['CC', 'CL', 'LC', 'LL']);
        allPerms.forEach((perm, idx) => {
            const patternLabel = perm.join('→');
            const patternKey = `pattern_seq_${perm.join('_').toLowerCase()}`;
            stats[patternKey] = findPatternSequenceStreaks(
                lotteryData, dateToIndexMap, perm,
                `Dạng ${patternLabel} tuần hoàn liên tiếp`
            );
        });

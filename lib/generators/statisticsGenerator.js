...
const { BO_GROUPS } = require('../utils/boGroups');
const {
...
function findOrderedSequenceStreaks(data, dateMap, {
    ...
    for (let i = 0; i < data.length - step; i++) {
        ...
        while (cursor + step < data.length) {
            ...
            const nextIndex = orderedSequence.indexOf(String(valueExtractor(nextItem)));
...
async function generateNumberStats(dataDir, statsDir, inMemoryData = null, onPatternGenerated = null) {
    try {
        ...
        res = findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
            typeCondition: () => true,
            valueExtractor: (item) => item.value,
            sequence: SETS.ALL,
            description: 'Các số về theo thứ tự'
        });
        await savePattern('cacSoVeTheoThuTu', 'cacSoVeTheoThuTu', null, res.description, res.streaks);

        res = findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
            typeCondition: () => true,
            valueExtractor: (item) => item.value,
            sequence: SETS.ALL,
            description: 'Các số về so le theo thứ tự',
            alternating: true
        });
        await savePattern('cacSoVeSoLeTheoThuTu', 'cacSoVeSoLeTheoThuTu', null, res.description, res.streaks);

        res = findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
            typeCondition: () => true,
            valueExtractor: (item) => item.value,
            sequence: SETS.ALL,
            description: 'Các số về so le theo thứ tự TIẾN',
            alternating: true,
            directionFilter: 1
        });
        await savePattern('cacSoVeSoLeTheoThuTuTien', 'cacSoVeSoLeTheoThuTuTien', null, res.description, res.streaks);

        res = findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
            typeCondition: () => true,
            valueExtractor: (item) => item.value,
            sequence: SETS.ALL,
            description: 'Các số về so le theo thứ tự LÙI',
            alternating: true,
            directionFilter: -1
        });
        await savePattern('cacSoVeSoLeTheoThuTuLui', 'cacSoVeSoLeTheoThuTuLui', null, res.description, res.streaks);
        ...
        for (const setName of headTailProgressiveSets) {
            ...
            const pStats = {
                ...
                veSoLeTheoThuTuTien: findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
                    ...
                    valueExtractor: (item) => item.value,
                    sequence: set,
                    description: `${desc} - Về so le theo thứ tự TIẾN`,
                    alternating: true,
                    directionFilter: 1
                }),
                veSoLeTheoThuTuLui: findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
                    ...
                    valueExtractor: (item) => item.value,
                    sequence: set,
                    description: `${desc} - Về so le theo thứ tự LÙI`,
                    alternating: true,
                    directionFilter: -1
                }),
        ...
        // 5. Traditional "Bộ" groups from the attached sheet.
        for (const boGroup of BO_GROUPS) {
            const setName = boGroup.setKey;
            const set = SETS[setName];
            const map = INDEX_MAPS[setName];
            const mapTrue = MAPS[setName];
            if (!set || !map || !mapTrue || set.length < 2) continue;

            const key = boGroup.key;
            const desc = boGroup.displayLabel || `${boGroup.label}: ${boGroup.tokens.join(' - ')}`;
            const typeCondition = (item) => mapTrue.has(item.value);
            const pStats = {
                veLienTiep: {
                    ...findConsecutiveTypeStreaks(lotteryData, dateToIndexMap, mapTrue),
                    description: `${desc} - Về liên tiếp`
                },
                veSole: findAlternatingTypeStreaks(lotteryData, dateToIndexMap, {
                    condition: typeCondition,
                    description: `${desc} - Về so le`
                }),
                veSoleMoi: {
                    ...findAlternatingTypeStreaksNew(lotteryData, dateToIndexMap, {
                        condition: typeCondition,
                        valueExtractor: (item) => item.value,
                        valueLabel: boGroup.label
                    }),
                    description: `${desc} - Về so le Mới`
                },
                ...
                tienLienTiep: findProgressiveStreaks(lotteryData, dateToIndexMap, false, set, map, typeCondition, `${desc} - Tiến liên tiếp`),
                tienDeuLienTiep: findProgressiveStreaks(lotteryData, dateToIndexMap, true, set, map, typeCondition, `${desc} - Tiến Đều`),
                luiLienTiep: findRegressiveStreaks(lotteryData, dateToIndexMap, false, set, map, typeCondition, `${desc} - Lùi liên tiếp`),
                luiDeuLienTiep: findRegressiveStreaks(lotteryData, dateToIndexMap, true, set, map, typeCondition, `${desc} - Lùi Đều`),
        ...
        // 8. So le theo cap nested patterns cho nhóm Số đề

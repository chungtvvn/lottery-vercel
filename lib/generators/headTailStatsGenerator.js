...
const { buildBlockAlternationStats } = require('../utils/blockAlternationPatterns');
...
function createStreakObject(data, dateMap, streak, typeSpecificData = {}) {
    ...
    const shouldStoreFullValues = process.env.STORE_FULL_STREAK_VALUES === '1';
    return {
        startDate: firstItem.date,
        endDate: lastItem.date,
        length: daySpan,
        ...(shouldStoreFullValues ? {
...
function findPairTypeABABStreaks(data, dateMap, { pairConfig, minLength = SO_LE_THEO_CAP_MIN_LENGTH }) {
    const allStreaks = [];
...
function findOrderedSequenceStreaks(data, dateMap, {
    ...
    for (let i = 0; i < data.length - step; i++) {
        ...
        if (exactOrder && startIndex !== 0) continue;

        const currentStreak = [first];
        let cursor = i;
        let direction = null; // 1 for forward, -1 for reverse
        let expectedIndex = exactOrder ? 1 % orderedSequence.length : null;
        ...
        while (cursor + step < data.length) {
            ...
            if (exactOrder) {
                ...
                if (nextIndex === fwdIndex) {
                    direction = 1;
                    if (directionFilter !== null && direction !== directionFilter) break;
        ...
        if (currentStreak.length >= minLength) {
            const finalStreak = createStreakObject(data, dateMap, currentStreak, {
                ...(process.env.STORE_FULL_STREAK_VALUES === '1' ? {
...
function analyzeType(data, dateMap, { typeName, descriptionPrefix, valueExtractor, digitSetKey, isTwoDigitSequence = false }) {
    ...
    return {
        ...
        ...buildBlockAlternationStats(data, dateMap, {
            condition: typeCondition,
            createStreakObject,
            isConsecutive,
            descriptionPrefix
        ...
        veSoLeTheoThuTuTien: findOrderedSequenceStreaks(data, dateMap, {
            typeCondition,
            valueExtractor,
            sequence: numberSet,
            description: `${descriptionPrefix} về so le theo thứ tự TIẾN`,
            alternating: true,
            directionFilter: 1
        }),
        veSoLeTheoThuTuLui: findOrderedSequenceStreaks(data, dateMap, {
            typeCondition,
            valueExtractor,
            sequence: numberSet,
            description: `${descriptionPrefix} về so le theo thứ tự LÙI`,
            alternating: true,
            directionFilter: -1
        }),
...
async function saveOrderedDigitGroupPermutations(savePattern, data, dateMap, {
    baseCategory,
    typeName,
    descriptionPrefix,
    valueExtractor,
    ...
    const typeCondition = (item) => MAPS[typeName].has(item.value);
    for (const permutation of buildPermutations(digitGroup)) {
        const orderedCategory = withOrderedPermutationCategory(baseCategory, permutation);
        const orderLabel = permutation.join('→');
        const orderedStats = findOrderedSequenceStreaks(data, dateMap, {
            typeCondition,
            valueExtractor,
            sequence: permutation,
            description: `${descriptionPrefix} về theo thứ tự ${orderLabel}`,
            exactOrder: true
        });
        await savePattern(
            `${orderedCategory}:veTheoThuTu`,
            orderedCategory,
            'veTheoThuTu',
            orderedStats.description,
            orderedStats.streaks
        );

        const alternatingStats = findOrderedSequenceStreaks(data, dateMap, {
            typeCondition,
            valueExtractor,
            sequence: permutation,
            description: `${descriptionPrefix} về so le theo thứ tự ${orderLabel}`,
            alternating: true,
            exactOrder: true
        });
        await savePattern(
            `${orderedCategory}:veSoLeTheoThuTu`,
            orderedCategory,
            'veSoLeTheoThuTu',
            alternatingStats.description,
            alternatingStats.streaks
        );

        const alternatingTienStats = findOrderedSequenceStreaks(data, dateMap, {
            typeCondition,
            valueExtractor,
            sequence: permutation,
            description: `${descriptionPrefix} về so le theo thứ tự TIẾN ${orderLabel}`,
            alternating: true,
            exactOrder: true,
            directionFilter: 1
        });
        await savePattern(
            `${orderedCategory}:veSoLeTheoThuTuTien`,
            orderedCategory,
            'veSoLeTheoThuTuTien',
            alternatingTienStats.description,
            alternatingTienStats.streaks
        );

        const alternatingLuiStats = findOrderedSequenceStreaks(data, dateMap, {
            typeCondition,
            valueExtractor,
            sequence: [...permutation].reverse(),
            description: `${descriptionPrefix} về so le theo thứ tự LÙI ${[...permutation].reverse().join('→')}`,
            alternating: true,
            exactOrder: true,
            directionFilter: 1
        });
        await savePattern(
            `${withOrderedPermutationCategory(baseCategory, [...permutation].reverse())}:veSoLeTheoThuTuLui`,
            withOrderedPermutationCategory(baseCategory, [...permutation].reverse()),
            'veSoLeTheoThuTuLui',
            alternatingLuiStats.description,
            alternatingLuiStats.streaks
        );
    }
}
...
async function generateHeadTailStats(dataDir, statsDir, inMemoryData = null, onPatternGenerated = null) {
    try {
        ...
        res = findOrderedSequenceStreaks(lotteryData, dateToIndexMap, { typeCondition: () => true, valueExtractor: getHead, sequence: DIGIT_SETS.DIGITS, description: "Các Đầu về so le theo thứ tự TIẾN", alternating: true, directionFilter: 1 });
        await savePattern('cacDauVeSoLeTheoThuTuTien', 'cacDauVeSoLeTheoThuTuTien', null, res.description, res.streaks);
        res = findOrderedSequenceStreaks(lotteryData, dateToIndexMap, { typeCondition: () => true, valueExtractor: getHead, sequence: DIGIT_SETS.DIGITS, description: "Các Đầu về so le theo thứ tự LÙI", alternating: true, directionFilter: -1 });
        await savePattern('cacDauVeSoLeTheoThuTuLui', 'cacDauVeSoLeTheoThuTuLui', null, res.description, res.streaks);
        ...
        res = findOrderedSequenceStreaks(lotteryData, dateToIndexMap, { typeCondition: () => true, valueExtractor: getTail, sequence: DIGIT_SETS.DIGITS, description: "Các Đít về so le theo thứ tự TIẾN", alternating: true, directionFilter: 1 });
        await savePattern('cacDitVeSoLeTheoThuTuTien', 'cacDitVeSoLeTheoThuTuTien', null, res.description, res.streaks);
        res = findOrderedSequenceStreaks(lotteryData, dateToIndexMap, { typeCondition: () => true, valueExtractor: getTail, sequence: DIGIT_SETS.DIGITS, description: "Các Đít về so le theo thứ tự LÙI", alternating: true, directionFilter: -1 });
        await savePattern('cacDitVeSoLeTheoThuTuLui', 'cacDitVeSoLeTheoThuTuLui', null, res.description, res.streaks);
        ...
        getSoLeTheoCapConfigs('head_tail').forEach(pairConfig => {
            const ababRes = findPairTypeABABStreaks(lotteryData, dateToIndexMap, { pairConfig });
        ...
        for (const typeName of fixedSetConfigs) {
            ...
            const pStats = {
                ...
                ...buildBlockAlternationStats(lotteryData, dateToIndexMap, {
                    condition: typeCondition,
                    createStreakObject,
                    isConsecutive,
                    descriptionPrefix: description
                }),
                veTheoThuTu: findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
                    typeCondition,
                    valueExtractor: getValue,
                    sequence: SETS[typeName],
                    description: `${description} về theo thứ tự`
                }),
                veSoLeTheoThuTu: findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
                    typeCondition,
                    valueExtractor: getValue,
                    sequence: SETS[typeName],
                    description: `${description} về so le theo thứ tự`,
                    alternating: true
                }),
                veSoLeTheoThuTuTien: findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
                    typeCondition,
                    valueExtractor: getValue,
                    sequence: SETS[typeName],
                    description: `${description} về so le theo thứ tự TIẾN`,
                    alternating: true,
                    directionFilter: 1
                }),
                veSoLeTheoThuTuLui: findOrderedSequenceStreaks(lotteryData, dateToIndexMap, {
                    typeCondition,
                    valueExtractor: getValue,
                    sequence: SETS[typeName],
                    description: `${description} về so le theo thứ tự LÙI`,
                    alternating: true,
                    directionFilter: -1
                }),
        ...
        // 5. 3-digit group configs, including consecutive windows such as 8,9,0.
        const { ALL_3_DIGIT_GROUPS } = require('../utils/numberAnalysis');
        for (const group of ALL_3_DIGIT_GROUPS) {
            ...
            if (MAPS[dauSetName]) {
                ...
                await saveOrderedDigitGroupPermutations(savePattern, lotteryData, dateToIndexMap, {
                    baseCategory: dauKey,
                    typeName: dauSetName,
                    descriptionPrefix: dauDesc,
                    valueExtractor: getHead,

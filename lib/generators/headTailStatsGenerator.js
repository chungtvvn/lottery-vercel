...
function findAlternatingTypeStreaksNew(data, dateMap, { condition, valueLabel }) {
    const allStreaks = [];
    const processedStreaks = new Set();
    for (let i = 0; i < data.length - 2; i++) {
        const dayA = data[i];
...
// [MỚI] Chuỗi ABAB liên tiếp (So le theo cặp). Ví dụ: 1, 4, 1, 4
function findABABStreaks(data, dateMap, { typeCondition, valueExtractor, descriptionPrefix, minLength = 3 }) {
    const allStreaks = [];
    const description = `${descriptionPrefix} so le theo cặp`;

    for (let i = 0; i < data.length - 1; i++) {
        if (!typeCondition(data[i])) continue;
        
        const valA = valueExtractor(data[i]);
        if (valA === undefined || valA === null) continue;
        ...
        for (let j = i; j < data.length - 1; j++) {
            ...
            const nextVal = valueExtractor(nextItem);
            ...
            if (currentStreak.length === 1) {
                ...
                if (nextVal !== valA) {
                    ...
                    break; // Not ABAB, it's AAAA
                }
            } else {
                // Expecting A or B depending on length
                const expectedVal = currentStreak.length % 2 === 0 ? valA : valB;
        ...
        // Only save if it meets minLength (default 3, meaning ABA, so we can predict B for day 4)
        if (currentStreak.length >= minLength) {
            allStreaks.push(createStreakObject(data, dateMap, currentStreak, {
                values: currentStreak.map(item => item.value)
            }));
...
function analyzeType(data, dateMap, { typeName, descriptionPrefix, valueExtractor, digitSetKey, isTwoDigitSequence = false }) {
    ...
    return {
        ...
        veSoleMoi: { // This is the fix from our previous conversation
            ...
            ...findAlternatingTypeStreaksNew(data, dateMap, { condition: (a) => MAPS[typeName].has(a.value), valueLabel: descriptionPrefix })
        },
        ...
        // [MỚI] So le theo cặp (ABAB)
        soLeTheoCap: findABABStreaks(data, dateMap, {
            typeCondition, valueExtractor, descriptionPrefix, minLength: 3
        }),
    };
}

async function generateHeadTailStats(dataDir, statsDir, inMemoryData = null) {
    try {
        let originalData;
        if (inMemoryData) {
        ...
        const stats = {
            ...
            cacDauSoLeTheoCap: findABABStreaks(lotteryData, dateToIndexMap, { typeCondition: () => true, valueExtractor: getHead, descriptionPrefix: "Các Đầu", minLength: 3 }),
            cacDitSoLeTheoCap: findABABStreaks(lotteryData, dateToIndexMap, { typeCondition: () => true, valueExtractor: getTail, descriptionPrefix: "Các Đít", minLength: 3 }),
        };
        ...
        fixedSetConfigs.forEach(typeName => {
            ...
            stats[key] = {
                ...
                veSoleMoi: {
                    ...
                    ...findAlternatingTypeStreaksNew(lotteryData, dateToIndexMap, { condition: typeCondition, valueLabel: description })
                },
                ...
                soLeTheoCap: findABABStreaks(lotteryData, dateToIndexMap, {
                    typeCondition, valueExtractor, descriptionPrefix: description, minLength: 3
                })
            };
        });

        // --- [MỚI] Phân tích Đầu/Đít 3 số ghép (cách nhau >= 2) ---
        const { VALID_3_DIGIT_GROUPS } = require('../utils/numberAnalysis');
        VALID_3_DIGIT_GROUPS.forEach(([a, b, c]) => {
            const groupKey = `${a}_${b}_${c}`;
            const groupLabel = `${a},${b},${c}`;

            // Đầu
            const dauSetName = `DAU_3D_${groupKey}`;
            const dauSet = SETS[dauSetName];
            const dauMap = MAPS[dauSetName];
            if (dauSet && dauMap) {
                const dauKey = dauSetName.toLowerCase();
                const dauDesc = `Đầu (${groupLabel})`;
                const dauCondition = (item) => dauMap.has(item.value);
                stats[dauKey] = {
                    veLienTiep: findStreaks(lotteryData, dateToIndexMap, {
                        condition: (a, b) => dauCondition(a) && dauCondition(b),
                        description: `${dauDesc} về liên tiếp`
                    }),
                    veSole: findAlternatingTypeStreaks(lotteryData, dateToIndexMap, {
                        description: `${dauDesc} về so le`,
                        condition: dauCondition
                    }),
                    veSoleMoi: {
                        description: `${dauDesc} về so le (mới)`,
                        ...findAlternatingTypeStreaksNew(lotteryData, dateToIndexMap, { condition: dauCondition, valueLabel: dauDesc })
                    }
                };
            }
            ...
            const ditSet = SETS[ditSetName];
            const ditMap = MAPS[ditSetName];
            if (ditSet && ditMap) {
                const ditKey = ditSetName.toLowerCase();
                const ditDesc = `Đít (${groupLabel})`;
                const ditCondition = (item) => ditMap.has(item.value);
                stats[ditKey] = {
                    veLienTiep: findStreaks(lotteryData, dateToIndexMap, {
                        condition: (a, b) => ditCondition(a) && ditCondition(b),
                        description: `${ditDesc} về liên tiếp`
                    }),
                    veSole: findAlternatingTypeStreaks(lotteryData, dateToIndexMap, {
                        description: `${ditDesc} về so le`,
                        condition: ditCondition
                    }),
                    veSoleMoi: {
                        description: `${ditDesc} về so le (mới)`,
                        ...findAlternatingTypeStreaksNew(lotteryData, dateToIndexMap, { condition: ditCondition, valueLabel: ditDesc })
                    }
                };
            }
        });

...
function findAlternatingStreaks(data, dateMap, { condition, description, valueExtractor }) {
    ...
    while (i < data.length - 2) {
        ...
        if (dayB && dayC && isConsecutive(data[i], dayB) && isConsecutive(dayB, dayC)) {
            if (condition(dayC) && startValue === valueExtractor(dayC) && valueExtractor(dayB) !== startValue) {
                let streak = [data[i], dayC];
                let lastIndex = i + 2;
                while (lastIndex < data.length - 2) {
                    const nextDayB = data[lastIndex + 1];
                    const nextDayC = data[lastIndex + 2];
                    if (nextDayB && nextDayC && isConsecutive(data[lastIndex], nextDayB) && isConsecutive(nextDayB, nextDayC) && condition(nextDayC) && startValue === valueExtractor(nextDayC) && valueExtractor(nextDayB) !== startValue) {
...
function findAlternatingValueStreaks(data, dateMap, { valueExtractor, description, isNewType }) {
    ...
    for (let i = 0; i < data.length - 2; i++) {
        const startValue = valueExtractor(data[i]);
        const gapValue = valueExtractor(data[i + 1]);
        ...
        while (lastIndex < data.length - 2) {
            ...
            if (dayB && dayC && isConsecutive(data[lastIndex], dayB) && isConsecutive(dayB, dayC)) {
                ...
                const gapMatch = isNewType ? (valueB === gapValue) : (valueB !== startValue);

                if (valueC === startValue && gapMatch) {
                    streak.push(dayC);
                    lastIndex += 2;
                } else {
                    break;
        ...
        if (streak.length >= 2) {
            const extraData = { value: startValue };
            if (isNewType) extraData.gapValue = gapValue;
            allStreaks.push(createStreakObject(data, dateMap, streak, extraData));
...
function findAlternatingTypeStreaks(data, dateMap, { condition, description }) {
    ...
    for (let i = 0; i < data.length - 2; i++) {
        const dayA = data[i];
        if (processedStreaks.has(dayA.date)) continue;
        const dayB = data[i + 1];
        const dayC = data[i + 2];
...
function analyzeNumberSet(data, dateMap, { typeName, descriptionPrefix }) {
    ...
    let valueExtractor;
    if (typeName.startsWith('TONG_TT_')) {
        valueExtractor = (item) => getTongTT(item.value);
    } else if (typeName.startsWith('TONG_MOI_')) {
        valueExtractor = (item) => getTongMoi(item.value);
    } else if (typeName.startsWith('HIEU_')) {
...
function analyzeValueSequence(data, dateMap, { valueExtractor, valueSet, valueMap, descriptionPrefix, typeCondition, includeOrderedSequence = false }) {
    ...
    if (isGroupAnalysis) {
        ...
        results.veSoleMoi = {
            description: `${descriptionPrefix} - Về so le (mới)`,
            ...findAlternatingTypeStreaksNew(data, dateMap, { condition: effectiveTypeCondition, valueExtractor, valueLabel: descriptionPrefix })
        };
        Object.assign(results, buildBlockAlternationStats(data, dateMap, {
    ...
    if (includeOrderedSequence) {
        ...
        results.veSoLeTheoThuTuTien = findOrderedSequenceStreaks(data, dateMap, {
            typeCondition: effectiveTypeCondition,
            valueExtractor,
            sequence: valueSet,
            description: `${descriptionPrefix} - Về so le theo thứ tự TIẾN`,
        ...
        results.veSoLeTheoThuTuLui = findOrderedSequenceStreaks(data, dateMap, {
            typeCondition: effectiveTypeCondition,
            valueExtractor,
            sequence: valueSet,
            description: `${descriptionPrefix} - Về so le theo thứ tự LÙI`,
...
async function saveOrderedValueGroupPermutations(savePattern, data, dateMap, {
    baseCategory,
    descriptionPrefix,
    valueExtractor,
    typeCondition,
    ...
    const baseSequence = (sequence || []).map(String);
    if (baseSequence.length !== 3) return;

    for (const permutation of buildPermutations(baseSequence)) {
        ...
        const reversedCategory = withOrderedPermutationCategory(baseCategory, reversedPermutation);
        const alternatingLuiStats = findOrderedSequenceStreaks(data, dateMap, {
            typeCondition,
            valueExtractor,
            sequence: reversedPermutation,
            description: `${descriptionPrefix} - Về so le theo thứ tự LÙI ${reversedPermutation.join('→')}`,
            alternating: true,
            exactOrder: true,
            directionFilter: 1
        });
        await savePattern(
            `${reversedCategory}:veSoLeTheoThuTuLui`,
            reversedCategory,
            'veSoLeTheoThuTuLui',
...
async function generateSumDifferenceStats(dataDir, statsDir, inMemoryData = null, onPatternGenerated = null) {
    try {
        ...
        for (const pairConfig of getSoLeTheoCapConfigs('sum_diff')) {
            const res = findPairTypeABABStreaks(lotteryData, dateToIndexMap, { pairConfig });
        ...
        for (const config of dangNhomConfigs) {
            ...
            const res = analyzeValueSequence(lotteryData, dateToIndexMap, {
                ...
                includeOrderedSequence: false
            });
            for (const [subcat, val] of Object.entries(res)) {
                await savePattern(`${key}:${subcat}`, key, subcat, val.description, val.streaks);
            }
            await saveOrderedValueGroupPermutations(savePattern, lotteryData, dateToIndexMap, {
                baseCategory: key,
                descriptionPrefix: config.descriptionPrefix,
                valueExtractor: (item) => config.getter(item.value),
                typeCondition: (item) => MAPS[config.typeName].has(item.value),

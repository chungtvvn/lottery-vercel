...
function findABABStreaks(data, dateMap, { typeCondition, valueExtractor, descriptionPrefix, minLength = 3 }) {
    ...
    for (let i = 0; i < data.length - 1; i++) {
        const effectiveCondition = typeCondition || (() => true);
        if (!effectiveCondition(data[i])) continue;
        ...
        for (let j = i; j < data.length - 1; j++) {
            ...
            if (!isConsecutive(currentItem, nextItem) || !effectiveCondition(nextItem)) {
                break;
            }
...
function analyzeNumberSet(data, dateMap, { typeName, descriptionPrefix }) {
    ...
    return {
        ...
        soLeTheoCap: findABABStreaks(data, dateMap, {
            typeCondition, valueExtractor: getValue, descriptionPrefix, minLength: 3
        }),
    };
}
...
function analyzeValueSequence(data, dateMap, { valueExtractor, valueSet, valueMap, descriptionPrefix, typeCondition }) {
    ...
    Object.assign(results, {
        ...
        // [MỚI] So le theo cặp (ABAB)
        soLeTheoCap: findABABStreaks(data, dateMap, {
            typeCondition: effectiveTypeCondition, valueExtractor, descriptionPrefix, minLength: 3
        }),
    });
